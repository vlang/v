...
fn (cfg DocConfig) gen_html(idx int) string {
	...
	mut doc_node_html := fn (dd doc.DocNode, link string, head bool) string {
		...
		content := if head {
			...
			'<code class="code-snippet">${dd.content}</code>
                <p>${md_content}</p>
				$src_link'
		}

		return '
			<section id="${slug(dd.name)}" class="doc-node">
				${heading}
				${content}
			</section>
		'
...
<head>
	...
	hw.write('</ul>\n</nav>\n</header>')
	// hw.write('<div class="doc-container">\n<div class="doc-content">\n<p class="text-gray-600">Generated by vdoc. Last generated: ${dcs.time_generated.str()}</p>')
	// println('test')
