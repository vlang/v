...
<head>
	...
	for cn in d.contents {
		...
		hw.write('        <li><a href="#${slug(cn.name)}">${cn.name}</a>')
		...
		if children.len != 0 {
			hw.writeln('        <ul>')
				for child in children {
					hw.writeln('                <li><a href="#${slug(child.name)}">${child.name}</a></li>')
				}
			hw.writeln('        </ul>')
		}
