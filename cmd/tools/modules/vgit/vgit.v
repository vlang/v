...
pub fn normalized_workpath_for_commit(workdir string, commit string) string {
	...
	return os.abs_path(workdir + os.path_separator + nc)
}
...
pub fn (vgit_context mut VGitContext) compile_oldv_if_needed() {
	...
	vgit_context.vexepath = os.abs_path(os.join_path(vgit_context.path_v, vgit_context.vexename))
...
pub fn add_common_tool_options<T>(context mut T, fp mut flag.FlagParser) []string {
	tdir := os.temp_dir()
	context.workdir = os.abs_path(fp.string_('workdir', `w`, tdir, 'A writable base folder. Default: $tdir'))
...
')
	...
	if os.is_dir(context.v_repo_url) {
		context.v_repo_url = os.abs_path( context.v_repo_url )
	}

	if os.is_dir(context.vc_repo_url) {
		context.vc_repo_url = os.abs_path( context.vc_repo_url )
	}
