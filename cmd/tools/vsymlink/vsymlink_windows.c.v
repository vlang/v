...
fn get_reg_value(reg_env_key voidptr, key string) !string {
	...
	query_result := C.RegQueryValueExW(reg_env_key, key.to_wide(), 0, 0, 0, voidptr(&reg_value_size))
	...
	read_result := C.RegQueryValueExW(reg_env_key, key.to_wide(), 0, 0, &u8(reg_value.data), voidptr(&reg_value_size))
	if read_result != 0 {
...
fn set_reg_value(reg_key voidptr, key string, value string) !bool {
	...
	if C.RegSetValueExW(reg_key, key.to_wide(), 0, C.REG_EXPAND_SZ, voidptr(wide_value), wide_value_size) != 0 {
...
fn send_setting_change_msg(message_data string) !bool {
	...
	if C.SendMessageTimeoutW(os.hwnd_broadcast, os.wm_settingchange, 0, unsafe { &u32(message_data_wide) }, os.smto_abortifhung, 5000, 0) == 0 {
