...
fn process_files(files []string) ! {
	...
	for f in files {
		...
		if !fuzzer_mode {
			println('${f_us:10}us ${p.scanner.all_tokens.len:10} ${p.scanner.text.len:10} ${ast_file.nr_lines:10} ${(f64(p.scanner.text.len) / p.scanner.all_tokens.len):13.3} ${p.error_handler.error_count():10}  ${formatted_content.len:8}   ${f}')
		}
	}
	hline()
	theader()
