...
fn (mut context Context) run() {
	for si in 0 .. context.series {
		for icmd, cmd in context.commands {
			...
			context.results[icmd].atiming = new_aints(context.results[icmd].timings, context.nmins, context.nmaxs)
			context.clear_line()
...
fn (mut context Context) parse_options() ! {
	...
	context.verbose = fp.bool('verbose', `v`, false, 'Print each command, its exit code, and its output.')
