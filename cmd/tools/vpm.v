...
fn vpm_install(module_names []string) {
	...
	for n in module_names {
		...
		final_module_path := os.abs_path(os.join_path(settings.vmodules_path,mod.name.replace('.', os.path_separator)))
...
fn vpm_remove(module_names []string) {
	...
	for name in module_names {
		...
		author := name.split('.')[0]
		author_dir := os.abs_path(os.join_path(settings.vmodules_path,author))
...
fn valid_final_path_of_existing_module(name string) ?string {
	...
	final_module_path := os.abs_path(name_of_vmodules_folder)
	if !os.exists(final_module_path) {
...
fn vcs_used_in_dir(dir string) ?[]string {
	...
	for repo_subfolder in supported_vcs_folders {
		checked_folder := os.abs_path(os.join_path(dir,repo_subfolder))
