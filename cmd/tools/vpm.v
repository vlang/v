...
fn get_all_modules() []string {
	...
	for read_len < s.len {
		...
		mut start_index := s.index_after_int(start_token, read_len)
		if start_index == -1 {
			start_token = '<a href="/mod'
			start_index = s.index_after_int(start_token, read_len)
			if start_index == -1 {
				break
			}
		}
		...
		start_token = '>'
		start_index = s.index_after_int(start_token, start_index) + start_token.len

		// get the index of the end of module entry
		end_index := s.index_after_int(end_token, start_index)
		if end_index == -1 {
			break
		}
