...
struct FormatOptions {
	...
	migrate_json2 bool // opt in to rewriting deprecated `json` module usage to `json2` (`-migrate-json2`); off by default so plain `v fmt` never changes json semantics
mut:
...
const vfmt_only_flags = ['-backup', '-c', '-diff', '-inprocess', '-l', '-migrate-json2', '-new_int',
	'-noerror', '-verbose', '--verbose', '-verify', '-w']
...
fn main() {
	...
	mut foptions := FormatOptions{
		...
		migrate_json2: '-migrate-json2' in args
	}
	if term_colors {
...
fn (foptions &FormatOptions) formated_content_from_file(prefs &pref.Preferences, file string) !string {
	...
	formated_content := fmt.fmt(file_ast, mut table, prefs, foptions.is_debug,
		migrate_json2: foptions.migrate_json2
	)
