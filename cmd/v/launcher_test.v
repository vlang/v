...
fn test_v1_fallback_installer_uses_last_v1_snapshot() {
	...
	assert installer.contains('if [ "$selected_cc" = cc ] && ! command -v "$selected_cc" >/dev/null 2>&1; then')
	assert installer.contains('for fallback_cc in clang gcc; do')
	assert installer.contains('selected_cc=$fallback_cc')
	...
	assert installer.contains('[ ! -f "$common_dir/objects/info/alternates" ] || return 1')
