...
V — це статично типізована скомпільована мова програмування, призначена для створення підтримуємого програмного забезпечення, яке підтримується.

Він схожий на Go, і на його дизайн також вплинули Oberon, Rust, Swift,
Kotlin і Python.

V — дуже проста мова. Перегляд цієї документації займе у вас близько години,
і до кінця ви вивчите майже всю мову.

Мова сприяє написанню простого і зрозумілого коду з мінімальною абстракцією.

Незважаючи на простоту, V дає розробнику багато можливостей.
Все, що ви можете зробити іншими мовами, ви можете зробити на V.

## Встановити з вихідного коду
Основний спосіб отримати найновіший і найкращий V — це __інсталювати його з вихідного коду__.
Це __легко__, і зазвичай це займає __лише кілька секунд__.


### Linux, macOS, FreeBSD, і так далі:
Вам потрібен `git`, і компілятор C, наприклад `tcc`, `gcc` або `clang`, і `make`:
```bash
...
### Windows:
Вам потрібен `git`, і компілятор C, наприклад `tcc`, `gcc`, `clang` or `msvc`:
```bash
git clone https://github.com/vlang/v
cd v
make.bat -tcc
...
### Android
Запуск графічних програм V на Android також можливий через [vab](https://github.com/vlang/vab).

V Залежності для Android: **V**, **Java JDK** >= 8, Android **SDK + NDK**.

  1. Встановлення залежності (див. [vab](https://github.com/vlang/vab))
  2. Підключіть свій Android пристрій.
  3. Запустіть:
  ```bash
  git clone https://github.com/vlang/vab && cd vab && v vab.v
  ./vab --device auto run /path/to/v/examples/sokol/particles
  ```
Для отримання додаткової інформації та усунення несправностей відвідайте [сховище GitHub vab](https://github.com/vlang/vab).
...
* [Запуск папки проекту](#running-a-project-folder-with-several-files)
* [Коментарі](#comments)
* [Функції](#functions)
    * [Повернення кількох значень](#returning-multiple-values)
    * [Підйомники](#hoistings)
* [Видимість символу](#symbol-visibility)
* [Змінні](#змінні)
* [V типи](#v-типи)
    * [Рядки](#рядки)
    * [Числа](#числа)
    * [Масиви](#масиви)
    * [Масиви фіксованого розміру](#масиви-фіксованого-розміру)
    * [карти](#карти)
* [Імпорт модуля](#імпорт-модуля)
* [Твердження та вирази](#Твердження-та-вирази)
    * [Якщо (if)](#якщо)
    * [In оператор](#in-оператор)
    * [цикл for](#цикл-for)
    * [Match](#match)
    * [Defer](#defer)
...
* [Advanced Topics](#advanced-topics)
	...
	* [Atomics](#atomics)
	* [Global Змінні](#global-Змінні)
    * [Debugging](#debugging)
    * [Conditional compilation](#conditional-compilation)
    * [Compile time pseudo Змінні](#compile-time-pseudo-Змінні)
...
Sorting arrays of all kinds is very simple and intuitive. Special Змінні `a` and `b`
...
Mutable Змінні can change, and doing a cast would be unsafe.
...
This is achieved by a lack of global Змінні and all function arguments being
immutable by default, even when [references](#references) are passed.

V is not a purely functional language however.

There is a compiler flag to enable global Змінні (`-enable-globals`), but this is
...
fn main() {
	...
	// Functions can be passed around without assigning them to Змінні:
...
This means that anonymous functions can inherit Змінні from the scope they were created in.
They must do so explicitly by listing all Змінні that are inherited.
...
Inherited Змінні are copied when the anonymous function is created.
...
Global Змінні are not normally allowed, so this can be really useful.
...
from local Змінні, the full path to consts must be specified. For example,
...
Operations are not allowed on enum Змінні; they must be explicitly cast to `int`.
...
> This works for both, simple Змінні and complex expressions like `user.name`
...
Such Змінні should be created as `shared` and passed to the thread as such, too.
...
```
Shared Змінні must be structs, arrays or maps.
...
V has no special support for atomics, yet, nevertheless it's possible to treat Змінні as atomics
...
// see section "Global Змінні" below
__global (
...
By default V does not allow global Змінні. However, in low level applications they have their
place so their usage can be enabled with the compiler flag `-enable-globals`.
Declarations of global Змінні must be surrounded with a `__global ( ... )`
specification &ndash; as in the example [above](#atomics).

An initializer for global Змінні must be explicitly converted to the
...
```
Be aware that in multi threaded applications the access to global Змінні is subject
...
- don't care &ndash; this approach is possible but makes only sense if the exact values
  of global Змінні do not really matter. An example can be found in the `rand` module
  where global Змінні are used to generate (non cryptographic) pseudo random numbers.
...
- `-D` for setting compile time Змінні
...
V can bring in values at compile time from environment Змінні.
...
## Compile time pseudo Змінні

V also gives your code access to a set of pseudo string Змінні,
