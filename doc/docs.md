...
Note that V by default makes implicit clone of the slice and displays a notice about this.
So without the .clone() call result of the code above will be the same. Make slice in
`unsafe {}` block if you want to reuse memory, othervise prefer to use explicit cloning.
