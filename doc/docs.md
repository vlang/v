...
instruction meaning changes with width, including shifts, rotates, bit counts, bit tests, byte
swaps, and CRC32 sources. It also rejects narrower signed operands of `cmp` and `test`. Named
operands cannot be sources of `movsx`, `movsxd`, or `movzx`. Named shift counts are not supported
