...
To test an entire module, use `v benchmark mymodule`. You can also use `v benchmark .` to test
everything inside your current folder (and subfolders). You can pass the `-stats`
option to see more details about the individual benchmark run.

You can put additional benchmark data, including .v source files in a folder, named
`benchmarkdata`, right next to your _benchmark.v files. V's test framework will *ignore*
such folders, while scanning for tests to run. This is useful, if you want to
