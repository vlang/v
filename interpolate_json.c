	...
	#define V_COMMIT_HASH "f32a76b"
#endif

#ifndef V_CURRENT_COMMIT_HASH
	#define V_CURRENT_COMMIT_HASH "0033380"
#endif
...
// defined by module `json` in file `/home/mark/dev/v/v/vlib/json/json_primitives.v`:
...
Option_Map_string_string json__decode_Map_string_string(cJSON* root);
cJSON* json__encode_Map_string_string(Map_string_string val);
...
VV_LOCAL_SYMBOL void map_free_nop(voidptr _t390);
...
VV_LOCAL_SYMBOL struct cJSON* json__encode_byte(byte val);
...
Option_Map_string_string json__decode_Map_string_string(cJSON* root) {
	Map_string_string res;
	if (!root) {
		const char *error_ptr = cJSON_GetErrorPtr();
		if (error_ptr != NULL)	{
			// fprintf(stderr, "Error in decode() for Map_string_string error_ptr=: %s\n", error_ptr);
			// printf("\nbad js=%%s\n", js.str);
			return (Option_Map_string_string){.state = 2,.err = v_error(tos2((byteptr)error_ptr)),.data = {0}};
		}
	}


	if(!cJSON_IsObject(root) && !cJSON_IsNull(root)) {
		return (Option_Map_string_string){ .state = 2, .err = v_error(string__plus(_SLIT("Json element is not an object: "), tos2((byteptr)cJSON_PrintUnformatted(root)))), .data = {0}};
	}
	res = new_map(sizeof(string), sizeof(string), &map_hash_string, &map_eq_string, &map_clone_string, &map_free_string);
	cJSON *jsval = NULL;
	cJSON_ArrayForEach(jsval, root)
	{
		string val = json__decode_string (js_get(root, jsval->string));
		string key = tos2((byteptr)jsval->string);
		map_set(&res, &key, &val);
	}

	Option_Map_string_string ret;
	opt_ok(&res, (Option*)&ret, sizeof(res));
	return ret;
}
...
cJSON* json__encode_Map_string_string(Map_string_string val) {
	...
	Array_string _t777 = map_keys(&val);
	for (int i = 0; i < _t777.len; ++i) {
		string key = ((string*)_t777.data)[i];
		cJSON_AddItemToObject(o, (char*) key.str, json__encode_string ( *(string*) map_get(&val, &key, &(string[]) { (string){.str=(byteptr)"", .is_lit=1} } ) ) );
	}
	array_free(&_t777);
...
string byte_str_escaped(byte b) {
	...
	if (b == (0)) {
		...
		_t371 = byte_ascii_str(b);
	}
	else {
		_t371 = string__plus(_SLIT("0x"), byte_hex(b));
	}string str = _t371;
	return str;
}
...
VV_LOCAL_SYMBOL voidptr map_get_and_set(map* m, voidptr key, voidptr zero) {
	...
	if (!(false)) {
		...
		v_assert_meta_info__t398.fpath = _SLIT("/home/mark/dev/v/v/vlib/builtin/map.v");
		...
		__print_assert_failure(&v_assert_meta_info__t398);
...
Option_voidptr json__decode(voidptr typ, string s) {
	Option_voidptr _t733;
	opt_ok(&(voidptr[]) { 0 }, (Option*)(&_t733), sizeof(voidptr));
	return _t733;
}
...
VV_LOCAL_SYMBOL i64 json__decode_i64(struct cJSON* root) {
	...
	i64 _t743 = ((i64)(root->valuedouble));
	return _t743;
}
...
VV_LOCAL_SYMBOL f64 json__decode_f64(struct cJSON* root) {
	...
	f64 _t755 = ((f64)(root->valuedouble));
	return _t755;
}
...
VV_LOCAL_SYMBOL string json__decode_string(struct cJSON* root) {
	...
	string _t758 = tos_clone(((byte*)(root->valuestring)));
	return _t758;
}
...
VV_LOCAL_SYMBOL bool json__decode_bool(struct cJSON* root) {
	...
	bool _t760 = cJSON_IsTrue(root);
	return _t760;
}

VV_LOCAL_SYMBOL struct cJSON* json__encode_int(int val) {
	struct cJSON* _t761 = cJSON_CreateNumber(val);
	return _t761;
}

VV_LOCAL_SYMBOL struct cJSON* json__encode_i8(i8 val) {
	struct cJSON* _t762 = cJSON_CreateNumber(val);
	return _t762;
}

VV_LOCAL_SYMBOL struct cJSON* json__encode_i16(i16 val) {
	struct cJSON* _t763 = cJSON_CreateNumber(val);
	return _t763;
}

VV_LOCAL_SYMBOL struct cJSON* json__encode_i64(i64 val) {
	struct cJSON* _t764 = cJSON_CreateNumber(val);
	return _t764;
}

VV_LOCAL_SYMBOL struct cJSON* json__encode_byte(byte val) {
	struct cJSON* _t765 = cJSON_CreateNumber(val);
	return _t765;
}

VV_LOCAL_SYMBOL struct cJSON* json__encode_u16(u16 val) {
	struct cJSON* _t766 = cJSON_CreateNumber(val);
	return _t766;
}

VV_LOCAL_SYMBOL struct cJSON* json__encode_u32(u32 val) {
	struct cJSON* _t767 = cJSON_CreateNumber(val);
	return _t767;
}

VV_LOCAL_SYMBOL struct cJSON* json__encode_u64(u64 val) {
	struct cJSON* _t768 = cJSON_CreateNumber(val);
	return _t768;
}

VV_LOCAL_SYMBOL struct cJSON* json__encode_f32(f32 val) {
	struct cJSON* _t769 = cJSON_CreateNumber(val);
	return _t769;
}

VV_LOCAL_SYMBOL struct cJSON* json__encode_f64(f64 val) {
	struct cJSON* _t770 = cJSON_CreateNumber(val);
	return _t770;
}

VV_LOCAL_SYMBOL struct cJSON* json__encode_bool(bool val) {
	struct cJSON* _t771 = cJSON_CreateBool(val);
	return _t771;
}

VV_LOCAL_SYMBOL struct cJSON* json__encode_string(string val) {
	struct cJSON* _t772 = cJSON_CreateString(((char*)(val.str)));
	return _t772;
}

VV_LOCAL_SYMBOL struct cJSON* json__json_parse(string s) {
	struct cJSON* _t773 = cJSON_Parse(((char*)(s.str)));
	return _t773;
}

VV_LOCAL_SYMBOL string json__json_print(struct cJSON* json) {
	char* s = cJSON_PrintUnformatted(json);
	string _t774 = tos(((byte*)(s)), strlen(((char*)(s))));
	return _t774;
}

VV_LOCAL_SYMBOL string json__json_print_pretty(struct cJSON* json) {
	char* s = cJSON_Print(json);
	string _t775 = tos(((byte*)(s)), strlen(((char*)(s))));
	return _t775;
}

VV_LOCAL_SYMBOL void main__main(void) {
	Map_string_string data = new_map_init(&map_hash_string, &map_eq_string, &map_clone_string, &map_free_string, 1, sizeof(string), sizeof(string), _MOV((string[1]){_SLIT("foo"), }), _MOV((string[1]){_SLIT("bar"), }));
	println( str_intp(2, _MOV((StrIntpData[]){{_SLIT("data: "), 0xfe10, {.d_s = cJSON* _t776 = json__encode_Map_string_string(data);
	string _t778 = json__json_print(_t776);
	cJSON_Delete(_t776); //del
(_t778)}}, {_SLIT0, 0, { .d_c = 0 }}})) );
}
