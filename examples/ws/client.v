module main

import net.websocket
import time


fn main() {
	ws_test('ws://echo.websocket.org')
	ws_test('wss://echo.websocket.org')
}

fn ws_test(uri string) {
	mut ws := websocket.new(uri)

	ws.subscriber.subscribe('on_open', on_open)
	ws.subscriber.subscribe('on_message', on_message)
	ws.subscriber.subscribe('on_error', on_error)
	ws.subscriber.subscribe('on_close', on_close)

	ws.connect()
	go ws.listen()

	for i := 0; i < 10; i++ {
		text := 'a'.repeat(i)
		println(text)
		ws.write(text.str, text.len, .text_frame)
		time.sleep_ms(100)//Only for test purposes, to give time to recieve message
	}
	time.sleep_ms(100)//Only for test purposes, to give time to recieve message
}

fn on_open(mut ws websocket.Client, x voidptr, y voidptr) {
	println('websocket opened.')
}

fn on_message(mut ws websocket.Client, msg &websocket.Message, x voidptr) {
	typ := msg.opcode
	if typ == .text_frame {
		println("Message: ${cstring_to_vstring(msg.payload)}")
	} else {
		println('Binary message: $msg')
	}
}

fn on_close(ws &websocket.Client, x voidptr, y voidptr) {
	println('websocket closed.')
}

fn on_error(ws &websocket.Client, x voidptr, y voidptr) {
	println('we have an error.')
}