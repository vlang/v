...
'BSTree.insert'(Bst, Value) ->
    case 'unknown.is_empty'(Bst) of
        true -> true;
        false -> 'unknown.insert_helper'(Bst, maps:get(root, Bst), Value)
        end.
    ...
    case maps:get(value, Node) < Value of
        true -> begin
            case todo andalso maps:get(is_init, maps:get(right, Node)) of
                true -> 'unknown.insert_helper'(Bst, maps:get(right, Node), Value);
                false -> ok
            end,
            true
        end;
        false -> case maps:get(value, Node) > Value of
            true -> begin
                case todo andalso maps:get(is_init, maps:get(left, Node)) of
                    true -> 'unknown.insert_helper'(Bst, maps:get(left, Node), Value);
                    false -> ok
                end,
                true
            end;
...
'BSTree.contains'(Bst, Value) ->
    'unknown.contains_helper'(Bst, maps:get(root, Bst), Value).
...
'BSTree.contains_helper'(Bst, Node, Value) ->
    case todo orelse not maps:get(is_init, Node) of
        ...
        false -> begin
            case maps:get(value, Node) < Value of
                true -> 'unknown.contains_helper'(Bst, maps:get(right, Node), Value);
                false -> case maps:get(value, Node) > Value of
                    true -> 'unknown.contains_helper'(Bst, maps:get(left, Node), Value);
                    false -> ok
                end
            end,
...
'BSTree.remove'(Bst, Value) ->
    case 'unknown.is_empty'(Bst) of
        true -> false;
        false -> 'unknown.remove_helper'(Bst, maps:get(root, Bst), Value, false)
        end.
    ...
    case not maps:get(is_init, Node) of
        ...
        false -> 
            case maps:get(value, Node) == Value of
                ...
                false -> 
                    case maps:get(value, Node) < Value of
                        true -> 'unknown.remove_helper'(Bst, maps:get(right, Node), Value, false);
                        false -> 'unknown.remove_helper'(Bst, maps:get(left, Node), Value, true)
                                        end
                                        end
                end.
    ...
    case todo of
        ...
        false -> begin
            ...
            case todo orelse not maps:get(is_init, Right_node) of
                true -> Node;
                false -> 'unknown.get_max_from_right'(Bst, Right_node)
                        end
        end
        end.
    ...
    case todo of
        ...
        false -> begin
            ...
            case todo orelse not maps:get(is_init, Left_node) of
                true -> Node;
                false -> 'unknown.get_min_from_left'(Bst, Left_node)
                        end
        end
        end.
...
'BSTree.in_order_traversal'(Bst) ->
    Result = [],
    'unknown.in_order_traversal_helper'(Bst, maps:get(root, Bst), Result),
    Result.
...
'BSTree.in_order_traversal_helper'(Bst, Node, Result) ->
    case todo orelse not maps:get(is_init, Node) of
        true -> ok;
        false -> begin
            'unknown.in_order_traversal_helper'(Bst, maps:get(left, Node), Result),
            Result bsl maps:get(value, Node),
            'unknown.in_order_traversal_helper'(Bst, maps:get(right, Node), Result),
            ok
        end
        end.
...
'BSTree.post_order_traversal'(Bst) ->
    Result = [],
    'unknown.post_order_traversal_helper'(Bst, maps:get(root, Bst), Result),
    Result.
...
'BSTree.post_order_traversal_helper'(Bst, Node, Result) ->
    case todo orelse not maps:get(is_init, Node) of
        true -> ok;
        false -> begin
            'unknown.post_order_traversal_helper'(Bst, maps:get(left, Node), Result),
            'unknown.post_order_traversal_helper'(Bst, maps:get(right, Node), Result),
            Result bsl maps:get(value, Node),
            ok
        end
        end.
...
'BSTree.pre_order_traversal'(Bst) ->
    Result = [],
    'unknown.pre_order_traversal_helper'(Bst, maps:get(root, Bst), Result),
    Result.
...
'BSTree.pre_order_traversal_helper'(Bst, Node, Result) ->
    case todo orelse not maps:get(is_init, Node) of
        ...
        false -> begin
            Result bsl maps:get(value, Node),
            'unknown.pre_order_traversal_helper'(Bst, maps:get(left, Node), Result),
            'unknown.pre_order_traversal_helper'(Bst, maps:get(right, Node), Result),
            ok
        end
        end.
    ...
    case todo orelse not maps:get(is_init, Node) of
        ...
        false -> 
            case maps:get(value, Node) == Value of
                ...
                false -> 
                    case maps:get(value, Node) < Value of
                        true -> 'unknown.get_node'(Bst, maps:get(right, Node), Value);
                        false -> 'unknown.get_node'(Bst, maps:get(left, Node), Value)
                                        end
                                        end
                end.
...
'BSTree.to_left'(Bst, Value) ->
    case 'unknown.is_empty'(Bst) of
        ...
        false -> begin
            Node = 'unknown.get_node'(Bst, maps:get(root, Bst), Value),
            case not maps:get(is_init, Node) of
...
'BSTree.max'(Bst) ->
    case 'unknown.is_empty'(Bst) of
        true -> error(<<"BSTree is empty">>);
        false -> begin
            Max = 'unknown.get_max_from_right'(Bst, maps:get(root, Bst)),
...
'BSTree.min'(Bst) ->
    case 'unknown.is_empty'(Bst) of
        true -> error(<<"BSTree is empty">>);
        false -> begin
            Min = 'unknown.get_min_from_left'(Bst, maps:get(root, Bst)),
...
'LinkedList.first'(List) ->
    case not 'LinkedList.is_empty'(List) of
...
'LinkedList.index'(List, Idx) ->
    case 'LinkedList.is_empty'(List) of
        ...
        false -> 
            case Idx < 0 orelse Idx >= length(List) of
                true -> error(<<"Index ", (integer_to_binary(Idx))/binary, " out of bounds">>);
                false -> begin
                    Node = maps:get(head, List),
...
'LinkedList.insert'(List, Idx, Item) ->
    case Idx < 0 orelse Idx > length(List) of
        true -> error(<<"Index ", (integer_to_binary(Idx))/binary, " out of bounds [0..", (integer_to_binary(length(List)))/binary, "]">>);
        false -> 
...
'LinkedList.prepend'(List, Item) ->
    'LinkedList.insert'(List, 0, Item),
    ok.

'LinkedList.str'(List) ->
    '[]T.str'('LinkedList.array'(List)).
...
'Queue.is_empty'(Queue) ->
    'LinkedList.is_empty'(maps:get(elements, Queue)).

'Queue.len'(Queue) ->
    'LinkedList.len'(maps:get(elements, Queue)).

'Queue.peek'(Queue) ->
    'LinkedList.first'(maps:get(elements, Queue)).

'Queue.last'(Queue) ->
    'LinkedList.last'(maps:get(elements, Queue)).

'Queue.index'(Queue, Idx) ->
    'LinkedList.index'(maps:get(elements, Queue), Idx).

'Queue.push'(Queue, Item) ->
    'LinkedList.push'(maps:get(elements, Queue), Item),
    ok.

'Queue.pop'(Queue) ->
    'LinkedList.shift'(maps:get(elements, Queue)).

'Queue.str'(Queue) ->
    'LinkedList.str'(maps:get(elements, Queue)).

'Queue.array'(Queue) ->
    'LinkedList.array'(maps:get(elements, Queue)).
