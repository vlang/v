...
% TODO: const lorem_corpora = map{ 'lorem': strings.lorem.lorem_corpus 'poe': strings.lorem.poe_corpus 'darwin': strings.lorem.darwin_corpus 'bard': strings.lorem.shakespeare_corpus };
% TODO: const lorem_corpus = '
...
';
% TODO: const lorem_seed_phrases = ['in the beginning', 'once upon a time', 'it was the first', 'when we consider', 'there was a moment'];
    ...
    Sentences = lists:foldl(fun(Pi, SentencesAcc) ->
        ...
        {Words, Sentence} = lists:foldl(fun(Si, {WordsAcc, SentenceAcc}) ->
            ...
            case length(Sentence) > 0 of
                true -> begin
                    Out << 'string.capitalize'('[]string.join'(Sentence, <<" ">>)),
                    Out << <<".">>
                end;
                false -> ok
            end,
...
lorem_build_markov(Tokens, Order) ->
    ...
    Key = lists:foldl(fun(I, KeyAcc) ->
        ...
        maps:get(Key, Model) << lists:nth(I + Order + 1, Tokens),
        KeyOut
