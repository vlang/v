        ...
        false -> begin
            ...
            case length(State) < maps:get(markov_order, Cfg) of
                true -> begin
                    ...
                    State1 = lists:nth(todo + 1, Tokens),
                end;
                false -> ok
            end,
            Out = [],
...
lorem_tokenize(Text) ->
    lists:filter(length(It) > 0, binary:split('string.replace_each'(Text, [<<"\\n">>, <<" ">>, <<"\\t">>, <<" ">>]), <<" ">>, [global])).

lorem_build_markov(Tokens, Order) ->
    Model = #{},
    Key = lists:foldl(fun(I, KeyAcc) ->
        KeyOut = iolist_to_binary(lists:join(<<"">>, lists:nth(todo + 1, Tokens))),
