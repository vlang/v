...
fn (mut app App) handle_mouse_event(x int, y int, btn_type gg.MouseButton){
	...
	match btn_type{
		.left{
			...
			else if !(row == app.start.y && col == app.start.x) && !(row == app.end.y && col == app.end.x) {
...
fn draw_gridlines(mut app App){
	dx := WINDOW_WIDTH / ROWS
	dy := WINDOW_HEIGHT / ROWS 
	for i:= 0; i < ROWS; i++ {
		// horizontal lines 
...
fn initialise_grid() ([][]Cell){
	mut grid := [][]Cell{len: ROWS, init: []Cell{len: ROWS}}
	gap := WINDOW_WIDTH / ROWS 
	for i:=0; i < ROWS; i++{
		for j := 0; j < ROWS; j++{
			grid[i][j] = &Cell{
...
fn update_neighbors(mut grid [][]Cell, row int, col int){
	if row < ROWS - 1 && grid[row+1][col].flag != 3 {
	...
	if col < ROWS - 1 && grid[row][col+1].flag != 3{
		grid[row][col].neighbors << &Point{
			x: row
			y: col+1
		}
...
fn reconstruct_path(mut grid [][]Cell, mut came_from [][]Point, start Point, end Point){
	...
	for !(x == -1 && y == -1){
		set_cell_type(mut grid, current.x, current.y, 'path')
		current = &Point{
			x: came_from[x][y].x
			y: came_from[x][y].y
		}
...
fn astar_path_finding(mut app App, mut grid [][]Cell, start Point, end Point){
	...
	mut g_score := [][]int{len: ROWS, init: []int{len: ROWS}}
	mut f_score := [][]int{len: ROWS, init: []int{len: ROWS}}
	mut came_from := [][]Point{len: ROWS, init: []Point{len: ROWS}}
	for i := 0; i < ROWS; i++ {
		for j := 0; j < ROWS; j++{
	...
	for priority_queue.len() > 0{
		curr_node := priority_queue.pop() or {panic("There is nothing in queue how did it reach here idk")}
