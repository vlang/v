	...
	#define V_CURRENT_COMMIT_HASH "4956f2a"
#endif
...
static void main__main() {
	...
	{ // 2comptime $for {
		// field 0
		FieldData field = (FieldData){.name = tos_lit("a"),
		.attrs = new_array_from_c_array(0, 0, sizeof(string), _MOV((string[0]){})),
		.typ = tos_lit("string"),
		.is_pub = false, .is_mut = true,};
		{ // $if ret_type typecheck start
			println(FieldData_str(field));
		} // typecheck end
	
		// field 1
		field = (FieldData){.name = tos_lit("test"),
		.attrs = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){tos_lit("test")})),
		.typ = tos_lit("string"),
