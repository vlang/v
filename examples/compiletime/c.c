	...
	#define V_CURRENT_COMMIT_HASH "6e2fc70"
#endif
...
string DataType_str(DataType it); // auto
string FieldData_str(FieldData x); // auto
string indent_FieldData_str(FieldData x, int indent_count); // auto
// variadic structs
struct varg_voidptr {
	int len;
	voidptr args[0];
...
string DataType_str(DataType it) {
	string tmp1 = string_add(tos_lit("DataType("), tos3(int_str((int)it).str));
	string tmp2 = string_add(tmp1, tos_lit(")"));
	string_free(&tmp1);
	return tmp2;
}
string FieldData_str(FieldData x) { return indent_FieldData_str(x,0);}
string indent_FieldData_str(FieldData x, int indent_count) {
	FieldData *it = &x;
	string indents = tos_lit("");
	for (int i = 0; i < indent_count; ++i) {
		indents = string_add(indents, tos_lit("    "));
	}
	return _STR("FieldData {\n"
		"%.*s\000    name: '%.*s\000'\n"
		"%.*s\000    attrs: %.*s\000\n"
		"%.*s\000    typ: '%.*s\000'\n"
		"%.*s\000    is_pub: %.*s\000\n"
		...
		"%.*s\000}", 14,
		indents, it->name,
		indents, array_string_str( it->attrs) ,
		indents, it->typ,
...
static void main__main() {
	...
	{ // 2comptime: $for method in main.App(methods) {
		...
		// method 0
		method.name = tos_lit("method_one");
		method.attrs = new_array_from_c_array(0, 0, sizeof(string), _MOV((string[0]){}));
		method.ret_type = tos_lit("void");
		method.type = 1;
		{ // $if ret_type is int, typecheck start, 1 == 7 => void == int => false 
		} // typecheck end
		println(_STR("%"PRId32"", 1, method.name.len));
		println(_STR("%p", 1, method.name.str));
		println(_STR("Method: %.*s", 1, method.name));
		println(_STR("Attributes: %.*s", 1, array_string_str(  method.attrs)));
		println(_STR("Return type: %.*s", 1, method.ret_type));
	
		// method 1
		method.name = tos_lit("method_two");
		method.attrs = new_array_from_c_array(0, 0, sizeof(string), _MOV((string[0]){}));
		method.ret_type = tos_lit("void");
		method.type = 1;
		{ // $if ret_type is int, typecheck start, 1 == 7 => void == int => false 
		...
		// method 2
		method.name = tos_lit("method_three");
		method.attrs = new_array_from_c_array(0, 0, sizeof(string), _MOV((string[0]){}));
		method.ret_type = tos_lit("int");
		method.type = 7;
		{ // $if ret_type is int, typecheck start, 7 == 7 => int == int => true 
			println(tos_lit("hi"));
		} // typecheck end
		println(_STR("%"PRId32"", 1, method.name.len));
		println(_STR("%p", 1, method.name.str));
		...
		// method 3
		method.name = tos_lit("method_four");
		method.attrs = new_array_from_c_array(0, 0, sizeof(string), _MOV((string[0]){}));
		method.ret_type = tos_lit("int");
		method.type = 7;
		{ // $if ret_type is int, typecheck start, 7 == 7 => int == int => true 
	...
	{ // 2comptime: $for field in main.App(fields) {
		FieldData field;
		memset(&field, 0, sizeof(FieldData));
		// field 0
		field.name = tos_lit("a");
		field.attrs = new_array_from_c_array(0, 0, sizeof(string), _MOV((string[0]){}));
		field.typ = tos_lit("string");
		field.type = 18;
		field.is_pub = false;
		field.is_mut = true;
		{ // $if ret_type is string, typecheck start, 18 == 18 => string == string => true 
			println(FieldData_str(field));
		} // typecheck end
	
		// field 1
		field.name = tos_lit("test");
		field.attrs = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){tos_lit("test")}));
		field.typ = tos_lit("string");
		field.type = 18;
		field.is_pub = false;
		field.is_mut = false;
