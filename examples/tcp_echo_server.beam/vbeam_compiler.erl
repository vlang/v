...
main(["--batch-direct", BatchFile, OutDir]) ->
    %% Direct BEAM assembly mode (Tier 4): same ETF input as --batch-etf,
    %% but compiles via vbeam_beam_direct (Core Erlang -> BEAM assembly ->
    %% compile:noenv_forms([from_asm])), bypassing Erlang compiler's
    %% Core Erlang optimization passes.
    %% Add OutDir to code path so vbeam_beam_direct.beam is loadable
    code:add_path(OutDir),
    case file:read_file(BatchFile) of
        {ok, Bin} ->
            Modules = split_batch(binary_to_list(Bin)),
            {OkCount, ErrCount, Errors} = compile_direct_batch(Modules, OutDir),
            case Errors of
                [] -> ok;
                _ ->
                    lists:foreach(fun({N, E}) ->
...
%% Compile a list of Erlang term text modules via direct BEAM assembly (Tier 4)
compile_direct_batch(Modules, OutDir) ->
    Results = lists:map(fun({N, Text}) ->
        try compile_direct_one(Text, OutDir) of
            {ok, _Mod, _Out} -> {ok, N};
            {error, Reason} -> {error, N, Reason}
        catch
            error:Reason ->
                {error, N, {direct_crash, Reason}};
            throw:Reason ->
                {error, N, {direct_throw, Reason}}
        end
    end, lists:zip(lists:seq(1, length(Modules)), Modules)),
    Errors = [{N, E} || {error, N, E} <- Results],
    OkCount = length([ok || {ok, _} <- Results]),
...
compile_direct_one(Text, OutDir) ->
    case parse_etf_text(Text) of
        {ok, CoreMod} ->
            case vbeam_beam_direct:compile(CoreMod) of
                {ok, ModName, Binary} ->
                    write_beam(ModName, Binary, OutDir);
                {ok, ModName, Binary, _Warnings} ->
                    write_beam(ModName, Binary, OutDir);
                {error, Errors, _Warnings} ->
                    %% Tier 4 failed — fall back to Tier 3 (Core Erlang → compile:noenv_forms)
                    ModName = cerl:concrete(cerl:module_name(CoreMod)),
                    AllErrs = lists:flatten([Es || {_, Es} <- Errors]),
                    io:format(standard_error, "NOTE: ~p fell back to Tier 3 (~p errors)~n",
                              [ModName, length(AllErrs)]),
                    lists:foreach(fun({_Line, Mod, Detail}) ->
                        io:format(standard_error, "  ~p: ~p~n", [Mod, Detail])
                    end, lists:sublist(AllErrs, 5)),
                    compile_core_forms(CoreMod, OutDir);
                OtherError ->
                    ModName2 = cerl:concrete(cerl:module_name(CoreMod)),
                    io:format(standard_error, "NOTE: ~p fell back to Tier 3 (error: ~p)~n",
                              [ModName2, OtherError]),
                    compile_core_forms(CoreMod, OutDir)
            end;
        {error, Reason} ->
