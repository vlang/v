...
VV_LOCAL_SYMBOL void main__mysql(void) {
	...
	Option_mysql__Result _t164 = mysql__Connection_query(&conn, _SLIT("create table if not exists Module (id SERIAL, name TEXT NOT NULL, nr_downloads INT DEFAULT 0, creator INT DEFAULT 0, PRIMARY KEY(`id`));"));
	if (_t164.state != 0) { /*or block*/ 
		IError err = _t164.err;
		v_panic(IError_str(err));
	};
	Option_mysql__Result _t165 = mysql__Connection_query(&conn, _SLIT("create table if not exists User (id SERIAL, age INT DEFAULT 0, name TEXT NOT NULL, is_customer INT DEFAULT 0, PRIMARY KEY(`id`));"));
