	...
	#define V_CURRENT_COMMIT_HASH "2e50d3c"
#endif
...
VV_LOCAL_SYMBOL void map_set_1(map* m, voidptr key, voidptr value) {
	...
	multi_return_u32_u32 mr_12726 = map_key_to_index(m, key);
	u32 index = mr_12726.arg0;
	u32 meta = mr_12726.arg1;
	multi_return_u32_u32 mr_12762 = map_meta_less(m, index, meta);
	index = mr_12762.arg0;
	meta = mr_12762.arg1;
	for (;;) {
		if (!(meta == m->metas[index])) break;
...
VV_LOCAL_SYMBOL void map_rehash(map* m) {
	...
	for (int i = 0; i < m->key_values.len; i++) {
		...
		multi_return_u32_u32 mr_14427 = map_key_to_index(m, pkey);
		u32 index = mr_14427.arg0;
		u32 meta = mr_14427.arg1;
		multi_return_u32_u32 mr_14465 = map_meta_less(m, index, meta);
		index = mr_14465.arg0;
		meta = mr_14465.arg1;
...
VV_LOCAL_SYMBOL void map_cached_rehash(map* m, u32 old_cap) {
	...
	for (u32 i = ((u32)(0U)); i <= old_cap + old_extra_metas; i += 2) {
		...
		multi_return_u32_u32 mr_15264 = map_meta_less(m, index, meta);
		index = mr_15264.arg0;
		meta = mr_15264.arg1;
...
VV_LOCAL_SYMBOL voidptr map_get_and_set_1(map* m, voidptr key, voidptr zero) {
	for (;;) {
		multi_return_u32_u32 mr_15714 = map_key_to_index(m, key);
		u32 index = mr_15714.arg0;
		u32 meta = mr_15714.arg1;
		for (;;) {
			if (meta == m->metas[index]) {
				int kv_index = ((int)(m->metas[index + 1]));
				voidptr pkey = DenseArray_key(&m->key_values, kv_index);
...
VV_LOCAL_SYMBOL voidptr map_get_1(map* m, voidptr key, voidptr zero) {
	multi_return_u32_u32 mr_16457 = map_key_to_index(m, key);
	u32 index = mr_16457.arg0;
	u32 meta = mr_16457.arg1;
	for (;;) {
		if (meta == m->metas[index]) {
			int kv_index = ((int)(m->metas[index + 1]));
			voidptr pkey = DenseArray_key(&m->key_values, kv_index);
...
VV_LOCAL_SYMBOL voidptr map_get_1_check(map* m, voidptr key) {
	multi_return_u32_u32 mr_17126 = map_key_to_index(m, key);
	u32 index = mr_17126.arg0;
	u32 meta = mr_17126.arg1;
	for (;;) {
		if (meta == m->metas[index]) {
			int kv_index = ((int)(m->metas[index + 1]));
			voidptr pkey = DenseArray_key(&m->key_values, kv_index);
...
VV_LOCAL_SYMBOL bool map_exists_1(map* m, voidptr key) {
	multi_return_u32_u32 mr_17640 = map_key_to_index(m, key);
	u32 index = mr_17640.arg0;
	u32 meta = mr_17640.arg1;
	for (;;) {
		if (meta == m->metas[index]) {
			int kv_index = ((int)(m->metas[index + 1]));
			voidptr pkey = DenseArray_key(&m->key_values, kv_index);
...
// Attr: [unsafe]
void map_delete(map* m, voidptr key) {
	multi_return_u32_u32 mr_18269 = map_key_to_index(m, key);
	u32 index = mr_18269.arg0;
	u32 meta = mr_18269.arg1;
	multi_return_u32_u32 mr_18305 = map_meta_less(m, index, meta);
	index = mr_18305.arg0;
	meta = mr_18305.arg1;
	for (;;) {
		if (!(meta == m->metas[index])) break;
...
VV_LOCAL_SYMBOL void main__main(void) {
	...
	// sqlite3 table creator
	sqlite__DB_exec(db, _SLIT("CREATE TABLE IF NOT EXISTS `User` (`id` INTEGER PRIMARY KEY, `age` INTEGER, `name` TEXT NOT NULL, `is_customer` INTEGER, `skipped_string` TEXT);"));
	...
	// sql select
	sqlite__DB _t153 = db;
	string _t154 = _SLIT("SELECT `id`, `name`, `nr_downloads`, `creator` FROM `Module` WHERE id = ?1 ORDER BY id ");
	sqlite3_stmt* _t152 = sqlite__DB_init_stmt(_t153, _t154);sqlite3_bind_int(_t152, 1, 1);

	int _t155 = sqlite3_extended_errcode(_t153.conn);
	if (_t155 != SQLITE_OK) { puts(sqlite3_errmsg(_t153.conn)); }
	main__Module _t156 = (main__Module){
	.id = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .nr_downloads = 0, .creator = {0}};
	int _step_res_t156 = sqlite3_step(_t152);
		if (_step_res_t156 == SQLITE_OK || _step_res_t156 == SQLITE_ROW) {
	_t156.id = sqlite3_column_int(_t152, 0);
	byteptr _t157 = sqlite3_column_text(_t152, 1);
	if (_t157 != NULL) {
		_t156.name = tos_clone(_t157);
	}
	_t156.nr_downloads = sqlite3_column_int(_t152, 2);
	//parse struct start
	int _t158 = sqlite3_column_int(_t152, 3);
	
	// sql select
	sqlite__DB _t160 = db;
	string _t161 = _SLIT("SELECT `id`, `age`, `name`, `is_customer` FROM `User` WHERE id = ?1 ORDER BY id ");
	sqlite3_stmt* _t159 = sqlite__DB_init_stmt(_t160, _t161);sqlite3_bind_int(_t159, 1, _t158);

	int _t162 = sqlite3_extended_errcode(_t160.conn);
	if (_t162 != SQLITE_OK) { puts(sqlite3_errmsg(_t160.conn)); }
	main__User _t163 = (main__User){
	.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	int _step_res_t163 = sqlite3_step(_t159);
		if (_step_res_t163 == SQLITE_OK || _step_res_t163 == SQLITE_ROW) {
	_t163.id = sqlite3_column_int(_t159, 0);
	_t163.age = sqlite3_column_int(_t159, 1);
	byteptr _t164 = sqlite3_column_text(_t159, 2);
	if (_t164 != NULL) {
		_t163.name = tos_clone(_t164);
	}
	_t163.is_customer = sqlite3_column_int(_t159, 3);
	}
	sqlite3_finalize(_t159);
		_t156.creator = _t163; 
	//parse struct end
	}
	sqlite3_finalize(_t152);
		main__Module modul =  _t156; 
	;
	main__mysql();
}

VV_LOCAL_SYMBOL void main__mysql(void) {
	mysql__Connection conn = (mysql__Connection){.conn = mysql_init(0),.host = _SLIT("localhost"),.port = 3306,.username = _SLIT("root"),.password = _SLIT("abc"),.dbname = _SLIT("test"),.flag = 0,};
	Option_bool _t165 = mysql__Connection_connect(&conn);
	...
	// mysql table creator
	// mysql table creator
	Option_mysql__Result _t166 = mysql__Connection_query(&conn, _SLIT("CREATE TABLE IF NOT EXISTS `User` (`id` SERIAL, `age` INT, `name` TEXT NOT NULL, `is_customer` TINYINT, `skipped_string` TEXT, PRIMARY KEY(`id`));"));
	if (_t166.state != 0) { IError err = _t166.err; _STR("Something went wrong\000%.*s", 2, IError_str(err)); }
	Option_mysql__Result _t167 = mysql__Connection_query(&conn, _SLIT("CREATE TABLE IF NOT EXISTS `Module` (`id` SERIAL, `name` TEXT, `nr_downloads` BIGINT, `creator` INT, PRIMARY KEY(`id`));"));
	if (_t167.state != 0) { IError err = _t167.err; _STR("Something went wrong\000%.*s", 2, IError_str(err)); }
	main__Module mod = (main__Module){.id = 0,.name = _SLIT("test"),.nr_downloads = 10,.creator = (main__User){.id = 0,.age = 21,.name = _SLIT("VUser"),.is_customer = true,.skipped_string = (string){.str=(byteptr)"", .is_lit=1},},};
	
	//mysql insert
	mysql__Connection _t168 = conn;
	string _t170 = _SLIT("INSERT INTO `Module` (`name`, `nr_downloads`, `creator`) values (?, ?, ?)");
	MYSQL_STMT* _t169 = mysql_stmt_init(_t168.conn);
	mysql_stmt_prepare(_t169, _t170.str, _t170.len);
	MYSQL_BIND _t171[3];
	memset(_t171, 0, sizeof(MYSQL_BIND)*3);
	//name (18)
	_t171[0].buffer_type = MYSQL_TYPE_STRING;
	_t171[0].buffer = (char*) &mod.name;
	_t171[0].is_null = 0;
	_t171[0].length = 0;
	//nr_downloads (7)
	_t171[1].buffer_type = MYSQL_TYPE_LONGLONG;
	_t171[1].buffer = (u64*) &mod.nr_downloads;
	_t171[1].is_null = 0;
	_t171[1].length = 0;
	//creator (71)
	
	//mysql insert
	mysql__Connection _t172 = conn;
	string _t174 = _SLIT("INSERT INTO `User` (`age`, `name`, `is_customer`) values (?, ?, ?)");
	MYSQL_STMT* _t173 = mysql_stmt_init(_t172.conn);
	mysql_stmt_prepare(_t173, _t174.str, _t174.len);
	MYSQL_BIND _t175[3];
	memset(_t175, 0, sizeof(MYSQL_BIND)*3);
	//age (7)
	_t175[0].buffer_type = MYSQL_TYPE_LONG;
	_t175[0].buffer = (int*) &mod.creator.age;
	_t175[0].is_null = 0;
	_t175[0].length = 0;
	//name (18)
	_t175[1].buffer_type = MYSQL_TYPE_STRING;
	_t175[1].buffer = (char*) &mod.creator.name;
	_t175[1].is_null = 0;
	_t175[1].length = 0;
	//is_customer (16)
	_t175[2].buffer_type = MYSQL_TYPE_TINY;
	_t175[2].buffer = (bool*) &mod.creator.is_customer;
	_t175[2].is_null = 0;
	_t175[2].length = 0;
	
	int _t176 = mysql_stmt_bind_param(_t173, _t175);
	if (_t176 != 0) { puts(mysql_error(_t172.conn)); }
	_t176 = mysql_stmt_execute(_t173);
	if (_t176 != 0) { puts(mysql_error(_t172.conn)); puts(mysql_stmt_error(_t173)); }
	mysql_stmt_close(_t173);
	mysql_stmt_free_result(_t173);
	int _t177_err = mysql_real_query(_t168.conn, "SELECT LAST_INSERT_ID();", 24);
	if (_t177_err != 0) { puts(mysql_error(_t168.conn)); }
	MYSQL_RES* _t177 = mysql_store_result(_t168.conn);
	if (mysql_num_rows(_t177) != 1) { puts("Something went wrong"); }
	MYSQL_ROW _t177_row = mysql_fetch_row(_t177);
	mod.creator.id = string_int(tos_clone(_t177_row[0]));
	mysql_free_result(_t177);
	_t171[2].buffer_type = MYSQL_TYPE_LONG;
	_t171[2].buffer = &mod.creator.id;
	_t171[2].is_null = 0;
	_t171[2].length = 0;
	
	int _t178 = mysql_stmt_bind_param(_t169, _t171);
	if (_t178 != 0) { puts(mysql_error(_t168.conn)); }
	_t178 = mysql_stmt_execute(_t169);
	if (_t178 != 0) { puts(mysql_error(_t168.conn)); puts(mysql_stmt_error(_t169)); }
	mysql_stmt_close(_t169);
	mysql_stmt_free_result(_t169);

	// sql select
	mysql__Connection _t181 = conn;
	string _t182 = _SLIT("SELECT `id`, `name`, `nr_downloads`, `creator` FROM `Module` WHERE id = 1 ORDER BY id ");
	Option_mysql__Result _t183 = mysql__Connection_real_query(&_t181, _t182);
	if (_t183.state != 0) { IError err = _t183.err; _STR("Something went wrong\000%.*s", 2, IError_str(err)); }
	Array_mysql__Row _t183_rows = mysql__Result_rows(*(mysql__Result*)_t183.data);
	mysql__Result_free((mysql__Result*)&_t183.data);
	int _t185 = 0;
	main__Module _t184 = (main__Module){
	.id = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .nr_downloads = 0, .creator = {0}};
	Array_string _t186 = (*(mysql__Row*)array_get(_t183_rows, _t185)).vals;
	_t184.id = string_int((*(string*) array_get(_t186, 0)));
	_t184.name = (*(string*) array_get(_t186, 1));
	_t184.nr_downloads = string_int((*(string*) array_get(_t186, 2)));
	//parse struct start
	int _t187 = string_int((*(string*) array_get(_t186, 3)));
	
	// sql select
	mysql__Connection _t190 = conn;
	string _t191 = _SLIT("SELECT `id`, `age`, `name`, `is_customer` FROM `User` WHERE id = _t187 ORDER BY id ");
	Option_mysql__Result _t192 = mysql__Connection_real_query(&_t190, _t191);
	if (_t192.state != 0) { IError err = _t192.err; _STR("Something went wrong\000%.*s", 2, IError_str(err)); }
	Array_mysql__Row _t192_rows = mysql__Result_rows(*(mysql__Result*)_t192.data);
	mysql__Result_free((mysql__Result*)&_t192.data);
	int _t194 = 0;
	main__User _t193 = (main__User){
	.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	Array_string _t195 = (*(mysql__Row*)array_get(_t192_rows, _t194)).vals;
	_t193.id = string_int((*(string*) array_get(_t195, 0)));
	_t193.age = string_int((*(string*) array_get(_t195, 1)));
	_t193.name = (*(string*) array_get(_t195, 2));
	_t193.is_customer = string_bool((*(string*) array_get(_t195, 3)));
	string_free(&_t191);
		_t184.creator = _t193; 
	//parse struct end
	string_free(&_t182);
		main__Module modul =  _t184; 
	;
