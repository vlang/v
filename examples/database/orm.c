	...
	#define V_CURRENT_COMMIT_HASH "c83b86a"
#endif
...
VV_LOCAL_SYMBOL void main__mysql(void) {
	...
	string _t182 = _SLIT("SELECT `id`, `age`, `name`, `is_customer` FROM `User` ORDER BY id ");
	int _t183 = mysql_real_query(_t181.conn, _t182.str, _t182.len);
	if (_t183 != 0) { puts(mysql_error(_t181.conn)); }
	MYSQL_RES* _t184 = mysql_store_result(_t181.conn);
	int _t186 = 0;
	Array_main__User _t185_array = __new_array(0, 10, sizeof(main__User));
	for (_t186 = 0; _t186 < mysql_num_rows(_t184); _t186++) {
		main__User _t185 = (main__User) {
	.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	MYSQL_ROW _t187 = mysql_fetch_row(_t184);
	_t185.id = string_int(tos_clone(_t187[0]));
	_t185.age = string_int(tos_clone(_t187[1]));
	_t185.name = tos_clone(_t187[2]);
	_t185.is_customer = string_bool(tos_clone(_t187[3]));
		 array_push(&_t185_array, _MOV((main__User[]) { _t185 }));
	}
	string_free(&_t182);
	mysql_free_result(_t184);
		Array_main__User m =  _t185_array; 
	;
}
