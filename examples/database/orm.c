	...
	#define V_CURRENT_COMMIT_HASH "e05da04"
#endif
...
VV_LOCAL_SYMBOL void map_free_nop(voidptr _t366);
...
VV_LOCAL_SYMBOL string None___str(None__ _t388);
...
Option_void io__cp(io__Writer dst, io__Reader src);
io__Reader io__make_reader(io__Reader r);
int _const_io__read_all_len; // inited later
...
// ^^^ number of types for interface io__ReaderWriter: 0

// Methods wrapper for interface "io__ReaderWriter"

typedef Option_int (*_io__ReaderWriter_read_fn)(void* _, Array_byte* buf);
typedef Option_int (*_io__ReaderWriter_write_fn)(void* _, Array_byte buf);

struct _io__ReaderWriter_interface_methods {
	_io__ReaderWriter_read_fn _method_read;
	_io__ReaderWriter_write_fn _method_write;
};

struct _io__ReaderWriter_interface_methods io__ReaderWriter_name_table[1];



// ^^^ number of types for interface io__Reader: 0

// Methods wrapper for interface "io__Reader"

typedef Option_int (*_io__Reader_read_fn)(void* _, Array_byte* buf);

struct _io__Reader_interface_methods {
	_io__Reader_read_fn _method_read;
};

struct _io__Reader_interface_methods io__Reader_name_table[1];



// ^^^ number of types for interface io__Writer: 0

// Methods wrapper for interface "io__Writer"

typedef Option_int (*_io__Writer_write_fn)(void* _, Array_byte buf);

struct _io__Writer_interface_methods {
	_io__Writer_write_fn _method_write;
};

struct _io__Writer_interface_methods io__Writer_name_table[1];
...
typedef Option_int (*_io__RandomReader_read_from_fn)(void* _, u64 pos, Array_byte* buf);

struct _io__RandomReader_interface_methods {
	_io__RandomReader_read_from_fn _method_read_from;
};
...
typedef Option_int (*_io__RandomWriter_write_to_fn)(void* _, u64 pos, Array_byte buf);

struct _io__RandomWriter_interface_methods {
	_io__RandomWriter_write_to_fn _method_write_to;
};
...
static string indent_main__Module_str(main__Module it, int indent_count) {
	...
	return _STR("Module{\n"		"%.*s\000    id: %d\000\n"
		"%.*s\000    name: '%.*s\000'\n"
		"%.*s\000    nr_downloads: %d\000\n"
		"%.*s\000    creator: %.*s\000\n"
		"%.*s\000}", 10,
		indents, it.id,
		indents, it.name,
		indents, it.nr_downloads,
		indents, indent_main__User_str(it.creator, indent_count + 1),
		indents);
}
...
inline u64 hash__sum64(Array_byte key, u64 seed) {
	u64 _t30 = hash__wyhash64(((byteptr)(key.data)), ((u64)(key.len)), seed);
...
string strconv__f64_to_str_lnd(f64 f, int dec_digit) {
	...
	int exp_sgn = 1;
	int dot_res_sp = -1;
	for (int _t143 = 0; _t143 < s.len; ++_t143) {
		byte c = s.str[_t143];
		if (c == L'-') {
			sgn = -1;
			i++;
	...
	for (int _t145 = 0; _t145 < string_substr(s, i, s.len).len; ++_t145) {
		byte c = string_substr(s, i, s.len).str[_t145];
...
string strconv__f64_to_str_l(f64 f) {
	...
	int exp = 0;
	int exp_sgn = 1;
	for (int _t208 = 0; _t208 < s.len; ++_t208) {
		byte c = s.str[_t208];
		if (c == L'-') {
			sgn = -1;
			i++;
	...
	for (int _t210 = 0; _t210 < string_substr(s, i, s.len).len; ++_t210) {
		byte c = string_substr(s, i, s.len).str[_t210];
		exp = exp * 10 + ((int)(c - L'0'));
	}
	Array_byte res = __new_array_with_default(exp + 32, 0, sizeof(byte), &(byte[]){0});
	int r_i = 0;
...
void Array_string_free(Array_string* a) {
	// FOR IN array
	for (int _t233 = 0; _t233 < a->len; ++_t233) {
		string s = ((string*)a->data)[_t233];
		string_free(&s);
	}
	v_free(a->data);
}
...
string Array_byte_hex(Array_byte b) {
	...
	int dst_i = 0;
	// FOR IN array
	for (int _t235 = 0; _t235 < b.len; ++_t235) {
		byte i = ((byte*)b.data)[_t235];
		byte n0 = i >> 4;
		{ // Unsafe block
...
int Array_int_reduce(Array_int a, int (*iter)(int , int ), int accum_start) {
	...
	// FOR IN array
	for (int _t246 = 0; _t246 < a.len; ++_t246) {
		int i = ((int*)a.data)[_t246];
...
VV_LOCAL_SYMBOL voidptr __as_cast(voidptr obj, int obj_type, int expected_type) {
	if (obj_type != expected_type) {
		...
		// FOR IN array
		for (int _t277 = 0; _t277 < as_cast_type_indexes.len; ++_t277) {
			VCastTypeIndexName x = ((VCastTypeIndexName*)as_cast_type_indexes.data)[_t277];
...
VV_LOCAL_SYMBOL bool print_backtrace_skipping_top_frames_linux(int skipframes) {
	...
	#else
	{
		#if !defined(_VFREESTANDING)
		{
			...
			// FOR IN array
			for (int _t286 = 0; _t286 < sframes.len; ++_t286) {
				string sframe = ((string*)sframes.data)[_t286];
...
string byte_str_escaped(byte b) {
	...
	if (b == (0)) {
		...
		_t347 = byte_ascii_str(b);
	}
	else {
		_t347 = string_add(_SLIT("0x"), byte_hex(b));
	}	string str = _t347;
	return str;
}
...
VV_LOCAL_SYMBOL voidptr map_get_and_set(map* m, voidptr key, voidptr zero) {
	...
	if (!(false)) {
		...
		v_assert_meta_info__t374.fpath = _SLIT("/home/louis/v/vlib/builtin/map.v");
		v_assert_meta_info__t374.line_nr = 559;
		v_assert_meta_info__t374.fn_name = _SLIT("get_and_set");
		v_assert_meta_info__t374.src = _SLIT("false");
		__print_assert_failure(&v_assert_meta_info__t374);
...
Array_string string_split_nth(string s, string delim, int nth) {
	...
	if (delim.len == (0)) {
		i = 1;
		for (int _t470 = 0; _t470 < s.len; ++_t470) {
			byte ch = s.str[_t470];
			if (nth > 0 && i >= nth) {
...
Option_int string_index(string s, string p) {
	...
	Option_int _t491;
	opt_ok(&(int[]) { idx }, (Option*)(&_t491), sizeof(int));
	return _t491;
}
...
int string_index_any(string s, string chars) {
	for (int _t495 = 0; _t495 < chars.len; ++_t495) {
		byte c = chars.str[_t495];
...
Option_int string_last_index(string s, string p) {
	...
	Option_int _t502;
	opt_ok(&(int[]) { idx }, (Option*)(&_t502), sizeof(int));
	return _t502;
}
...
int string_count(string s, string substr) {
	...
	if (substr.len == 1) {
		...
		for (int _t513 = 0; _t513 < s.len; ++_t513) {
			byte letter = s.str[_t513];
			if (letter == target) {
				n++;
			}
		}
...
bool string_contains_any(string s, string chars) {
	for (int _t520 = 0; _t520 < chars.len; ++_t520) {
		byte c = chars.str[_t520];
...
bool string_contains_any_substr(string s, Array_string substrs) {
	...
	// FOR IN array
	for (int _t524 = 0; _t524 < substrs.len; ++_t524) {
		string sub = ((string*)substrs.data)[_t524];
...
string string_title(string s) {
	...
	// FOR IN array
	for (int _t545 = 0; _t545 < words.len; ++_t545) {
		string word = ((string*)words.data)[_t545];
...
bool string_is_title(string s) {
	Array_string words = string_split(s, _SLIT(" "));
	// FOR IN array
	for (int _t548 = 0; _t548 < words.len; ++_t548) {
		string word = ((string*)words.data)[_t548];
...
string string_trim(string s, string cutset) {
	...
	for (;;) {
		...
		cs_match = false;
		for (int _t557 = 0; _t557 < cutset.len; ++_t557) {
			byte cs = cutset.str[_t557];
		...
		for (int _t558 = 0; _t558 < cutset.len; ++_t558) {
			byte cs = cutset.str[_t558];
...
string string_trim_left(string s, string cutset) {
	...
	for (;;) {
		if (!(pos < s.len)) break;
		bool found = false;
		for (int _t562 = 0; _t562 < cutset.len; ++_t562) {
			byte cs = cutset.str[_t562];
...
string Array_string_join(Array_string a, string del) {
	...
	int len = 0;
	// FOR IN array
	for (int _t631 = 0; _t631 < a.len; ++_t631) {
		string val = ((string*)a.data)[_t631];
		len += val.len + del.len;
	}
	len -= del.len;
	string res = (string){.str = v_malloc(len + 1), .len = len};
...
int string_hash(string s) {
	u32 h = ((u32)(0U));
	if (h == 0U && s.len > 0) {
		for (int _t638 = 0; _t638 < s.len; ++_t638) {
			byte c = s.str[_t638];
			h = h * 31U + ((u32)(c));
		}
	}
...
string sqlite__DB_str(sqlite__DB db) {
	string _t679 = string_add(string_add(_SLIT("sqlite.DB{ conn: "), ptr_str(db.conn)), _SLIT(" }"));
	// autofree_scope_vars(pos=569 line_nr=29 scope.pos=546 scope.end_pos=623)
	// var "db" var.pos=547 var.line_nr=28
	// af parent scope:
	// af parent scope:
	return _t679;
}
...
Option_sqlite__DB sqlite__connect(string path) {
	...
	if (sqlite3_open(((char*)(path.str)), &db) != 0) {
		return (Option_sqlite__DB){ .state=2, .err=v_error(_SLIT("sqlite db error")), .data={0} };
	}
	Option_sqlite__DB _t681;
	opt_ok(&(sqlite__DB[]) { (sqlite__DB){.is_open = true,.conn = db,} }, (Option*)(&_t681), sizeof(sqlite__DB));
	// autofree_scope_vars(pos=1487 line_nr=73 scope.pos=1354 scope.end_pos=1529)
	// var "path" var.pos=1362 var.line_nr=68
	// var "db" var.pos=1382 var.line_nr=69
	// af parent scope:
	// af parent scope:
	return _t681;
}
...
Option_bool sqlite__DB_close(sqlite__DB* db) {
	...
	if (code == 0) {
		...
		return (Option_bool){ .state=2, .err=v_error(_STR("sqlite db error: failed to close with code: %"PRId32"", 1, code)), .data={0} };
	}
	Option_bool _t683;
	opt_ok(&(bool[]) { true }, (Option*)(&_t683), sizeof(bool));
	// autofree_scope_vars(pos=1858 line_nr=89 scope.pos=1678 scope.end_pos=1894)
	// var "db" var.pos=1683 var.line_nr=82
	// var "code" var.pos=1707 var.line_nr=83
	// af parent scope:
	// af parent scope:
	return _t683;
}
...
VV_LOCAL_SYMBOL struct sqlite3_stmt* sqlite__DB_init_stmt(sqlite__DB db, string query) {
	...
	// autofree_scope_vars(pos=2066 line_nr=96 scope.pos=1918 scope.end_pos=2079)
	// var "db" var.pos=1919 var.line_nr=93
	// var "query" var.pos=1936 var.line_nr=93
	// var "stmt" var.pos=1969 var.line_nr=94
	// af parent scope:
	// af parent scope:
	return stmt;
}
...
VV_LOCAL_SYMBOL int sqlite__get_int_from_stmt(struct sqlite3_stmt* stmt) {
	...
	// autofree_scope_vars(pos=2318 line_nr=107 scope.pos=2103 scope.end_pos=2330)
	// var "stmt" var.pos=2121 var.line_nr=100
	// var "x" var.pos=2150 var.line_nr=101
	// var "res" var.pos=2255 var.line_nr=105
	// af parent scope:
	// af parent scope:
	return res;
}
...
int sqlite__DB_q_int(sqlite__DB db, string query) {
	...
	int res = sqlite3_column_int(stmt, 0);
	sqlite3_finalize(stmt);
	// autofree_scope_vars(pos=2599 line_nr=117 scope.pos=2381 scope.end_pos=2611)
	// var "db" var.pos=2382 var.line_nr=111
	// var "query" var.pos=2395 var.line_nr=111
	// var "stmt" var.pos=2416 var.line_nr=112
	// var "res" var.pos=2536 var.line_nr=115
	// af parent scope:
	// af parent scope:
	return res;
}
...
string sqlite__DB_q_string(sqlite__DB db, string query) {
	...
	sqlite3_prepare_v2(db.conn, ((char*)(query.str)), query.len, &stmt, 0);
	sqlite3_step(stmt);
	string res = tos_clone(((byte*)(sqlite3_column_text(stmt, 0))));
	sqlite3_finalize(stmt);
	// autofree_scope_vars(pos=2919 line_nr=127 scope.pos=2665 scope.end_pos=2931)
	// var "db" var.pos=2666 var.line_nr=121
	// var "query" var.pos=2682 var.line_nr=121
	// var "stmt" var.pos=2706 var.line_nr=122
	// var "res" var.pos=2826 var.line_nr=125
	// af parent scope:
	// af parent scope:
	return res;
}
...
multi_return_Array_sqlite__Row_int sqlite__DB_exec(sqlite__DB db, string query) {
	...
	for (;;) {
		...
		for (int i = 0; i < nr_cols; ++i) {
			string val = tos_clone(((byte*)(sqlite3_column_text(stmt, i))));
			array_push((array*)&row.vals, _MOV((string[]){ string_clone(val) }));
		}
...
Option_sqlite__Row sqlite__DB_exec_one(sqlite__DB db, string query) {
	multi_return_Array_sqlite__Row_int mr_3804 = sqlite__DB_exec(db, query);
	Array_sqlite__Row rows = mr_3804.arg0;
	int code = mr_3804.arg1;
	if (rows.len == 0 || code != 101) {
		return (Option_sqlite__Row){ .state=2, .err=v_error(_STR("SQL Error: Rows #%"PRId32"\000 Return code %"PRId32"", 2, rows.len, code)), .data={0} };
	}
	Option_sqlite__Row _t692;
	opt_ok(&(sqlite__Row[]) { (*(sqlite__Row*)/*ee elem_typ */array_get(rows, 0)) }, (Option*)(&_t692), sizeof(sqlite__Row));
	// autofree_scope_vars(pos=3923 line_nr=163 scope.pos=3754 scope.end_pos=3939)
	// var "db" var.pos=3755 var.line_nr=158
	// var "query" var.pos=3771 var.line_nr=158
	// var "rows" var.pos=3793 var.line_nr=159
	// var "code" var.pos=3799 var.line_nr=159
	// af parent scope:
	// af parent scope:
	return _t692;
}
...
int sqlite__DB_exec_none(sqlite__DB db, string query) {
	multi_return_Array_sqlite__Row_int mr_4102 = sqlite__DB_exec(db, query);
	int code = mr_4102.arg1;
	// autofree_scope_vars(pos=4120 line_nr=170 scope.pos=4055 scope.end_pos=4133)
	// var "db" var.pos=4056 var.line_nr=168
	// var "query" var.pos=4073 var.line_nr=168
	// var "code" var.pos=4097 var.line_nr=169
	// af parent scope:
	// af parent scope:
	return code;
}
...
Option_bool mysql__Connection_connect(mysql__Connection* conn) {
	...
	Option_bool _t696;
	opt_ok(&(bool[]) { true }, (Option*)(&_t696), sizeof(bool));
	// autofree_scope_vars(pos=1500 line_nr=42 scope.pos=1184 scope.end_pos=1513)
	// var "conn" var.pos=1189 var.line_nr=35
	// var "instance" var.pos=1225 var.line_nr=36
	// af parent scope:
	// af parent scope:
	return _t696;
}
...
Option_mysql__Result mysql__Connection_query(mysql__Connection* conn, string q) {
	...
	Option_mysql__Result _t698;
	opt_ok(&(mysql__Result[]) { (mysql__Result){.result = res,} }, (Option*)(&_t698), sizeof(mysql__Result));
	// autofree_scope_vars(pos=1884 line_nr=53 scope.pos=1677 scope.end_pos=1904)
	// var "conn" var.pos=1682 var.line_nr=48
	// var "q" var.pos=1705 var.line_nr=48
	// var "res" var.pos=1845 var.line_nr=52
	// af parent scope:
	// af parent scope:
	return _t698;
}
...
Option_mysql__Result mysql__Connection_real_query(mysql__Connection* conn, string q) {
	...
	MYSQL_RES* res = mysql_store_result(conn->conn);
	Option_mysql__Result _t700;
	opt_ok(&(mysql__Result[]) { (mysql__Result){.result = res,} }, (Option*)(&_t700), sizeof(mysql__Result));
	// autofree_scope_vars(pos=2440 line_nr=66 scope.pos=2216 scope.end_pos=2460)
	// var "conn" var.pos=2221 var.line_nr=61
	// var "q" var.pos=2249 var.line_nr=61
	// var "res" var.pos=2401 var.line_nr=65
	// af parent scope:
	// af parent scope:
	return _t700;
}
...
Option_bool mysql__Connection_select_db(mysql__Connection* conn, string dbname) {
	...
	Option_bool _t702;
	opt_ok(&(bool[]) { true }, (Option*)(&_t702), sizeof(bool));
	// autofree_scope_vars(pos=2718 line_nr=74 scope.pos=2535 scope.end_pos=2731)
	// var "conn" var.pos=2540 var.line_nr=70
	// var "dbname" var.pos=2567 var.line_nr=70
	// af parent scope:
	// af parent scope:
	return _t702;
}
...
Option_bool mysql__Connection_change_user(mysql__Connection* conn, string username, string password, string dbname) {
	...
	Option_bool _t704;
	opt_ok(&(bool[]) { ret }, (Option*)(&_t704), sizeof(bool));
	// autofree_scope_vars(pos=3327 line_nr=90 scope.pos=2951 scope.end_pos=3339)
	// var "conn" var.pos=2956 var.line_nr=80
	// var "username" var.pos=2985 var.line_nr=80
	// var "password" var.pos=3002 var.line_nr=80
	// var "dbname" var.pos=3019 var.line_nr=80
	// var "ret" var.pos=3047 var.line_nr=81
	// af parent scope:
	// af parent scope:
	return _t704;
}
...
u64 mysql__Connection_affected_rows(mysql__Connection* conn) {
	u64 _t705 = mysql_affected_rows(conn->conn);
...
Option_Array_string mysql__Connection_tables(mysql__Connection* conn, string wildcard) {
	...
	// FOR IN array
	Array_mysql__Row _t707 = mysql__Result_rows(res);
	for (int _t708 = 0; _t708 < _t707.len; ++_t708) {
		mysql__Row row = ((mysql__Row*)_t707.data)[_t708];
		array_push((array*)&tables, _MOV((string[]){ string_clone((*(string*)/*ee elem_typ */array_get(row.vals, 0))) }));
	}
	Option_Array_string _t710;
	opt_ok(&(Array_string[]) { tables }, (Option*)(&_t710), sizeof(Array_string));
	// autofree_scope_vars(pos=4473 line_nr=120 scope.pos=4171 scope.end_pos=4488)
	// var "conn" var.pos=4172 var.line_nr=110
	// var "wildcard" var.pos=4197 var.line_nr=110
	// var "cres" var.pos=4227 var.line_nr=111
	// var "res" var.pos=4375 var.line_nr=115
	// var "tables" var.pos=4400 var.line_nr=116
	// af parent scope:
	// af parent scope:
	return _t710;
}
...
Option_voidptr mysql__Connection_get_option(mysql__Connection* conn, int option_type) {
	...
	Option_voidptr _t713;
	opt_ok(&(voidptr[]) { ret }, (Option*)(&_t713), sizeof(voidptr));
	// autofree_scope_vars(pos=5601 line_nr=148 scope.pos=5388 scope.end_pos=5613)
	// var "conn" var.pos=5389 var.line_nr=143
	// var "option_type" var.pos=5418 var.line_nr=143
	// var "ret" var.pos=5447 var.line_nr=144
	// af parent scope:
	// af parent scope:
	return _t713;
}
...
Option_bool mysql__Connection_refresh(mysql__Connection* conn, u32 options) {
	...
	Option_bool _t715;
	opt_ok(&(bool[]) { true }, (Option*)(&_t715), sizeof(bool));
	// autofree_scope_vars(pos=5937 line_nr=157 scope.pos=5763 scope.end_pos=5950)
	// var "conn" var.pos=5768 var.line_nr=153
	// var "options" var.pos=5793 var.line_nr=153
	// af parent scope:
	// af parent scope:
	return _t715;
}
...
Option_bool mysql__Connection_reset(mysql__Connection* conn) {
	...
	Option_bool _t717;
	opt_ok(&(bool[]) { true }, (Option*)(&_t717), sizeof(bool));
	// autofree_scope_vars(pos=6184 line_nr=165 scope.pos=6023 scope.end_pos=6197)
	// var "conn" var.pos=6028 var.line_nr=161
	// af parent scope:
	// af parent scope:
	return _t717;
}
...
Option_bool mysql__Connection_ping(mysql__Connection* conn) {
	...
	Option_bool _t719;
	opt_ok(&(bool[]) { true }, (Option*)(&_t719), sizeof(bool));
	// autofree_scope_vars(pos=6451 line_nr=174 scope.pos=6303 scope.end_pos=6464)
	// var "conn" var.pos=6308 var.line_nr=170
	// af parent scope:
	// af parent scope:
	return _t719;
}
...
string mysql__Connection_info(mysql__Connection* conn) {
	string _t720 = mysql__resolve_nil_str(mysql_info(conn->conn));
...
string mysql__Connection_get_host_info(mysql__Connection* conn) {
	string _t721 = byteptr_vstring(mysql_get_host_info(conn->conn));
...
string mysql__Connection_get_server_info(mysql__Connection* conn) {
	string _t722 = byteptr_vstring(mysql_get_server_info(conn->conn));
...
u64 mysql__Connection_get_server_version(mysql__Connection* conn) {
	u64 _t723 = mysql_get_server_version(conn->conn);
...
Option_bool mysql__Connection_dump_debug_info(mysql__Connection* conn) {
	...
	Option_bool _t725;
	opt_ok(&(bool[]) { true }, (Option*)(&_t725), sizeof(bool));
	// autofree_scope_vars(pos=7983 line_nr=214 scope.pos=7813 scope.end_pos=7996)
	// var "conn" var.pos=7818 var.line_nr=210
	// af parent scope:
	// af parent scope:
	return _t725;
}
...
string mysql__get_client_info(void) {
	string _t726 = byteptr_vstring(mysql_get_client_info());
...
u64 mysql__get_client_version(void) {
	u64 _t727 = mysql_get_client_version();
...
byteptr* mysql__Result_fetch_row(mysql__Result r) {
	byteptr* _t728 = mysql_fetch_row(r.result);
...
u64 mysql__Result_n_rows(mysql__Result r) {
	u64 _t729 = mysql_num_rows(r.result);
...
int mysql__Result_n_fields(mysql__Result r) {
	int _t730 = mysql_num_fields(r.result);
	...
	for (byteptr* rr = mysql__Result_fetch_row(r); rr; rr = mysql__Result_fetch_row(r)) {
		...
		for (int i = 0; i < nr_cols; ++i) {
			if (rr[i] == 0) {
				...
				array_push((array*)&row.vals, _MOV((string[]){ string_clone(mysql__mystring(((byteptr)(rr[i])))) }));
			}
		}
...
VV_LOCAL_SYMBOL string mysql__get_error_msg(MYSQL* conn) {
	string _t740 = byteptr_vstring(mysql_error(conn));
...
VV_LOCAL_SYMBOL string mysql__resolve_nil_str(byteptr ptr) {
	...
	string _t743 = byteptr_vstring(ptr);
...
inline VV_LOCAL_SYMBOL string mysql__mystring(byteptr b) {
	{ // Unsafe block
		string _t744 = byteptr_vstring(b);
...
io__BufferedReader* io__new_buffered_reader(io__BufferedReaderConfig o) {
	if (o.cap <= 0) {
		v_panic(_SLIT("new_buffered_reader should be called with a positive `cap`"));
	}
	...
	// autofree_scope_vars(pos=1023 line_nr=34 scope.pos=745 scope.end_pos=1033)
	// var "o" var.pos=765 var.line_nr=23
	// var "r" var.pos=909 var.line_nr=28
	// af parent scope:
	// af parent scope:
	return r;
}
...
Option_int io__BufferedReader_read(io__BufferedReader* r, Array_byte* buf) {
	...
	r->offset += read;
	Option_int _t749;
	opt_ok(&(int[]) { read }, (Option*)(&_t749), sizeof(int));
	// autofree_scope_vars(pos=1403 line_nr=54 scope.pos=1080 scope.end_pos=1416)
	// var "r" var.pos=1085 var.line_nr=38
	// var "buf" var.pos=1112 var.line_nr=38
	// var "read" var.pos=1310 var.line_nr=49
	// af parent scope:
	// af parent scope:
	return _t749;
}
...
VV_LOCAL_SYMBOL bool io__BufferedReader_fill_buffer(io__BufferedReader* r) {
	if (r->end_of_stream) {
		bool _t750 = true;
		// autofree_scope_vars(pos=1728 line_nr=69 scope.pos=1649 scope.end_pos=1742)
		// af parent scope:
		// var "r" var.pos=1590 var.line_nr=65
		// af parent scope:
		// af parent scope:
		return _t750;
	}
	r->offset = 0;
	r->len = 0;
	Option_int _t751 = io__Reader_name_table[r->reader._typ]._method_read(r->reader._object, &/*111*/(array[]){r->buf}[0]);
	if (_t751.state != 0) { /*or block*/ 
		IError err = _t751.err;
		r->end_of_stream = true;
		bool _t752 = false;
		// autofree_scope_vars(pos=1865 line_nr=76 scope.pos=1806 scope.end_pos=1880)
		// var "err" var.pos=1806 var.line_nr=73
		// af parent scope:
		// var "r" var.pos=1590 var.line_nr=65
		// af parent scope:
		// af parent scope:
	...
	if (r->fails >= r->mfails) {
		r->end_of_stream = true;
		bool _t753 = false;
		// autofree_scope_vars(pos=2121 line_nr=87 scope.pos=1962 scope.end_pos=2136)
		// af parent scope:
		// var "r" var.pos=1590 var.line_nr=65
		// af parent scope:
		// af parent scope:
	...
	bool _t754 = true;
	// autofree_scope_vars(pos=2159 line_nr=90 scope.pos=1585 scope.end_pos=2172)
	// var "r" var.pos=1590 var.line_nr=65
	// af parent scope:
	// af parent scope:
	return _t754;
...
VV_LOCAL_SYMBOL bool io__BufferedReader_needs_fill(io__BufferedReader r) {
	bool _t755 = r.offset >= r.len;
	// autofree_scope_vars(pos=2274 line_nr=95 scope.pos=2235 scope.end_pos=2300)
	// var "r" var.pos=2236 var.line_nr=94
	// af parent scope:
	// af parent scope:
	return _t755;
}
...
bool io__BufferedReader_end_of_stream(io__BufferedReader r) {
	bool _t756 = r.end_of_stream;
	// autofree_scope_vars(pos=2419 line_nr=100 scope.pos=2377 scope.end_pos=2443)
	// var "r" var.pos=2378 var.line_nr=99
	// af parent scope:
	// af parent scope:
	return _t756;
}
	...
	for (;;) {
		if (io__BufferedReader_needs_fill(/*rec*/*r)) {
			if (!io__BufferedReader_fill_buffer(r)) {
				...
				Option_string _t759;
				opt_ok(&(string[]) { Array_byte_bytestr(line) }, (Option*)(&_t759), sizeof(string));
				// autofree_scope_vars(pos=2914 line_nr=120 scope.pos=2783 scope.end_pos=2940)
				// af parent scope:
				// af parent scope:
				// var "i" var.pos=2990 var.line_nr=124
				// af parent scope:
				// var "r" var.pos=2601 var.line_nr=106
				// var "line" var.pos=2685 var.line_nr=110
				// af parent scope:
				// af parent scope:
				return _t759;
			}
		...
		for (; i < r->len; i++) {
			...
			if (c == L'\n') {
				if (i != 0 && (*(byte*)/*ee elem_typ */array_get(r->buf, i - 1)) == L'\r') {
					int x = i - 1;
					_PUSH_MANY(&line, (array_slice(r->buf, r->offset, x)), _t760, Array_byte);
				} else {
					_PUSH_MANY(&line, (array_slice(r->buf, r->offset, i)), _t761, Array_byte);
				}
				r->offset = i + 1;
				Option_string _t762;
				opt_ok(&(string[]) { Array_byte_bytestr(line) }, (Option*)(&_t762), sizeof(string));
				// autofree_scope_vars(pos=3317 line_nr=137 scope.pos=3062 scope.end_pos=3343)
				// af parent scope:
				// var "c" var.pos=3032 var.line_nr=126
				// af parent scope:
				// var "i" var.pos=2990 var.line_nr=124
				// af parent scope:
				// var "r" var.pos=2601 var.line_nr=106
		...
		_PUSH_MANY(&line, (array_slice(r->buf, r->offset, i)), _t763, Array_byte);
		r->offset = i;
	}
	return (Option_string){ .state=2, .err=_const_none__, .data={0} };
}
...
Option_void io__cp(io__Writer dst, io__Reader src) {
	...
	for (;;) {
		Option_int _t765 = io__Reader_name_table[src._typ]._method_read(src._object, &/*arr*/buf);
		if (_t765.state != 0) { /*or block*/ 
			IError err = _t765.err;
			break;
		}
 		int len =  *(int*)_t765.data;
		Option_int _t766 = io__Writer_name_table[dst._typ]._method_write(dst._object, array_slice(buf, 0, len));
		if (_t766.state != 0) { /*or block*/ 
			IError err = _t766.err;
...
io__Reader io__make_reader(io__Reader r) {
	// autofree_scope_vars(pos=523 line_nr=16 scope.pos=492 scope.end_pos=533)
	// var "r" var.pos=504 var.line_nr=15
	// af parent scope:
	// af parent scope:
	return r;
}
...
Option_Array_byte io__read_all(io__ReadAllConfig config) {
	...
	int read = 0;
	for (;;) {
		Option_int _t769 = io__Reader_name_table[r._typ]._method_read(r._object, &/*111*/(array[]){array_slice(b, read, b.len)}[0]);
		if (_t769.state != 0) { /*or block*/ 
			IError err = _t769.err;
			break;
		}
 		int new_read =  *(int*)_t769.data;
		read += new_read;
	...
	Option_Array_byte _t770;
	opt_ok(&(Array_byte[]) { array_slice(b, 0, read) }, (Option*)(&_t770), sizeof(Array_byte));
	// autofree_scope_vars(pos=1284 line_nr=49 scope.pos=923 scope.end_pos=1302)
	// var "config" var.pos=932 var.line_nr=33
	// var "r" var.pos=965 var.line_nr=34
	// var "read_till_eof" var.pos=985 var.line_nr=35
	// var "b" var.pos=1037 var.line_nr=37
	// var "read" var.pos=1076 var.line_nr=38
	// af parent scope:
	// af parent scope:
	return _t770;
}
...
Option_Array_byte io__read_any(io__Reader r) {
	...
	int read = 0;
	for (;;) {
		Option_int _t771 = io__Reader_name_table[r._typ]._method_read(r._object, &/*111*/(array[]){array_slice(b, read, b.len)}[0]);
		if (_t771.state != 0) { /*or block*/ 
			IError err = _t771.err;
			break;
		}
 		int new_read =  *(int*)_t771.data;
		read += new_read;
		if (new_read == 0) {
			break;
		}
	...
	Option_Array_byte _t772;
	opt_ok(&(Array_byte[]) { array_slice(b, 0, read) }, (Option*)(&_t772), sizeof(Array_byte));
	// autofree_scope_vars(pos=1680 line_nr=67 scope.pos=1413 scope.end_pos=1698)
	// var "r" var.pos=1422 var.line_nr=54
	// var "b" var.pos=1447 var.line_nr=55
	// var "read" var.pos=1486 var.line_nr=56
	// af parent scope:
	// af parent scope:
	return _t772;
}
...
Option_int io__ReaderWriterImpl_read(io__ReaderWriterImpl* r, Array_byte* buf) {
	Option_int _t773 = io__Reader_name_table[r->r._typ]._method_read(r->r._object, buf);
	// autofree_scope_vars(pos=425 line_nr=18 scope.pos=372 scope.end_pos=451)
	// var "r" var.pos=377 var.line_nr=17
	// var "buf" var.pos=406 var.line_nr=17
	// af parent scope:
	// af parent scope:
	return _t773;
}
...
Option_int io__ReaderWriterImpl_write(io__ReaderWriterImpl* r, Array_byte buf) {
	Option_int _t774 = io__Writer_name_table[r->w._typ]._method_write(r->w._object, buf);
	// autofree_scope_vars(pos=511 line_nr=22 scope.pos=461 scope.end_pos=534)
	// var "r" var.pos=466 var.line_nr=21
	// var "buf" var.pos=492 var.line_nr=21
	// af parent scope:
	// af parent scope:
	return _t774;
}
...
io__ReaderWriterImpl io__make_readerwriter(io__Reader r, io__Writer w) {
	io__ReaderWriterImpl _t775 = (io__ReaderWriterImpl){.r = r,.w = w,};
	// autofree_scope_vars(pos=687 line_nr=28 scope.pos=630 scope.end_pos=714)
	// var "r" var.pos=648 var.line_nr=27
	// var "w" var.pos=658 var.line_nr=27
	// af parent scope:
	// af parent scope:
	return _t775;
}
...
Option_pg__DB pg__connect(pg__Config config) {
	string conninfo = _STR("host=%.*s\000 port=%"PRId32"\000 user=%.*s\000 dbname=%.*s\000 password=%.*s", 5, config.host, config.port, config.user, config.dbname, config.password);
	PGconn* conn = PQconnectdb(conninfo.str);
	if (conn == 0) {
		return (Option_pg__DB){ .state=2, .err=v_error(_SLIT("libpq memory allocation error")), .data={0} };
	}
	int status = PQstatus(conn);
	if (status != CONNECTION_OK) {
		string c_error_msg = byteptr_vstring(PQerrorMessage(conn));
		string error_msg = _STR("%.*s", 1, c_error_msg);
		PQfinish(conn);
		return (Option_pg__DB){ .state=2, .err=v_error(_STR("Connection to a PG database failed: %.*s", 1, error_msg)), .data={0} };
	}
	Option_pg__DB _t778;
	opt_ok(&(pg__DB[]) { (pg__DB){.conn = conn,} }, (Option*)(&_t778), sizeof(pg__DB));
	// autofree_scope_vars(pos=2197 line_nr=92 scope.pos=1589 scope.end_pos=2225)
	// var "config" var.pos=1597 var.line_nr=76
	// var "conninfo" var.pos=1619 var.line_nr=77
	// var "conn" var.pos=1736 var.line_nr=78
	// var "status" var.pos=1840 var.line_nr=82
	// af parent scope:
	// af parent scope:
	return _t778;
}
...
VV_LOCAL_SYMBOL Array_pg__Row pg__res_to_rows(voidptr res) {
	...
	for (int i = 0; i < nr_rows; ++i) {
		pg__Row row = (pg__Row){.vals = __new_array(0, 1, sizeof(string)),};
		for (int j = 0; j < nr_cols; ++j) {
			byteptr val = PQgetvalue(res, i, j);
			string sval = byteptr_vstring(val);
			array_push((array*)&row.vals, _MOV((string[]){ string_clone(sval) }));
		}
		array_push((array*)&rows, _MOV((pg__Row[]){ row }));
	}
	PQclear(res);
	// autofree_scope_vars(pos=2542 line_nr=113 scope.pos=2231 scope.end_pos=2555)
	// var "res" var.pos=2243 var.line_nr=97
	// var "nr_rows" var.pos=2265 var.line_nr=98
	// var "nr_cols" var.pos=2294 var.line_nr=99
	// var "rows" var.pos=2328 var.line_nr=101
	// af parent scope:
	// af parent scope:
	return rows;
}
...
Option_int pg__DB_q_int(pg__DB db, string query) {
	Option_Array_pg__Row _t782 = pg__DB_exec(db, query);
	if (_t782.state != 0) { /*or block*/ 
		Option_int _t783;
		memcpy(&_t783, &_t782, sizeof(Option));
		return _t783;
	}
 	Array_pg__Row rows =  *(Array_pg__Row*)_t782.data;
	if (rows.len == 0) {
		return (Option_int){ .state=2, .err=v_error(_STR("q_int \"%.*s\000\" not found", 2, query)), .data={0} };
	}
	pg__Row row = (*(pg__Row*)/*ee elem_typ */array_get(rows, 0));
	if (row.vals.len == 0) {
		Option_int _t785;
		opt_ok(&(int[]) { 0 }, (Option*)(&_t785), sizeof(int));
		// autofree_scope_vars(pos=3050 line_nr=132 scope.pos=3047 scope.end_pos=3061)
		// af parent scope:
		// var "db" var.pos=2883 var.line_nr=125
		// var "query" var.pos=2896 var.line_nr=125
		// var "rows" var.pos=2918 var.line_nr=126
		// var "row" var.pos=3010 var.line_nr=130
		// var "val" var.pos=3064 var.line_nr=134
		// af parent scope:
		// af parent scope:
		return _t785;
	}
	...
	Option_int _t786;
	opt_ok(&(int[]) { string_int(val) }, (Option*)(&_t786), sizeof(int));
	// autofree_scope_vars(pos=3083 line_nr=135 scope.pos=2882 scope.end_pos=3101)
	// var "db" var.pos=2883 var.line_nr=125
	// var "query" var.pos=2896 var.line_nr=125
	// var "rows" var.pos=2918 var.line_nr=126
	// var "row" var.pos=3010 var.line_nr=130
...
Option_string pg__DB_q_string(pg__DB db, string query) {
	Option_Array_pg__Row _t787 = pg__DB_exec(db, query);
	if (_t787.state != 0) { /*or block*/ 
		Option_string _t788;
		memcpy(&_t788, &_t787, sizeof(Option));
		return _t788;
	}
 	Array_pg__Row rows =  *(Array_pg__Row*)_t787.data;
	if (rows.len == 0) {
		return (Option_string){ .state=2, .err=v_error(_STR("q_string \"%.*s\000\" not found", 2, query)), .data={0} };
	}
	pg__Row row = (*(pg__Row*)/*ee elem_typ */array_get(rows, 0));
	if (row.vals.len == 0) {
		Option_string _t790;
		opt_ok(&(string[]) { _SLIT("") }, (Option*)(&_t790), sizeof(string));
		// autofree_scope_vars(pos=3475 line_nr=149 scope.pos=3472 scope.end_pos=3487)
		// af parent scope:
		// var "db" var.pos=3299 var.line_nr=142
		// var "query" var.pos=3315 var.line_nr=142
		// var "rows" var.pos=3340 var.line_nr=143
		// var "row" var.pos=3435 var.line_nr=147
		// var "val" var.pos=3490 var.line_nr=151
		// af parent scope:
		// af parent scope:
		return _t790;
	}
	string val = (*(string*)/*ee elem_typ */array_get(row.vals, 0));
	Option_string _t791;
	opt_ok(&(string[]) { val }, (Option*)(&_t791), sizeof(string));
	// autofree_scope_vars(pos=3509 line_nr=152 scope.pos=3298 scope.end_pos=3521)
	// var "db" var.pos=3299 var.line_nr=142
	// var "query" var.pos=3315 var.line_nr=142
	// var "rows" var.pos=3340 var.line_nr=143
	// var "row" var.pos=3435 var.line_nr=147
...
Option_Array_pg__Row pg__DB_q_strings(pg__DB db, string query) {
	Option_Array_pg__Row _t792 = pg__DB_exec(db, query);
	// autofree_scope_vars(pos=3679 line_nr=158 scope.pos=3638 scope.end_pos=3702)
	// var "db" var.pos=3639 var.line_nr=157
	// var "query" var.pos=3656 var.line_nr=157
	// af parent scope:
	// af parent scope:
	return _t792;
}
...
Option_Array_pg__Row pg__DB_exec(pg__DB db, string query) {
	...
	Option_Array_pg__Row _t793 = pg__DB_handle_error_or_result(db, res, _SLIT("exec"));
	// autofree_scope_vars(pos=3919 line_nr=166 scope.pos=3846 scope.end_pos=3966)
	// var "db" var.pos=3847 var.line_nr=164
	// var "query" var.pos=3859 var.line_nr=164
	// var "res" var.pos=3883 var.line_nr=165
	// af parent scope:
	// af parent scope:
	return _t793;
}
...
VV_LOCAL_SYMBOL Option_pg__Row pg__rows_first_or_empty(Array_pg__Row rows) {
	if (rows.len == 0) {
		return (Option_pg__Row){ .state=2, .err=v_error(_SLIT("no row")), .data={0} };
	}
	Option_pg__Row _t795;
	opt_ok(&(pg__Row[]) { (*(pg__Row*)/*ee elem_typ */array_get(rows, 0)) }, (Option*)(&_t795), sizeof(pg__Row));
	// autofree_scope_vars(pos=4059 line_nr=173 scope.pos=3972 scope.end_pos=4075)
	// var "rows" var.pos=3992 var.line_nr=169
	// af parent scope:
	// af parent scope:
	return _t795;
}
...
Option_pg__Row pg__DB_exec_one(pg__DB db, string query) {
	struct PGResult* res = PQexec(db.conn, query.str);
	string e = byteptr_vstring(PQerrorMessage(db.conn));
	if ((e).len != 0) {
		return (Option_pg__Row){ .state=2, .err=v_error(_STR("pg exec error: \"%.*s\000\"", 2, e)), .data={0} };
	}
	Option_pg__Row _t797 = pg__rows_first_or_empty(pg__res_to_rows(res));
	if (_t797.state != 0) { /*or block*/ 
		Option_pg__Row _t798;
		memcpy(&_t798, &_t797, sizeof(Option));
		return _t798;
	}
 	pg__Row row =  *(pg__Row*)_t797.data;
	Option_pg__Row _t799;
	opt_ok(&(pg__Row[]) { row }, (Option*)(&_t799), sizeof(pg__Row));
	// autofree_scope_vars(pos=4316 line_nr=183 scope.pos=4085 scope.end_pos=4328)
	// var "db" var.pos=4086 var.line_nr=176
	// var "query" var.pos=4102 var.line_nr=176
	// var "res" var.pos=4124 var.line_nr=177
	// var "e" var.pos=4161 var.line_nr=178
	// var "row" var.pos=4269 var.line_nr=182
	// af parent scope:
	// af parent scope:
	return _t799;
}
...
Option_Array_pg__Row pg__DB_exec_param_many(pg__DB db, string query, Array_string params) {
	Array_charptr param_vals = __new_array_with_default(params.len, 0, sizeof(charptr), 0);
	for (int i = 0; i < params.len; ++i) {
		array_set(&param_vals, i, &(charptr[]) { (*(string*)/*ee elem_typ */array_get(params, i)).str });
	}
	struct PGResult* res = PQexecParams(db.conn, query.str, params.len, 0, param_vals.data, 0, 0, 0);
	Option_Array_pg__Row _t800 = pg__DB_handle_error_or_result(db, res, _SLIT("exec_param_many"));
	// autofree_scope_vars(pos=4661 line_nr=194 scope.pos=4403 scope.end_pos=4719)
	// var "db" var.pos=4404 var.line_nr=187
	// var "query" var.pos=4427 var.line_nr=187
	// var "params" var.pos=4441 var.line_nr=187
	// var "param_vals" var.pos=4472 var.line_nr=188
	// var "res" var.pos=4578 var.line_nr=193
	// af parent scope:
	// af parent scope:
	return _t800;
}
...
Option_Array_pg__Row pg__DB_exec_param2(pg__DB db, string query, string param, string param2) {
	Option_Array_pg__Row _t801 = pg__DB_exec_param_many(db, query, new_array_from_c_array(2, 2, sizeof(string), _MOV((string[2]){param, param2})));
	// autofree_scope_vars(pos=4801 line_nr=198 scope.pos=4729 scope.end_pos=4852)
	// var "db" var.pos=4730 var.line_nr=197
	// var "query" var.pos=4749 var.line_nr=197
	// var "param" var.pos=4763 var.line_nr=197
	// var "param2" var.pos=4777 var.line_nr=197
	// af parent scope:
	// af parent scope:
	return _t801;
}
...
Option_Array_pg__Row pg__DB_exec_param(pg__DB db, string query, string param) {
	Option_Array_pg__Row _t802 = pg__DB_exec_param_many(db, query, new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){param})));
	// autofree_scope_vars(pos=4918 line_nr=202 scope.pos=4862 scope.end_pos=4961)
	// var "db" var.pos=4863 var.line_nr=201
	// var "query" var.pos=4881 var.line_nr=201
	// var "param" var.pos=4895 var.line_nr=201
	// af parent scope:
	// af parent scope:
	return _t802;
}
...
VV_LOCAL_SYMBOL Option_Array_pg__Row pg__DB_handle_error_or_result(pg__DB db, voidptr res, string elabel) {
	...
	if ((e).len != 0) {
		PQclear(res);
		return (Option_Array_pg__Row){ .state=2, .err=v_error(_STR("pg %.*s\000 error:\n%.*s", 2, elabel, e)), .data={0} };
	}
	Option_Array_pg__Row _t804;
	opt_ok(&(Array_pg__Row[]) { pg__res_to_rows(res) }, (Option*)(&_t804), sizeof(Array_pg__Row));
	// autofree_scope_vars(pos=5162 line_nr=211 scope.pos=4967 scope.end_pos=5187)
	// var "db" var.pos=4968 var.line_nr=205
	// var "res" var.pos=4998 var.line_nr=205
	// var "elabel" var.pos=5011 var.line_nr=205
	// var "e" var.pos=5036 var.line_nr=206
	// af parent scope:
	// af parent scope:
	return _t804;
}
...
bool pg__DB_copy_expert_defer_0 = false;
	...
	string e = byteptr_vstring(PQerrorMessage(db.conn));
	if ((e).len != 0) {
		Option_int _t805 = (Option_int){ .state=2, .err=v_error(_STR("pg copy error:\n%.*s", 1, e)), .data={0} };
		// Defer begin
	...
	if (status == PGRES_COPY_IN) {
		...
		for (;;) {
			Option_int _t806 = io__ReaderWriter_name_table[file._typ]._method_read(file._object, &/*arr*/buf);
			if (_t806.state != 0) { /*or block*/ 
				IError err = _t806.err;
				string msg = _SLIT("pg copy error: Failed to read from input");
				PQputCopyEnd(db.conn, msg.str);
				Option_int _t807 = (Option_int){ .state=2, .err=err, .data={0} };
				// Defer begin
				if (pg__DB_copy_expert_defer_0) {
					PQclear(res);
				}
 			...
 			int n =  *(int*)_t806.data;
			if (n <= 0) {
				break;
			}
			int code = PQputCopyData(db.conn, buf.data, n);
			if (code == -1) {
				Option_int _t808 = (Option_int){ .state=2, .err=v_error(_STR("pg copy error: Failed to send data, code=%"PRId32"", 1, code)), .data={0} };
				// Defer begin
				if (pg__DB_copy_expert_defer_0) {
					PQclear(res);
				}
		...
		if (code != 1) {
			Option_int _t809 = (Option_int){ .state=2, .err=v_error(_STR("pg copy error: Failed to finish copy command, code: %"PRId32"", 1, code)), .data={0} };
			// Defer begin
			if (pg__DB_copy_expert_defer_0) {
				PQclear(res);
			}
		...
		for (;;) {
			byteptr address = ((byteptr)(0));
			int n_bytes = PQgetCopyData(db.conn, &address, 0);
			if (n_bytes > 0) {
				Array_byte local_buf = __new_array_with_default(n_bytes, 0, sizeof(byte), 0);
				memcpy(((byteptr)(local_buf.data)), address, n_bytes);
				Option_int _t810 = io__ReaderWriter_name_table[file._typ]._method_write(file._object, local_buf);
				if (_t810.state != 0) { /*or block*/ 
					IError err = _t810.err;
					PQfreemem(address);
					Option_int _t811 = (Option_int){ .state=2, .err=err, .data={0} };
					// Defer begin
					if (pg__DB_copy_expert_defer_0) {
						PQclear(res);
					}
				...
				Option_int _t812 = (Option_int){ .state=2, .err=v_error(_SLIT("pg copy error: read error")), .data={0} };
				// Defer begin
				if (pg__DB_copy_expert_defer_0) {
					PQclear(res);
				}
	...
	Option_int _t813;
	opt_ok(&(int[]) { 0 }, (Option*)(&_t813), sizeof(int));
	// Defer begin
	if (pg__DB_copy_expert_defer_0) {
		PQclear(res);
	}
	// Defer end
	// autofree_scope_vars(pos=6647 line_nr=275 scope.pos=5289 scope.end_pos=6657)
	// var "db" var.pos=5290 var.line_nr=216
	// var "query" var.pos=5309 var.line_nr=216
	// var "file" var.pos=5323 var.line_nr=216
	// var "res" var.pos=5353 var.line_nr=217
	// var "status" var.pos=5390 var.line_nr=218
	// var "e" var.pos=5454 var.line_nr=224
	// af parent scope:
	// af parent scope:
	return _t813;
}
...
VV_LOCAL_SYMBOL void main__main(void) {
	Option_sqlite__DB _t814 = sqlite__connect(_SLIT(":memory:"));
	if (_t814.state != 0) { /*or block*/ 
		IError err = _t814.err;
		v_panic(IError_str(err));
	}
 	sqlite__DB db =  *(sqlite__DB*)_t814.data;
	// sqlite3 table creator
	// sqlite3 table creator
	sqlite__DB_exec(db, _SLIT("CREATE TABLE IF NOT EXISTS `User` (`id` INTEGER PRIMARY KEY, `age` INTEGER, `username` TEXT UNIQUE, `abc` INTEGER, /* user */UNIQUE(`age`, `abc`));"));
	sqlite__DB_exec(db, _SLIT("CREATE TABLE IF NOT EXISTS `modules` (`id` INTEGER PRIMARY KEY, `name` TEXT, `nr_downloads` INTEGER, `creator` INTEGER);"));
	...
	// sql insert
	sqlite__DB _t815 = db;
	sqlite3_stmt* _t816 = sqlite__DB_init_stmt(_t815, _SLIT("INSERT INTO `modules` (`name`, `nr_downloads`, `creator`) VALUES (?1, ?2, ?3);"));
	sqlite3_bind_text(_t816, 1, (char*)mod.name.str, mod.name.len, 0);
	sqlite3_bind_int(_t816, 2 , mod.nr_downloads); // stmt
	
	// sql insert
	sqlite__DB _t817 = db;
	sqlite3_stmt* _t818 = sqlite__DB_init_stmt(_t817, _SLIT("INSERT INTO `User` (`age`, `username`, `abc`) VALUES (?1, ?2, ?3);"));
	sqlite3_bind_int(_t818, 1 , mod.creator.age); // stmt
	sqlite3_bind_text(_t818, 2, (char*)mod.creator.name.str, mod.creator.name.len, 0);
	sqlite3_bind_int(_t818, 3 , mod.creator.is_customer); // stmt
	
		int _t819 = sqlite3_step(_t818);
		if( (_t819 != SQLITE_OK) && (_t819 != SQLITE_DONE)){ puts(sqlite3_errmsg(_t817.conn)); }
		sqlite3_finalize(_t818);
	Array_sqlite__Row _t820 = sqlite__DB_exec(_t815, _SLIT("SELECT last_insert_rowid()")).arg0;
	int _t821 = string_int((*(string*)array_get((*(sqlite__Row*)array_get(_t820, 0)).vals, 0)));
	sqlite3_bind_int(_t816, 3 , _t821); // id
	
		int _t822 = sqlite3_step(_t816);
		if( (_t822 != SQLITE_OK) && (_t822 != SQLITE_DONE)){ puts(sqlite3_errmsg(_t815.conn)); }
		sqlite3_finalize(_t816);

	// sql select
	sqlite__DB _t824 = db;
	string _t825 = _SLIT("SELECT `id`, `name`, `nr_downloads`, `creator` FROM `modules` WHERE id = ?1 ORDER BY id ");
	sqlite3_stmt* _t823 = sqlite__DB_init_stmt(_t824, _t825);sqlite3_bind_int(_t823, 1, 1);

	int _t826 = sqlite3_extended_errcode(_t824.conn);
	if (_t826 != SQLITE_OK) { puts(sqlite3_errmsg(_t824.conn)); }
	main__Module _t827 = (main__Module){
	.id = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .nr_downloads = 0, .creator = (main__User){.id = 0,.age = 0,.name = (string){.str=(byteptr)"", .is_lit=1},.is_customer = 0,.skipped_string = (string){.str=(byteptr)"", .is_lit=1},}};
	int _step_res_t827 = sqlite3_step(_t823);
		if (_step_res_t827 == SQLITE_OK || _step_res_t827 == SQLITE_ROW) {
	_t827.id = sqlite3_column_int(_t823, 0);
	byte* _t828 = (byte*)sqlite3_column_text(_t823, 1);
	if (_t828 != NULL) {
		_t827.name = tos_clone(_t828);
	}
	_t827.nr_downloads = sqlite3_column_int(_t823, 2);
	//parse struct start
	int _t829 = sqlite3_column_int(_t823, 3);
	
	// sql select
	sqlite__DB _t831 = db;
	string _t832 = _SLIT("SELECT `id`, `age`, `username`, `abc` FROM `User` WHERE id = ?1 ORDER BY id ");
	sqlite3_stmt* _t830 = sqlite__DB_init_stmt(_t831, _t832);sqlite3_bind_int(_t830, 1, _t829);

	int _t833 = sqlite3_extended_errcode(_t831.conn);
	if (_t833 != SQLITE_OK) { puts(sqlite3_errmsg(_t831.conn)); }
	main__User _t834 = (main__User){
	.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	int _step_res_t834 = sqlite3_step(_t830);
		if (_step_res_t834 == SQLITE_OK || _step_res_t834 == SQLITE_ROW) {
	_t834.id = sqlite3_column_int(_t830, 0);
	_t834.age = sqlite3_column_int(_t830, 1);
	byte* _t835 = (byte*)sqlite3_column_text(_t830, 2);
	if (_t835 != NULL) {
		_t834.name = tos_clone(_t835);
	}
	_t834.is_customer = sqlite3_column_int(_t830, 3);
	}
	sqlite3_finalize(_t830);
		_t827.creator = _t834; 
	//parse struct end
	}
	sqlite3_finalize(_t823);
		main__Module modul =  _t827; 
	;
	// sqlite3 table drop
	sqlite__DB_exec(db, _SLIT("DROP TABLE `modules`;"));
...
VV_LOCAL_SYMBOL void main__mysql(void) {
	...
	Option_bool _t836 = mysql__Connection_connect(&conn);
	if (_t836.state != 0) { /*or block*/ 
		IError err = _t836.err;
		v_panic(IError_str(err));
	};
	// mysql table creator
	// mysql table creator
	Option_mysql__Result _t837 = mysql__Connection_query(&conn, _SLIT("CREATE TABLE IF NOT EXISTS `User` (`id` SERIAL, `age` INT, `username` TEXT UNIQUE, `abc` TINYINT, /* user */UNIQUE(`age`, `abc`), PRIMARY KEY(`id`));"));
	if (_t837.state != 0) { IError err = _t837.err; eprintln(_STR("Something went wrong\000%.*s", 2, IError_str(err))); }
	Option_mysql__Result _t838 = mysql__Connection_query(&conn, _SLIT("CREATE TABLE IF NOT EXISTS `modules` (`id` SERIAL, `name` TEXT, `nr_downloads` BIGINT, `creator` INT, PRIMARY KEY(`id`));"));
	if (_t838.state != 0) { IError err = _t838.err; eprintln(_STR("Something went wrong\000%.*s", 2, IError_str(err))); }
	main__Module mod = (main__Module){.id = 0,.name = _SLIT("test"),.nr_downloads = 10,.creator = (main__User){.id = 0,.age = 21,.name = _SLIT("VUser"),.is_customer = true,.skipped_string = (string){.str=(byteptr)"", .is_lit=1},},};
	
	//mysql insert
	mysql__Connection _t839 = conn;
	string _t841 = _SLIT("INSERT INTO `modules` (`name`, `nr_downloads`, `creator`) VALUES (?, ?, ?);");
	MYSQL_STMT* _t840 = mysql_stmt_init(_t839.conn);
	mysql_stmt_prepare(_t840, _t841.str, _t841.len);
	MYSQL_BIND _t842[3];
	memset(_t842, 0, sizeof(MYSQL_BIND)*3);
	//name (18)
	_t842[0].buffer_type = MYSQL_TYPE_STRING;
	_t842[0].buffer = (char*) mod.name.str;
	_t842[0].buffer_length = mod.name.len;
	_t842[0].is_null = 0;
	_t842[0].length = 0;
	//nr_downloads (7)
	_t842[1].buffer_type = MYSQL_TYPE_LONGLONG;
	_t842[1].buffer = (u64*) &mod.nr_downloads;
	_t842[1].is_null = 0;
	_t842[1].length = 0;
	//creator (75)
	
	//mysql insert
	mysql__Connection _t843 = conn;
	string _t845 = _SLIT("INSERT INTO `User` (`age`, `username`, `abc`) VALUES (?, ?, ?);");
	MYSQL_STMT* _t844 = mysql_stmt_init(_t843.conn);
	mysql_stmt_prepare(_t844, _t845.str, _t845.len);
	MYSQL_BIND _t846[3];
	memset(_t846, 0, sizeof(MYSQL_BIND)*3);
	//age (7)
	_t846[0].buffer_type = MYSQL_TYPE_LONG;
	_t846[0].buffer = (int*) &mod.creator.age;
	_t846[0].is_null = 0;
	_t846[0].length = 0;
	//name (18)
	_t846[1].buffer_type = MYSQL_TYPE_STRING;
	_t846[1].buffer = (char*) mod.creator.name.str;
	_t846[1].buffer_length = mod.creator.name.len;
	_t846[1].is_null = 0;
	_t846[1].length = 0;
	//is_customer (16)
	_t846[2].buffer_type = MYSQL_TYPE_TINY;
	_t846[2].buffer = (bool*) &mod.creator.is_customer;
	_t846[2].is_null = 0;
	_t846[2].length = 0;
	
	int _t847 = mysql_stmt_bind_param(_t844, _t846);
	if (_t847 != 0) { puts(mysql_error(_t843.conn)); }
	_t847 = mysql_stmt_execute(_t844);
	if (_t847 != 0) { puts(mysql_error(_t843.conn)); puts(mysql_stmt_error(_t844)); }
	mysql_stmt_close(_t844);
	mysql_stmt_free_result(_t844);
	int _t848_err = mysql_real_query(_t839.conn, "SELECT LAST_INSERT_ID();", 24);
	if (_t848_err != 0) { puts(mysql_error(_t839.conn)); }
	MYSQL_RES* _t848 = mysql_store_result(_t839.conn);
	if (mysql_num_rows(_t848) != 1) { puts("Something went wrong"); }
	MYSQL_ROW _t848_row = mysql_fetch_row(_t848);
	mod.creator.id = string_int(tos_clone(_t848_row[0]));
	mysql_free_result(_t848);
	_t842[2].buffer_type = MYSQL_TYPE_LONG;
	_t842[2].buffer = &mod.creator.id;
	_t842[2].is_null = 0;
	_t842[2].length = 0;
	
	int _t849 = mysql_stmt_bind_param(_t840, _t842);
	if (_t849 != 0) { puts(mysql_error(_t839.conn)); }
	_t849 = mysql_stmt_execute(_t840);
	if (_t849 != 0) { puts(mysql_error(_t839.conn)); puts(mysql_stmt_error(_t840)); }
	mysql_stmt_close(_t840);
	mysql_stmt_free_result(_t840);

	// sql select
	mysql__Connection _t852 = conn;
	char* _t853_raw = "SELECT `id`, `name`, `nr_downloads`, `creator` FROM `modules` WHERE id = ?1 ORDER BY id ";
	string _t853 = tos_clone(_t853_raw);
	int _t854 = mysql_real_query(_t852.conn, _t853.str, _t853.len);
	if (_t854 != 0) { puts(mysql_error(_t852.conn)); }
	MYSQL_RES* _t855 = mysql_store_result(_t852.conn);
	MYSQL_ROW _t856 = mysql_fetch_row(_t855);
	int _t858 = 0;
	main__Module _t857 = (main__Module){
	.id = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .nr_downloads = 0, .creator = (main__User){.id = 0,.age = 0,.name = (string){.str=(byteptr)"", .is_lit=1},.is_customer = 0,.skipped_string = (string){.str=(byteptr)"", .is_lit=1},}};
	char* _t859 = "";
	_t859 = _t856[0];
	if (_t859 == NULL) { _t859 = ""; }
	_t857.id = string_int(tos_clone(_t859));
	_t859 = _t856[1];
	if (_t859 == NULL) { _t859 = ""; }
	_t857.name = tos_clone(_t859);
	_t859 = _t856[2];
	if (_t859 == NULL) { _t859 = ""; }
	_t857.nr_downloads = string_int(tos_clone(_t859));
	_t859 = _t856[3];
	if (_t859 == NULL) { _t859 = ""; }
	string_free(&_t853);
	mysql_free_result(_t855);
		main__Module m =  _t857; 
	;
	eprintln(main__Module_str(m));
}

VV_LOCAL_SYMBOL void main__psql(void) {
	Option_pg__DB _t860 = pg__connect((pg__Config){.host = _SLIT("localhost"),.port = 5432,.user = _SLIT("test"),.password = _SLIT("abc"),.dbname = _SLIT("test"),});
	if (_t860.state != 0) { /*or block*/ 
		IError err = _t860.err;
		v_panic(IError_str(err));
	}
 	pg__DB db =  *(pg__DB*)_t860.data;
	main__Module mod = (main__Module){.id = 0,.name = _SLIT("test"),.nr_downloads = 10,.creator = (main__User){.id = 0,.age = 21,.name = _SLIT("VUser"),.is_customer = true,.skipped_string = (string){.str=(byteptr)"", .is_lit=1},},};
	// psql table creator
	// psql table creator
	Option_Array_pg__Row _t861 = pg__DB_exec(db, _SLIT("CREATE TABLE IF NOT EXISTS \"User\" (\"id\" SERIAL, \"age\" INT, \"username\" TEXT UNIQUE, \"abc\" CHAR(1), /* user */UNIQUE(\"age\", \"abc\"), PRIMARY KEY(\"id\"));"));
	if (_t861.state != 0) { IError err = _t861.err; eprintln(_STR("Something went wrong\000%.*s", 2, IError_str(err))); }
	Option_Array_pg__Row _t862 = pg__DB_exec(db, _SLIT("CREATE TABLE IF NOT EXISTS \"modules\" (\"id\" SERIAL, \"name\" TEXT, \"nr_downloads\" BIGINT, \"creator\" INT, PRIMARY KEY(\"id\"));"));
	if (_t862.state != 0) { IError err = _t862.err; eprintln(_STR("Something went wrong\000%.*s", 2, IError_str(err))); }
	
	//psql insert
	pg__DB _t863 = db;
	string _t864 = _SLIT("INSERT INTO \"modules\" (\"name\", \"nr_downloads\", \"creator\") VALUES ($1, $2, $3);");
	//name (18)
	_t864 = string_replace(_t864, _SLIT("$1"), string_add(_SLIT("'"), string_add(((string) mod.name), _SLIT("'"))));

	//nr_downloads (7)
	_t864 = string_replace(_t864, _SLIT("$2"), u64_str(((u64) mod.nr_downloads)));

	//creator (75)
	
	//psql insert
	pg__DB _t865 = db;
	string _t866 = _SLIT("INSERT INTO \"User\" (\"age\", \"username\", \"abc\") VALUES ($1, $2, $3);");
	//age (7)
	_t866 = string_replace(_t866, _SLIT("$1"), int_str(((int) mod.creator.age)));

	//name (18)
	_t866 = string_replace(_t866, _SLIT("$2"), string_add(_SLIT("'"), string_add(((string) mod.creator.name), _SLIT("'"))));

	//is_customer (16)
	_t866 = string_replace(_t866, _SLIT("$3"), ((bool) mod.creator.is_customer)? _SLIT("1") : _SLIT("0"));

	
	pg__DB_exec(_t865, _t866);
	Option_pg__Row _t867 = pg__DB_exec_one(_t863, _SLIT("SELECT LASTVAL();"));
	if (_t867.state != 0) { IError err = _t867.err; eprintln(_STR("\000%.*s", 2, IError_str(err))); }
	_t864 = string_replace(_t864, _SLIT("$3"), int_str(((int) string_int((*(string*)array_get((*(pg__Row*)_t867.data).vals, 0))))));

	
	pg__DB_exec(_t863, _t864);

	// psql select
	pg__DB _t869 = db;
	string _t868 = _SLIT("SELECT \"id\", \"name\", \"nr_downloads\", \"creator\" FROM \"modules\" WHERE id = $1 ORDER BY id ");
	_t868 = string_replace(_t868, _SLIT("$1"), int_str(((int) 1)));

	Option_Array_pg__Row _t870 = pg__DB_exec(_t869, _t868);
	if (_t870.state != 0) { IError err = _t870.err; eprintln(_STR("Something went wrong\000%.*s", 2, IError_str(err))); }
	Array_pg__Row _t871 = *(Array_pg__Row*) _t870.data;
	int _t873 = 0;
	main__Module _t872 = (main__Module){
	.id = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .nr_downloads = 0, .creator = (main__User){.id = 0,.age = 0,.name = (string){.str=(byteptr)"", .is_lit=1},.is_customer = 0,.skipped_string = (string){.str=(byteptr)"", .is_lit=1},}};
	Array_string _t874 = (*(pg__Row*) array_get(_t871, _t873)).vals;
	string _t875;
	_t875 = (*(string*)array_get(_t874, 0));
	_t872.id = string_int(_t875);
	_t875 = (*(string*)array_get(_t874, 1));
	_t872.name = _t875;
	_t875 = (*(string*)array_get(_t874, 2));
	_t872.nr_downloads = string_int(_t875);
	_t875 = (*(string*)array_get(_t874, 3));
	//parse struct start
	
	// psql select
	pg__DB _t877 = db;
	string _t876 = _SLIT("SELECT \"id\", \"age\", \"username\", \"abc\" FROM \"User\" WHERE id = $1 ORDER BY id ");
	_t876 = string_replace(_t876, _SLIT("$1"), int_str(((int) 1)));

	Option_Array_pg__Row _t878 = pg__DB_exec(_t877, _t876);
	if (_t878.state != 0) { IError err = _t878.err; eprintln(_STR("Something went wrong\000%.*s", 2, IError_str(err))); }
	Array_pg__Row _t879 = *(Array_pg__Row*) _t878.data;
	int _t881 = 0;
	main__User _t880 = (main__User){
	.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	Array_string _t882 = (*(pg__Row*) array_get(_t879, _t881)).vals;
	string _t883;
	_t883 = (*(string*)array_get(_t882, 0));
	_t880.id = string_int(_t883);
	_t883 = (*(string*)array_get(_t882, 1));
	_t880.age = string_int(_t883);
	_t883 = (*(string*)array_get(_t882, 2));
	_t880.name = _t883;
	_t883 = (*(string*)array_get(_t882, 3));
	_t880.is_customer = string_eq(_t883, _SLIT("0")) ? false : true;
	string_free(&_t876);
		_t872.creator = _t880; 
	//parse struct end
	string_free(&_t868);
		main__Module modul =  _t872; 
	;
	// psql table drop
	Option_Array_pg__Row _t884 = pg__DB_exec(db, _SLIT("DROP TABLE \"modules\";"));
	if (_t884.state != 0) { IError err = _t884.err; eprintln(_STR("Something went wrong\000%.*s", 2, IError_str(err))); }
	eprintln(main__Module_str(modul));
}
...
void _vinit(int ___argc, voidptr ___argv) {
	...
	_const_io__read_all_len = 10 * 1024;
