...
fn (v mut V) add_user_v_files() {
	...
	for i := 0; i < v.table.imports.len; i++ {
		...
		vfiles := v.v_files_from_dir(v.module_path(mod))
		...
		for file in vfiles {
			...
			for _, m in p.import_table.imports {
				for lfit in lib_imports {
					if lfit.module_name == m && lfit.known_import(p.import_table.module_name) {
						panic('Circular import detected between modules: ${m} & ${mod}.')
					}
				}
			}
...
fn (v &V) module_dir(mod string) string {
	// submodule support
	if mod.contains('.') {
...
fn (v &V) module_path(mod string) string {
	mod_dir := v.module_dir(mod)
	idir := os.getwd()
	mut mod_path := '$idir/$mod_dir'
	if v.pref.build_mode == .default_mode || v.pref.build_mode == .build {
		mod_path = '$ModPath/vlib/$mod_dir'
	}
	if !os.file_exists(mod_path) {
		mod_path = '$v.lang_dir/vlib/$mod_dir'
	}
	return mod_path
}
