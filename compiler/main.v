...
*/
	...
	if v.pref.build_mode == .build {
		...
		libs = filepath.join([TmpPath, 'vlib', 'builtin.o'])
		if !os.file_exists(libs) {
			println('`builtin.o` not found')
			exit(1)
		}
		for imp in v.table.imports {
			...
			libs += filepath.join([' ' + TmpPath, 'vlib', '${imp}.o'])
		}
	}
	// -I flags
	/* 
...
fn (v mut V) add_user_v_files() {
	...
	if v.pref.build_mode == .default_mode {
		for i := 0; i < v.table.imports.len; i++ {
			...
			vfiles := v.v_files_from_dir(filepath.join([TmpPath, 'vlib', pkg]))
		...
		for i := 0; i < v.table.imports.len; i++ {
			pkg := v.table.imports[i]
			idir := os.getwd()
			mut import_path := filepath.join([idir, pkg])
			if(!os.file_exists(import_path)) {
				import_path = filepath.join([v.lang_dir, 'vlib', pkg])
			}
	...
	for pkg in v.table.imports {
		idir := os.getwd()
		mut module_path := filepath.join([idir, pkg])
		...
		if v.pref.build_mode == .default_mode || v.pref.build_mode == .build {
			module_path = filepath.join([TmpPath, 'vlib', pkg])
		}
		if(!os.file_exists(module_path)) {
			module_path = filepath.join([v.lang_dir, 'vlib', pkg])
		}
...
*/ 
	...
	vroot_path := filepath.join([TmpPath, 'VROOT'])
	if os.file_exists(vroot_path) {
		...
		if os.dir_exists(vroot) && os.dir_exists(filepath.join([vroot, 'vlib', 'builtin'])) {
			lang_dir = vroot
		}
	}
	break
	...
	if lang_dir == ''  {
		...
		// from "v/compiler"
		lang_dir = os.getwd() 
		if os.dir_exists(filepath.join([lang_dir, 'vlib', 'builtin'])) {
			println('Setting VROOT to "$lang_dir".')
			os.write_file(filepath.join([TmpPath, 'VROOT']), lang_dir)
		} else {
...
fn run_repl() []string {
	...
	file := filepath.join([TmpPath, 'vrepl.v'])
	temp_file := filepath.join([TmpPath, 'vrepl_temp.v'])
	mut lines := []string
	for {
		print('>>> ')
		...
		if line.starts_with('print') {
			...
			s := os.exec('v run '+filepath.join([TmpPath, 'vrepl.v']))
			mut vals := s.split('\n')
			if s.contains('panic: ') {
			...
			s := os.exec('v run '+filepath.join([TmpPath, 'vrepl_temp.v']))
			if s.contains('panic: ') {
				if !s.contains('declared and not used') 	{
					mut vals := s.split('\n')
					for i:=1; i<vals.len; i++ {
