...
fn (c mut V) add_user_v_files() {
	...
	if c.build_mode == DEFAULT_MODE {
		...
		for i := 0; i < c.table.imports.len; i++ {
			pkg := c.table.imports[i]
			mut dir_pkg := ''
			// Compiling with .o files doesnt work
			// if os.dir_exists('$TmpPath/vlib/$pkg') {
			//   dir_pkg = '$TmpPath/vlib/$pkg'
			// } else
			if os.dir_exists('$c.lang_dir/../modules/$pkg') {
				dir_pkg = '$c.lang_dir/../modules/$pkg'
			} else {
				dir_pkg = '$c.lang_dir/$pkg'
			}
			// mut import_path := '$c.lang_dir/$pkg'
			vfiles := c.v_files_from_dir(dir_pkg)
			// Add all imports referenced by these libs
			for file in vfiles {
				mut p := c.new_parser(file, RUN_IMPORTS)
				p.parse()
	...
	for pkg in c.table.imports {
		...
		// } else
		if os.dir_exists('$c.lang_dir/../modules/$pkg') {
			module_path = '$c.lang_dir/../modules/$pkg'
		} else {
			module_path = '$c.lang_dir/$pkg'
		}
...
fn new_v(args[]string) *V {
	...
	if !out_name.contains('builtin.o') {
		for builtin in builtins {
			...
			if (build_mode == DEFAULT_MODE || build_mode == BUILD) && os.file_exists('$TmpPath/vlib/builtin/${builtin}h') {
