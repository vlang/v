...
*/
	...
	if v.pref.build_mode == .build {
		...
		libs = path.join([TmpPath, 'vlib', 'builtin.o'])
		if !os.file_exists(libs) {
			println('`builtin.o` not found')
			exit(1)
		}
		for imp in v.table.imports {
			...
			libs += path.join([' ' + TmpPath, 'vlib', '${imp}.o'])
		}
	}
	// -I flags
	/* 
...
fn (v mut V) add_user_v_files() {
	...
	if v.pref.build_mode == .default_mode {
		for i := 0; i < v.table.imports.len; i++ {
			...
			vfiles := v.v_files_from_dir(path.join([TmpPath, 'vlib', pkg]))
		...
		for i := 0; i < v.table.imports.len; i++ {
			pkg := v.table.imports[i]
			idir := os.getwd()
			mut import_path := path.join([idir, pkg])
			if(!os.file_exists(import_path)) {
				import_path = path.join([v.lang_dir, 'vlib', pkg])
			}
	...
	for pkg in v.table.imports {
		idir := os.getwd()
		mut module_path := path.join([idir, pkg])
		...
		if v.pref.build_mode == .default_mode || v.pref.build_mode == .build {
			module_path = path.join([TmpPath, 'vlib', pkg])
		}
		if(!os.file_exists(module_path)) {
			module_path = path.join([v.lang_dir, 'vlib', pkg])
		}
...
*/ 
	...
	vroot_path := path.join([TmpPath, 'VROOT'])
	if os.file_exists(vroot_path) {
		...
		if os.dir_exists(vroot) && os.dir_exists(path.join([vroot, 'vlib', 'builtin'])) {
			lang_dir = vroot
		}
	}
	break
	...
	if lang_dir == ''  {
		...
		// from "v/compiler"
		lang_dir = os.getwd() 
		if os.dir_exists(path.join([lang_dir, 'vlib', 'builtin'])) {
			println('Setting VROOT to "$lang_dir".')
			os.write_file(path.join([TmpPath, 'VROOT']), lang_dir)
		} else {
