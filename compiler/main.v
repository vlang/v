...
*/
	...
	if v.pref.build_mode == .build {
		...
		libs = TmpPath + path_sep + 'vlib' + path_sep + 'builtin.o'
		if !os.file_exists(libs) {
			println('`builtin.o` not found')
			exit(1)
		}
		for imp in v.table.imports {
			...
			libs += ' ' + TmpPath + path_sep + 'vlib' + path_sep + '${imp}.o'
		}
	}
	// -I flags
	/* 
...
fn (v mut V) add_user_v_files() {
	...
	if v.pref.build_mode == .default_mode {
		for i := 0; i < v.table.imports.len; i++ {
			...
			vfiles := v.v_files_from_dir(TmpPath + path_sep + 'vlib' + path_sep + pkg)
		...
		for i := 0; i < v.table.imports.len; i++ {
			pkg := v.table.imports[i]
			idir := os.getwd()
			mut import_path := idir + path_sep + pkg
			if(!os.file_exists(import_path)) {
				import_path = v.lang_dir + path_sep + 'vlib' + path_sep + pkg
			}
	...
	for pkg in v.table.imports {
		idir := os.getwd()
		mut module_path := idir + path_sep + pkg
		...
		if v.pref.build_mode == .default_mode || v.pref.build_mode == .build {
			module_path = TmpPath + path_sep + 'vlib' + path_sep + pkg
		}
		if(!os.file_exists(module_path)) {
			module_path = v.lang_dir + path_sep + 'vlib' + path_sep + pkg
		}
...
*/ 
	...
	vroot_path := TmpPath + path_sep + 'VROOT'
	if os.file_exists(vroot_path) {
		...
		if os.dir_exists(vroot) && os.dir_exists(vroot + path_sep + 'vlib' + path_sep + 'builtin') {
			lang_dir = vroot
		}
	}
	break
	...
	if lang_dir == ''  {
		...
		// from "v/compiler"
		lang_dir = os.getwd() 
		if os.dir_exists(lang_dir + path_sep +'vlib' + path_sep + 'builtin') {
			println('Setting VROOT to "$lang_dir".')
			os.write_file(TmpPath + path_sep + 'VROOT', lang_dir)
		} else {
	...
	if !out_name.contains('builtin.o') {
		for builtin in builtins {
			mut f := lang_dir + path_sep + 'vlib' + path_sep + 'builtin' + path_sep + builtin
			// In default mode we use precompiled vlib.o, point to .vh files with signatures
			if build_mode == .default_mode || build_mode == .build {
				f = TmpPath + path_sep + 'vlib' + path_sep + 'builtin' + path_sep + '${builtin}h'
			}
			files << f
		}
	}
