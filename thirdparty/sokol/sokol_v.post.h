	...
	void v_sapp_screenshot(const char * filename) {
		...
		const int num_pixels = window_width * windowHeight * 3;
		unsigned char pixels[num_pixels];

		glPixelStorei(GL_PACK_ALIGNMENT, 1);
		glReadBuffer(GL_FRONT);
		glReadPixels(0, 0, window_width, windowHeight, GL_BGR_EXT, GL_UNSIGNED_BYTE, pixels);

		FILE *outputFile = fopen(filename, "w");
		short header[] = {0, 2, 0, 0, 0, 0, (short) window_width, (short) windowHeight, 24};
