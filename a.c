	...
	#define V_COMMIT_HASH "19b4504"
#endif

#ifndef V_CURRENT_COMMIT_HASH
	#define V_CURRENT_COMMIT_HASH "9cb958c"
#endif
...
typedef struct main__ABC main__ABC;
...
VV_LOCAL_SYMBOL voidptr map_get_and_set(map* m, voidptr key, voidptr zero) {
	...
	if (!(false)) {
		...
		v_assert_meta_info__t380.fpath = _SLIT("/home/louis/v/vlib/builtin/map.v");
...
Array_int* _t441; // idxs
	...
	int s_idx = 0;
	// FOR IN array
	for (int _t445 = 0; _t445 < idxs.len; ++_t445) {
		int rep_pos = ((int*)idxs.data)[_t445];
...
Array_string string_split_nth(string s, string delim, int nth) {
	...
	if (delim.len == (0)) {
		i = 1;
		for (int _t477 = 0; _t477 < s.len; ++_t477) {
			byte ch = s.str[_t477];
			if (nth > 0 && i >= nth) {
...
int string_index_any(string s, string chars) {
	for (int _t503 = 0; _t503 < chars.len; ++_t503) {
		byte c = chars.str[_t503];
...
Option_int string_last_index(string s, string p) {
	...
	Option_int _t510;
	opt_ok(&(int[]) { idx }, (Option*)(&_t510), sizeof(int));
	return _t510;
}
...
int string_count(string s, string substr) {
	...
	if (substr.len == 1) {
		...
		for (int _t521 = 0; _t521 < s.len; ++_t521) {
			byte letter = s.str[_t521];
			if (letter == target) {
				n++;
			}
		}
...
bool string_contains_any(string s, string chars) {
	for (int _t528 = 0; _t528 < chars.len; ++_t528) {
		byte c = chars.str[_t528];
...
bool string_contains_any_substr(string s, Array_string substrs) {
	...
	// FOR IN array
	for (int _t532 = 0; _t532 < substrs.len; ++_t532) {
		string sub = ((string*)substrs.data)[_t532];
...
string string_title(string s) {
	...
	// FOR IN array
	for (int _t553 = 0; _t553 < words.len; ++_t553) {
		string word = ((string*)words.data)[_t553];
...
bool string_is_title(string s) {
	Array_string words = string_split(s, _SLIT(" "));
	// FOR IN array
	for (int _t556 = 0; _t556 < words.len; ++_t556) {
		string word = ((string*)words.data)[_t556];
...
string string_trim(string s, string cutset) {
	...
	for (;;) {
		...
		for (int _t566 = 0; _t566 < cutset.len; ++_t566) {
			byte cs = cutset.str[_t566];
...
string string_trim_left(string s, string cutset) {
	...
	for (;;) {
		if (!(pos < s.len)) break;
		bool found = false;
		for (int _t570 = 0; _t570 < cutset.len; ++_t570) {
			byte cs = cutset.str[_t570];
			if (s.str[ pos] == cs) {
				found = true;
				break;
			}
...
string string_trim_right(string s, string cutset) {
	...
	for (;;) {
		if (!(pos >= 0)) break;
		bool found = false;
		for (int _t573 = 0; _t573 < cutset.len; ++_t573) {
			byte cs = cutset.str[_t573];
			if (s.str[ pos] == cs) {
				found = true;
			}
		}
...
string Array_string_join(Array_string a, string sep) {
	...
	int len = 0;
	// FOR IN array
	for (int _t639 = 0; _t639 < a.len; ++_t639) {
		string val = ((string*)a.data)[_t639];
		len += val.len + sep.len;
	}
	len -= sep.len;
	string res = (string){.str = v_malloc(len + 1), .len = len};
...
int string_hash(string s) {
	u32 h = ((u32)(0U));
	if (h == 0U && s.len > 0) {
		for (int _t646 = 0; _t646 < s.len; ++_t646) {
			byte c = s.str[_t646];
			h = h * 31U + ((u32)(c));
		}
	}
...
main__ABC* _t687; // abc
	main__ABC abc = (main__ABC){.ia = __new_array(0, 1, sizeof(int)),};
	main__main_defer_0 = true;
	_t687 = &(abc);
// Defer begin
if (main__main_defer_0) {
	array_push((array*)&_t687.ia, _MOV((int[]){ main__add() }));
}
// Defer end
}

VV_LOCAL_SYMBOL int main__add(void) {
	int _t689 = 2;
	// autofree_scope_vars(pos=110 line_nr=14 scope.pos=98 scope.end_pos=120)
	// af parent scope:
	// af parent scope:
	return _t689;
}
