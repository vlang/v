	...
	#define V_COMMIT_HASH "59d80e2"
#endif

#ifndef V_CURRENT_COMMIT_HASH
	#define V_CURRENT_COMMIT_HASH "005e1e2"
#endif
...
VV_LOCAL_SYMBOL void map_free_nop(voidptr _t398);
...
VV_LOCAL_SYMBOL string None___str(None__ _t421);
...
VV_LOCAL_SYMBOL void main__test_comptime_pkgconfig();
...
Option_u64 strconv__common_parse_uint(string s, int _base, int _bit_size, bool error_on_non_digit, bool error_on_high_digit) {
	...
	if (err != 0 && (error_on_non_digit || error_on_high_digit)) {
		...
		if (err == (-1)) {
			return (Option_u64){ .state=2, .err=v_error( str_intp(3, _MOV((StrIntpData[]){{_SLIT("common_parse_uint: wrong base "), 0xfe07, {.d_i32 = _base}}, {_SLIT(" for "), 0xfe10, {.d_s = s}}, {_SLIT0, 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
		}
		else if (err == (-2)) {
			return (Option_u64){ .state=2, .err=v_error( str_intp(3, _MOV((StrIntpData[]){{_SLIT("common_parse_uint: wrong bit size "), 0xfe07, {.d_i32 = _bit_size}}, {_SLIT(" for "), 0xfe10, {.d_s = s}}, {_SLIT0, 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
		}
		else if (err == (-3)) {
			return (Option_u64){ .state=2, .err=v_error( str_intp(2, _MOV((StrIntpData[]){{_SLIT("common_parse_uint: integer overflow "), 0xfe10, {.d_s = s}}, {_SLIT0, 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
		}
		else {
			return (Option_u64){ .state=2, .err=v_error( str_intp(2, _MOV((StrIntpData[]){{_SLIT("common_parse_uint: syntax error "), 0xfe10, {.d_s = s}}, {_SLIT0, 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
		};
	}
	Option_u64 _t89;
	opt_ok(&(u64[]) { result }, (Option*)(&_t89), sizeof(u64));
	return _t89;
}
...
Option_i64 strconv__common_parse_int(string _s, int base, int _bit_size, bool error_on_non_digit, bool error_on_high_digit) {
	...
	if (s.len < 1) {
		Option_i64 _t99;
		opt_ok(&(i64[]) { ((i64)(0)) }, (Option*)(&_t99), sizeof(i64));
		return _t99;
	}
	bool neg = false;
	if (string_at(s, 0) == '+') {
	...
	Option_u64 _t100 = strconv__common_parse_uint(s, base, bit_size, error_on_non_digit, error_on_high_digit);
	if (_t100.state != 0) { /*or block*/ 
		Option_i64 _t101;
		memcpy(&_t101, &_t100, sizeof(Option));
		return _t101;
	}
	
 	u64 un =  (*(u64*)_t100.data);
	if (un == 0U) {
		Option_i64 _t102;
		opt_ok(&(i64[]) { ((i64)(0)) }, (Option*)(&_t102), sizeof(i64));
		return _t102;
	}
	if (bit_size == 0) {
		bit_size = _const_strconv__int_size;
	...
	if (!neg && un >= cutoff) {
		Option_i64 _t103;
		opt_ok(&(i64[]) { ((i64)(cutoff - ((u64)(1U)))) }, (Option*)(&_t103), sizeof(i64));
		return _t103;
	}
	if (neg && un > cutoff) {
		Option_i64 _t104;
		opt_ok(&(i64[]) { -((i64)(cutoff)) }, (Option*)(&_t104), sizeof(i64));
		return _t104;
	}
	Option_i64 _t106; /* if prepend */
	if (neg) {
		opt_ok(&(i64[]) { -((i64)(un)) }, (Option*)(&_t106), sizeof(i64));
	} else {
		opt_ok(&(i64[]) { ((i64)(un)) }, (Option*)(&_t106), sizeof(i64));
	}
	return  _t106;
...
Option_int strconv__atoi(string s) {
	if ((s).len == 0) {
		return (Option_int){ .state=2, .err=v_error( str_intp(2, _MOV((StrIntpData[]){{_SLIT("strconv.atoi: parsing \""), 0xfe10, {.d_s = s}}, {_SLIT("\": invalid syntax "), 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	if ((_const_strconv__int_size == 32 && (0 < s.len && s.len < 10)) || (_const_strconv__int_size == 64 && (0 < s.len && s.len < 19))) {
		...
		Option_int _t112; /* if prepend */
		if (string_at(s, 0) == '-') {
			opt_ok(&(int[]) { -n }, (Option*)(&_t112), sizeof(int));
		} else {
			opt_ok(&(int[]) { n }, (Option*)(&_t112), sizeof(int));
		}
		return  _t112;
	}
	Option_i64 _t113 = strconv__parse_int(s, 10, 0);
	if (_t113.state != 0) { /*or block*/ 
 	...
 	i64 int64 =  (*(i64*)_t113.data);
	Option_int _t115;
	opt_ok(&(int[]) { ((int)(int64)) }, (Option*)(&_t115), sizeof(int));
	return _t115;
}
...
string strconv__fxx_to_str_l_parse(string s) {
	...
	int exp = 0;
	int exp_sgn = 1;
	for (int _t202 = 0; _t202 < s.len; ++_t202) {
		byte c = s.str[_t202];
		if (c == '-') {
			sgn = -1;
			i++;
...
string strconv__fxx_to_str_l_parse_no_dot(string s) {
	...
	int exp = 0;
	int exp_sgn = 1;
	for (int _t206 = 0; _t206 < s.len; ++_t206) {
		byte c = s.str[_t206];
		if (c == '-') {
			sgn = -1;
			i++;
...
// Attr: [unsafe]
void Array_string_free(Array_string* a) {
	for (int _t260 = 0; _t260 < a->len; ++_t260) {
		string s = ((string*)a->data)[_t260];
		string_free(&s);
	}
	v_free(a->data);
}
...
string Array_byte_hex(Array_byte b) {
	...
	int dst_i = 0;
	for (int _t262 = 0; _t262 < b.len; ++_t262) {
		byte i = ((byte*)b.data)[_t262];
		byte n0 = i >> 4;
		{ // Unsafe block
...
int Array_int_reduce(Array_int a, int (*iter)(int , int ), int accum_start) {
	...
	for (int _t273 = 0; _t273 < a.len; ++_t273) {
		int i = ((int*)a.data)[_t273];
...
// Attr: [noreturn]
VNORETURN void v_exit(int code) {
	exit(code);
	VUNREACHABLE();
	while(1);
}
...
VV_LOCAL_SYMBOL voidptr __as_cast(voidptr obj, int obj_type, int expected_type) {
	if (obj_type != expected_type) {
		...
		for (int _t298 = 0; _t298 < as_cast_type_indexes.len; ++_t298) {
			VCastTypeIndexName x = ((VCastTypeIndexName*)as_cast_type_indexes.data)[_t298];
...
VV_LOCAL_SYMBOL bool print_backtrace_skipping_top_frames_linux(int skipframes) {
	...
	#else
	{
		#if defined(__linux__) && !defined(_VFREESTANDING)
		{
			...
			for (int _t307 = 0; _t307 < sframes.len; ++_t307) {
				string sframe = ((string*)sframes.data)[_t307];
...
string byte_str_escaped(byte b) {
	...
	if (b == (0)) {
		...
		_t379 = byte_ascii_str(b);
	}
	else {
		_t379 = string__plus(_SLIT("0x"), byte_hex(b));
	}string str = _t379;
	return str;
}
...
VV_LOCAL_SYMBOL voidptr map_get_and_set(map* m, voidptr key, voidptr zero) {
	...
	if (false) {
		...
		v_assert_meta_info__t406.fpath = _SLIT("/root/v/vlib/builtin/map.v");
		v_assert_meta_info__t406.line_nr = 560;
		v_assert_meta_info__t406.fn_name = _SLIT("get_and_set");
		v_assert_meta_info__t406.src = _SLIT("false");
		main__cb_assertion_ok(&v_assert_meta_info__t406);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t407 = {0};
		v_assert_meta_info__t407.fpath = _SLIT("/root/v/vlib/builtin/map.v");
		v_assert_meta_info__t407.line_nr = 560;
		v_assert_meta_info__t407.fn_name = _SLIT("get_and_set");
		v_assert_meta_info__t407.src = _SLIT("false");
...
string Array_rune_string(Array_rune ra) {
	string res = _SLIT("");
	for (int _t431 = 0; _t431 < ra.len; ++_t431) {
		rune r = ((rune*)ra.data)[_t431];
...
Array_int idxs;
	...
	int b_i = 0;
	int s_idx = 0;
	for (int _t482 = 0; _t482 < idxs.len; ++_t482) {
		int rep_pos = ((int*)idxs.data)[_t482];
...
int string_int(string s) {
	Option_i64 _t494 = strconv__common_parse_int(s, 0, 32, false, false);
	if (_t494.state != 0) { /*or block*/ 
		IError err = _t494.err;
		*(i64*) _t494.data = 0;
	}
	
 	return ((int)( (*(i64*)_t494.data)));
}

i64 string_i64(string s) {
	Option_i64 _t496 = strconv__common_parse_int(s, 0, 64, false, false);
	if (_t496.state != 0) { /*or block*/ 
		IError err = _t496.err;
		*(i64*) _t496.data = 0;
	}
	
 	return  (*(i64*)_t496.data);
}

i8 string_i8(string s) {
	Option_i64 _t498 = strconv__common_parse_int(s, 0, 8, false, false);
	if (_t498.state != 0) { /*or block*/ 
		IError err = _t498.err;
		*(i64*) _t498.data = 0;
	}
	
 	return ((i8)( (*(i64*)_t498.data)));
}

i16 string_i16(string s) {
	Option_i64 _t500 = strconv__common_parse_int(s, 0, 16, false, false);
	if (_t500.state != 0) { /*or block*/ 
		IError err = _t500.err;
		*(i64*) _t500.data = 0;
	}
	
 	return ((i16)( (*(i64*)_t500.data)));
}
...
u16 string_u16(string s) {
	Option_u64 _t504 = strconv__common_parse_uint(s, 0, 16, false, false);
	if (_t504.state != 0) { /*or block*/ 
		IError err = _t504.err;
		*(u64*) _t504.data = 0U;
	}
	
 	return ((u16)( (*(u64*)_t504.data)));
}

u32 string_u32(string s) {
	Option_u64 _t506 = strconv__common_parse_uint(s, 0, 32, false, false);
	if (_t506.state != 0) { /*or block*/ 
		IError err = _t506.err;
		*(u64*) _t506.data = 0U;
	}
	
 	return ((u32)( (*(u64*)_t506.data)));
}

u64 string_u64(string s) {
	Option_u64 _t508 = strconv__common_parse_uint(s, 0, 64, false, false);
	if (_t508.state != 0) { /*or block*/ 
		IError err = _t508.err;
		*(u64*) _t508.data = 0U;
	}
	
 	return  (*(u64*)_t508.data);
}
...
Array_string string_split_nth(string s, string delim, int nth) {
	...
	if (delim.len == (0)) {
		i = 1;
		for (int _t518 = 0; _t518 < s.len; ++_t518) {
			byte ch = s.str[_t518];
			if (nth > 0 && i >= nth) {
...
Option_int string_index(string s, string p) {
	...
	Option_int _t540;
	opt_ok(&(int[]) { idx }, (Option*)(&_t540), sizeof(int));
	return _t540;
}
...
int string_index_any(string s, string chars) {
	for (int _t544 = 0; _t544 < chars.len; ++_t544) {
		byte c = chars.str[_t544];
...
Option_int string_last_index(string s, string p) {
	...
	Option_int _t551;
	opt_ok(&(int[]) { idx }, (Option*)(&_t551), sizeof(int));
	return _t551;
}
...
int string_count(string s, string substr) {
	...
	if (substr.len == 1) {
		...
		for (int _t562 = 0; _t562 < s.len; ++_t562) {
			byte letter = s.str[_t562];
			if (letter == target) {
				n++;
			}
		}
...
bool string_contains_any(string s, string chars) {
	for (int _t569 = 0; _t569 < chars.len; ++_t569) {
		byte c = chars.str[_t569];
...
bool string_contains_any_substr(string s, Array_string substrs) {
	...
	for (int _t573 = 0; _t573 < substrs.len; ++_t573) {
		string sub = ((string*)substrs.data)[_t573];
...
string string_title(string s) {
	...
	for (int _t594 = 0; _t594 < words.len; ++_t594) {
		string word = ((string*)words.data)[_t594];
...
bool string_is_title(string s) {
	Array_string words = string_split(s, _SLIT(" "));
	for (int _t597 = 0; _t597 < words.len; ++_t597) {
		string word = ((string*)words.data)[_t597];
...
string string_trim(string s, string cutset) {
	...
	for (;;) {
		...
		cs_match = false;
		for (int _t605 = 0; _t605 < cutset.len; ++_t605) {
			byte cs = cutset.str[_t605];
		...
		for (int _t606 = 0; _t606 < cutset.len; ++_t606) {
			byte cs = cutset.str[_t606];
...
string string_trim_left(string s, string cutset) {
	...
	for (;;) {
		if (!(pos < s.len)) break;
		bool found = false;
		for (int _t610 = 0; _t610 < cutset.len; ++_t610) {
			byte cs = cutset.str[_t610];
			if (s.str[ pos] == cs) {
				found = true;
				break;
			}
...
string string_trim_right(string s, string cutset) {
	...
	for (;;) {
		if (!(pos >= 0)) break;
		bool found = false;
		for (int _t613 = 0; _t613 < cutset.len; ++_t613) {
			byte cs = cutset.str[_t613];
			if (s.str[ pos] == cs) {
				found = true;
			}
		}
...
VV_LOCAL_SYMBOL Option_byte string_at_with_check(string s, int idx) {
	if (idx < 0 || idx >= s.len) {
		return (Option_byte){ .state=2, .err=v_error(_SLIT("string index out of range")), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	{ // Unsafe block
		Option_byte _t633;
		opt_ok(&(byte[]) { s.str[idx] }, (Option*)(&_t633), sizeof(byte));
		return _t633;
	}
...
Option_os__File os__open_file(string path, string mode, Array_int options) {
	...
	if (fd == -1) {
		return (Option_os__File){ .state=2, .err=v_error(os__posix_get_error_msg(errno)), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	FILE* cfile = fdopen(fd, ((char*)(mode.str)));
	if (isnil(cfile)) {
		return (Option_os__File){ .state=2, .err=v_error( str_intp(2, _MOV((StrIntpData[]){{_SLIT("Failed to open or create file \""), 0xfe10, {.d_s = path}}, {_SLIT("\""), 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
...
Option_int os__File_read(os__File* f, Array_byte* buf) {
	...
	Option_int _t773 = os__fread(buf->data, 1, buf->len, f->cfile);
	if (_t773.state != 0) { /*or block*/ 
 	...
 	int nbytes =  (*(int*)_t773.data);
	Option_int _t775;
	opt_ok(&(int[]) { nbytes }, (Option*)(&_t775), sizeof(int));
	return _t775;
}
...
Option_int os__File_write(os__File* f, Array_byte buf) {
	...
	if (written == 0 && buf.len != 0) {
		return (Option_int){ .state=2, .err=v_error(_SLIT("0 bytes written")), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	Option_int _t778;
	opt_ok(&(int[]) { written }, (Option*)(&_t778), sizeof(int));
	return _t778;
}
...
Option_int os__File_writeln(os__File* f, string s) {
	...
	int x = fputs("\n", f->cfile);
	if (x < 0) {
		return (Option_int){ .state=2, .err=v_error(_SLIT("could not add newline")), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	Option_int _t782;
	opt_ok(&(int[]) { (written + 1) }, (Option*)(&_t782), sizeof(int));
	return _t782;
}

Option_int os__File_write_string(os__File* f, string s) {
		Option_void _t783 = os__File_write_full_buffer(f, s.str, ((size_t)(s.len)));
	if (_t783.state != 0 && _t783.err._typ != _IError_None___index) {
		Option_int _t784;
		memcpy(&_t784, &_t783, sizeof(Option));
		return _t784;
	}
	
 ;
	Option_int _t785;
	opt_ok(&(int[]) { s.len }, (Option*)(&_t785), sizeof(int));
	return _t785;
}
...
Option_int os__File_write_to(os__File* f, u64 pos, Array_byte buf) {
	...
	#if defined(TARGET_IS_64BIT)
	{
		...
		#else
		{
			...
			Option_int _t788;
			opt_ok(&(int[]) { res }, (Option*)(&_t788), sizeof(int));
			return _t788;
		}
		#endif
	}
	...
	#if defined(TARGET_IS_32BIT)
	{
		...
		Option_int _t790;
		opt_ok(&(int[]) { res }, (Option*)(&_t790), sizeof(int));
		return _t790;
	}
	#endif
	return (Option_int){ .state=2, .err=v_error(_SLIT("Could not write to file")), .data={EMPTY_STRUCT_INITIALIZATION} };
}
...
Option_void os__File_write_full_buffer(os__File* f, voidptr buffer, size_t buffer_len) {
	...
	for (;;) {
		...
		{ // Unsafe block
			...
			remaining_bytes -= x;
			if (x <= 0) {
				return (Option_void){ .state=2, .err=v_error(_SLIT("C.fwrite returned 0")), .data={EMPTY_STRUCT_INITIALIZATION} };
			}
		}
	}
	return (Option_void){0};
...
VV_LOCAL_SYMBOL Option_int os__fread(voidptr ptr, int item_size, int items, FILE* stream) {
	...
	if (nbytes <= 0) {
		...
		if (ferror(stream) != 0) {
			return (Option_int){ .state=2, .err=v_error(_SLIT("file read error")), .data={EMPTY_STRUCT_INITIALIZATION} };
		}
	}
	Option_int _t800;
	opt_ok(&(int[]) { nbytes }, (Option*)(&_t800), sizeof(int));
	return _t800;
}
...
Array_byte os__File_read_bytes_at(os__File* f, int size, u64 pos) {
	...
	Option_int _t802 = os__File_read_bytes_into(f, pos, &/*arr*/arr);
	if (_t802.state != 0) { /*or block*/ 
		IError err = _t802.err;
		return __new_array_with_default(0, 0, sizeof(byte), 0);
	}
	
 	int nreadbytes =  (*(int*)_t802.data);
...
Option_int os__File_read_bytes_into_newline(os__File* f, Array_byte* buf) {
	...
	for (;;) {
		...
		if (c == (EOF)) {
			if (feof(stream) != 0) {
				Option_int _t805;
				opt_ok(&(int[]) { nbytes }, (Option*)(&_t805), sizeof(int));
				return _t805;
			}
			if (ferror(stream) != 0) {
				return (Option_int){ .state=2, .err=v_error(_SLIT("file read error")), .data={EMPTY_STRUCT_INITIALIZATION} };
			...
			nbytes++;
			Option_int _t807;
			opt_ok(&(int[]) { nbytes }, (Option*)(&_t807), sizeof(int));
			return _t807;
		}
		else {
			array_set(buf, buf_ptr, &(byte[]) { ((byte)(c)) });
...
Option_int os__File_read_bytes_into(os__File* f, u64 pos, Array_byte* buf) {
	...
	#if defined(TARGET_IS_64BIT)
	{
		...
		{
			fseeko(f->cfile, pos, SEEK_SET);
			Option_int _t809 = os__fread(buf->data, 1, buf->len, f->cfile);
			if (_t809.state != 0) { /*or block*/ 
				Option_int _t810;
				memcpy(&_t810, &_t809, sizeof(Option));
				return _t810;
			}
			
 			int nbytes =  (*(int*)_t809.data);
			Option_int _t811;
	...
	#endif
	return (Option_int){ .state=2, .err=v_error(_SLIT("Could not read file")), .data={EMPTY_STRUCT_INITIALIZATION} };
}

Option_int os__File_read_from(os__File* f, u64 pos, Array_byte* buf) {
	if (buf->len == 0) {
		Option_int _t816;
		opt_ok(&(int[]) { 0 }, (Option*)(&_t816), sizeof(int));
		return _t816;
	}
	#if defined(TARGET_IS_64BIT)
	{
		...
		#endif
		Option_int _t817 = os__fread(buf->data, 1, buf->len, f->cfile);
		if (_t817.state != 0) { /*or block*/ 
			Option_int _t818;
			memcpy(&_t818, &_t817, sizeof(Option));
			return _t818;
		}
		
 		int nbytes =  (*(int*)_t817.data);
		Option_int _t819;
		opt_ok(&(int[]) { nbytes }, (Option*)(&_t819), sizeof(int));
		return _t819;
	}
	#endif
	#if defined(TARGET_IS_32BIT)
...
Option_void os__cp_all(string src, string dst, bool overwrite) {
	...
	if (!os__exists(source_path)) {
		return (Option_void){ .state=2, .err=v_error(_SLIT("Source path doesn't exist")), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	if (!os__is_dir(source_path)) {
		...
		if (os__exists(adjusted_path)) {
			if (overwrite) {
				Option_void _t828 = os__rm(adjusted_path);
				if (_t828.state != 0 && _t828.err._typ != _IError_None___index) {
					Option_void _t829;
				...
				return (Option_void){ .state=2, .err=v_error(_SLIT("Destination file path already exist")), .data={EMPTY_STRUCT_INITIALIZATION} };
			}
		}
		Option_void _t831 = os__cp(source_path, adjusted_path);
		if (_t831.state != 0 && _t831.err._typ != _IError_None___index) {
			Option_void _t832;
			memcpy(&_t832, &_t831, sizeof(Option));
			return _t832;
		}
		;
		return (Option_void){0};
	...
	if (!os__exists(dest_path)) {
		Option_bool _t833 = os__mkdir(dest_path);
		if (_t833.state != 0) { /*or block*/ 
			Option_void _t834;
			memcpy(&_t834, &_t833, sizeof(Option));
			return _t834;
		}
		;
	}
	if (!os__is_dir(dest_path)) {
		return (Option_void){ .state=2, .err=v_error(_SLIT("Destination path is not a valid directory")), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	Option_Array_string _t836 = os__ls(source_path);
	if (_t836.state != 0) { /*or block*/ 
 	...
 	Array_string files =  (*(Array_string*)_t836.data);
	for (int _t838 = 0; _t838 < files.len; ++_t838) {
		string file = ((string*)files.data)[_t838];
		...
		if (os__is_dir(sp)) {
			if (!os__exists(dp)) {
				Option_bool _t839 = os__mkdir(dp);
				if (_t839.state != 0) { /*or block*/ 
					Option_void _t840;
		...
		Option_void _t841 = os__cp_all(sp, dp, overwrite);
		if (_t841.state != 0 && _t841.err._typ != _IError_None___index) {
			IError err = _t841.err;
			Option_void _t842 = os__rmdir(dp);
...
Option_void os__mv_by_cp(string source, string target) {
	Option_void _t845 = os__cp(source, target);
	...
	;
	Option_void _t847 = os__rm(source);
	if (_t847.state != 0 && _t847.err._typ != _IError_None___index) {
		Option_void _t848;
...
Option_Array_string os__read_lines(string path) {
	Option_string _t849 = os__read_file(path);
	if (_t849.state != 0) { /*or block*/ 
		Option_Array_string _t850;
		memcpy(&_t850, &_t849, sizeof(Option));
		return _t850;
	}
	
 	string buf =  (*(string*)_t849.data);
	Array_string res = string_split_into_lines(buf);
	string_free(&buf);
	Option_Array_string _t851;
	opt_ok(&(Array_string[]) { res }, (Option*)(&_t851), sizeof(Array_string));
	return _t851;
}
...
Option_void os__rmdir_all(string path) {
	...
	Option_Array_string _t874 = os__ls(path);
	if (_t874.state != 0) { /*or block*/ 
		Option_void _t875;
		memcpy(&_t875, &_t874, sizeof(Option));
		return _t875;
	}
	
 	Array_string items =  (*(Array_string*)_t874.data);
	for (int _t876 = 0; _t876 < items.len; ++_t876) {
		string item = ((string*)items.data)[_t876];
		string fullpath = os__join_path(path, new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){item})));
		if (os__is_dir(fullpath)) {
			Option_void _t877 = os__rmdir_all(fullpath);
			if (_t877.state != 0 && _t877.err._typ != _IError_None___index) {
				IError err = _t877.err;
				ret_err = (*(err.msg));
			}
			;
		} else {
			Option_void _t878 = os__rm(fullpath);
			if (_t878.state != 0 && _t878.err._typ != _IError_None___index) {
				IError err = _t878.err;
				ret_err = (*(err.msg));
			}
			;
		}
	}
	Option_void _t879 = os__rmdir(path);
	if (_t879.state != 0 && _t879.err._typ != _IError_None___index) {
		IError err = _t879.err;
		ret_err = (*(err.msg));
	}
	;
	if (ret_err.len > 0) {
		return (Option_void){ .state=2, .err=v_error(ret_err), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	return (Option_void){0};
}

bool os__is_dir_empty(string path) {
	Option_Array_string _t881 = os__ls(path);
	if (_t881.state != 0) { /*or block*/ 
		IError err = _t881.err;
		return true;
	}
	
 	Array_string items =  (*(Array_string*)_t881.data);
	return items.len == 0;
}

string os__file_ext(string path) {
	Option_int _t884 = string_last_index(path, _SLIT("."));
	if (_t884.state != 0) { /*or block*/ 
		IError err = _t884.err;
		return _SLIT("");
	}
	
 	int pos =  (*(int*)_t884.data);
...
string os__dir(string path) {
	...
	Option_int _t888 = string_last_index(path, _const_os__path_separator);
	if (_t888.state != 0) { /*or block*/ 
		IError err = _t888.err;
		return _SLIT(".");
	}
	
 	int pos =  (*(int*)_t888.data);
...
string os__base(string path) {
	...
	if (string_ends_with(path, _const_os__path_separator)) {
		...
		Option_int _t893 = string_last_index(path2, _const_os__path_separator);
	...
	Option_int _t896 = string_last_index(path, _const_os__path_separator);
	if (_t896.state != 0) { /*or block*/ 
		IError err = _t896.err;
		return string_clone(path);
	}
	
 	int pos =  (*(int*)_t896.data);
...
string os__input(string prompt) {
	Option_string _t902 = os__input_opt(prompt);
	if (_t902.state != 0) { /*or block*/ 
		IError err = _t902.err;
		return _SLIT("<EOF>");
	}
	
 	string res =  (*(string*)_t902.data);
	return res;
}
...
Option_void os__write_file(string path, string text) {
	Option_os__File _t914 = os__create(path);
	if (_t914.state != 0) { /*or block*/ 
		Option_void _t915;
		memcpy(&_t915, &_t914, sizeof(Option));
		return _t915;
	}
	
 	os__File f =  (*(os__File*)_t914.data);
		Option_void _t916 = os__File_write_full_buffer(&f, text.str, ((size_t)(text.len)));
	if (_t916.state != 0 && _t916.err._typ != _IError_None___index) {
		Option_void _t917;
		memcpy(&_t917, &_t916, sizeof(Option));
		return _t917;
	}
...
Option_void os__write_file_array(string path, array buffer) {
	Option_os__File _t918 = os__create(path);
	if (_t918.state != 0) { /*or block*/ 
		Option_void _t919;
		memcpy(&_t919, &_t918, sizeof(Option));
		return _t919;
	}
	
 	os__File f =  (*(os__File*)_t918.data);
		Option_void _t920 = os__File_write_full_buffer(&f, buffer.data, ((size_t)(buffer.len * buffer.element_size)));
	if (_t920.state != 0 && _t920.err._typ != _IError_None___index) {
		Option_void _t921;
		memcpy(&_t921, &_t920, sizeof(Option));
		return _t921;
	}
...
VV_LOCAL_SYMBOL string os__executable_fallback(void) {
	...
	if (!os__is_abs_path(exepath)) {
		if (string_contains(exepath, _const_os__path_separator)) {
			...
			Option_string _t923 = os__find_abs_path_of_executable(exepath);
			if (_t923.state != 0) { /*or block*/ 
				IError err = _t923.err;
				*(string*) _t923.data = _SLIT("");
			}
			
 			string foundpath =  (*(string*)_t923.data);
			if (foundpath.len > 0) {
				exepath = foundpath;
			}
		}
...
Option_string os__find_abs_path_of_executable(string exepath) {
	if ((exepath).len == 0) {
		return (Option_string){ .state=2, .err=v_error(_SLIT("expected non empty `exepath`")), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	if (os__is_abs_path(exepath)) {
		Option_string _t926;
		opt_ok(&(string[]) { os__real_path(exepath) }, (Option*)(&_t926), sizeof(string));
		return _t926;
	}
	string res = _SLIT("");
	Array_string paths = string_split(os__getenv(_SLIT("PATH")), _const_os__path_delimiter);
	for (int _t927 = 0; _t927 < paths.len; ++_t927) {
		string p = ((string*)paths.data)[_t927];
	...
	if (res.len > 0) {
		Option_string _t928;
		opt_ok(&(string[]) { os__real_path(res) }, (Option*)(&_t928), sizeof(string));
		return _t928;
	}
	return (Option_string){ .state=2, .err=v_error(_SLIT("failed to find executable")), .data={EMPTY_STRUCT_INITIALIZATION} };
}

bool os__exists_in_system_path(string prog) {
	Option_string _t930 = os__find_abs_path_of_executable(prog);
...
string os__join_path(string base, Array_string dirs) {
	...
	for (int _t937 = 0; _t937 < dirs.len; ++_t937) {
		string d = ((string*)dirs.data)[_t937];
...
void os__walk(string path, void (*f)(string )) {
	...
	Option_Array_string _t947 = os__ls(path);
	if (_t947.state != 0) { /*or block*/ 
		IError err = _t947.err;
		return;
	}
	
 	Array_string files =  (*(Array_string*)_t947.data);
	string local_path_separator = _const_os__path_separator;
	if (string_ends_with(path, _const_os__path_separator)) {
		local_path_separator = _SLIT("");
	}
	for (int _t948 = 0; _t948 < files.len; ++_t948) {
		string file = ((string*)files.data)[_t948];
...
Option_void os__mkdir_all(string path) {
	...
	for (int _t949 = 0; _t949 < path_parts.len; ++_t949) {
		string subdir = ((string*)path_parts.data)[_t949];
		p = /*f*/string__plus(p, string__plus(subdir, _const_os__path_separator));
		if (os__exists(p) && os__is_dir(p)) {
			continue;
		}
		Option_bool _t950 = os__mkdir(p);
		if (_t950.state != 0) { /*or block*/ 
			IError err = _t950.err;
			return (Option_void){ .state=2, .err=v_error( str_intp(3, _MOV((StrIntpData[]){{_SLIT("folder: "), 0xfe10, {.d_s = p}}, {_SLIT(", error: "), 0xfe10, {.d_s = IError_str(err)}}, {_SLIT0, 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
		}
		;
	}
	return (Option_void){0};
...
string os__cache_dir(void) {
	...
	if (!os__is_dir(cdir) && !os__is_link(cdir)) {
		Option_bool _t953 = os__mkdir(cdir);
...
Array_string os__vmodules_paths(void) {
	...
	Array_string _t959_orig = string_split(path, _const_os__path_delimiter);
	int _t959_len = _t959_orig.len;
	Array_string _t959 = __new_array(0, _t959_len, sizeof(string));

	for (int _t960 = 0; _t960 < _t959_len; ++_t960) {
		string it = ((string*) _t959_orig.data)[_t960];
		string ti = string_trim_right(it, _const_os__path_separator);
		array_push((array*)&_t959, &ti);
	}
	
	Array_string list = _t959;
	return list;
}
...
// Attr: [manualfree]
Option_Array_byte os__read_bytes(string path) {
	Option_FILE_ptr _t965 = os__vfopen(path, _SLIT("rb"));
	if (_t965.state != 0) { /*or block*/ 
		Option_Array_byte _t966;
		memcpy(&_t966, &_t965, sizeof(Option));
		return _t966;
	}
	
 	FILE* fp =  (*(FILE**)_t965.data);
	...
	Option_Array_byte _t970;
	opt_ok(&(Array_byte[]) { res }, (Option*)(&_t970), sizeof(Array_byte));
	return _t970;
}
...
FILE* fp;
	...
	Option_FILE_ptr _t971 = os__vfopen(path, mode);
	if (_t971.state != 0) { /*or block*/ 
		Option_string _t972;
		memcpy(&_t972, &_t971, sizeof(Option));
		return _t972;
	}
	...
	int cseek = fseek(fp, 0, SEEK_END);
	if (cseek != 0) {
		Option_string _t973 = (Option_string){ .state=2, .err=v_error(_SLIT("fseek failed")), .data={EMPTY_STRUCT_INITIALIZATION} };
		// Defer begin
	...
	int fsize = ftell(fp);
	if (fsize < 0) {
		Option_string _t974 = (Option_string){ .state=2, .err=v_error(_SLIT("ftell failed")), .data={EMPTY_STRUCT_INITIALIZATION} };
		// Defer begin
		if (os__read_file_defer_0) {
			fclose(fp);
		}
	...
	{ // Unsafe block
		...
		if (is_eof == 0 && is_error != 0) {
			v_free(str);
			Option_string _t975 = (Option_string){ .state=2, .err=v_error(_SLIT("fread failed")), .data={EMPTY_STRUCT_INITIALIZATION} };
			// Defer begin
			if (os__read_file_defer_0) {
				fclose(fp);
			}
		...
		if (nelements == 0) {
			Option_string _t976;
			opt_ok(&(string[]) { byte_vstring(str) }, (Option*)(&_t976), sizeof(string));
			// Defer begin
			if (os__read_file_defer_0) {
				fclose(fp);
			}
		...
		Option_string _t977;
		opt_ok(&(string[]) { byte_vstring_with_len(str, nelements) }, (Option*)(&_t977), sizeof(string));
		// Defer begin
		if (os__read_file_defer_0) {
			fclose(fp);
		}
...
int fp;
	...
	if (fp < 0) {
		Option_void _t978 = (Option_void){ .state=2, .err=error_with_code(os__posix_get_error_msg(errno), errno), .data={EMPTY_STRUCT_INITIALIZATION} };
		// Defer begin
	...
	#else
	{
		if (ftruncate(fp, len) != 0) {
			Option_void _t979 = (Option_void){ .state=2, .err=error_with_code(os__posix_get_error_msg(errno), errno), .data={EMPTY_STRUCT_INITIALIZATION} };
			// Defer begin
			if (os__truncate_defer_0) {
				close(fp);
			}
...
Option_FILE_ptr os__vfopen(string path, string mode) {
	if (path.len == 0) {
		return (Option_FILE_ptr){ .state=2, .err=v_error(_SLIT("vfopen called with \"\"")), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	...
	#endif
	if (isnil(fp)) {
		return (Option_FILE_ptr){ .state=2, .err=v_error( str_intp(2, _MOV((StrIntpData[]){{_SLIT("failed to open file \""), 0xfe10, {.d_s = path}}, {_SLIT("\""), 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
	} else {
		Option_FILE_ptr _t992;
		opt_ok(&(FILE*[]) { fp }, (Option*)(&_t992), sizeof(FILE*));
		return _t992;
	}
...
Option_void os__rm(string path) {
	...
	#endif
	if (rc == -1) {
		return (Option_void){ .state=2, .err=v_error(string__plus( str_intp(2, _MOV((StrIntpData[]){{_SLIT("Failed to remove \""), 0xfe10, {.d_s = path}}, {_SLIT("\": "), 0, { .d_c = 0 }}})), os__posix_get_error_msg(errno))), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	return (Option_void){0};
}
...
Option_void os__rmdir(string path) {
	...
	#else
	{
		...
		if (rc == -1) {
			return (Option_void){ .state=2, .err=v_error(os__posix_get_error_msg(errno)), .data={EMPTY_STRUCT_INITIALIZATION} };
		}
	}
	#endif
	return (Option_void){0};
...
Option_os__File os__open_append(string path) {
	...
	#endif
	if (isnil(file.cfile)) {
		return (Option_os__File){ .state=2, .err=v_error( str_intp(2, _MOV((StrIntpData[]){{_SLIT("failed to create(append) file \""), 0xfe10, {.d_s = path}}, {_SLIT("\""), 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	file.is_opened = true;
	Option_os__File _t1024;
	opt_ok(&(os__File[]) { file }, (Option*)(&_t1024), sizeof(os__File));
	return _t1024;
}
...
Option_Array_string os__glob(Array_string patterns) {
	...
	if (patterns.len == 0) {
		Option_Array_string _t1035;
		opt_ok(&(Array_string[]) { matches }, (Option*)(&_t1035), sizeof(Array_string));
		return _t1035;
	}
	...
	globfree(&globdata);
	Option_Array_string _t1038;
	opt_ok(&(Array_string[]) { matches }, (Option*)(&_t1038), sizeof(Array_string));
	return _t1038;
}
...
Option_Array_string os__ls(string path) {
	...
	if (isnil(dir)) {
		return (Option_Array_string){ .state=2, .err=v_error( str_intp(2, _MOV((StrIntpData[]){{_SLIT("ls() couldnt open dir \""), 0xfe10, {.d_s = path}}, {_SLIT("\""), 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	...
	closedir(dir);
	Option_Array_string _t1049;
	opt_ok(&(Array_string[]) { res }, (Option*)(&_t1049), sizeof(Array_string));
	return _t1049;
}
...
Option_bool os__mkdir(string path) {
	if (string__eq(path, _SLIT("."))) {
		Option_bool _t1050;
		opt_ok(&(bool[]) { true }, (Option*)(&_t1050), sizeof(bool));
		return _t1050;
	}
	string apath = os__real_path(path);
	int r = mkdir(((char*)(apath.str)), 511U);
	if (r == -1) {
		return (Option_bool){ .state=2, .err=v_error(os__posix_get_error_msg(errno)), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	Option_bool _t1052;
	opt_ok(&(bool[]) { true }, (Option*)(&_t1052), sizeof(bool));
	return _t1052;
}
...
strings__Builder res;
	...
	v_free(buf);
	os__Result _t1054 = (os__Result){.exit_code = exit_code,.output = soutput,};
	// Defer begin
...
string pcmd;
	...
	c->f = os__vpopen(pcmd);
	if (isnil(c->f)) {
		Option_void _t1055 = (Option_void){ .state=2, .err=v_error( str_intp(2, _MOV((StrIntpData[]){{_SLIT("exec(\""), 0xfe10, {.d_s = c->path}}, {_SLIT("\") failed"), 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
		// Defer begin
...
Option_bool os__symlink(string origin, string target) {
	...
	if (res == 0) {
		Option_bool _t1059;
		opt_ok(&(bool[]) { true }, (Option*)(&_t1059), sizeof(bool));
		return _t1059;
	}
	return (Option_bool){ .state=2, .err=v_error(os__posix_get_error_msg(errno)), .data={EMPTY_STRUCT_INITIALIZATION} };
}
...
Option_bool os__link(string origin, string target) {
	...
	if (res == 0) {
		Option_bool _t1061;
		opt_ok(&(bool[]) { true }, (Option*)(&_t1061), sizeof(bool));
		return _t1061;
	}
	return (Option_bool){ .state=2, .err=v_error(os__posix_get_error_msg(errno)), .data={EMPTY_STRUCT_INITIALIZATION} };
}
...
Option_bool os__is_writable_folder(string folder) {
	if (!os__exists(folder)) {
		return (Option_bool){ .state=2, .err=v_error( str_intp(2, _MOV((StrIntpData[]){{_SLIT("`"), 0xfe10, {.d_s = folder}}, {_SLIT("` does not exist"), 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	if (!os__is_dir(folder)) {
		return (Option_bool){ .state=2, .err=v_error(_SLIT("`folder` is not a folder")), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	...
	{ // Unsafe block
		...
		if (-1 == x) {
			return (Option_bool){ .state=2, .err=v_error( str_intp(2, _MOV((StrIntpData[]){{_SLIT("folder `"), 0xfe10, {.d_s = folder}}, {_SLIT("` is not writable"), 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
		}
		close(x);
	}
	Option_void _t1069 = os__rm(tmp_perm_check);
	if (_t1069.state != 0 && _t1069.err._typ != _IError_None___index) {
		Option_bool _t1070;
		memcpy(&_t1070, &_t1069, sizeof(Option));
		return _t1070;
	}
	;
	Option_bool _t1071;
...
void os__Process_set_environment(os__Process* p, Map_string_string envs) {
	...
	int _t1080 = envs.key_values.len;
	for (int _t1079 = 0; _t1079 < _t1080; ++_t1079 ) {
		int _t1081 = envs.key_values.len - _t1080;
		_t1080 = envs.key_values.len;
		if (_t1081 < 0) {
			_t1079 = -1;
			continue;
		}
		if (!DenseArray_has_index(&envs.key_values, _t1079)) {continue;}
		string k = /*key*/ *(string*)DenseArray_key(&envs.key_values, _t1079);
		k = string_clone(k);
		string v = (*(string*)DenseArray_value(&envs.key_values, _t1079));
...
VV_LOCAL_SYMBOL int os__Process__spawn(os__Process* p) {
	if (!p->env_is_custom) {
		...
		int _t1084 = current_environment.key_values.len;
		for (int _t1083 = 0; _t1083 < _t1084; ++_t1083 ) {
			int _t1085 = current_environment.key_values.len - _t1084;
			_t1084 = current_environment.key_values.len;
			if (_t1085 < 0) {
				_t1083 = -1;
				continue;
			}
			if (!DenseArray_has_index(&current_environment.key_values, _t1083)) {continue;}
			string k = /*key*/ *(string*)DenseArray_key(&current_environment.key_values, _t1083);
			k = string_clone(k);
			string v = (*(string*)DenseArray_value(&current_environment.key_values, _t1083));
			array_push((array*)&p->env, _MOV((string[]){  str_intp(3, _MOV((StrIntpData[]){{_SLIT0, 0xfe10, {.d_s = k}}, {_SLIT("="), 0xfe10, {.d_s = v}}, {_SLIT0, 0, { .d_c = 0 }}})) }));
		}
	}
	int pid = 0;
...
VV_LOCAL_SYMBOL int os__Process_unix_spawn_process(os__Process* p) {
	...
	Option_void _t1096 = os__execve(p->filename, p->args, p->env);
	if (_t1096.state != 0 && _t1096.err._typ != _IError_None___index) {
		IError err = _t1096.err;
...
Option_os__SignalHandler os__signal_opt(os__Signal signum, void (*handler)(os__Signal )) {
	...
	Option_os__SignalHandler _t1106;
	opt_ok(&(os__SignalHandler[]) { (voidptr)((os__SignalHandler)(prev_handler)) }, (Option*)(&_t1106), sizeof(os__SignalHandler));
	return _t1106;
}

strings__textscanner__TextScanner strings__textscanner__new(string input) {
	strings__textscanner__TextScanner _t1107 = (strings__textscanner__TextScanner){.input = input,.ilen = input.len,.pos = 0,};
	return _t1107;
}
...
inline int strings__textscanner__TextScanner_peek(strings__textscanner__TextScanner* ss) {
	if (ss->pos < ss->ilen) {
		int _t1111 = ss->input.str[ ss->pos];
		return _t1111;
	}
	int _t1112 = -1;
	return _t1112;
...
inline int strings__textscanner__TextScanner_peek_n(strings__textscanner__TextScanner* ss, int n) {
	if (ss->pos + n < ss->ilen) {
		int _t1113 = ss->input.str[ ss->pos + n];
		return _t1113;
	}
	int _t1114 = -1;
	return _t1114;
...
inline int strings__textscanner__TextScanner_peek_back(strings__textscanner__TextScanner* ss) {
	int _t1115 = strings__textscanner__TextScanner_peek_back_n(ss, 1);
	return _t1115;
}
...
inline int strings__textscanner__TextScanner_peek_back_n(strings__textscanner__TextScanner* ss, int n) {
	int offset = n + 1;
	if (ss->pos >= offset) {
		int _t1116 = ss->input.str[ ss->pos - offset];
		return _t1116;
	}
	int _t1117 = -1;
	return _t1117;
...
inline int strings__textscanner__TextScanner_current(strings__textscanner__TextScanner* ss) {
	if (ss->pos > 0) {
		int _t1118 = ss->input.str[ ss->pos - 1];
		return _t1118;
	}
	int _t1119 = -1;
	return _t1119;
...
string term__format(string msg, string open, string close) {
	string _t1120 = string__plus(string__plus(string__plus(string__plus(string__plus(string__plus(_SLIT("\x1b["), open), _SLIT("m")), msg), _SLIT("\x1b[")), close), _SLIT("m"));
	return _t1120;
}

string term__format_rgb(int r, int g, int b, string msg, string open, string close) {
	string _t1121 = string__plus(string__plus(string__plus(string__plus(string__plus(string__plus(string__plus(string__plus(string__plus(string__plus(string__plus(string__plus(_SLIT("\x1b["), open), _SLIT(";2;")), int_str(r)), _SLIT(";")), int_str(g)), _SLIT(";")), int_str(b)), _SLIT("m")), msg), _SLIT("\x1b[")), close), _SLIT("m"));
	return _t1121;
}

string term__rgb(int r, int g, int b, string msg) {
	string _t1122 = term__format_rgb(r, g, b, msg, _SLIT("38"), _SLIT("39"));
	return _t1122;
}

string term__bg_rgb(int r, int g, int b, string msg) {
	string _t1123 = term__format_rgb(r, g, b, msg, _SLIT("48"), _SLIT("49"));
	return _t1123;
}

string term__hex(int hex, string msg) {
	string _t1124 = term__format_rgb(hex >> 16, (hex >> 8 & 0xFF), (hex & 0xFF), msg, _SLIT("38"), _SLIT("39"));
	return _t1124;
}

string term__bg_hex(int hex, string msg) {
	string _t1125 = term__format_rgb(hex >> 16, (hex >> 8 & 0xFF), (hex & 0xFF), msg, _SLIT("48"), _SLIT("49"));
	return _t1125;
}

string term__bg_black(string msg) {
	string _t1126 = term__format(msg, _SLIT("40"), _SLIT("49"));
	return _t1126;
}

string term__bright_bg_black(string msg) {
	string _t1127 = term__format(msg, _SLIT("100"), _SLIT("49"));
	return _t1127;
}

string term__bg_blue(string msg) {
	string _t1128 = term__format(msg, _SLIT("44"), _SLIT("49"));
	return _t1128;
}

string term__bright_bg_blue(string msg) {
	string _t1129 = term__format(msg, _SLIT("104"), _SLIT("49"));
	return _t1129;
}

string term__bg_cyan(string msg) {
	string _t1130 = term__format(msg, _SLIT("46"), _SLIT("49"));
	return _t1130;
}

string term__bright_bg_cyan(string msg) {
	string _t1131 = term__format(msg, _SLIT("106"), _SLIT("49"));
	return _t1131;
}

string term__bg_green(string msg) {
	string _t1132 = term__format(msg, _SLIT("42"), _SLIT("49"));
	return _t1132;
}

string term__bright_bg_green(string msg) {
	string _t1133 = term__format(msg, _SLIT("102"), _SLIT("49"));
	return _t1133;
}

string term__bg_magenta(string msg) {
	string _t1134 = term__format(msg, _SLIT("45"), _SLIT("49"));
	return _t1134;
}

string term__bright_bg_magenta(string msg) {
	string _t1135 = term__format(msg, _SLIT("105"), _SLIT("49"));
	return _t1135;
}

string term__bg_red(string msg) {
	string _t1136 = term__format(msg, _SLIT("41"), _SLIT("49"));
	return _t1136;
}

string term__bright_bg_red(string msg) {
	string _t1137 = term__format(msg, _SLIT("101"), _SLIT("49"));
	return _t1137;
}

string term__bg_white(string msg) {
	string _t1138 = term__format(msg, _SLIT("47"), _SLIT("49"));
	return _t1138;
}

string term__bright_bg_white(string msg) {
	string _t1139 = term__format(msg, _SLIT("107"), _SLIT("49"));
	return _t1139;
}

string term__bg_yellow(string msg) {
	string _t1140 = term__format(msg, _SLIT("43"), _SLIT("49"));
	return _t1140;
}

string term__bright_bg_yellow(string msg) {
	string _t1141 = term__format(msg, _SLIT("103"), _SLIT("49"));
	return _t1141;
}

string term__black(string msg) {
	string _t1142 = term__format(msg, _SLIT("30"), _SLIT("39"));
	return _t1142;
}

string term__bright_black(string msg) {
	string _t1143 = term__format(msg, _SLIT("90"), _SLIT("39"));
	return _t1143;
}

string term__blue(string msg) {
	string _t1144 = term__format(msg, _SLIT("34"), _SLIT("39"));
	return _t1144;
}

string term__bright_blue(string msg) {
	string _t1145 = term__format(msg, _SLIT("94"), _SLIT("39"));
	return _t1145;
}

string term__bold(string msg) {
	string _t1146 = term__format(msg, _SLIT("1"), _SLIT("22"));
	return _t1146;
}

string term__cyan(string msg) {
	string _t1147 = term__format(msg, _SLIT("36"), _SLIT("39"));
	return _t1147;
}

string term__bright_cyan(string msg) {
	string _t1148 = term__format(msg, _SLIT("96"), _SLIT("39"));
	return _t1148;
}

string term__dim(string msg) {
	string _t1149 = term__format(msg, _SLIT("2"), _SLIT("22"));
	return _t1149;
}

string term__green(string msg) {
	string _t1150 = term__format(msg, _SLIT("32"), _SLIT("39"));
	return _t1150;
}

string term__bright_green(string msg) {
	string _t1151 = term__format(msg, _SLIT("92"), _SLIT("39"));
	return _t1151;
}

string term__gray(string msg) {
	string _t1152 = term__bright_black(msg);
	return _t1152;
}

string term__hidden(string msg) {
	string _t1153 = term__format(msg, _SLIT("8"), _SLIT("28"));
	return _t1153;
}

string term__italic(string msg) {
	string _t1154 = term__format(msg, _SLIT("3"), _SLIT("23"));
	return _t1154;
}

string term__inverse(string msg) {
	string _t1155 = term__format(msg, _SLIT("7"), _SLIT("27"));
	return _t1155;
}

string term__magenta(string msg) {
	string _t1156 = term__format(msg, _SLIT("35"), _SLIT("39"));
	return _t1156;
}

string term__bright_magenta(string msg) {
	string _t1157 = term__format(msg, _SLIT("95"), _SLIT("39"));
	return _t1157;
}

string term__reset(string msg) {
	string _t1158 = term__format(msg, _SLIT("0"), _SLIT("0"));
	return _t1158;
}

string term__red(string msg) {
	string _t1159 = term__format(msg, _SLIT("31"), _SLIT("39"));
	return _t1159;
}

string term__bright_red(string msg) {
	string _t1160 = term__format(msg, _SLIT("91"), _SLIT("39"));
	return _t1160;
}

string term__strikethrough(string msg) {
	string _t1161 = term__format(msg, _SLIT("9"), _SLIT("29"));
	return _t1161;
}

string term__underline(string msg) {
	string _t1162 = term__format(msg, _SLIT("4"), _SLIT("24"));
	return _t1162;
}

string term__white(string msg) {
	string _t1163 = term__format(msg, _SLIT("37"), _SLIT("39"));
	return _t1163;
}

string term__bright_white(string msg) {
	string _t1164 = term__format(msg, _SLIT("97"), _SLIT("39"));
	return _t1164;
}

string term__yellow(string msg) {
	string _t1165 = term__format(msg, _SLIT("33"), _SLIT("39"));
	return _t1165;
}

string term__bright_yellow(string msg) {
	string _t1166 = term__format(msg, _SLIT("93"), _SLIT("39"));
	return _t1166;
}
...
bool term__can_show_color_on_stdout(void) {
	bool _t1167 = term__supports_escape_sequences(1);
	return _t1167;
}

bool term__can_show_color_on_stderr(void) {
	bool _t1168 = term__supports_escape_sequences(2);
	return _t1168;
}

string term__ok_message(string s) {
	if (term__can_show_color_on_stdout()) {
		string _t1169 = term__green( str_intp(2, _MOV((StrIntpData[]){{_SLIT(" "), 0xfe10, {.d_s = s}}, {_SLIT(" "), 0, { .d_c = 0 }}})));
		return _t1169;
	}
	return s;
}
...
string term__fail_message(string s) {
	if (term__can_show_color_on_stdout()) {
		string _t1171 = term__inverse(term__bg_white(term__bold(term__red( str_intp(2, _MOV((StrIntpData[]){{_SLIT(" "), 0xfe10, {.d_s = s}}, {_SLIT(" "), 0, { .d_c = 0 }}}))))));
		return _t1171;
	}
	return s;
}
...
string term__warn_message(string s) {
	if (term__can_show_color_on_stdout()) {
		string _t1173 = term__bright_yellow( str_intp(2, _MOV((StrIntpData[]){{_SLIT(" "), 0xfe10, {.d_s = s}}, {_SLIT(" "), 0, { .d_c = 0 }}})));
		return _t1173;
	}
	return s;
}
...
string term__strip_ansi(string text) {
	...
	string _t1178 = Array_byte_bytestr(output);
	return _t1178;
}
...
string term__h_divider(string divider) {
	...
	string _t1179 = string_substr(result, 0, cols);
	return _t1179;
}
...
string term__header_left(string text, string divider) {
	...
	string _t1180 =  str_intp(4, _MOV((StrIntpData[]){{_SLIT0, 0xfe10, {.d_s = hstart}}, {_SLIT(" "), 0xfe10, {.d_s = text}}, {_SLIT(" "), 0xfe10, {.d_s = hend}}, {_SLIT0, 0, { .d_c = 0 }}}));
	return _t1180;
}

string term__header(string text, string divider) {
	if (text.len == 0) {
		string _t1181 = term__h_divider(divider);
		return _t1181;
	}
	...
	if (ln.len == 1) {
		string _t1182 = string__plus(string__plus(string__plus(string__plus(ln, _SLIT(" ")), string_substr(text, 0, tlimit)), _SLIT(" ")), ln);
		return _t1182;
	}
	string _t1183 = string__plus(string__plus(string__plus(string__plus(string_substr(ln, 0, tstart), _SLIT(" ")), string_substr(text, 0, tlimit)), _SLIT(" ")), string_substr(ln, tstart + tlimit + 2, cols));
	return _t1183;
}
...
term__Coord term__get_cursor_position(void) {
	if (os__is_atty(1) <= 0 || string__eq(os__getenv(_SLIT("TERM")), _SLIT("dumb"))) {
		term__Coord _t1191 = (term__Coord){.x = 0,.y = 0,};
		return _t1191;
	}
	...
	term__Coord _t1192 = (term__Coord){.x = x,.y = y,};
	return _t1192;
}
...
VV_LOCAL_SYMBOL bool main__can_use_relative_paths(void) {
	string _t1196 = os__getenv(_SLIT("VERROR_PATHS"));
		bool _t1195 = ((string__eq(_t1196, _SLIT("absolute"))) ? (false) : (true));
	return _t1195;
}
...
VV_LOCAL_SYMBOL void main__cb_propagate_test_error(int line_nr, string file, string mod, string fn_name, string errmsg) {
	...
	if (os__is_file(file)) {
		Option_Array_string _t1197 = os__read_lines(file);
		if (_t1197.state != 0) { /*or block*/ 
			IError err = _t1197.err;
			*(Array_string*) _t1197.data = __new_array_with_default(line_nr + 1, 0, sizeof(string), &(string[]){_SLIT("")});
		}
		
 		Array_string source_lines =  (*(Array_string*)_t1197.data);
...
VV_LOCAL_SYMBOL void main__test_comptime_pkgconfig(void) {
	#if true
	{
		// assert
		if (true) {
			g_test_oks++;
			VAssertMetaInfo v_assert_meta_info__t1198 = {0};
			v_assert_meta_info__t1198.fpath = _SLIT("vlib/v/tests/comptime_if_pkgconfig_test.v");
			v_assert_meta_info__t1198.line_nr = 2;
			v_assert_meta_info__t1198.fn_name = _SLIT("main.test_comptime_pkgconfig");
			v_assert_meta_info__t1198.src = _SLIT("true");
			main__cb_assertion_ok(&v_assert_meta_info__t1198);
		} else {
			g_test_fails++;
			VAssertMetaInfo v_assert_meta_info__t1199 = {0};
			v_assert_meta_info__t1199.fpath = _SLIT("vlib/v/tests/comptime_if_pkgconfig_test.v");
			v_assert_meta_info__t1199.line_nr = 2;
			v_assert_meta_info__t1199.fn_name = _SLIT("main.test_comptime_pkgconfig");
			v_assert_meta_info__t1199.src = _SLIT("true");
			main__cb_assertion_failed(&v_assert_meta_info__t1199);
			longjmp(g_jump_buffer, 1);
			// TODO
			// Maybe print all vars in a test function if it fails?
		}
	...
	if (false) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t1200 = {0};
		v_assert_meta_info__t1200.fpath = _SLIT("vlib/v/tests/comptime_if_pkgconfig_test.v");
		v_assert_meta_info__t1200.line_nr = 8;
		v_assert_meta_info__t1200.fn_name = _SLIT("main.test_comptime_pkgconfig");
		v_assert_meta_info__t1200.src = _SLIT("false");
		main__cb_assertion_ok(&v_assert_meta_info__t1200);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t1201 = {0};
		v_assert_meta_info__t1201.fpath = _SLIT("vlib/v/tests/comptime_if_pkgconfig_test.v");
		v_assert_meta_info__t1201.line_nr = 8;
		v_assert_meta_info__t1201.fn_name = _SLIT("main.test_comptime_pkgconfig");
		v_assert_meta_info__t1201.src = _SLIT("false");
		main__cb_assertion_failed(&v_assert_meta_info__t1201);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
...
int main(int ___argc, char** ___argv){
	...
	if (!setjmp(g_jump_buffer)) main__test_comptime_pkgconfig();
