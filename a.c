	...
	#define V_CURRENT_COMMIT_HASH "820669b"
#endif
...
typedef struct main__TestTime main__TestTime;
...
typedef array Array_main__TestTime;
...
int _IError_sqlite__SQLError_index = 4;
...
int _orm__Connection_sqlite__DB_index = 0;
...
static inline orm__Primitive time__Time_to_sumtype_orm__Primitive(time__Time* x) {
	...
	return (orm__Primitive){ ._time__Time = ptr, ._typ = 88};
}
...
static inline orm__Primitive orm__InfixType_to_sumtype_orm__Primitive(orm__InfixType* x) {
	...
	return (orm__Primitive){ ._orm__InfixType = ptr, ._typ = 162};
}
...
static char * v_typeof_sumtype_orm__Primitive(int sidx) { /* orm.Primitive */ 
	switch(sidx) {
		case 156: return "orm.Primitive";
		case 162: return "orm.InfixType";
		case 16: return "bool";
		case 9: return "byte";
		case 13: return "f32";
		case 14: return "f64";
...
VV_LOCAL_SYMBOL voidptr map_get_and_set(map* m, voidptr key, voidptr zero) {
	...
	if (false) {
		...
		v_assert_meta_info__t2.line_nr = 560;
		v_assert_meta_info__t2.fn_name = _SLIT("get_and_set");
		v_assert_meta_info__t2.src = _SLIT("false");
		main__cb_assertion_ok(&v_assert_meta_info__t2);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t3 = {0};
		v_assert_meta_info__t3.fpath = _SLIT("/home/louis/v/vlib/builtin/map.v");
		v_assert_meta_info__t3.line_nr = 560;
		v_assert_meta_info__t3.fn_name = _SLIT("get_and_set");
		v_assert_meta_info__t3.src = _SLIT("false");
		main__cb_assertion_failed(&v_assert_meta_info__t3);
...
Option_int os__File_read_bytes_into_newline(os__File* f, Array_byte* buf) {
	if (buf->len == 0) {
		_v_panic(string__plus(_SLIT("read_bytes_into_newline"), _SLIT(": `buf.len` == 0")));
		VUNREACHABLE();
	}
	int newline = 10;
	int c = 0;
	...
	for (;;) {
		...
		if (c == (EOF)) {
			if (feof(stream) != 0) {
				Option_int _t1;
				opt_ok(&(int[]) { nbytes }, (Option*)(&_t1), sizeof(int));
				return _t1;
			}
			if (ferror(stream) != 0) {
				return (Option_int){ .state=2, .err=_v_error(_SLIT("file read error")), .data={EMPTY_STRUCT_INITIALIZATION} };
...
Option_int os__File_read_bytes_into(os__File* f, u64 pos, Array_byte* buf) {
	if (buf->len == 0) {
		_v_panic(string__plus(_SLIT("read_bytes_into"), _SLIT(": `buf.len` == 0")));
		VUNREACHABLE();
	}
	#if defined(TARGET_IS_64BIT)
	{
		...
		{
			fseeko(f->cfile, pos, SEEK_SET);
			Option_int _t1 = os__fread(buf->data, 1, buf->len, f->cfile);
			if (_t1.state != 0) { /*or block*/ 
				Option_int _t2;
				memcpy(&_t2, &_t1, sizeof(Option));
				return _t2;
	...
	{
		fseek(f->cfile, pos, SEEK_SET);
		Option_int _t4 = os__fread(buf->data, 1, buf->len, f->cfile);
		...
		Option_int _t6;
		opt_ok(&(int[]) { nbytes }, (Option*)(&_t6), sizeof(int));
		return _t6;
	}
	#endif
...
VV_LOCAL_SYMBOL IError os__error_file_not_opened(void) {
	return /*&IError*/I_os__ErrFileNotOpened_to_Interface_IError((os__ErrFileNotOpened*)memdup(&(os__ErrFileNotOpened){.msg = _SLIT("os: file not opened"),.code = 0,}, sizeof(os__ErrFileNotOpened)));
}

VV_LOCAL_SYMBOL IError os__error_size_of_type_0(void) {
	return /*&IError*/I_os__ErrSizeOfTypeIs0_to_Interface_IError((os__ErrSizeOfTypeIs0*)memdup(&(os__ErrSizeOfTypeIs0){.msg = _SLIT("os: size of type is 0"),.code = 0,}, sizeof(os__ErrSizeOfTypeIs0)));
}
...
Option_string os__find_abs_path_of_executable(string exepath) {
	...
	return (Option_string){ .state=2, .err=_v_error(_SLIT("failed to find executable")), .data={EMPTY_STRUCT_INITIALIZATION} };
}
...
string os__join_path(string base, Array_string dirs) {
	...
	for (int _t2 = 0; _t2 < dirs.len; ++_t2) {
		string d = ((string*)dirs.data)[_t2];
...
Array_string os__walk_ext(string path, string ext) {
	...
	for (int _t4 = 0; _t4 < files.len; ++_t4) {
		...
		if (os__is_dir(p) && !os__is_link(p)) {
			_PUSH_MANY(&res, (os__walk_ext(p, ext)), _t5, Array_string);
...
Array_string os__vmodules_paths(void) {
	...
	Array_string _t1_orig = string_split(path, _const_os__path_delimiter);
...
FILE* fp;
	...
	rewind(fp);
	Array_byte res = __new_array_with_default(((int)(fsize)), 0, sizeof(byte), 0);
	int nr_read_elements = ((int)(fread(res.data, fsize, 1, fp)));
	if (nr_read_elements == 0 && fsize > 0) {
		Option_Array_byte _t5 = (Option_Array_byte){ .state=2, .err=_v_error(_SLIT("fread failed")), .data={EMPTY_STRUCT_INITIALIZATION} };
		// Defer begin
		if (os__read_bytes_defer_0) {
			fclose(fp);
		}
	...
	array_trim(&res, nr_read_elements * ((int)(fsize)));
...
FILE* fp;
	...
	rewind(fp);
	{ // Unsafe block
		byte* str = malloc_noscan(((int)(fsize)) + 1);
		int nelements = ((int)(fread(str, 1, fsize, fp)));
		int is_eof = ((int)(feof(fp)));
		int is_error = ((int)(ferror(fp)));
		if (is_eof == 0 && is_error != 0) {
			_v_free(str);
			Option_string _t5 = (Option_string){ .state=2, .err=_v_error(_SLIT("fread failed")), .data={EMPTY_STRUCT_INITIALIZATION} };
			// Defer begin
			if (os__read_file_defer_0) {
				fclose(fp);
			}
		...
		if (nelements == 0) {
			Option_string _t6;
			opt_ok(&(string[]) { byte_vstring(str) }, (Option*)(&_t6), sizeof(string));
			// Defer begin
			if (os__read_file_defer_0) {
				fclose(fp);
			}
		...
		Option_string _t7;
		opt_ok(&(string[]) { byte_vstring_with_len(str, nelements) }, (Option*)(&_t7), sizeof(string));
		// Defer begin
		if (os__read_file_defer_0) {
			fclose(fp);
		}
...
string os__real_path(string fpath) {
	...
	#else
	{
		byte* fullpath = vcalloc_noscan(_const_os__max_path_len);
...
inline bool os__debugger_present(void) {
	...
	{
		return ptrace(PTRACE_TRACEME, 0U, 1, 0) == -1;
	}
...
Option_bool os__is_writable_folder(string folder) {
	...
	{ // Unsafe block
		...
		if (-1 == x) {
			return (Option_bool){ .state=2, .err=_v_error( str_intp(2, _MOV((StrIntpData[]){{_SLIT("folder `"), 0xfe10, {.d_s = folder}}, {_SLIT("` is not writable"), 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
		}
		close(x);
	}
...
os__Process* os__new_process(string filename) {
	return (os__Process*)memdup(&(os__Process){.filename = filename,.pid = 0,.code = -1,.status = os__ProcessState__not_started,.err = (string){.str=(byteptr)"", .is_lit=1},.args = __new_array(0, 0, sizeof(string)),.env_is_custom = 0,.env = __new_array(0, 0, sizeof(string)),.use_stdio_ctl = 0,.use_pgroup = 0,.stdio_fd = {-1, -1, -1},.wdata = 0,}, sizeof(os__Process));
}
...
term__Coord term__get_cursor_position(void) {
	if (os__is_atty(1) <= 0 || string__eq(os__getenv(_SLIT("TERM")), _SLIT("dumb"))) {
		term__Coord _t1 = (term__Coord){.x = 0,.y = 0,};
		return _t1;
	}
	...
	for (;;) {
		...
		i++;
		if (i >= 15) {
			_v_panic(_SLIT("C.getchar() called too many times"));
			VUNREACHABLE();
		}
		if (b == 'R') {
			break;
	...
	term__Coord _t2 = (term__Coord){.x = x,.y = y,};
	return _t2;
}
...
Option_string orm__orm_table_gen(string table, string para, bool defaults, int def_unique_len, Array_orm__TableField fields, Option_string (*sql_from_v)(int ), bool alternative) {
	...
	for (int _t1 = 0; _t1 < fields.len; ++_t1) {
		...
		if (_t4.state != 0) { /*or block*/ 
			...
			Option_string _t5 = sql_from_v(8);
			if (_t5.state != 0) { /*or block*/ 
				Option_string _t6;
				memcpy(&_t6, &_t5, sizeof(Option));
				return _t6;
...
VV_LOCAL_SYMBOL int sqlite__bind(sqlite__Stmt stmt, int* c, orm__Primitive data) {
	...
	else if (data._typ == 88 /* time.Time */) {
		eprintln(u64_str((*data._time__Time)._v_unix));
		err = sqlite__Stmt_bind_int(stmt, *c, ((int)((*data._time__Time)._v_unix)));
	}
	else if (data._typ == 162 /* orm.InfixType */) {
...
VV_LOCAL_SYMBOL bool main__can_use_relative_paths(void) {
	string _t2 = os__getenv(_SLIT("VERROR_PATHS"));
		bool _t1 = ((string__eq(_t2, _SLIT("absolute"))) ? (false) : (true));
	return _t1;
}
...
VV_LOCAL_SYMBOL void main__cb_propagate_test_error(int line_nr, string file, string mod, string fn_name, string errmsg) {
	...
	if (os__is_file(file)) {
		Option_Array_string _t1 = os__read_lines(file);
		if (_t1.state != 0) { /*or block*/ 
			IError err = _t1.err;
...
VV_LOCAL_SYMBOL void main__test_orm_sqlite(void) {
	...
	Option_void _t3 = orm__Connection_name_table[_t2._typ]._method_create(_t2._object, _SLIT("userlist"), new_array_from_c_array(4, 4, sizeof(orm__TableField), _MOV((orm__TableField[4]){(orm__TableField){.name = _SLIT("id"),.typ = 7,.is_arr = false, .is_time = 0,.default_val = (string){.str = (byteptr) "", .is_lit = 1},.attrs = new_array_from_c_array(2, 2, sizeof(StructAttribute), _MOV((StructAttribute[2]){(StructAttribute){.name = _SLIT("primary"),.has_arg = 0,.arg = _SLIT(""),.kind = 0,},(StructAttribute){.name = _SLIT("sql"),.has_arg = 1,.arg = _SLIT("serial"),.kind = 0,},}))},(orm__TableField){.name = _SLIT("age"),.typ = 7,.is_arr = false, .is_time = 0,.default_val = (string){.str = (byteptr) "", .is_lit = 1},.attrs = new_array_from_c_array(0, 0, sizeof(StructAttribute),NULL)},(orm__TableField){.name = _SLIT("name"),.typ = 18,.is_arr = false, .is_time = 0,.default_val = (string){.str = (byteptr) "", .is_lit = 1},.attrs = new_array_from_c_array(1, 1, sizeof(StructAttribute), _MOV((StructAttribute[1]){(StructAttribute){.name = _SLIT("sql"),.has_arg = 1,.arg = _SLIT("username"),.kind = 1,},}))},(orm__TableField){.name = _SLIT("is_customer"),.typ = 16,.is_arr = false, .is_time = 0,.default_val = (string){.str = (byteptr) "", .is_lit = 1},.attrs = new_array_from_c_array(0, 0, sizeof(StructAttribute),NULL)},})));
	if (_t3.state != 0 && _t3.err._typ != _IError_None___index) { _v_panic(IError_str(_t3.err)); }
	string name = _SLIT("Peter");
	main__User sam = (main__User){.id = 0,.age = 29,.name = _SLIT("Sam"),.is_customer = 0,.skipped_string = (string){.str=(byteptr)"", .is_lit=1},};
	main__User peter = (main__User){.id = 0,.age = 31,.name = _SLIT("Peter"),.is_customer = 0,.skipped_string = (string){.str=(byteptr)"", .is_lit=1},};
	main__User k = (main__User){.id = 0,.age = 30,.name = _SLIT("Kate"),.is_customer = true,.skipped_string = (string){.str=(byteptr)"", .is_lit=1},};
	
	// orm
	orm__Connection _t4 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Array_orm__Primitive _t6 = new_array_from_c_array(0, 0, sizeof(orm__Primitive), NULL);
	Option_void _t5 = orm__Connection_name_table[_t4._typ]._method_insert(_t4._object, _SLIT("userlist"), (orm__QueryData){.fields = new_array_from_c_array(3, 3, sizeof(string),_MOV((string[3]){_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.data = new_array_from_c_array(3, 3, sizeof(orm__Primitive), _MOV((orm__Primitive[3]){orm__int_to_primitive(sam.age),orm__string_to_primitive(sam.name),orm__bool_to_primitive(sam.is_customer),})),.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),});
	if (_t5.state != 0 && _t5.err._typ != _IError_None___index) { _v_panic(IError_str(_t5.err)); }
	Array_orm__Primitive _t8 = new_array_from_c_array(0, 0, sizeof(orm__Primitive), NULL);
	Option_void _t7 = orm__Connection_name_table[_t4._typ]._method_insert(_t4._object, _SLIT("userlist"), (orm__QueryData){.fields = new_array_from_c_array(3, 3, sizeof(string),_MOV((string[3]){_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.data = new_array_from_c_array(3, 3, sizeof(orm__Primitive), _MOV((orm__Primitive[3]){orm__int_to_primitive(peter.age),orm__string_to_primitive(peter.name),orm__bool_to_primitive(peter.is_customer),})),.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),});
	if (_t7.state != 0 && _t7.err._typ != _IError_None___index) { _v_panic(IError_str(_t7.err)); }
	Array_orm__Primitive _t10 = new_array_from_c_array(0, 0, sizeof(orm__Primitive), NULL);
	Option_void _t9 = orm__Connection_name_table[_t4._typ]._method_insert(_t4._object, _SLIT("userlist"), (orm__QueryData){.fields = new_array_from_c_array(3, 3, sizeof(string),_MOV((string[3]){_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.data = new_array_from_c_array(3, 3, sizeof(orm__Primitive), _MOV((orm__Primitive[3]){orm__int_to_primitive(k.age),orm__string_to_primitive(k.name),orm__bool_to_primitive(k.is_customer),})),.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),});
	if (_t9.state != 0 && _t9.err._typ != _IError_None___index) { _v_panic(IError_str(_t9.err)); }

	// orm
	orm__Connection _t11 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t12 = orm__Connection_name_table[_t11._typ]._method_select(_t11._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = true,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(1),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__neq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t12.state != 0 && _o_t12.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t12.err)); }
	Array_Array_orm__Primitive _t12 = (*(Array_Array_orm__Primitive*)_o_t12.data);
		int c =  *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*)array_get(_t12, 0)), 0))._int);
	;
	// assert
	if (c == 2) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t13 = {0};
		v_assert_meta_info__t13.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t13.line_nr = 65;
		v_assert_meta_info__t13.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t13.src = _SLIT("c == 2");
		v_assert_meta_info__t13.op = _SLIT("==");
		v_assert_meta_info__t13.llabel = _SLIT("c");
		v_assert_meta_info__t13.rlabel = _SLIT("2");
		v_assert_meta_info__t13.lvalue = int_str(c) /* typeof: v.ast.Ident type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t13.rvalue = int_str(2) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t13);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t14 = {0};
		v_assert_meta_info__t14.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t14.line_nr = 65;
		v_assert_meta_info__t14.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t14.src = _SLIT("c == 2");
		v_assert_meta_info__t14.op = _SLIT("==");
		v_assert_meta_info__t14.llabel = _SLIT("c");
		v_assert_meta_info__t14.rlabel = _SLIT("2");
		v_assert_meta_info__t14.lvalue = int_str(c) /* typeof: v.ast.Ident type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t14.rvalue = int_str(2) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t14);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t15 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t16 = orm__Connection_name_table[_t15._typ]._method_select(_t15._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = true,.has_where = false,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData) {});
	if (_o_t16.state != 0 && _o_t16.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t16.err)); }
	Array_Array_orm__Primitive _t16 = (*(Array_Array_orm__Primitive*)_o_t16.data);
		int nr_all_users =  *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*)array_get(_t16, 0)), 0))._int);
	;
	// assert
	if (nr_all_users == 3) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t17 = {0};
		v_assert_meta_info__t17.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t17.line_nr = 70;
		v_assert_meta_info__t17.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t17.src = _SLIT("nr_all_users == 3");
		v_assert_meta_info__t17.op = _SLIT("==");
		v_assert_meta_info__t17.llabel = _SLIT("nr_all_users");
		v_assert_meta_info__t17.rlabel = _SLIT("3");
		v_assert_meta_info__t17.lvalue = int_str(nr_all_users) /* typeof: v.ast.Ident type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t17.rvalue = int_str(3) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t17);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t18 = {0};
		v_assert_meta_info__t18.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t18.line_nr = 70;
		v_assert_meta_info__t18.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t18.src = _SLIT("nr_all_users == 3");
		v_assert_meta_info__t18.op = _SLIT("==");
		v_assert_meta_info__t18.llabel = _SLIT("nr_all_users");
		v_assert_meta_info__t18.rlabel = _SLIT("3");
		v_assert_meta_info__t18.lvalue = int_str(nr_all_users) /* typeof: v.ast.Ident type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t18.rvalue = int_str(3) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t18);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	println( str_intp(2, _MOV((StrIntpData[]){{_SLIT("nr_all_users="), 0xfe07, {.d_i32 = nr_all_users}}, {_SLIT0, 0, { .d_c = 0 }}})));

	// orm
	orm__Connection _t19 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t20 = orm__Connection_name_table[_t19._typ]._method_select(_t19._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = true,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(1),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t20.state != 0 && _o_t20.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t20.err)); }
	Array_Array_orm__Primitive _t20 = (*(Array_Array_orm__Primitive*)_o_t20.data);
		int nr_users1 =  *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*)array_get(_t20, 0)), 0))._int);
	;
	// assert
	if (nr_users1 == 1) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t21 = {0};
		v_assert_meta_info__t21.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t21.line_nr = 76;
		v_assert_meta_info__t21.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t21.src = _SLIT("nr_users1 == 1");
		v_assert_meta_info__t21.op = _SLIT("==");
		v_assert_meta_info__t21.llabel = _SLIT("nr_users1");
		v_assert_meta_info__t21.rlabel = _SLIT("1");
		v_assert_meta_info__t21.lvalue = int_str(nr_users1) /* typeof: v.ast.Ident type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t21.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t21);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t22 = {0};
		v_assert_meta_info__t22.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t22.line_nr = 76;
		v_assert_meta_info__t22.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t22.src = _SLIT("nr_users1 == 1");
		v_assert_meta_info__t22.op = _SLIT("==");
		v_assert_meta_info__t22.llabel = _SLIT("nr_users1");
		v_assert_meta_info__t22.rlabel = _SLIT("1");
		v_assert_meta_info__t22.lvalue = int_str(nr_users1) /* typeof: v.ast.Ident type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t22.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t22);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	println( str_intp(2, _MOV((StrIntpData[]){{_SLIT("nr_users1="), 0xfe07, {.d_i32 = nr_users1}}, {_SLIT0, 0, { .d_c = 0 }}})));

	// orm
	orm__Connection _t23 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t24 = orm__Connection_name_table[_t23._typ]._method_select(_t23._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = true,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(2, 2, sizeof(string), _MOV((string[2]){_SLIT("id"),_SLIT("username"),})),.data = new_array_from_c_array(2, 2, sizeof(orm__Primitive), _MOV((orm__Primitive[2]){orm__int_to_primitive(2),orm__string_to_primitive(_SLIT("Peter")),})),.kinds = new_array_from_c_array(2, 2, sizeof(orm__OperationKind), _MOV((orm__OperationKind[2]){orm__OperationKind__eq,orm__OperationKind__eq,})),.is_and = new_array_from_c_array(1, 1, sizeof(bool), _MOV((bool[1]){true, })),});
	if (_o_t24.state != 0 && _o_t24.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t24.err)); }
	Array_Array_orm__Primitive _t24 = (*(Array_Array_orm__Primitive*)_o_t24.data);
		int nr_peters =  *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*)array_get(_t24, 0)), 0))._int);
	;
	// assert
	if (nr_peters == 1) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t25 = {0};
		v_assert_meta_info__t25.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t25.line_nr = 82;
		v_assert_meta_info__t25.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t25.src = _SLIT("nr_peters == 1");
		v_assert_meta_info__t25.op = _SLIT("==");
		v_assert_meta_info__t25.llabel = _SLIT("nr_peters");
		v_assert_meta_info__t25.rlabel = _SLIT("1");
		v_assert_meta_info__t25.lvalue = int_str(nr_peters) /* typeof: v.ast.Ident type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t25.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t25);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t26 = {0};
		v_assert_meta_info__t26.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t26.line_nr = 82;
		v_assert_meta_info__t26.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t26.src = _SLIT("nr_peters == 1");
		v_assert_meta_info__t26.op = _SLIT("==");
		v_assert_meta_info__t26.llabel = _SLIT("nr_peters");
		v_assert_meta_info__t26.rlabel = _SLIT("1");
		v_assert_meta_info__t26.lvalue = int_str(nr_peters) /* typeof: v.ast.Ident type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t26.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t26);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	println( str_intp(2, _MOV((StrIntpData[]){{_SLIT("nr_peters="), 0xfe07, {.d_i32 = nr_peters}}, {_SLIT0, 0, { .d_c = 0 }}})));

	// orm
	orm__Connection _t27 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t28 = orm__Connection_name_table[_t27._typ]._method_select(_t27._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = true,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(2, 2, sizeof(string), _MOV((string[2]){_SLIT("id"),_SLIT("username"),})),.data = new_array_from_c_array(2, 2, sizeof(orm__Primitive), _MOV((orm__Primitive[2]){orm__int_to_primitive(2),orm__string_to_primitive(name),})),.kinds = new_array_from_c_array(2, 2, sizeof(orm__OperationKind), _MOV((orm__OperationKind[2]){orm__OperationKind__eq,orm__OperationKind__eq,})),.is_and = new_array_from_c_array(1, 1, sizeof(bool), _MOV((bool[1]){true, })),});
	if (_o_t28.state != 0 && _o_t28.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t28.err)); }
	Array_Array_orm__Primitive _t28 = (*(Array_Array_orm__Primitive*)_o_t28.data);
		int nr_peters2 =  *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*)array_get(_t28, 0)), 0))._int);
	;
	// assert
	if (nr_peters2 == 1) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t29 = {0};
		v_assert_meta_info__t29.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t29.line_nr = 88;
		v_assert_meta_info__t29.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t29.src = _SLIT("nr_peters2 == 1");
		v_assert_meta_info__t29.op = _SLIT("==");
		v_assert_meta_info__t29.llabel = _SLIT("nr_peters2");
		v_assert_meta_info__t29.rlabel = _SLIT("1");
		v_assert_meta_info__t29.lvalue = int_str(nr_peters2) /* typeof: v.ast.Ident type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t29.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t29);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t30 = {0};
		v_assert_meta_info__t30.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t30.line_nr = 88;
		v_assert_meta_info__t30.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t30.src = _SLIT("nr_peters2 == 1");
		v_assert_meta_info__t30.op = _SLIT("==");
		v_assert_meta_info__t30.llabel = _SLIT("nr_peters2");
		v_assert_meta_info__t30.rlabel = _SLIT("1");
		v_assert_meta_info__t30.lvalue = int_str(nr_peters2) /* typeof: v.ast.Ident type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t30.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t30);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t31 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t32 = orm__Connection_name_table[_t31._typ]._method_select(_t31._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = true,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("username"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__string_to_primitive(name),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t32.state != 0 && _o_t32.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t32.err)); }
	Array_Array_orm__Primitive _t32 = (*(Array_Array_orm__Primitive*)_o_t32.data);
		int nr_peters3 =  *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*)array_get(_t32, 0)), 0))._int);
	;
	// assert
	if (nr_peters3 == 1) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t33 = {0};
		v_assert_meta_info__t33.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t33.line_nr = 92;
		v_assert_meta_info__t33.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t33.src = _SLIT("nr_peters3 == 1");
		v_assert_meta_info__t33.op = _SLIT("==");
		v_assert_meta_info__t33.llabel = _SLIT("nr_peters3");
		v_assert_meta_info__t33.rlabel = _SLIT("1");
		v_assert_meta_info__t33.lvalue = int_str(nr_peters3) /* typeof: v.ast.Ident type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t33.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t33);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t34 = {0};
		v_assert_meta_info__t34.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t34.line_nr = 92;
		v_assert_meta_info__t34.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t34.src = _SLIT("nr_peters3 == 1");
		v_assert_meta_info__t34.op = _SLIT("==");
		v_assert_meta_info__t34.llabel = _SLIT("nr_peters3");
		v_assert_meta_info__t34.rlabel = _SLIT("1");
		v_assert_meta_info__t34.lvalue = int_str(nr_peters3) /* typeof: v.ast.Ident type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t34.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t34);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t35 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t36 = orm__Connection_name_table[_t35._typ]._method_select(_t35._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("username"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__string_to_primitive(name),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t36.state != 0 && _o_t36.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t36.err)); }
	Array_Array_orm__Primitive _t36 = (*(Array_Array_orm__Primitive*)_o_t36.data);
	int _t38 = 0;
	Array_main__User _t37_array = __new_array(0, _t36.len, sizeof(main__User));
	for (; _t38 < _t36.len; _t38++) {
		main__User _t37 = (main__User) {.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t36.len > 0) {
	_t37.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t36, _t38)), 0))._int);
	_t37.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t36, _t38)), 1))._int);
	_t37.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t36, _t38)), 2))._string);
	_t37.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t36, _t38)), 3))._bool);
	}
	array_push(&_t37_array, _MOV((main__User[]){ _t37 }));
	}
		Array_main__User peters =  _t37_array;
	;
	// assert
	if (peters.len == 1) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t39 = {0};
		v_assert_meta_info__t39.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t39.line_nr = 96;
		v_assert_meta_info__t39.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t39.src = _SLIT("peters.len == 1");
		v_assert_meta_info__t39.op = _SLIT("==");
		v_assert_meta_info__t39.llabel = _SLIT("peters.len");
		v_assert_meta_info__t39.rlabel = _SLIT("1");
		v_assert_meta_info__t39.lvalue = int_str(peters.len) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t39.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t39);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t40 = {0};
		v_assert_meta_info__t40.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t40.line_nr = 96;
		v_assert_meta_info__t40.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t40.src = _SLIT("peters.len == 1");
		v_assert_meta_info__t40.op = _SLIT("==");
		v_assert_meta_info__t40.llabel = _SLIT("peters.len");
		v_assert_meta_info__t40.rlabel = _SLIT("1");
		v_assert_meta_info__t40.lvalue = int_str(peters.len) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t40.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t40);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	// assert
	if (string__eq((*(main__User*)/*ee elem_typ */array_get(peters, 0)).name, _SLIT("Peter"))) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t41 = {0};
		v_assert_meta_info__t41.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t41.line_nr = 97;
		v_assert_meta_info__t41.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t41.src = _SLIT("peters[0].name == 'Peter'");
		v_assert_meta_info__t41.op = _SLIT("==");
		v_assert_meta_info__t41.llabel = _SLIT("peters[0].name");
		v_assert_meta_info__t41.rlabel = _SLIT("'Peter'");
		v_assert_meta_info__t41.lvalue = (*(main__User*)/*ee elem_typ */array_get(peters, 0)).name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t41.rvalue = _SLIT("Peter") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t41);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t42 = {0};
		v_assert_meta_info__t42.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t42.line_nr = 97;
		v_assert_meta_info__t42.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t42.src = _SLIT("peters[0].name == 'Peter'");
		v_assert_meta_info__t42.op = _SLIT("==");
		v_assert_meta_info__t42.llabel = _SLIT("peters[0].name");
		v_assert_meta_info__t42.rlabel = _SLIT("'Peter'");
		v_assert_meta_info__t42.lvalue = (*(main__User*)/*ee elem_typ */array_get(peters, 0)).name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t42.rvalue = _SLIT("Peter") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t42);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t43 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t44 = orm__Connection_name_table[_t43._typ]._method_select(_t43._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = true,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(1),}))},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("username"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__string_to_primitive(name),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t44.state != 0 && _o_t44.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t44.err)); }
	Array_Array_orm__Primitive _t44 = (*(Array_Array_orm__Primitive*)_o_t44.data);
	int _t46 = 0;
	main__User _t45 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t44.len > 0) {
	_t45.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t44, _t46)), 0))._int);
	_t45.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t44, _t46)), 1))._int);
	_t45.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t44, _t46)), 2))._string);
	_t45.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t44, _t46)), 3))._bool);
	}
		main__User one_peter =  _t45;
	;
	// assert
	if (string__eq(one_peter.name, _SLIT("Peter"))) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t47 = {0};
		v_assert_meta_info__t47.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t47.line_nr = 101;
		v_assert_meta_info__t47.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t47.src = _SLIT("one_peter.name == 'Peter'");
		v_assert_meta_info__t47.op = _SLIT("==");
		v_assert_meta_info__t47.llabel = _SLIT("one_peter.name");
		v_assert_meta_info__t47.rlabel = _SLIT("'Peter'");
		v_assert_meta_info__t47.lvalue = one_peter.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t47.rvalue = _SLIT("Peter") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t47);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t48 = {0};
		v_assert_meta_info__t48.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t48.line_nr = 101;
		v_assert_meta_info__t48.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t48.src = _SLIT("one_peter.name == 'Peter'");
		v_assert_meta_info__t48.op = _SLIT("==");
		v_assert_meta_info__t48.llabel = _SLIT("one_peter.name");
		v_assert_meta_info__t48.rlabel = _SLIT("'Peter'");
		v_assert_meta_info__t48.lvalue = one_peter.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t48.rvalue = _SLIT("Peter") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t48);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	if (one_peter.id == 2) {
		...
		v_assert_meta_info__t49.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t49.line_nr = 102;
		v_assert_meta_info__t49.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t49.src = _SLIT("one_peter.id == 2");
		v_assert_meta_info__t49.op = _SLIT("==");
		v_assert_meta_info__t49.llabel = _SLIT("one_peter.id");
		v_assert_meta_info__t49.rlabel = _SLIT("2");
		v_assert_meta_info__t49.lvalue = int_str(one_peter.id) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t49.rvalue = int_str(2) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t49);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t50 = {0};
		v_assert_meta_info__t50.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t50.line_nr = 102;
		v_assert_meta_info__t50.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t50.src = _SLIT("one_peter.id == 2");
		v_assert_meta_info__t50.op = _SLIT("==");
		v_assert_meta_info__t50.llabel = _SLIT("one_peter.id");
		v_assert_meta_info__t50.rlabel = _SLIT("2");
		v_assert_meta_info__t50.lvalue = int_str(one_peter.id) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t50.rvalue = int_str(2) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t50);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t51 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t52 = orm__Connection_name_table[_t51._typ]._method_select(_t51._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(1),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t52.state != 0 && _o_t52.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t52.err)); }
	Array_Array_orm__Primitive _t52 = (*(Array_Array_orm__Primitive*)_o_t52.data);
	int _t54 = 0;
	main__User _t53 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t52.len > 0) {
	_t53.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t52, _t54)), 0))._int);
	_t53.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t52, _t54)), 1))._int);
	_t53.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t52, _t54)), 2))._string);
	_t53.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t52, _t54)), 3))._bool);
	}
		main__User user =  _t53;
	;
	println(main__User_str(user));
	// assert
	if (string__eq(user.name, _SLIT("Sam"))) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t55 = {0};
		v_assert_meta_info__t55.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t55.line_nr = 108;
		v_assert_meta_info__t55.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t55.src = _SLIT("user.name == 'Sam'");
		v_assert_meta_info__t55.op = _SLIT("==");
		v_assert_meta_info__t55.llabel = _SLIT("user.name");
		v_assert_meta_info__t55.rlabel = _SLIT("'Sam'");
		v_assert_meta_info__t55.lvalue = user.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t55.rvalue = _SLIT("Sam") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t55);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t56 = {0};
		v_assert_meta_info__t56.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t56.line_nr = 108;
		v_assert_meta_info__t56.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t56.src = _SLIT("user.name == 'Sam'");
		v_assert_meta_info__t56.op = _SLIT("==");
		v_assert_meta_info__t56.llabel = _SLIT("user.name");
		v_assert_meta_info__t56.rlabel = _SLIT("'Sam'");
		v_assert_meta_info__t56.lvalue = user.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t56.rvalue = _SLIT("Sam") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t56);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	if (user.id == 1) {
		...
		v_assert_meta_info__t57.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t57.line_nr = 109;
		v_assert_meta_info__t57.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t57.src = _SLIT("user.id == 1");
		v_assert_meta_info__t57.op = _SLIT("==");
		v_assert_meta_info__t57.llabel = _SLIT("user.id");
		v_assert_meta_info__t57.rlabel = _SLIT("1");
		v_assert_meta_info__t57.lvalue = int_str(user.id) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t57.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t57);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t58 = {0};
		v_assert_meta_info__t58.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t58.line_nr = 109;
		v_assert_meta_info__t58.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t58.src = _SLIT("user.id == 1");
		v_assert_meta_info__t58.op = _SLIT("==");
		v_assert_meta_info__t58.llabel = _SLIT("user.id");
		v_assert_meta_info__t58.rlabel = _SLIT("1");
		v_assert_meta_info__t58.lvalue = int_str(user.id) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t58.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t58);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	if (user.age == 29) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t59 = {0};
		v_assert_meta_info__t59.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t59.line_nr = 110;
		v_assert_meta_info__t59.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t59.src = _SLIT("user.age == 29");
		v_assert_meta_info__t59.op = _SLIT("==");
		v_assert_meta_info__t59.llabel = _SLIT("user.age");
		v_assert_meta_info__t59.rlabel = _SLIT("29");
		v_assert_meta_info__t59.lvalue = int_str(user.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t59.rvalue = int_str(29) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t59);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t60 = {0};
		v_assert_meta_info__t60.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t60.line_nr = 110;
		v_assert_meta_info__t60.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t60.src = _SLIT("user.age == 29");
		v_assert_meta_info__t60.op = _SLIT("==");
		v_assert_meta_info__t60.llabel = _SLIT("user.age");
		v_assert_meta_info__t60.rlabel = _SLIT("29");
		v_assert_meta_info__t60.lvalue = int_str(user.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t60.rvalue = int_str(29) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t60);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t61 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t62 = orm__Connection_name_table[_t61._typ]._method_select(_t61._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(0),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__gt,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t62.state != 0 && _o_t62.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t62.err)); }
	Array_Array_orm__Primitive _t62 = (*(Array_Array_orm__Primitive*)_o_t62.data);
	int _t64 = 0;
	Array_main__User _t63_array = __new_array(0, _t62.len, sizeof(main__User));
	for (; _t64 < _t62.len; _t64++) {
		main__User _t63 = (main__User) {.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t62.len > 0) {
	_t63.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t62, _t64)), 0))._int);
	_t63.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t62, _t64)), 1))._int);
	_t63.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t62, _t64)), 2))._string);
	_t63.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t62, _t64)), 3))._bool);
	}
	array_push(&_t63_array, _MOV((main__User[]){ _t63 }));
	}
		Array_main__User users =  _t63_array;
	;
	...
	if (users.len == 3) {
		...
		v_assert_meta_info__t65.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t65.line_nr = 116;
		v_assert_meta_info__t65.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t65.src = _SLIT("users.len == 3");
		v_assert_meta_info__t65.op = _SLIT("==");
		v_assert_meta_info__t65.llabel = _SLIT("users.len");
		v_assert_meta_info__t65.rlabel = _SLIT("3");
		v_assert_meta_info__t65.lvalue = int_str(users.len) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t65.rvalue = int_str(3) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t65);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t66 = {0};
		v_assert_meta_info__t66.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t66.line_nr = 116;
		v_assert_meta_info__t66.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t66.src = _SLIT("users.len == 3");
		v_assert_meta_info__t66.op = _SLIT("==");
		v_assert_meta_info__t66.llabel = _SLIT("users.len");
		v_assert_meta_info__t66.rlabel = _SLIT("3");
		v_assert_meta_info__t66.lvalue = int_str(users.len) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t66.rvalue = int_str(3) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t66);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	// assert
	if (string__eq((*(main__User*)/*ee elem_typ */array_get(users, 0)).name, _SLIT("Sam"))) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t67 = {0};
		v_assert_meta_info__t67.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t67.line_nr = 117;
		v_assert_meta_info__t67.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t67.src = _SLIT("users[0].name == 'Sam'");
		v_assert_meta_info__t67.op = _SLIT("==");
		v_assert_meta_info__t67.llabel = _SLIT("users[0].name");
		v_assert_meta_info__t67.rlabel = _SLIT("'Sam'");
		v_assert_meta_info__t67.lvalue = (*(main__User*)/*ee elem_typ */array_get(users, 0)).name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t67.rvalue = _SLIT("Sam") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t67);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t68 = {0};
		v_assert_meta_info__t68.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t68.line_nr = 117;
		v_assert_meta_info__t68.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t68.src = _SLIT("users[0].name == 'Sam'");
		v_assert_meta_info__t68.op = _SLIT("==");
		v_assert_meta_info__t68.llabel = _SLIT("users[0].name");
		v_assert_meta_info__t68.rlabel = _SLIT("'Sam'");
		v_assert_meta_info__t68.lvalue = (*(main__User*)/*ee elem_typ */array_get(users, 0)).name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t68.rvalue = _SLIT("Sam") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t68);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	// assert
	if (string__eq((*(main__User*)/*ee elem_typ */array_get(users, 1)).name, _SLIT("Peter"))) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t69 = {0};
		v_assert_meta_info__t69.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t69.line_nr = 118;
		v_assert_meta_info__t69.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t69.src = _SLIT("users[1].name == 'Peter'");
		v_assert_meta_info__t69.op = _SLIT("==");
		v_assert_meta_info__t69.llabel = _SLIT("users[1].name");
		v_assert_meta_info__t69.rlabel = _SLIT("'Peter'");
		v_assert_meta_info__t69.lvalue = (*(main__User*)/*ee elem_typ */array_get(users, 1)).name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t69.rvalue = _SLIT("Peter") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t69);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t70 = {0};
		v_assert_meta_info__t70.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t70.line_nr = 118;
		v_assert_meta_info__t70.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t70.src = _SLIT("users[1].name == 'Peter'");
		v_assert_meta_info__t70.op = _SLIT("==");
		v_assert_meta_info__t70.llabel = _SLIT("users[1].name");
		v_assert_meta_info__t70.rlabel = _SLIT("'Peter'");
		v_assert_meta_info__t70.lvalue = (*(main__User*)/*ee elem_typ */array_get(users, 1)).name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t70.rvalue = _SLIT("Peter") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
	...
	if ((*(main__User*)/*ee elem_typ */array_get(users, 1)).age == 31) {
		...
		v_assert_meta_info__t71.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t71.line_nr = 119;
		v_assert_meta_info__t71.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t71.src = _SLIT("users[1].age == 31");
		v_assert_meta_info__t71.op = _SLIT("==");
		v_assert_meta_info__t71.llabel = _SLIT("users[1].age");
		v_assert_meta_info__t71.rlabel = _SLIT("31");
		v_assert_meta_info__t71.lvalue = int_str((*(main__User*)/*ee elem_typ */array_get(users, 1)).age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t71.rvalue = int_str(31) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t71);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t72 = {0};
		v_assert_meta_info__t72.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t72.line_nr = 119;
		v_assert_meta_info__t72.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t72.src = _SLIT("users[1].age == 31");
		v_assert_meta_info__t72.op = _SLIT("==");
		v_assert_meta_info__t72.llabel = _SLIT("users[1].age");
		v_assert_meta_info__t72.rlabel = _SLIT("31");
		v_assert_meta_info__t72.lvalue = int_str((*(main__User*)/*ee elem_typ */array_get(users, 1)).age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t72.rvalue = int_str(31) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t72);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t73 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t74 = orm__Connection_name_table[_t73._typ]._method_select(_t73._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(0),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__lt,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t74.state != 0 && _o_t74.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t74.err)); }
	Array_Array_orm__Primitive _t74 = (*(Array_Array_orm__Primitive*)_o_t74.data);
	int _t76 = 0;
	Array_main__User _t75_array = __new_array(0, _t74.len, sizeof(main__User));
	for (; _t76 < _t74.len; _t76++) {
		main__User _t75 = (main__User) {.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t74.len > 0) {
	_t75.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t74, _t76)), 0))._int);
	_t75.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t74, _t76)), 1))._int);
	_t75.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t74, _t76)), 2))._string);
	_t75.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t74, _t76)), 3))._bool);
	}
	array_push(&_t75_array, _MOV((main__User[]){ _t75 }));
	}
		Array_main__User users2 =  _t75_array;
	;
	println(Array_main__User_str(users2));
	// assert
	if (users2.len == 0) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t77 = {0};
		v_assert_meta_info__t77.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t77.line_nr = 125;
		v_assert_meta_info__t77.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t77.src = _SLIT("users2.len == 0");
		v_assert_meta_info__t77.op = _SLIT("==");
		v_assert_meta_info__t77.llabel = _SLIT("users2.len");
		v_assert_meta_info__t77.rlabel = _SLIT("0");
		v_assert_meta_info__t77.lvalue = int_str(users2.len) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t77.rvalue = int_str(0) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t77);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t78 = {0};
		v_assert_meta_info__t78.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t78.line_nr = 125;
		v_assert_meta_info__t78.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t78.src = _SLIT("users2.len == 0");
		v_assert_meta_info__t78.op = _SLIT("==");
		v_assert_meta_info__t78.llabel = _SLIT("users2.len");
		v_assert_meta_info__t78.rlabel = _SLIT("0");
		v_assert_meta_info__t78.lvalue = int_str(users2.len) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t78.rvalue = int_str(0) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t78);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t79 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t80 = orm__Connection_name_table[_t79._typ]._method_select(_t79._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(2, 2, sizeof(string), _MOV((string[2]){_SLIT("age"),_SLIT("age"),})),.data = new_array_from_c_array(2, 2, sizeof(orm__Primitive), _MOV((orm__Primitive[2]){orm__int_to_primitive(29),orm__int_to_primitive(31),})),.kinds = new_array_from_c_array(2, 2, sizeof(orm__OperationKind), _MOV((orm__OperationKind[2]){orm__OperationKind__eq,orm__OperationKind__eq,})),.is_and = new_array_from_c_array(1, 1, sizeof(bool), _MOV((bool[1]){false, })),});
	if (_o_t80.state != 0 && _o_t80.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t80.err)); }
	Array_Array_orm__Primitive _t80 = (*(Array_Array_orm__Primitive*)_o_t80.data);
	int _t82 = 0;
	Array_main__User _t81_array = __new_array(0, _t80.len, sizeof(main__User));
	for (; _t82 < _t80.len; _t82++) {
		main__User _t81 = (main__User) {.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t80.len > 0) {
	_t81.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t80, _t82)), 0))._int);
	_t81.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t80, _t82)), 1))._int);
	_t81.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t80, _t82)), 2))._string);
	_t81.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t80, _t82)), 3))._bool);
	}
	array_push(&_t81_array, _MOV((main__User[]){ _t81 }));
	}
		Array_main__User users3 =  _t81_array;
	;
	println(Array_main__User_str(users3));
	// assert
	if (users3.len == 2) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t83 = {0};
		v_assert_meta_info__t83.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t83.line_nr = 131;
		v_assert_meta_info__t83.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t83.src = _SLIT("users3.len == 2");
		v_assert_meta_info__t83.op = _SLIT("==");
		v_assert_meta_info__t83.llabel = _SLIT("users3.len");
		v_assert_meta_info__t83.rlabel = _SLIT("2");
		v_assert_meta_info__t83.lvalue = int_str(users3.len) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t83.rvalue = int_str(2) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t83);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t84 = {0};
		v_assert_meta_info__t84.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t84.line_nr = 131;
		v_assert_meta_info__t84.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t84.src = _SLIT("users3.len == 2");
		v_assert_meta_info__t84.op = _SLIT("==");
		v_assert_meta_info__t84.llabel = _SLIT("users3.len");
		v_assert_meta_info__t84.rlabel = _SLIT("2");
		v_assert_meta_info__t84.lvalue = int_str(users3.len) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t84.rvalue = int_str(2) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t84);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	if ((*(main__User*)/*ee elem_typ */array_get(users3, 0)).age == 29) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t85 = {0};
		v_assert_meta_info__t85.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t85.line_nr = 132;
		v_assert_meta_info__t85.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t85.src = _SLIT("users3[0].age == 29");
		v_assert_meta_info__t85.op = _SLIT("==");
		v_assert_meta_info__t85.llabel = _SLIT("users3[0].age");
		v_assert_meta_info__t85.rlabel = _SLIT("29");
		v_assert_meta_info__t85.lvalue = int_str((*(main__User*)/*ee elem_typ */array_get(users3, 0)).age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t85.rvalue = int_str(29) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t85);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t86 = {0};
		v_assert_meta_info__t86.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t86.line_nr = 132;
		v_assert_meta_info__t86.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t86.src = _SLIT("users3[0].age == 29");
		v_assert_meta_info__t86.op = _SLIT("==");
		v_assert_meta_info__t86.llabel = _SLIT("users3[0].age");
		v_assert_meta_info__t86.rlabel = _SLIT("29");
		v_assert_meta_info__t86.lvalue = int_str((*(main__User*)/*ee elem_typ */array_get(users3, 0)).age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t86.rvalue = int_str(29) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t86);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	if ((*(main__User*)/*ee elem_typ */array_get(users3, 1)).age == 31) {
		...
		v_assert_meta_info__t87.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t87.line_nr = 133;
		v_assert_meta_info__t87.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t87.src = _SLIT("users3[1].age == 31");
		v_assert_meta_info__t87.op = _SLIT("==");
		v_assert_meta_info__t87.llabel = _SLIT("users3[1].age");
		v_assert_meta_info__t87.rlabel = _SLIT("31");
		v_assert_meta_info__t87.lvalue = int_str((*(main__User*)/*ee elem_typ */array_get(users3, 1)).age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t87.rvalue = int_str(31) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t87);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t88 = {0};
		v_assert_meta_info__t88.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t88.line_nr = 133;
		v_assert_meta_info__t88.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t88.src = _SLIT("users3[1].age == 31");
		v_assert_meta_info__t88.op = _SLIT("==");
		v_assert_meta_info__t88.llabel = _SLIT("users3[1].age");
		v_assert_meta_info__t88.rlabel = _SLIT("31");
		v_assert_meta_info__t88.lvalue = int_str((*(main__User*)/*ee elem_typ */array_get(users3, 1)).age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t88.rvalue = int_str(31) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t88);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t89 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t90 = orm__Connection_name_table[_t89._typ]._method_select(_t89._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(8777),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t90.state != 0 && _o_t90.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t90.err)); }
	Array_Array_orm__Primitive _t90 = (*(Array_Array_orm__Primitive*)_o_t90.data);
	int _t92 = 0;
	main__User _t91 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t90.len > 0) {
	_t91.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t90, _t92)), 0))._int);
	_t91.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t90, _t92)), 1))._int);
	_t91.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t90, _t92)), 2))._string);
	_t91.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t90, _t92)), 3))._bool);
	}
		main__User missing_user =  _t91;
	;
	println(_SLIT("missing_user:"));
	println(main__User_str(missing_user));
	main__User new_user = (main__User){.id = 0,.age = 30,.name = _SLIT("New user"),.is_customer = 0,.skipped_string = (string){.str=(byteptr)"", .is_lit=1},};
	
	// orm
	orm__Connection _t93 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Array_orm__Primitive _t95 = new_array_from_c_array(0, 0, sizeof(orm__Primitive), NULL);
	Option_void _t94 = orm__Connection_name_table[_t93._typ]._method_insert(_t93._object, _SLIT("userlist"), (orm__QueryData){.fields = new_array_from_c_array(3, 3, sizeof(string),_MOV((string[3]){_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.data = new_array_from_c_array(3, 3, sizeof(orm__Primitive), _MOV((orm__Primitive[3]){orm__int_to_primitive(new_user.age),orm__string_to_primitive(new_user.name),orm__bool_to_primitive(new_user.is_customer),})),.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),});
	if (_t94.state != 0 && _t94.err._typ != _IError_None___index) { _v_panic(IError_str(_t94.err)); }

	// orm
	orm__Connection _t96 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t97 = orm__Connection_name_table[_t96._typ]._method_select(_t96._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(4),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t97.state != 0 && _o_t97.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t97.err)); }
	Array_Array_orm__Primitive _t97 = (*(Array_Array_orm__Primitive*)_o_t97.data);
	int _t99 = 0;
	main__User _t98 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t97.len > 0) {
	_t98.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t97, _t99)), 0))._int);
	_t98.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t97, _t99)), 1))._int);
	_t98.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t97, _t99)), 2))._string);
	_t98.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t97, _t99)), 3))._bool);
	}
		main__User x =  _t98;
	;
	...
	if (x.age == 30) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t100 = {0};
		v_assert_meta_info__t100.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t100.line_nr = 154;
		v_assert_meta_info__t100.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t100.src = _SLIT("x.age == 30");
		v_assert_meta_info__t100.op = _SLIT("==");
		v_assert_meta_info__t100.llabel = _SLIT("x.age");
		v_assert_meta_info__t100.rlabel = _SLIT("30");
		v_assert_meta_info__t100.lvalue = int_str(x.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t100.rvalue = int_str(30) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t100);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t101 = {0};
		v_assert_meta_info__t101.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t101.line_nr = 154;
		v_assert_meta_info__t101.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t101.src = _SLIT("x.age == 30");
		v_assert_meta_info__t101.op = _SLIT("==");
		v_assert_meta_info__t101.llabel = _SLIT("x.age");
		v_assert_meta_info__t101.rlabel = _SLIT("30");
		v_assert_meta_info__t101.lvalue = int_str(x.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t101.rvalue = int_str(30) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t101);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	if (x.id == 4) {
		...
		v_assert_meta_info__t102.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t102.line_nr = 155;
		v_assert_meta_info__t102.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t102.src = _SLIT("x.id == 4");
		v_assert_meta_info__t102.op = _SLIT("==");
		v_assert_meta_info__t102.llabel = _SLIT("x.id");
		v_assert_meta_info__t102.rlabel = _SLIT("4");
		v_assert_meta_info__t102.lvalue = int_str(x.id) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t102.rvalue = int_str(4) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t102);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t103 = {0};
		v_assert_meta_info__t103.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t103.line_nr = 155;
		v_assert_meta_info__t103.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t103.src = _SLIT("x.id == 4");
		v_assert_meta_info__t103.op = _SLIT("==");
		v_assert_meta_info__t103.llabel = _SLIT("x.id");
		v_assert_meta_info__t103.rlabel = _SLIT("4");
		v_assert_meta_info__t103.lvalue = int_str(x.id) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t103.rvalue = int_str(4) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t103);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	// assert
	if (string__eq(x.name, _SLIT("New user"))) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t104 = {0};
		v_assert_meta_info__t104.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t104.line_nr = 156;
		v_assert_meta_info__t104.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t104.src = _SLIT("x.name == 'New user'");
		v_assert_meta_info__t104.op = _SLIT("==");
		v_assert_meta_info__t104.llabel = _SLIT("x.name");
		v_assert_meta_info__t104.rlabel = _SLIT("'New user'");
		v_assert_meta_info__t104.lvalue = x.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t104.rvalue = _SLIT("New user") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t104);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t105 = {0};
		v_assert_meta_info__t105.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t105.line_nr = 156;
		v_assert_meta_info__t105.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t105.src = _SLIT("x.name == 'New user'");
		v_assert_meta_info__t105.op = _SLIT("==");
		v_assert_meta_info__t105.llabel = _SLIT("x.name");
		v_assert_meta_info__t105.rlabel = _SLIT("'New user'");
		v_assert_meta_info__t105.lvalue = x.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t105.rvalue = _SLIT("New user") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t105);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t106 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t107 = orm__Connection_name_table[_t106._typ]._method_select(_t106._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(3),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t107.state != 0 && _o_t107.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t107.err)); }
	Array_Array_orm__Primitive _t107 = (*(Array_Array_orm__Primitive*)_o_t107.data);
	int _t109 = 0;
	main__User _t108 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t107.len > 0) {
	_t108.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t107, _t109)), 0))._int);
	_t108.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t107, _t109)), 1))._int);
	_t108.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t107, _t109)), 2))._string);
	_t108.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t107, _t109)), 3))._bool);
	}
		main__User kate =  _t108;
	;
	// assert
	if (kate.is_customer == true) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t110 = {0};
		v_assert_meta_info__t110.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t110.line_nr = 161;
		v_assert_meta_info__t110.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t110.src = _SLIT("kate.is_customer == true");
		v_assert_meta_info__t110.op = _SLIT("==");
		v_assert_meta_info__t110.llabel = _SLIT("kate.is_customer");
		v_assert_meta_info__t110.rlabel = _SLIT("true");
		v_assert_meta_info__t110.lvalue = kate.is_customer ? _SLIT("true") : _SLIT("false") /* typeof: v.ast.SelectorExpr type: ast.Type(0x10 = 16) */ ;
		v_assert_meta_info__t110.rvalue = true ? _SLIT("true") : _SLIT("false") /* typeof: v.ast.BoolLiteral type: ast.Type(0x10 = 16) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t110);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t111 = {0};
		v_assert_meta_info__t111.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t111.line_nr = 161;
		v_assert_meta_info__t111.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t111.src = _SLIT("kate.is_customer == true");
		v_assert_meta_info__t111.op = _SLIT("==");
		v_assert_meta_info__t111.llabel = _SLIT("kate.is_customer");
		v_assert_meta_info__t111.rlabel = _SLIT("true");
		v_assert_meta_info__t111.lvalue = kate.is_customer ? _SLIT("true") : _SLIT("false") /* typeof: v.ast.SelectorExpr type: ast.Type(0x10 = 16) */ ;
		v_assert_meta_info__t111.rvalue = true ? _SLIT("true") : _SLIT("false") /* typeof: v.ast.BoolLiteral type: ast.Type(0x10 = 16) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t111);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t112 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t113 = orm__Connection_name_table[_t112._typ]._method_select(_t112._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = true,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(1),}))},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("is_customer"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__bool_to_primitive(true),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t113.state != 0 && _o_t113.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t113.err)); }
	Array_Array_orm__Primitive _t113 = (*(Array_Array_orm__Primitive*)_o_t113.data);
	int _t115 = 0;
	main__User _t114 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t113.len > 0) {
	_t114.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t113, _t115)), 0))._int);
	_t114.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t113, _t115)), 1))._int);
	_t114.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t113, _t115)), 2))._string);
	_t114.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t113, _t115)), 3))._bool);
	}
		main__User customer =  _t114;
	;
	// assert
	if (customer.is_customer == true) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t116 = {0};
		v_assert_meta_info__t116.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t116.line_nr = 166;
		v_assert_meta_info__t116.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t116.src = _SLIT("customer.is_customer == true");
		v_assert_meta_info__t116.op = _SLIT("==");
		v_assert_meta_info__t116.llabel = _SLIT("customer.is_customer");
		v_assert_meta_info__t116.rlabel = _SLIT("true");
		v_assert_meta_info__t116.lvalue = customer.is_customer ? _SLIT("true") : _SLIT("false") /* typeof: v.ast.SelectorExpr type: ast.Type(0x10 = 16) */ ;
		v_assert_meta_info__t116.rvalue = true ? _SLIT("true") : _SLIT("false") /* typeof: v.ast.BoolLiteral type: ast.Type(0x10 = 16) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t116);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t117 = {0};
		v_assert_meta_info__t117.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t117.line_nr = 166;
		v_assert_meta_info__t117.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t117.src = _SLIT("customer.is_customer == true");
		v_assert_meta_info__t117.op = _SLIT("==");
		v_assert_meta_info__t117.llabel = _SLIT("customer.is_customer");
		v_assert_meta_info__t117.rlabel = _SLIT("true");
		v_assert_meta_info__t117.lvalue = customer.is_customer ? _SLIT("true") : _SLIT("false") /* typeof: v.ast.SelectorExpr type: ast.Type(0x10 = 16) */ ;
		v_assert_meta_info__t117.rvalue = true ? _SLIT("true") : _SLIT("false") /* typeof: v.ast.BoolLiteral type: ast.Type(0x10 = 16) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t117);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	// assert
	if (string__eq(customer.name, _SLIT("Kate"))) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t118 = {0};
		v_assert_meta_info__t118.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t118.line_nr = 167;
		v_assert_meta_info__t118.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t118.src = _SLIT("customer.name == 'Kate'");
		v_assert_meta_info__t118.op = _SLIT("==");
		v_assert_meta_info__t118.llabel = _SLIT("customer.name");
		v_assert_meta_info__t118.rlabel = _SLIT("'Kate'");
		v_assert_meta_info__t118.lvalue = customer.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t118.rvalue = _SLIT("Kate") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t118);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t119 = {0};
		v_assert_meta_info__t119.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t119.line_nr = 167;
		v_assert_meta_info__t119.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t119.src = _SLIT("customer.name == 'Kate'");
		v_assert_meta_info__t119.op = _SLIT("==");
		v_assert_meta_info__t119.llabel = _SLIT("customer.name");
		v_assert_meta_info__t119.rlabel = _SLIT("'Kate'");
		v_assert_meta_info__t119.lvalue = customer.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t119.rvalue = _SLIT("Kate") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t119);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t120 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_void _t121 = orm__Connection_name_table[_t120._typ]._method_update(_t120._object, _SLIT("userlist"), (orm__QueryData){.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("age"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(31),})),},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("username"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__string_to_primitive(_SLIT("Kate")),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_t121.state != 0 && _t121.err._typ != _IError_None___index) { _v_panic(IError_str(_t121.err)); }

	// orm
	orm__Connection _t122 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t123 = orm__Connection_name_table[_t122._typ]._method_select(_t122._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(3),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t123.state != 0 && _o_t123.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t123.err)); }
	Array_Array_orm__Primitive _t123 = (*(Array_Array_orm__Primitive*)_o_t123.data);
	int _t125 = 0;
	main__User _t124 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t123.len > 0) {
	_t124.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t123, _t125)), 0))._int);
	_t124.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t123, _t125)), 1))._int);
	_t124.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t123, _t125)), 2))._string);
	_t124.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t123, _t125)), 3))._bool);
	}
		main__User kate2 =  _t124;
	;
	// assert
	if (kate2.age == 31) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t126 = {0};
		v_assert_meta_info__t126.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t126.line_nr = 176;
		v_assert_meta_info__t126.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t126.src = _SLIT("kate2.age == 31");
		v_assert_meta_info__t126.op = _SLIT("==");
		v_assert_meta_info__t126.llabel = _SLIT("kate2.age");
		v_assert_meta_info__t126.rlabel = _SLIT("31");
		v_assert_meta_info__t126.lvalue = int_str(kate2.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t126.rvalue = int_str(31) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t126);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t127 = {0};
		v_assert_meta_info__t127.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t127.line_nr = 176;
		v_assert_meta_info__t127.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t127.src = _SLIT("kate2.age == 31");
		v_assert_meta_info__t127.op = _SLIT("==");
		v_assert_meta_info__t127.llabel = _SLIT("kate2.age");
		v_assert_meta_info__t127.rlabel = _SLIT("31");
		v_assert_meta_info__t127.lvalue = int_str(kate2.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t127.rvalue = int_str(31) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t127);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	// assert
	if (string__eq(kate2.name, _SLIT("Kate"))) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t128 = {0};
		v_assert_meta_info__t128.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t128.line_nr = 177;
		v_assert_meta_info__t128.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t128.src = _SLIT("kate2.name == 'Kate'");
		v_assert_meta_info__t128.op = _SLIT("==");
		v_assert_meta_info__t128.llabel = _SLIT("kate2.name");
		v_assert_meta_info__t128.rlabel = _SLIT("'Kate'");
		v_assert_meta_info__t128.lvalue = kate2.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t128.rvalue = _SLIT("Kate") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t128);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t129 = {0};
		v_assert_meta_info__t129.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t129.line_nr = 177;
		v_assert_meta_info__t129.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t129.src = _SLIT("kate2.name == 'Kate'");
		v_assert_meta_info__t129.op = _SLIT("==");
		v_assert_meta_info__t129.llabel = _SLIT("kate2.name");
		v_assert_meta_info__t129.rlabel = _SLIT("'Kate'");
		v_assert_meta_info__t129.lvalue = kate2.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t129.rvalue = _SLIT("Kate") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t129);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t130 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_void _t131 = orm__Connection_name_table[_t130._typ]._method_update(_t130._object, _SLIT("userlist"), (orm__QueryData){.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(2, 2, sizeof(string), _MOV((string[2]){_SLIT("age"),_SLIT("username"),})),.data = new_array_from_c_array(2, 2, sizeof(orm__Primitive), _MOV((orm__Primitive[2]){orm__int_to_primitive(32),orm__string_to_primitive(_SLIT("Kate N")),})),},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("username"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__string_to_primitive(_SLIT("Kate")),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_t131.state != 0 && _t131.err._typ != _IError_None___index) { _v_panic(IError_str(_t131.err)); }

	// orm
	orm__Connection _t132 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t133 = orm__Connection_name_table[_t132._typ]._method_select(_t132._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(3),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t133.state != 0 && _o_t133.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t133.err)); }
	Array_Array_orm__Primitive _t133 = (*(Array_Array_orm__Primitive*)_o_t133.data);
	int _t135 = 0;
	main__User _t134 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t133.len > 0) {
	_t134.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t133, _t135)), 0))._int);
	_t134.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t133, _t135)), 1))._int);
	_t134.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t133, _t135)), 2))._string);
	_t134.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t133, _t135)), 3))._bool);
	}
		main__User kate3 =  _t134;
	;
	// assert
	if (kate3.age == 32) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t136 = {0};
		v_assert_meta_info__t136.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t136.line_nr = 186;
		v_assert_meta_info__t136.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t136.src = _SLIT("kate3.age == 32");
		v_assert_meta_info__t136.op = _SLIT("==");
		v_assert_meta_info__t136.llabel = _SLIT("kate3.age");
		v_assert_meta_info__t136.rlabel = _SLIT("32");
		v_assert_meta_info__t136.lvalue = int_str(kate3.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t136.rvalue = int_str(32) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t136);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t137 = {0};
		v_assert_meta_info__t137.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t137.line_nr = 186;
		v_assert_meta_info__t137.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t137.src = _SLIT("kate3.age == 32");
		v_assert_meta_info__t137.op = _SLIT("==");
		v_assert_meta_info__t137.llabel = _SLIT("kate3.age");
		v_assert_meta_info__t137.rlabel = _SLIT("32");
		v_assert_meta_info__t137.lvalue = int_str(kate3.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t137.rvalue = int_str(32) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t137);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	// assert
	if (string__eq(kate3.name, _SLIT("Kate N"))) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t138 = {0};
		v_assert_meta_info__t138.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t138.line_nr = 187;
		v_assert_meta_info__t138.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t138.src = _SLIT("kate3.name == 'Kate N'");
		v_assert_meta_info__t138.op = _SLIT("==");
		v_assert_meta_info__t138.llabel = _SLIT("kate3.name");
		v_assert_meta_info__t138.rlabel = _SLIT("'Kate N'");
		v_assert_meta_info__t138.lvalue = kate3.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t138.rvalue = _SLIT("Kate N") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t138);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t139 = {0};
		v_assert_meta_info__t139.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t139.line_nr = 187;
		v_assert_meta_info__t139.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t139.src = _SLIT("kate3.name == 'Kate N'");
		v_assert_meta_info__t139.op = _SLIT("==");
		v_assert_meta_info__t139.llabel = _SLIT("kate3.name");
		v_assert_meta_info__t139.rlabel = _SLIT("'Kate N'");
		v_assert_meta_info__t139.lvalue = kate3.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t139.rvalue = _SLIT("Kate N") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t139);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t140 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_void _t141 = orm__Connection_name_table[_t140._typ]._method_update(_t140._object, _SLIT("userlist"), (orm__QueryData){.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(2, 2, sizeof(string), _MOV((string[2]){_SLIT("age"),_SLIT("username"),})),.data = new_array_from_c_array(2, 2, sizeof(orm__Primitive), _MOV((orm__Primitive[2]){orm__int_to_primitive(new_age),orm__string_to_primitive(_SLIT("Kate N")),})),},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(3),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_t141.state != 0 && _t141.err._typ != _IError_None___index) { _v_panic(IError_str(_t141.err)); }

	// orm
	orm__Connection _t142 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t143 = orm__Connection_name_table[_t142._typ]._method_select(_t142._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(3),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t143.state != 0 && _o_t143.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t143.err)); }
	Array_Array_orm__Primitive _t143 = (*(Array_Array_orm__Primitive*)_o_t143.data);
	int _t145 = 0;
	main__User _t144 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t143.len > 0) {
	_t144.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t143, _t145)), 0))._int);
	_t144.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t143, _t145)), 1))._int);
	_t144.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t143, _t145)), 2))._string);
	_t144.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t143, _t145)), 3))._bool);
	}
		kate3 =  _t144;
	;
	// assert
	if (kate3.age == 33) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t146 = {0};
		v_assert_meta_info__t146.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t146.line_nr = 208;
		v_assert_meta_info__t146.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t146.src = _SLIT("kate3.age == 33");
		v_assert_meta_info__t146.op = _SLIT("==");
		v_assert_meta_info__t146.llabel = _SLIT("kate3.age");
		v_assert_meta_info__t146.rlabel = _SLIT("33");
		v_assert_meta_info__t146.lvalue = int_str(kate3.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t146.rvalue = int_str(33) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t146);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t147 = {0};
		v_assert_meta_info__t147.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t147.line_nr = 208;
		v_assert_meta_info__t147.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t147.src = _SLIT("kate3.age == 33");
		v_assert_meta_info__t147.op = _SLIT("==");
		v_assert_meta_info__t147.llabel = _SLIT("kate3.age");
		v_assert_meta_info__t147.rlabel = _SLIT("33");
		v_assert_meta_info__t147.lvalue = int_str(kate3.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t147.rvalue = int_str(33) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t147);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	if (string__eq(kate3.name, _SLIT("Kate N"))) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t148 = {0};
		v_assert_meta_info__t148.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t148.line_nr = 209;
		v_assert_meta_info__t148.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t148.src = _SLIT("kate3.name == 'Kate N'");
		v_assert_meta_info__t148.op = _SLIT("==");
		v_assert_meta_info__t148.llabel = _SLIT("kate3.name");
		v_assert_meta_info__t148.rlabel = _SLIT("'Kate N'");
		v_assert_meta_info__t148.lvalue = kate3.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t148.rvalue = _SLIT("Kate N") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t148);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t149 = {0};
		v_assert_meta_info__t149.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t149.line_nr = 209;
		v_assert_meta_info__t149.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t149.src = _SLIT("kate3.name == 'Kate N'");
		v_assert_meta_info__t149.op = _SLIT("==");
		v_assert_meta_info__t149.llabel = _SLIT("kate3.name");
		v_assert_meta_info__t149.rlabel = _SLIT("'Kate N'");
		v_assert_meta_info__t149.lvalue = kate3.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t149.rvalue = _SLIT("Kate N") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t149);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t150 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_void _t151 = orm__Connection_name_table[_t150._typ]._method_update(_t150._object, _SLIT("userlist"), (orm__QueryData){.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(2, 2, sizeof(string), _MOV((string[2]){_SLIT("age"),_SLIT("username"),})),.data = new_array_from_c_array(2, 2, sizeof(orm__Primitive), _MOV((orm__Primitive[2]){orm__int_to_primitive(foo.age),orm__string_to_primitive(_SLIT("Kate N")),})),},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(3),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_t151.state != 0 && _t151.err._typ != _IError_None___index) { _v_panic(IError_str(_t151.err)); }

	// orm
	orm__Connection _t152 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t153 = orm__Connection_name_table[_t152._typ]._method_select(_t152._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(3),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t153.state != 0 && _o_t153.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t153.err)); }
	Array_Array_orm__Primitive _t153 = (*(Array_Array_orm__Primitive*)_o_t153.data);
	int _t155 = 0;
	main__User _t154 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t153.len > 0) {
	_t154.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t153, _t155)), 0))._int);
	_t154.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t153, _t155)), 1))._int);
	_t154.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t153, _t155)), 2))._string);
	_t154.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t153, _t155)), 3))._bool);
	}
		kate3 =  _t154;
	;
	// assert
	if (kate3.age == 34) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t156 = {0};
		v_assert_meta_info__t156.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t156.line_nr = 219;
		v_assert_meta_info__t156.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t156.src = _SLIT("kate3.age == 34");
		v_assert_meta_info__t156.op = _SLIT("==");
		v_assert_meta_info__t156.llabel = _SLIT("kate3.age");
		v_assert_meta_info__t156.rlabel = _SLIT("34");
		v_assert_meta_info__t156.lvalue = int_str(kate3.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t156.rvalue = int_str(34) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t156);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t157 = {0};
		v_assert_meta_info__t157.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t157.line_nr = 219;
		v_assert_meta_info__t157.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t157.src = _SLIT("kate3.age == 34");
		v_assert_meta_info__t157.op = _SLIT("==");
		v_assert_meta_info__t157.llabel = _SLIT("kate3.age");
		v_assert_meta_info__t157.rlabel = _SLIT("34");
		v_assert_meta_info__t157.lvalue = int_str(kate3.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t157.rvalue = int_str(34) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t157);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	if (string__eq(kate3.name, _SLIT("Kate N"))) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t158 = {0};
		v_assert_meta_info__t158.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t158.line_nr = 220;
		v_assert_meta_info__t158.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t158.src = _SLIT("kate3.name == 'Kate N'");
		v_assert_meta_info__t158.op = _SLIT("==");
		v_assert_meta_info__t158.llabel = _SLIT("kate3.name");
		v_assert_meta_info__t158.rlabel = _SLIT("'Kate N'");
		v_assert_meta_info__t158.lvalue = kate3.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t158.rvalue = _SLIT("Kate N") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t158);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t159 = {0};
		v_assert_meta_info__t159.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t159.line_nr = 220;
		v_assert_meta_info__t159.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t159.src = _SLIT("kate3.name == 'Kate N'");
		v_assert_meta_info__t159.op = _SLIT("==");
		v_assert_meta_info__t159.llabel = _SLIT("kate3.name");
		v_assert_meta_info__t159.rlabel = _SLIT("'Kate N'");
		v_assert_meta_info__t159.lvalue = kate3.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t159.rvalue = _SLIT("Kate N") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t159);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t160 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t161 = orm__Connection_name_table[_t160._typ]._method_select(_t160._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(30),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t161.state != 0 && _o_t161.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t161.err)); }
	Array_Array_orm__Primitive _t161 = (*(Array_Array_orm__Primitive*)_o_t161.data);
	int _t163 = 0;
	main__User _t162 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t161.len > 0) {
	_t162.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t161, _t163)), 0))._int);
	_t162.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t161, _t163)), 1))._int);
	_t162.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t161, _t163)), 2))._string);
	_t162.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t161, _t163)), 3))._bool);
	}
		main__User no_user =  _t162;
	;
	// assert
	if ((no_user.name).len == 0) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t164 = {0};
		v_assert_meta_info__t164.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t164.line_nr = 225;
		v_assert_meta_info__t164.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t164.src = _SLIT("no_user.name == ''");
		v_assert_meta_info__t164.op = _SLIT("==");
		v_assert_meta_info__t164.llabel = _SLIT("no_user.name");
		v_assert_meta_info__t164.rlabel = _SLIT("''");
		v_assert_meta_info__t164.lvalue = no_user.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t164.rvalue = _SLIT("") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t164);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t165 = {0};
		v_assert_meta_info__t165.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t165.line_nr = 225;
		v_assert_meta_info__t165.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t165.src = _SLIT("no_user.name == ''");
		v_assert_meta_info__t165.op = _SLIT("==");
		v_assert_meta_info__t165.llabel = _SLIT("no_user.name");
		v_assert_meta_info__t165.rlabel = _SLIT("''");
		v_assert_meta_info__t165.lvalue = no_user.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t165.rvalue = _SLIT("") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t165);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	if (no_user.age == 0) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t166 = {0};
		v_assert_meta_info__t166.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t166.line_nr = 226;
		v_assert_meta_info__t166.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t166.src = _SLIT("no_user.age == 0");
		v_assert_meta_info__t166.op = _SLIT("==");
		v_assert_meta_info__t166.llabel = _SLIT("no_user.age");
		v_assert_meta_info__t166.rlabel = _SLIT("0");
		v_assert_meta_info__t166.lvalue = int_str(no_user.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t166.rvalue = int_str(0) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t166);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t167 = {0};
		v_assert_meta_info__t167.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t167.line_nr = 226;
		v_assert_meta_info__t167.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t167.src = _SLIT("no_user.age == 0");
		v_assert_meta_info__t167.op = _SLIT("==");
		v_assert_meta_info__t167.llabel = _SLIT("no_user.age");
		v_assert_meta_info__t167.rlabel = _SLIT("0");
		v_assert_meta_info__t167.lvalue = int_str(no_user.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t167.rvalue = int_str(0) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t167);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t168 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t169 = orm__Connection_name_table[_t168._typ]._method_select(_t168._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = false,.has_order = false,.has_limit = true,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(2),}))},(orm__QueryData) {});
	if (_o_t169.state != 0 && _o_t169.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t169.err)); }
	Array_Array_orm__Primitive _t169 = (*(Array_Array_orm__Primitive*)_o_t169.data);
	int _t171 = 0;
	Array_main__User _t170_array = __new_array(0, _t169.len, sizeof(main__User));
	for (; _t171 < _t169.len; _t171++) {
		main__User _t170 = (main__User) {.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t169.len > 0) {
	_t170.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t169, _t171)), 0))._int);
	_t170.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t169, _t171)), 1))._int);
	_t170.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t169, _t171)), 2))._string);
	_t170.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t169, _t171)), 3))._bool);
	}
	array_push(&_t170_array, _MOV((main__User[]){ _t170 }));
	}
		Array_main__User two_users =  _t170_array;
	;
	// assert
	if (two_users.len == 2) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t172 = {0};
		v_assert_meta_info__t172.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t172.line_nr = 231;
		v_assert_meta_info__t172.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t172.src = _SLIT("two_users.len == 2");
		v_assert_meta_info__t172.op = _SLIT("==");
		v_assert_meta_info__t172.llabel = _SLIT("two_users.len");
		v_assert_meta_info__t172.rlabel = _SLIT("2");
		v_assert_meta_info__t172.lvalue = int_str(two_users.len) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t172.rvalue = int_str(2) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t172);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t173 = {0};
		v_assert_meta_info__t173.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t173.line_nr = 231;
		v_assert_meta_info__t173.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t173.src = _SLIT("two_users.len == 2");
		v_assert_meta_info__t173.op = _SLIT("==");
		v_assert_meta_info__t173.llabel = _SLIT("two_users.len");
		v_assert_meta_info__t173.rlabel = _SLIT("2");
		v_assert_meta_info__t173.lvalue = int_str(two_users.len) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t173.rvalue = int_str(2) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t173);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	if ((*(main__User*)/*ee elem_typ */array_get(two_users, 0)).id == 1) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t174 = {0};
		v_assert_meta_info__t174.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t174.line_nr = 232;
		v_assert_meta_info__t174.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t174.src = _SLIT("two_users[0].id == 1");
		v_assert_meta_info__t174.op = _SLIT("==");
		v_assert_meta_info__t174.llabel = _SLIT("two_users[0].id");
		v_assert_meta_info__t174.rlabel = _SLIT("1");
		v_assert_meta_info__t174.lvalue = int_str((*(main__User*)/*ee elem_typ */array_get(two_users, 0)).id) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t174.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t174);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t175 = {0};
		v_assert_meta_info__t175.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t175.line_nr = 232;
		v_assert_meta_info__t175.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t175.src = _SLIT("two_users[0].id == 1");
		v_assert_meta_info__t175.op = _SLIT("==");
		v_assert_meta_info__t175.llabel = _SLIT("two_users[0].id");
		v_assert_meta_info__t175.rlabel = _SLIT("1");
		v_assert_meta_info__t175.lvalue = int_str((*(main__User*)/*ee elem_typ */array_get(two_users, 0)).id) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t175.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t175);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t176 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t177 = orm__Connection_name_table[_t176._typ]._method_select(_t176._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = false,.has_order = false,.has_limit = true,.has_offset = true,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(2, 2, sizeof(orm__Primitive), _MOV((orm__Primitive[2]){orm__int_to_primitive(2),orm__int_to_primitive(1),}))},(orm__QueryData) {});
	if (_o_t177.state != 0 && _o_t177.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t177.err)); }
	Array_Array_orm__Primitive _t177 = (*(Array_Array_orm__Primitive*)_o_t177.data);
	int _t179 = 0;
	Array_main__User _t178_array = __new_array(0, _t177.len, sizeof(main__User));
	for (; _t179 < _t177.len; _t179++) {
		main__User _t178 = (main__User) {.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t177.len > 0) {
	_t178.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t177, _t179)), 0))._int);
	_t178.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t177, _t179)), 1))._int);
	_t178.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t177, _t179)), 2))._string);
	_t178.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t177, _t179)), 3))._bool);
	}
	array_push(&_t178_array, _MOV((main__User[]){ _t178 }));
	}
		Array_main__User y =  _t178_array;
	;
	// assert
	if (y.len == 2) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t180 = {0};
		v_assert_meta_info__t180.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t180.line_nr = 237;
		v_assert_meta_info__t180.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t180.src = _SLIT("y.len == 2");
		v_assert_meta_info__t180.op = _SLIT("==");
		v_assert_meta_info__t180.llabel = _SLIT("y.len");
		v_assert_meta_info__t180.rlabel = _SLIT("2");
		v_assert_meta_info__t180.lvalue = int_str(y.len) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t180.rvalue = int_str(2) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t180);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t181 = {0};
		v_assert_meta_info__t181.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t181.line_nr = 237;
		v_assert_meta_info__t181.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t181.src = _SLIT("y.len == 2");
		v_assert_meta_info__t181.op = _SLIT("==");
		v_assert_meta_info__t181.llabel = _SLIT("y.len");
		v_assert_meta_info__t181.rlabel = _SLIT("2");
		v_assert_meta_info__t181.lvalue = int_str(y.len) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t181.rvalue = int_str(2) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t181);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	if ((*(main__User*)/*ee elem_typ */array_get(y, 0)).id == 2) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t182 = {0};
		v_assert_meta_info__t182.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t182.line_nr = 238;
		v_assert_meta_info__t182.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t182.src = _SLIT("y[0].id == 2");
		v_assert_meta_info__t182.op = _SLIT("==");
		v_assert_meta_info__t182.llabel = _SLIT("y[0].id");
		v_assert_meta_info__t182.rlabel = _SLIT("2");
		v_assert_meta_info__t182.lvalue = int_str((*(main__User*)/*ee elem_typ */array_get(y, 0)).id) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t182.rvalue = int_str(2) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t182);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t183 = {0};
		v_assert_meta_info__t183.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t183.line_nr = 238;
		v_assert_meta_info__t183.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t183.src = _SLIT("y[0].id == 2");
		v_assert_meta_info__t183.op = _SLIT("==");
		v_assert_meta_info__t183.llabel = _SLIT("y[0].id");
		v_assert_meta_info__t183.rlabel = _SLIT("2");
		v_assert_meta_info__t183.lvalue = int_str((*(main__User*)/*ee elem_typ */array_get(y, 0)).id) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t183.rvalue = int_str(2) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t183);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t184 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t185 = orm__Connection_name_table[_t184._typ]._method_select(_t184._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = false,.has_order = false,.has_limit = true,.has_offset = true,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(2, 2, sizeof(orm__Primitive), _MOV((orm__Primitive[2]){orm__int_to_primitive(2),orm__int_to_primitive(offset_const),}))},(orm__QueryData) {});
	if (_o_t185.state != 0 && _o_t185.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t185.err)); }
	Array_Array_orm__Primitive _t185 = (*(Array_Array_orm__Primitive*)_o_t185.data);
	int _t187 = 0;
	Array_main__User _t186_array = __new_array(0, _t185.len, sizeof(main__User));
	for (; _t187 < _t185.len; _t187++) {
		main__User _t186 = (main__User) {.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t185.len > 0) {
	_t186.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t185, _t187)), 0))._int);
	_t186.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t185, _t187)), 1))._int);
	_t186.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t185, _t187)), 2))._string);
	_t186.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t185, _t187)), 3))._bool);
	}
	array_push(&_t186_array, _MOV((main__User[]){ _t186 }));
	}
		Array_main__User z =  _t186_array;
	;
	// assert
	if (z.len == 2) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t188 = {0};
		v_assert_meta_info__t188.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t188.line_nr = 244;
		v_assert_meta_info__t188.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t188.src = _SLIT("z.len == 2");
		v_assert_meta_info__t188.op = _SLIT("==");
		v_assert_meta_info__t188.llabel = _SLIT("z.len");
		v_assert_meta_info__t188.rlabel = _SLIT("2");
		v_assert_meta_info__t188.lvalue = int_str(z.len) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t188.rvalue = int_str(2) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t188);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t189 = {0};
		v_assert_meta_info__t189.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t189.line_nr = 244;
		v_assert_meta_info__t189.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t189.src = _SLIT("z.len == 2");
		v_assert_meta_info__t189.op = _SLIT("==");
		v_assert_meta_info__t189.llabel = _SLIT("z.len");
		v_assert_meta_info__t189.rlabel = _SLIT("2");
		v_assert_meta_info__t189.lvalue = int_str(z.len) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t189.rvalue = int_str(2) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t189);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	if ((*(main__User*)/*ee elem_typ */array_get(z, 0)).id == 3) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t190 = {0};
		v_assert_meta_info__t190.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t190.line_nr = 245;
		v_assert_meta_info__t190.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t190.src = _SLIT("z[0].id == 3");
		v_assert_meta_info__t190.op = _SLIT("==");
		v_assert_meta_info__t190.llabel = _SLIT("z[0].id");
		v_assert_meta_info__t190.rlabel = _SLIT("3");
		v_assert_meta_info__t190.lvalue = int_str((*(main__User*)/*ee elem_typ */array_get(z, 0)).id) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t190.rvalue = int_str(3) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t190);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t191 = {0};
		v_assert_meta_info__t191.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t191.line_nr = 245;
		v_assert_meta_info__t191.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t191.src = _SLIT("z[0].id == 3");
		v_assert_meta_info__t191.op = _SLIT("==");
		v_assert_meta_info__t191.llabel = _SLIT("z[0].id");
		v_assert_meta_info__t191.rlabel = _SLIT("3");
		v_assert_meta_info__t191.lvalue = int_str((*(main__User*)/*ee elem_typ */array_get(z, 0)).id) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t191.rvalue = int_str(3) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t191);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t192 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t193 = orm__Connection_name_table[_t192._typ]._method_select(_t192._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = false,.has_order = true,.order = _SLIT("age"),.order_type = orm__OrderType__desc,.has_limit = true,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(1),}))},(orm__QueryData) {});
	if (_o_t193.state != 0 && _o_t193.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t193.err)); }
	Array_Array_orm__Primitive _t193 = (*(Array_Array_orm__Primitive*)_o_t193.data);
	int _t195 = 0;
	main__User _t194 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t193.len > 0) {
	_t194.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t193, _t195)), 0))._int);
	_t194.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t193, _t195)), 1))._int);
	_t194.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t193, _t195)), 2))._string);
	_t194.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t193, _t195)), 3))._bool);
	}
		main__User oldest =  _t194;
	;
	// assert
	if (oldest.age == 34) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t196 = {0};
		v_assert_meta_info__t196.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t196.line_nr = 249;
		v_assert_meta_info__t196.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t196.src = _SLIT("oldest.age == 34");
		v_assert_meta_info__t196.op = _SLIT("==");
		v_assert_meta_info__t196.llabel = _SLIT("oldest.age");
		v_assert_meta_info__t196.rlabel = _SLIT("34");
		v_assert_meta_info__t196.lvalue = int_str(oldest.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t196.rvalue = int_str(34) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t196);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t197 = {0};
		v_assert_meta_info__t197.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t197.line_nr = 249;
		v_assert_meta_info__t197.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t197.src = _SLIT("oldest.age == 34");
		v_assert_meta_info__t197.op = _SLIT("==");
		v_assert_meta_info__t197.llabel = _SLIT("oldest.age");
		v_assert_meta_info__t197.rlabel = _SLIT("34");
		v_assert_meta_info__t197.lvalue = int_str(oldest.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t197.rvalue = int_str(34) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t197);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t198 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t199 = orm__Connection_name_table[_t198._typ]._method_select(_t198._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = false,.has_order = true,.order = _SLIT("age"),.order_type = orm__OrderType__desc,.has_limit = true,.has_offset = true,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(2, 2, sizeof(orm__Primitive), _MOV((orm__Primitive[2]){orm__int_to_primitive(1),orm__int_to_primitive(offs),}))},(orm__QueryData) {});
	if (_o_t199.state != 0 && _o_t199.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t199.err)); }
	Array_Array_orm__Primitive _t199 = (*(Array_Array_orm__Primitive*)_o_t199.data);
	int _t201 = 0;
	main__User _t200 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t199.len > 0) {
	_t200.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t199, _t201)), 0))._int);
	_t200.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t199, _t201)), 1))._int);
	_t200.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t199, _t201)), 2))._string);
	_t200.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t199, _t201)), 3))._bool);
	}
		main__User second_oldest =  _t200;
	;
	// assert
	if (second_oldest.age == 31) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t202 = {0};
		v_assert_meta_info__t202.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t202.line_nr = 254;
		v_assert_meta_info__t202.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t202.src = _SLIT("second_oldest.age == 31");
		v_assert_meta_info__t202.op = _SLIT("==");
		v_assert_meta_info__t202.llabel = _SLIT("second_oldest.age");
		v_assert_meta_info__t202.rlabel = _SLIT("31");
		v_assert_meta_info__t202.lvalue = int_str(second_oldest.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t202.rvalue = int_str(31) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t202);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t203 = {0};
		v_assert_meta_info__t203.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t203.line_nr = 254;
		v_assert_meta_info__t203.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t203.src = _SLIT("second_oldest.age == 31");
		v_assert_meta_info__t203.op = _SLIT("==");
		v_assert_meta_info__t203.llabel = _SLIT("second_oldest.age");
		v_assert_meta_info__t203.rlabel = _SLIT("31");
		v_assert_meta_info__t203.lvalue = int_str(second_oldest.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t203.rvalue = int_str(31) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t203);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t204 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_void _t205 = orm__Connection_name_table[_t204._typ]._method__v_delete(_t204._object, _SLIT("userlist"),(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("age"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(34),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_t205.state != 0 && _t205.err._typ != _IError_None___index) { _v_panic(IError_str(_t205.err)); }

	// orm
	orm__Connection _t206 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t207 = orm__Connection_name_table[_t206._typ]._method_select(_t206._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = false,.has_order = true,.order = _SLIT("age"),.order_type = orm__OrderType__desc,.has_limit = true,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(1),}))},(orm__QueryData) {});
	if (_o_t207.state != 0 && _o_t207.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t207.err)); }
	Array_Array_orm__Primitive _t207 = (*(Array_Array_orm__Primitive*)_o_t207.data);
	int _t209 = 0;
	main__User _t208 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t207.len > 0) {
	_t208.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t207, _t209)), 0))._int);
	_t208.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t207, _t209)), 1))._int);
	_t208.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t207, _t209)), 2))._string);
	_t208.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t207, _t209)), 3))._bool);
	}
		main__User updated_oldest =  _t208;
	;
	// assert
	if (updated_oldest.age == 31) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t210 = {0};
		v_assert_meta_info__t210.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t210.line_nr = 262;
		v_assert_meta_info__t210.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t210.src = _SLIT("updated_oldest.age == 31");
		v_assert_meta_info__t210.op = _SLIT("==");
		v_assert_meta_info__t210.llabel = _SLIT("updated_oldest.age");
		v_assert_meta_info__t210.rlabel = _SLIT("31");
		v_assert_meta_info__t210.lvalue = int_str(updated_oldest.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t210.rvalue = int_str(31) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t210);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t211 = {0};
		v_assert_meta_info__t211.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t211.line_nr = 262;
		v_assert_meta_info__t211.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t211.src = _SLIT("updated_oldest.age == 31");
		v_assert_meta_info__t211.op = _SLIT("==");
		v_assert_meta_info__t211.llabel = _SLIT("updated_oldest.age");
		v_assert_meta_info__t211.rlabel = _SLIT("31");
		v_assert_meta_info__t211.lvalue = int_str(updated_oldest.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t211.rvalue = int_str(31) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t211);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t212 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t213 = orm__Connection_name_table[_t212._typ]._method_select(_t212._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(5),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t213.state != 0 && _o_t213.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t213.err)); }
	Array_Array_orm__Primitive _t213 = (*(Array_Array_orm__Primitive*)_o_t213.data);
	int _t215 = 0;
	main__User _t214 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t213.len > 0) {
	_t214.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t213, _t215)), 0))._int);
	_t214.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t213, _t215)), 1))._int);
	_t214.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t213, _t215)), 2))._string);
	_t214.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t213, _t215)), 3))._bool);
	}
		main__User null_user =  _t214;
	;
	// assert
	if ((null_user.name).len == 0) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t216 = {0};
		v_assert_meta_info__t216.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t216.line_nr = 268;
		v_assert_meta_info__t216.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t216.src = _SLIT("null_user.name == ''");
		v_assert_meta_info__t216.op = _SLIT("==");
		v_assert_meta_info__t216.llabel = _SLIT("null_user.name");
		v_assert_meta_info__t216.rlabel = _SLIT("''");
		v_assert_meta_info__t216.lvalue = null_user.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t216.rvalue = _SLIT("") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t216);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t217 = {0};
		v_assert_meta_info__t217.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t217.line_nr = 268;
		v_assert_meta_info__t217.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t217.src = _SLIT("null_user.name == ''");
		v_assert_meta_info__t217.op = _SLIT("==");
		v_assert_meta_info__t217.llabel = _SLIT("null_user.name");
		v_assert_meta_info__t217.rlabel = _SLIT("''");
		v_assert_meta_info__t217.lvalue = null_user.name /* typeof: v.ast.SelectorExpr type: ast.Type(0x12 = 18) */ ;
		v_assert_meta_info__t217.rvalue = _SLIT("") /* typeof: v.ast.StringLiteral type: ast.Type(0x12 = 18) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t217);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t218 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t219 = orm__Connection_name_table[_t218._typ]._method_select(_t218._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(1),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t219.state != 0 && _o_t219.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t219.err)); }
	Array_Array_orm__Primitive _t219 = (*(Array_Array_orm__Primitive*)_o_t219.data);
	int _t221 = 0;
	main__User _t220 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t219.len > 0) {
	_t220.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t219, _t221)), 0))._int);
	_t220.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t219, _t221)), 1))._int);
	_t220.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t219, _t221)), 2))._string);
	_t220.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t219, _t221)), 3))._bool);
	}
		main__User age_test =  _t220;
	;
	// assert
	if (age_test.age == 29) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t222 = {0};
		v_assert_meta_info__t222.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t222.line_nr = 274;
		v_assert_meta_info__t222.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t222.src = _SLIT("age_test.age == 29");
		v_assert_meta_info__t222.op = _SLIT("==");
		v_assert_meta_info__t222.llabel = _SLIT("age_test.age");
		v_assert_meta_info__t222.rlabel = _SLIT("29");
		v_assert_meta_info__t222.lvalue = int_str(age_test.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t222.rvalue = int_str(29) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t222);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t223 = {0};
		v_assert_meta_info__t223.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t223.line_nr = 274;
		v_assert_meta_info__t223.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t223.src = _SLIT("age_test.age == 29");
		v_assert_meta_info__t223.op = _SLIT("==");
		v_assert_meta_info__t223.llabel = _SLIT("age_test.age");
		v_assert_meta_info__t223.rlabel = _SLIT("29");
		v_assert_meta_info__t223.lvalue = int_str(age_test.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t223.rvalue = int_str(29) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t223);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t224 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_void _t225 = orm__Connection_name_table[_t224._typ]._method_update(_t224._object, _SLIT("userlist"), (orm__QueryData){.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("age"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__infix_to_primitive((orm__InfixType){.name = _SLIT("age"),.operator = orm__MathOperationKind__add,.right = orm__int_to_primitive(1),}),})),},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(1),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_t225.state != 0 && _t225.err._typ != _IError_None___index) { _v_panic(IError_str(_t225.err)); }

	// orm
	orm__Connection _t226 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t227 = orm__Connection_name_table[_t226._typ]._method_select(_t226._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(1),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t227.state != 0 && _o_t227.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t227.err)); }
	Array_Array_orm__Primitive _t227 = (*(Array_Array_orm__Primitive*)_o_t227.data);
	int _t229 = 0;
	main__User _t228 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t227.len > 0) {
	_t228.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t227, _t229)), 0))._int);
	_t228.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t227, _t229)), 1))._int);
	_t228.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t227, _t229)), 2))._string);
	_t228.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t227, _t229)), 3))._bool);
	}
		main__User first =  _t228;
	;
	// assert
	if (first.age == 30) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t230 = {0};
		v_assert_meta_info__t230.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t230.line_nr = 284;
		v_assert_meta_info__t230.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t230.src = _SLIT("first.age == 30");
		v_assert_meta_info__t230.op = _SLIT("==");
		v_assert_meta_info__t230.llabel = _SLIT("first.age");
		v_assert_meta_info__t230.rlabel = _SLIT("30");
		v_assert_meta_info__t230.lvalue = int_str(first.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t230.rvalue = int_str(30) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t230);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t231 = {0};
		v_assert_meta_info__t231.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t231.line_nr = 284;
		v_assert_meta_info__t231.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t231.src = _SLIT("first.age == 30");
		v_assert_meta_info__t231.op = _SLIT("==");
		v_assert_meta_info__t231.llabel = _SLIT("first.age");
		v_assert_meta_info__t231.rlabel = _SLIT("30");
		v_assert_meta_info__t231.lvalue = int_str(first.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t231.rvalue = int_str(30) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t231);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t232 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_void _t233 = orm__Connection_name_table[_t232._typ]._method_update(_t232._object, _SLIT("userlist"), (orm__QueryData){.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("age"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__infix_to_primitive((orm__InfixType){.name = _SLIT("age"),.operator = orm__MathOperationKind__mul,.right = orm__int_to_primitive(2),}),})),},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(1),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_t233.state != 0 && _t233.err._typ != _IError_None___index) { _v_panic(IError_str(_t233.err)); }

	// orm
	orm__Connection _t234 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t235 = orm__Connection_name_table[_t234._typ]._method_select(_t234._object, (orm__SelectConfig){.table = _SLIT("userlist"),.is_count = false,.has_where = true,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(4, 4, sizeof(string), _MOV((string[4]){_SLIT("id"),_SLIT("age"),_SLIT("username"),_SLIT("is_customer"),})),.types = new_array_from_c_array(4, 4, sizeof(int), _MOV((int[4]){7,7,18,16,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData){.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.fields = new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){_SLIT("id"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__int_to_primitive(1),})),.kinds = new_array_from_c_array(1, 1, sizeof(orm__OperationKind), _MOV((orm__OperationKind[1]){orm__OperationKind__eq,})),.is_and = new_array_from_c_array(0, 0, sizeof(bool),NULL),});
	if (_o_t235.state != 0 && _o_t235.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t235.err)); }
	Array_Array_orm__Primitive _t235 = (*(Array_Array_orm__Primitive*)_o_t235.data);
	int _t237 = 0;
	main__User _t236 = (main__User){.id = 0, .age = 0, .name = (string){.str=(byteptr)"", .is_lit=1}, .is_customer = 0, .skipped_string = (string){.str=(byteptr)"", .is_lit=1}};
	if (_t235.len > 0) {
	_t236.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t235, _t237)), 0))._int);
	_t236.age = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t235, _t237)), 1))._int);
	_t236.name = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t235, _t237)), 2))._string);
	_t236.is_customer = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t235, _t237)), 3))._bool);
	}
		first =  _t236;
	;
	// assert
	if (first.age == 60) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t238 = {0};
		v_assert_meta_info__t238.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t238.line_nr = 294;
		v_assert_meta_info__t238.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t238.src = _SLIT("first.age == 60");
		v_assert_meta_info__t238.op = _SLIT("==");
		v_assert_meta_info__t238.llabel = _SLIT("first.age");
		v_assert_meta_info__t238.rlabel = _SLIT("60");
		v_assert_meta_info__t238.lvalue = int_str(first.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t238.rvalue = int_str(60) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t238);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t239 = {0};
		v_assert_meta_info__t239.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t239.line_nr = 294;
		v_assert_meta_info__t239.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t239.src = _SLIT("first.age == 60");
		v_assert_meta_info__t239.op = _SLIT("==");
		v_assert_meta_info__t239.llabel = _SLIT("first.age");
		v_assert_meta_info__t239.rlabel = _SLIT("60");
		v_assert_meta_info__t239.lvalue = int_str(first.age) /* typeof: v.ast.SelectorExpr type: ast.Type(0x7 = 7) */ ;
		v_assert_meta_info__t239.rvalue = int_str(60) /* typeof: v.ast.IntegerLiteral type: ast.Type(0x7 = 7) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t239);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	// orm
	orm__Connection _t240 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_void _t241 = orm__Connection_name_table[_t240._typ]._method_create(_t240._object, _SLIT("TestTime"), new_array_from_c_array(2, 2, sizeof(orm__TableField), _MOV((orm__TableField[2]){(orm__TableField){.name = _SLIT("id"),.typ = 7,.is_arr = false, .is_time = 0,.default_val = (string){.str = (byteptr) "", .is_lit = 1},.attrs = new_array_from_c_array(2, 2, sizeof(StructAttribute), _MOV((StructAttribute[2]){(StructAttribute){.name = _SLIT("primary"),.has_arg = 0,.arg = _SLIT(""),.kind = 0,},(StructAttribute){.name = _SLIT("sql"),.has_arg = 1,.arg = _SLIT("serial"),.kind = 0,},}))},(orm__TableField){.name = _SLIT("create"),.typ = 88,.is_arr = false, .is_time = 0,.default_val = (string){.str = (byteptr) "", .is_lit = 1},.attrs = new_array_from_c_array(0, 0, sizeof(StructAttribute),NULL)},})));
	if (_t241.state != 0 && _t241.err._typ != _IError_None___index) { _v_panic(IError_str(_t241.err)); }
	time__Time tnow = time__now();
	main__TestTime time_test = (main__TestTime){.id = 0,.create = tnow,};
	
	// orm
	orm__Connection _t242 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Array_orm__Primitive _t244 = new_array_from_c_array(0, 0, sizeof(orm__Primitive), NULL);
	Option_void _t243 = orm__Connection_name_table[_t242._typ]._method_insert(_t242._object, _SLIT("TestTime"), (orm__QueryData){.fields = new_array_from_c_array(1, 1, sizeof(string),_MOV((string[1]){_SLIT("create"),})),.data = new_array_from_c_array(1, 1, sizeof(orm__Primitive), _MOV((orm__Primitive[1]){orm__time_to_primitive(time_test.create),})),.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),});
	if (_t243.state != 0 && _t243.err._typ != _IError_None___index) { _v_panic(IError_str(_t243.err)); }

	// orm
	orm__Connection _t245 = (orm__Connection){._sqlite__DB = &db, ._typ = _orm__Connection_sqlite__DB_index};
	Option_Array_Array_orm__Primitive _o_t246 = orm__Connection_name_table[_t245._typ]._method_select(_t245._object, (orm__SelectConfig){.table = _SLIT("TestTime"),.is_count = false,.has_where = false,.has_order = false,.has_limit = false,.has_offset = false,.primary = _SLIT("id"),.fields = new_array_from_c_array(2, 2, sizeof(string), _MOV((string[2]){_SLIT("id"),_SLIT("create"),})),.types = new_array_from_c_array(2, 2, sizeof(int), _MOV((int[2]){7,-2,})),},(orm__QueryData) {.types = new_array_from_c_array(0, 0, sizeof(int), NULL),.kinds = new_array_from_c_array(0, 0, sizeof(orm__OperationKind), NULL),.is_and = new_array_from_c_array(0, 0, sizeof(bool), NULL),.data = new_array_from_c_array(0, 0, sizeof(orm__Primitive),NULL)},(orm__QueryData) {});
	if (_o_t246.state != 0 && _o_t246.err._typ != _IError_None___index) { _v_panic(IError_str(_o_t246.err)); }
	Array_Array_orm__Primitive _t246 = (*(Array_Array_orm__Primitive*)_o_t246.data);
	int _t248 = 0;
	Array_main__TestTime _t247_array = __new_array(0, _t246.len, sizeof(main__TestTime));
	for (; _t248 < _t246.len; _t248++) {
		main__TestTime _t247 = (main__TestTime) {.id = 0, .create = (time__Time){.year = 0,.month = 0,.day = 0,.hour = 0,.minute = 0,.second = 0,.microsecond = 0,._v_unix = 0,}};
	if (_t246.len > 0) {
	_t247.id = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t246, _t248)), 0))._int);
	_t247.create = *((*(orm__Primitive*) array_get((*(Array_orm__Primitive*) array_get(_t246, _t248)), 1))._time__Time);
	}
	array_push(&_t247_array, _MOV((main__TestTime[]){ _t247 }));
	}
		Array_main__TestTime data =  _t247_array;
	;
	// assert
	if (time__Time__eq(tnow, (*(main__TestTime*)/*ee elem_typ */array_get(data, 0)).create)) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t249 = {0};
		v_assert_meta_info__t249.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t249.line_nr = 314;
		v_assert_meta_info__t249.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t249.src = _SLIT("tnow == data[0].create");
		v_assert_meta_info__t249.op = _SLIT("==");
		v_assert_meta_info__t249.llabel = _SLIT("tnow");
		v_assert_meta_info__t249.rlabel = _SLIT("data[0].create");
		v_assert_meta_info__t249.lvalue = time__Time_str(tnow) /* typeof: v.ast.Ident type: ast.Type(0x58 = 88) */ ;
		v_assert_meta_info__t249.rvalue = time__Time_str((*(main__TestTime*)/*ee elem_typ */array_get(data, 0)).create) /* typeof: v.ast.SelectorExpr type: ast.Type(0x58 = 88) */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t249);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t250 = {0};
		v_assert_meta_info__t250.fpath = _SLIT("vlib/orm/orm_test.v");
		v_assert_meta_info__t250.line_nr = 314;
		v_assert_meta_info__t250.fn_name = _SLIT("main.test_orm_sqlite");
		v_assert_meta_info__t250.src = _SLIT("tnow == data[0].create");
		v_assert_meta_info__t250.op = _SLIT("==");
		v_assert_meta_info__t250.llabel = _SLIT("tnow");
		v_assert_meta_info__t250.rlabel = _SLIT("data[0].create");
		v_assert_meta_info__t250.lvalue = time__Time_str(tnow) /* typeof: v.ast.Ident type: ast.Type(0x58 = 88) */ ;
		v_assert_meta_info__t250.rvalue = time__Time_str((*(main__TestTime*)/*ee elem_typ */array_get(data, 0)).create) /* typeof: v.ast.SelectorExpr type: ast.Type(0x58 = 88) */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t250);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
