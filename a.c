	...
	#define V_COMMIT_HASH "2abbbcc"
#endif

#ifndef V_CURRENT_COMMIT_HASH
	#define V_CURRENT_COMMIT_HASH "d464f54"
#endif
...
VV_LOCAL_SYMBOL void map_free_nop(voidptr _t391);
...
VV_LOCAL_SYMBOL void main__test_defer_arrays();
VV_LOCAL_SYMBOL void main__test_defer_str_interpol();
...
VV_LOCAL_SYMBOL bool print_backtrace_skipping_top_frames_linux(int skipframes) {
	...
	#else
	{
		#if !defined(_VFREESTANDING)
		{
			...
			// FOR IN array
			for (int _t301 = 0; _t301 < sframes.len; ++_t301) {
				string sframe = ((string*)sframes.data)[_t301];
...
string byte_str_escaped(byte b) {
	...
	if (b == (0)) {
		...
		_t372 = byte_ascii_str(b);
		;
	}
	else {
		_t372 = string__plus(_SLIT("0x"), byte_hex(b));
		;
	}	string str = _t372;
	return str;
}
...
VV_LOCAL_SYMBOL voidptr map_get_and_set(map* m, voidptr key, voidptr zero) {
	...
	if (false) {
		...
		v_assert_meta_info__t399.fpath = _SLIT("/home/louis/v/vlib/builtin/map.v");
		v_assert_meta_info__t399.line_nr = 560;
		v_assert_meta_info__t399.fn_name = _SLIT("get_and_set");
		v_assert_meta_info__t399.src = _SLIT("false");
		main__cb_assertion_ok(&v_assert_meta_info__t399);
	} else {
		g_test_fails++;
		...
		main__cb_assertion_failed(&v_assert_meta_info__t400);
...
Option_int string_last_index(string s, string p) {
	...
	Option_int _t534;
	opt_ok(&(int[]) { idx }, (Option*)(&_t534), sizeof(int));
	return _t534;
}
...
int string_count(string s, string substr) {
	...
	if (substr.len == 1) {
		...
		for (int _t545 = 0; _t545 < s.len; ++_t545) {
			byte letter = s.str[_t545];
			if (letter == target) {
				n++;
			}
		}
...
bool string_contains_any(string s, string chars) {
	for (int _t552 = 0; _t552 < chars.len; ++_t552) {
		byte c = chars.str[_t552];
...
bool string_contains_any_substr(string s, Array_string substrs) {
	...
	// FOR IN array
	for (int _t556 = 0; _t556 < substrs.len; ++_t556) {
		string sub = ((string*)substrs.data)[_t556];
...
string string_title(string s) {
	...
	// FOR IN array
	for (int _t577 = 0; _t577 < words.len; ++_t577) {
		string word = ((string*)words.data)[_t577];
...
bool string_is_title(string s) {
	Array_string words = string_split(s, _SLIT(" "));
	// FOR IN array
	for (int _t580 = 0; _t580 < words.len; ++_t580) {
		string word = ((string*)words.data)[_t580];
...
string string_trim_left(string s, string cutset) {
	...
	for (;;) {
		if (!(pos < s.len)) break;
		bool found = false;
		for (int _t594 = 0; _t594 < cutset.len; ++_t594) {
			byte cs = cutset.str[_t594];
			if (s.str[ pos] == cs) {
				found = true;
				break;
			}
...
string string_trim_right(string s, string cutset) {
	...
	for (;;) {
		if (!(pos >= 0)) break;
		bool found = false;
		for (int _t597 = 0; _t597 < cutset.len; ++_t597) {
			byte cs = cutset.str[_t597];
			if (s.str[ pos] == cs) {
				found = true;
			}
		}
...
string Array_string_join(Array_string a, string sep) {
	...
	int len = 0;
	// FOR IN array
	for (int _t658 = 0; _t658 < a.len; ++_t658) {
		string val = ((string*)a.data)[_t658];
		len += val.len + sep.len;
	}
	len -= sep.len;
	string res = (string){.str = v_malloc(len + 1), .len = len};
...
int string_hash(string s) {
	u32 h = ((u32)(0U));
	if (h == 0U && s.len > 0) {
		for (int _t665 = 0; _t665 < s.len; ++_t665) {
			byte c = s.str[_t665];
			h = h * 31U + ((u32)(c));
		}
	}
...
// Attr: [manualfree]
string str_intp_sub(string base_str, string in_str) {
	Option_int _t718 = string_index(base_str, _SLIT("%%"));
	if (_t718.state != 0) { /*or block*/ 
		IError err = _t718.err;
		eprintln(_SLIT("No strin interpolation %% parameteres"));
		v_exit(1);
	}
 	int index =  (*(int*)_t718.data);
	{ // Unsafe block
...
Array_string os__args_after(string cut_word) {
	...
	if (!(Array_string_contains(_const_os__args, cut_word))) {
		...
		// FOR IN array
		Array_string _t742 = array_slice(_const_os__args, 1, _const_os__args.len);
		for (int _t743 = 0; _t743 < _t742.len; ++_t743) {
			string a = ((string*)_t742.data)[_t743];
...
Array_string os__args_before(string cut_word) {
	...
	if (!(Array_string_contains(_const_os__args, cut_word))) {
		...
		array_push((array*)&cargs, _MOV((string[]){ (*(string*)/*ee elem_typ */array_get(_const_os__args, 0)) }));
		// FOR IN array
		Array_string _t748 = array_slice(_const_os__args, 1, _const_os__args.len);
		for (int _t749 = 0; _t749 < _t748.len; ++_t749) {
			string a = ((string*)_t748.data)[_t749];
			if (string__eq(a, cut_word)) {
				break;
			}
			array_push((array*)&cargs, _MOV((string[]){ a }));
...
Option_os__File os__open_file(string path, string mode, Array_int options) {
	int flags = 0;
	for (int _t766 = 0; _t766 < mode.len; ++_t766) {
		byte m = mode.str[_t766];
	...
	Option_os__File _t769;
	opt_ok(&(os__File[]) { (os__File){.cfile = cfile,.fd = fd,.is_opened = true,} }, (Option*)(&_t769), sizeof(os__File));
	return _t769;
}

Option_os__File os__open(string path) {
	Option_FILE_ptr _t770 = os__vfopen(path, _SLIT("rb"));
	if (_t770.state != 0) { /*or block*/ 
		Option_os__File _t771;
		memcpy(&_t771, &_t770, sizeof(Option));
		return _t771;
	}
 	FILE* cfile =  (*(FILE**)_t770.data);
	int fd = os__fileno(cfile);
	Option_os__File _t772;
	opt_ok(&(os__File[]) { (os__File){.cfile = cfile,.fd = fd,.is_opened = true,} }, (Option*)(&_t772), sizeof(os__File));
	return _t772;
}

Option_os__File os__create(string path) {
	Option_FILE_ptr _t773 = os__vfopen(path, _SLIT("wb"));
	if (_t773.state != 0) { /*or block*/ 
		Option_os__File _t774;
		memcpy(&_t774, &_t773, sizeof(Option));
		return _t774;
	}
 	FILE* cfile =  (*(FILE**)_t773.data);
	int fd = os__fileno(cfile);
	Option_os__File _t775;
	opt_ok(&(os__File[]) { (os__File){.cfile = cfile,.fd = fd,.is_opened = true,} }, (Option*)(&_t775), sizeof(os__File));
	return _t775;
}
...
Option_int os__File_write(os__File* f, Array_byte buf) {
	...
	Option_int _t782;
	opt_ok(&(int[]) { written }, (Option*)(&_t782), sizeof(int));
	return _t782;
}
...
Option_int os__File_writeln(os__File* f, string s) {
	...
	Option_int _t786;
	opt_ok(&(int[]) { (written + 1) }, (Option*)(&_t786), sizeof(int));
	return _t786;
}
...
Option_int os__File_write_string(os__File* f, string s) {
	...
	Option_int _t789;
	opt_ok(&(int[]) { written }, (Option*)(&_t789), sizeof(int));
	return _t789;
}
...
Option_int os__File_write_to(os__File* f, u64 pos, Array_byte buf) {
	...
	#if defined(TARGET_IS_64BIT)
	{
		...
		#else
		{
			...
			Option_int _t792;
			opt_ok(&(int[]) { res }, (Option*)(&_t792), sizeof(int));
			return _t792;
		}
		#endif
	}
	...
	#if defined(TARGET_IS_32BIT)
	{
		...
		Option_int _t794;
		opt_ok(&(int[]) { res }, (Option*)(&_t794), sizeof(int));
		return _t794;
	}
	#endif
...
VV_LOCAL_SYMBOL Option_int os__fread(voidptr ptr, int item_size, int items, FILE* stream) {
	...
	Option_int _t804;
	opt_ok(&(int[]) { nbytes }, (Option*)(&_t804), sizeof(int));
	return _t804;
}
...
Array_byte os__File_read_bytes_at(os__File* f, int size, u64 pos) {
	...
	Option_int _t806 = os__File_read_bytes_into(f, pos, &/*arr*/arr);
	if (_t806.state != 0) { /*or block*/ 
		IError err = _t806.err;
		return __new_array_with_default(0, 0, sizeof(byte), 0);
	}
 	int nreadbytes =  (*(int*)_t806.data);
...
Option_int os__File_read_bytes_into(os__File* f, u64 pos, Array_byte* buf) {
	...
	{
		fseek(f->cfile, pos, SEEK_SET);
		Option_int _t816 = os__fread(buf->data, 1, buf->len, f->cfile);
		if (_t816.state != 0) { /*or block*/ 
			Option_int _t817;
			memcpy(&_t817, &_t816, sizeof(Option));
			return _t817;
		}
 		int nbytes =  (*(int*)_t816.data);
		Option_int _t818;
		opt_ok(&(int[]) { nbytes }, (Option*)(&_t818), sizeof(int));
		return _t818;
	}
	#endif
...
Option_int os__File_read(os__File* f, Array_byte* buf) {
	if (buf->len == 0) {
		Option_int _t820;
		opt_ok(&(int[]) { 0 }, (Option*)(&_t820), sizeof(int));
		return _t820;
	}
	Option_int _t821 = os__fread(buf->data, 1, buf->len, f->cfile);
	if (_t821.state != 0) { /*or block*/ 
		Option_int _t822;
		memcpy(&_t822, &_t821, sizeof(Option));
		return _t822;
	}
 	int nbytes =  (*(int*)_t821.data);
	Option_int _t823;
	opt_ok(&(int[]) { nbytes }, (Option*)(&_t823), sizeof(int));
	return _t823;
}
...
Option_int os__File_read_from(os__File* f, u64 pos, Array_byte* buf) {
	if (buf->len == 0) {
		Option_int _t825;
		opt_ok(&(int[]) { 0 }, (Option*)(&_t825), sizeof(int));
		return _t825;
	}
	#if defined(TARGET_IS_64BIT)
	{
		...
		#endif
		Option_int _t826 = os__fread(buf->data, 1, buf->len, f->cfile);
		if (_t826.state != 0) { /*or block*/ 
			Option_int _t827;
			memcpy(&_t827, &_t826, sizeof(Option));
			return _t827;
		}
 		int nbytes =  (*(int*)_t826.data);
		Option_int _t828;
		opt_ok(&(int[]) { nbytes }, (Option*)(&_t828), sizeof(int));
		return _t828;
	}
	#endif
	#if defined(TARGET_IS_32BIT)
	{
		fseek(f->cfile, pos, SEEK_SET);
		Option_int _t829 = os__fread(buf->data, 1, buf->len, f->cfile);
		if (_t829.state != 0) { /*or block*/ 
			Option_int _t830;
			memcpy(&_t830, &_t829, sizeof(Option));
			return _t830;
		}
 		int nbytes =  (*(int*)_t829.data);
		Option_int _t831;
		opt_ok(&(int[]) { nbytes }, (Option*)(&_t831), sizeof(int));
		return _t831;
	}
	#endif
	return (Option_int){ .state=2, .err=v_error(_SLIT("Could not read file")), .data={0} };
...
// Attr: [deprecated]
Option_void os__File_write_str(os__File* f, string s) {
	Option_int _t833 = os__File_write_string(f, s);
	if (_t833.state != 0) { /*or block*/ 
		IError err = _t833.err;
...
Option_void os__cp_all(string src, string dst, bool overwrite) {
	...
	if (!os__is_dir(source_path)) {
		...
		if (os__exists(adjusted_path)) {
			if (overwrite) {
				Option_void _t837 = os__rm(adjusted_path);
				if (_t837.state != 0 && _t837.err._typ != _IError_None___index) {
					Option_void _t838;
		...
		Option_void _t840 = os__cp(source_path, adjusted_path);
	...
	if (!os__exists(dest_path)) {
		Option_bool _t842 = os__mkdir(dest_path);
		if (_t842.state != 0) { /*or block*/ 
			Option_void _t843;
			memcpy(&_t843, &_t842, sizeof(Option));
			return _t843;
		};
	}
	...
	Option_Array_string _t845 = os__ls(source_path);
	if (_t845.state != 0) { /*or block*/ 
		Option_void _t846;
		memcpy(&_t846, &_t845, sizeof(Option));
		return _t846;
	}
 	Array_string files =  (*(Array_string*)_t845.data);
	// FOR IN array
	for (int _t847 = 0; _t847 < files.len; ++_t847) {
		string file = ((string*)files.data)[_t847];
		...
		if (os__is_dir(sp)) {
			if (!os__exists(dp)) {
				Option_bool _t848 = os__mkdir(dp);
				if (_t848.state != 0) { /*or block*/ 
					Option_void _t849;
					memcpy(&_t849, &_t848, sizeof(Option));
					return _t849;
				};
			}
		}
		Option_void _t850 = os__cp_all(sp, dp, overwrite);
		if (_t850.state != 0 && _t850.err._typ != _IError_None___index) {
			IError err = _t850.err;
			Option_void _t851 = os__rmdir(dp);
...
Option_void os__mv_by_cp(string source, string target) {
	Option_void _t854 = os__cp(source, target);
	if (_t854.state != 0 && _t854.err._typ != _IError_None___index) {
		Option_void _t855;
		memcpy(&_t855, &_t854, sizeof(Option));
		return _t855;
	};
	Option_void _t856 = os__rm(source);
...
Option_Array_string os__read_lines(string path) {
	Option_string _t858 = os__read_file(path);
	if (_t858.state != 0) { /*or block*/ 
		Option_Array_string _t859;
		memcpy(&_t859, &_t858, sizeof(Option));
		return _t859;
	}
 	string buf =  (*(string*)_t858.data);
	Array_string res = string_split_into_lines(buf);
	string_free(&buf);
	Option_Array_string _t860;
	opt_ok(&(Array_string[]) { res }, (Option*)(&_t860), sizeof(Array_string));
	return _t860;
}

VV_LOCAL_SYMBOL Option_Array_ustring os__read_ulines(string path) {
	Option_Array_string _t861 = os__read_lines(path);
	if (_t861.state != 0) { /*or block*/ 
		Option_Array_ustring _t862;
		memcpy(&_t862, &_t861, sizeof(Option));
		return _t862;
	}
 	Array_string lines =  (*(Array_string*)_t861.data);
	Array_ustring ulines = __new_array_with_default(0, 0, sizeof(ustring), 0);
	// FOR IN array
	for (int _t863 = 0; _t863 < lines.len; ++_t863) {
		string myline = ((string*)lines.data)[_t863];
		array_push((array*)&ulines, _MOV((ustring[]){ string_ustring(myline) }));
	}
	Option_Array_ustring _t865;
	opt_ok(&(Array_ustring[]) { ulines }, (Option*)(&_t865), sizeof(Array_ustring));
	return _t865;
}
...
Option_void os__rmdir_all(string path) {
	...
	Option_Array_string _t888 = os__ls(path);
	if (_t888.state != 0) { /*or block*/ 
		Option_void _t889;
		memcpy(&_t889, &_t888, sizeof(Option));
		return _t889;
	}
 	Array_string items =  (*(Array_string*)_t888.data);
	// FOR IN array
	for (int _t890 = 0; _t890 < items.len; ++_t890) {
		string item = ((string*)items.data)[_t890];
		string fullpath = os__join_path(path, new_array_from_c_array(1, 1, sizeof(string), _MOV((string[1]){item})));
		if (os__is_dir(fullpath)) {
			Option_void _t891 = os__rmdir_all(fullpath);
			if (_t891.state != 0 && _t891.err._typ != _IError_None___index) {
				IError err = _t891.err;
				ret_err = (*(err.msg));
			};
		} else {
			Option_void _t892 = os__rm(fullpath);
	...
	Option_void _t893 = os__rmdir(path);
	if (_t893.state != 0 && _t893.err._typ != _IError_None___index) {
...
bool os__is_dir_empty(string path) {
	Option_Array_string _t895 = os__ls(path);
	if (_t895.state != 0) { /*or block*/ 
		IError err = _t895.err;
		return true;
	}
 	Array_string items =  (*(Array_string*)_t895.data);
	return items.len == 0;
}

string os__file_ext(string path) {
	Option_int _t898 = string_last_index(path, _SLIT("."));
	if (_t898.state != 0) { /*or block*/ 
		IError err = _t898.err;
		return _SLIT("");
	}
 	int pos =  (*(int*)_t898.data);
...
string os__dir(string path) {
	...
	Option_int _t902 = string_last_index(path, _const_os__path_separator);
...
string os__input(string prompt) {
	Option_string _t916 = os__input_opt(prompt);
	if (_t916.state != 0) { /*or block*/ 
		IError err = _t916.err;
		return _SLIT("<EOF>");
	}
 	string res =  (*(string*)_t916.data);
	return res;
}
...
Option_void os__write_file(string path, string text) {
	Option_os__File _t928 = os__create(path);
	if (_t928.state != 0) { /*or block*/ 
		Option_void _t929;
		memcpy(&_t929, &_t928, sizeof(Option));
		return _t929;
	}
 	os__File f =  (*(os__File*)_t928.data);
	Option_int _t930 = os__File_write_string(&f, text);
	if (_t930.state != 0) { /*or block*/ 
		Option_void _t931;
		memcpy(&_t931, &_t930, sizeof(Option));
		return _t931;
	};
	os__File_close(&f);
	return (Option_void){0};
...
Option_void os__write_file_array(string path, array buffer) {
	Option_os__File _t932 = os__create(path);
	if (_t932.state != 0) { /*or block*/ 
		Option_void _t933;
		memcpy(&_t933, &_t932, sizeof(Option));
		return _t933;
	}
 	os__File f =  (*(os__File*)_t932.data);
...
VV_LOCAL_SYMBOL string os__executable_fallback(void) {
	...
	if (!os__is_abs_path(exepath)) {
		if (string_contains(exepath, _const_os__path_separator)) {
			...
			Option_string _t935 = os__find_abs_path_of_executable(exepath);
			if (_t935.state != 0) { /*or block*/ 
				IError err = _t935.err;
				*(string*) _t935.data = _SLIT("");
			}
 			string foundpath =  (*(string*)_t935.data);
			if (foundpath.len > 0) {
				exepath = foundpath;
			}
		}
...
Option_string os__find_abs_path_of_executable(string exepath) {
	...
	if (os__is_abs_path(exepath)) {
		Option_string _t938;
		opt_ok(&(string[]) { os__real_path(exepath) }, (Option*)(&_t938), sizeof(string));
		return _t938;
	}
	string res = _SLIT("");
	Array_string paths = string_split(os__getenv(_SLIT("PATH")), _const_os__path_delimiter);
	// FOR IN array
	for (int _t939 = 0; _t939 < paths.len; ++_t939) {
		string p = ((string*)paths.data)[_t939];
	...
	if (res.len > 0) {
		Option_string _t940;
		opt_ok(&(string[]) { os__real_path(res) }, (Option*)(&_t940), sizeof(string));
		return _t940;
	}
...
bool os__exists_in_system_path(string prog) {
	Option_string _t942 = os__find_abs_path_of_executable(prog);
	if (_t942.state != 0) { /*or block*/ 
...
string os__join_path(string base, Array_string dirs) {
	...
	// FOR IN array
	for (int _t949 = 0; _t949 < dirs.len; ++_t949) {
		string d = ((string*)dirs.data)[_t949];
...
Array_string os__walk_ext(string path, string ext) {
	...
	Option_Array_string _t953 = os__ls(path);
	if (_t953.state != 0) { /*or block*/ 
		IError err = _t953.err;
		return __new_array_with_default(0, 0, sizeof(string), 0);
	}
 	Array_string files =  (*(Array_string*)_t953.data);
	Array_string res = __new_array_with_default(0, 0, sizeof(string), 0);
	...
	for (int _t955 = 0; _t955 < files.len; ++_t955) {
		...
		if (os__is_dir(p) && !os__is_link(p)) {
			_PUSH_MANY(&res, (os__walk_ext(p, ext)), _t956, Array_string);
...
void os__walk(string path, void (*f)(string )) {
	...
	Option_Array_string _t959 = os__ls(path);
	if (_t959.state != 0) { /*or block*/ 
		IError err = _t959.err;
		return;
	}
 	Array_string files =  (*(Array_string*)_t959.data);
	string local_path_separator = _const_os__path_separator;
	if (string_ends_with(path, _const_os__path_separator)) {
		local_path_separator = _SLIT("");
	}
	// FOR IN array
	for (int _t960 = 0; _t960 < files.len; ++_t960) {
		string file = ((string*)files.data)[_t960];
...
Option_void os__mkdir_all(string path) {
	...
	// FOR IN array
	for (int _t961 = 0; _t961 < path_parts.len; ++_t961) {
		string subdir = ((string*)path_parts.data)[_t961];
		p = /*f*/string__plus(p, string__plus(subdir, _const_os__path_separator));
		if (os__exists(p) && os__is_dir(p)) {
			continue;
		}
		Option_bool _t962 = os__mkdir(p);
		if (_t962.state != 0) { /*or block*/ 
			IError err = _t962.err;
...
string os__cache_dir(void) {
	...
	if (!os__is_dir(cdir) && !os__is_link(cdir)) {
		Option_bool _t965 = os__mkdir(cdir);
		if (_t965.state != 0) { /*or block*/ 
			IError err = _t965.err;
...
Array_string os__vmodules_paths(void) {
	...
	Array_string _t971_orig = string_split(path, _const_os__path_delimiter);
	int _t971_len = _t971_orig.len;
	Array_string _t971 = __new_array(0, _t971_len, sizeof(string));

	for (int _t972 = 0; _t972 < _t971_len; ++_t972) {
		string it = ((string*) _t971_orig.data)[_t972];
		string ti = string_trim_right(it, _const_os__path_separator);
		array_push((array*)&_t971, &ti);
	}
	
	Array_string list = _t971;
	return list;
}
...
Option_os__Result os__exec(string cmd) {
	...
	Option_os__Result _t976;
	opt_ok(&(os__Result[]) { res }, (Option*)(&_t976), sizeof(os__Result));
	return _t976;
}
...
// Attr: [manualfree]
Option_Array_byte os__read_bytes(string path) {
	Option_FILE_ptr _t979 = os__vfopen(path, _SLIT("rb"));
	if (_t979.state != 0) { /*or block*/ 
		Option_Array_byte _t980;
		memcpy(&_t980, &_t979, sizeof(Option));
		return _t980;
	}
 	FILE* fp =  (*(FILE**)_t979.data);
	...
	array_free(&res);
	Option_Array_byte _t984;
	opt_ok(&(Array_byte[]) { fres }, (Option*)(&_t984), sizeof(Array_byte));
	return _t984;
}
...
FILE** _t985; // fp
	...
	Option_FILE_ptr _t986 = os__vfopen(path, mode);
	if (_t986.state != 0) { /*or block*/ 
		Option_string _t987;
		memcpy(&_t987, &_t986, sizeof(Option));
		return _t987;
	}
 	FILE* fp =  (*(FILE**)_t986.data);
	os__read_file_defer_0 = true;
	_t985 = &(fp);
	int cseek = fseek(fp, 0, SEEK_END);
	if (cseek != 0) {
		Option_string _t988 = (Option_string){ .state=2, .err=v_error(_SLIT("fseek failed")), .data={0} };
		// Defer begin
	...
	int fsize = ftell(fp);
	if (fsize < 0) {
		Option_string _t989 = (Option_string){ .state=2, .err=v_error(_SLIT("ftell failed")), .data={0} };
		// Defer begin
		if (os__read_file_defer_0) {
			fclose(/*defer*/(*_t985));
		}
	...
	{ // Unsafe block
		...
		if (is_eof == 0 && is_error != 0) {
			v_free(str);
			Option_string _t990 = (Option_string){ .state=2, .err=v_error(_SLIT("fread failed")), .data={0} };
			// Defer begin
			if (os__read_file_defer_0) {
				fclose(/*defer*/(*_t985));
			}
		...
		if (nelements == 0) {
			Option_string _t991;
			opt_ok(&(string[]) { byte_vstring(str) }, (Option*)(&_t991), sizeof(string));
			// Defer begin
			if (os__read_file_defer_0) {
				fclose(/*defer*/(*_t985));
			}
		...
		Option_string _t992;
		opt_ok(&(string[]) { byte_vstring_with_len(str, fsize) }, (Option*)(&_t992), sizeof(string));
		// Defer begin
		if (os__read_file_defer_0) {
			fclose(/*defer*/(*_t985));
		}
...
int* _t993; // fp
	...
	_t993 = &(fp);
	if (fp < 0) {
		Option_void _t994 = (Option_void){ .state=2, .err=error_with_code(os__posix_get_error_msg(errno), errno), .data={0} };
		// Defer begin
	...
	#else
	{
		if (ftruncate(fp, len) != 0) {
			Option_void _t995 = (Option_void){ .state=2, .err=error_with_code(os__posix_get_error_msg(errno), errno), .data={0} };
			// Defer begin
			if (os__truncate_defer_0) {
				close(/*defer*/(*_t993));
			}
...
Option_FILE_ptr os__vfopen(string path, string mode) {
	...
	if (isnil(fp)) {
		...
		Option_FILE_ptr _t1008;
		opt_ok(&(FILE*[]) { fp }, (Option*)(&_t1008), sizeof(FILE*));
		return _t1008;
	}
...
Option_os__File os__open_append(string path) {
	...
	file.is_opened = true;
	Option_os__File _t1043;
	opt_ok(&(os__File[]) { file }, (Option*)(&_t1043), sizeof(os__File));
	return _t1043;
}
...
Option_Array_string os__ls(string path) {
	...
	closedir(dir);
	Option_Array_string _t1063;
	opt_ok(&(Array_string[]) { res }, (Option*)(&_t1063), sizeof(Array_string));
	return _t1063;
}
...
Option_bool os__mkdir(string path) {
	if (string__eq(path, _SLIT("."))) {
		Option_bool _t1064;
		opt_ok(&(bool[]) { true }, (Option*)(&_t1064), sizeof(bool));
		return _t1064;
	}
	...
	Option_bool _t1066;
	opt_ok(&(bool[]) { true }, (Option*)(&_t1066), sizeof(bool));
	return _t1066;
}
...
strings__Builder* _t1067; // res
	...
	v_free(buf);
	os__Result _t1069 = (os__Result){.exit_code = exit_code,.output = soutput,};
	// Defer begin
	if (os__execute_defer_0) {
		strings__Builder_free(&/*defer*/(*_t1067));
	}
	// Defer end
	return _t1069;
}
...
string* _t1070; // pcmd
	...
	c->f = os__vpopen(pcmd);
	if (isnil(c->f)) {
		Option_void _t1071 = (Option_void){ .state=2, .err=v_error( str_intp(2, _MOV((StrIntpData[]){{_SLIT("exec(\""), 0xfe10, {.d_s = c->path}}, {_SLIT("\") failed"), 0, { .d_c = 0 }}})) ), .data={0} };
		// Defer begin
		if (os__Command_start_defer_0) {
			string_free(&/*defer*/(*_t1070));
		}
		// Defer end
		return _t1071;
	}
			...
			for (int i = 0; i < len; ++i) {
				if (bufbp[i] == '\n') {
					...
					if (os__Command_read_line_defer_0) {
						strings__Builder_free(&/*defer*/(*_t1072));
					}
					// Defer end
					return final;
				}
...
Option_bool os__is_writable_folder(string folder) {
	...
	Option_void _t1083 = os__rm(tmp_perm_check);
	if (_t1083.state != 0 && _t1083.err._typ != _IError_None___index) {
		Option_bool _t1084;
		memcpy(&_t1084, &_t1083, sizeof(Option));
		return _t1084;
	};
	Option_bool _t1085;
	opt_ok(&(bool[]) { true }, (Option*)(&_t1085), sizeof(bool));
	return _t1085;
}
...
void os__Process_set_environment(os__Process* p, Map_string_string envs) {
	...
	// FOR IN map
	int _t1091 = envs.key_values.len;
	for (int _t1090 = 0; _t1090 < _t1091; ++_t1090 ) {
		int _t1092 = envs.key_values.len - _t1091;
		_t1091 = envs.key_values.len;
		if (_t1092 < 0) {
			_t1090 = -1;
			continue;
		}
		if (!DenseArray_has_index(&envs.key_values, _t1090)) {continue;}
		string k = /*key*/ *(string*)DenseArray_key(&envs.key_values, _t1090);
		k = string_clone(k);
		string v = (*(string*)DenseArray_value(&envs.key_values, _t1090));
...
VV_LOCAL_SYMBOL int os__Process__spawn(os__Process* p) {
	if (!p->env_is_custom) {
		...
		// FOR IN map
		int _t1095 = current_environment.key_values.len;
		for (int _t1094 = 0; _t1094 < _t1095; ++_t1094 ) {
			int _t1096 = current_environment.key_values.len - _t1095;
			_t1095 = current_environment.key_values.len;
			if (_t1096 < 0) {
				_t1094 = -1;
				continue;
			}
			if (!DenseArray_has_index(&current_environment.key_values, _t1094)) {continue;}
			string k = /*key*/ *(string*)DenseArray_key(&current_environment.key_values, _t1094);
			k = string_clone(k);
			string v = (*(string*)DenseArray_value(&current_environment.key_values, _t1094));
			array_push((array*)&p->env, _MOV((string[]){  str_intp(3, _MOV((StrIntpData[]){{_SLIT0, 0xfe10, {.d_s = k}}, {_SLIT("="), 0xfe10, {.d_s = v}}, {_SLIT0, 0, { .d_c = 0 }}}))  }));
		}
	}
	int pid = 0;
...
VV_LOCAL_SYMBOL int os__Process_unix_spawn_process(os__Process* p) {
	...
	Option_void _t1107 = os__execve(p->filename, p->args, p->env);
	if (_t1107.state != 0 && _t1107.err._typ != _IError_None___index) {
		IError err = _t1107.err;
...
// Attr: [deprecated_after]
voidptr os__signal(int signum, voidptr handler) {
	Option_os__SignalHandler _t1117 = os__signal_opt(((os__Signal)(signum)), (voidptr)handler);
	if (_t1117.state != 0) { /*or block*/ 
		IError err = _t1117.err;
		*(os__SignalHandler*) _t1117.data = (voidptr)SIG_ERR;
	}
 	return ((voidptr)( (*(os__SignalHandler*)_t1117.data)));
}
...
Option_os__SignalHandler os__signal_opt(os__Signal signum, void (*handler)(os__Signal )) {
	...
	Option_os__SignalHandler _t1119;
	opt_ok(&(os__SignalHandler[]) { (voidptr)((os__SignalHandler)(prev_handler)) }, (Option*)(&_t1119), sizeof(os__SignalHandler));
	return _t1119;
}
...
#endif
strings__textscanner__TextScanner strings__textscanner__new(string input) {
	strings__textscanner__TextScanner _t1120 = (strings__textscanner__TextScanner){.input = input,.ilen = input.len,.pos = 0,};
	return _t1120;
}
...
inline int strings__textscanner__TextScanner_peek(strings__textscanner__TextScanner* ss) {
	if (ss->pos < ss->ilen) {
		int _t1124 = ss->input.str[ ss->pos];
		return _t1124;
	}
	int _t1125 = -1;
	return _t1125;
...
inline int strings__textscanner__TextScanner_peek_n(strings__textscanner__TextScanner* ss, int n) {
	if (ss->pos + n < ss->ilen) {
		int _t1126 = ss->input.str[ ss->pos + n];
		return _t1126;
	}
	int _t1127 = -1;
	return _t1127;
...
string term__format(string msg, string open, string close) {
	string _t1128 = string__plus(string__plus(string__plus(string__plus(string__plus(string__plus(_SLIT("\x1b["), open), _SLIT("m")), msg), _SLIT("\x1b[")), close), _SLIT("m"));
	return _t1128;
}

string term__format_rgb(int r, int g, int b, string msg, string open, string close) {
	string _t1129 = string__plus(string__plus(string__plus(string__plus(string__plus(string__plus(string__plus(string__plus(string__plus(string__plus(string__plus(string__plus(_SLIT("\x1b["), open), _SLIT(";2;")), int_str(r)), _SLIT(";")), int_str(g)), _SLIT(";")), int_str(b)), _SLIT("m")), msg), _SLIT("\x1b[")), close), _SLIT("m"));
	return _t1129;
}

string term__rgb(int r, int g, int b, string msg) {
	string _t1130 = term__format_rgb(r, g, b, msg, _SLIT("38"), _SLIT("39"));
	return _t1130;
}

string term__bg_rgb(int r, int g, int b, string msg) {
	string _t1131 = term__format_rgb(r, g, b, msg, _SLIT("48"), _SLIT("49"));
	return _t1131;
}

string term__hex(int hex, string msg) {
	string _t1132 = term__format_rgb(hex >> 16, (hex >> 8 & 0xFF), (hex & 0xFF), msg, _SLIT("38"), _SLIT("39"));
	return _t1132;
}

string term__bg_hex(int hex, string msg) {
	string _t1133 = term__format_rgb(hex >> 16, (hex >> 8 & 0xFF), (hex & 0xFF), msg, _SLIT("48"), _SLIT("49"));
	return _t1133;
}

string term__bg_black(string msg) {
	string _t1134 = term__format(msg, _SLIT("40"), _SLIT("49"));
	return _t1134;
}

string term__bright_bg_black(string msg) {
	string _t1135 = term__format(msg, _SLIT("100"), _SLIT("49"));
	return _t1135;
}

string term__bg_blue(string msg) {
	string _t1136 = term__format(msg, _SLIT("44"), _SLIT("49"));
	return _t1136;
}

string term__bright_bg_blue(string msg) {
	string _t1137 = term__format(msg, _SLIT("104"), _SLIT("49"));
	return _t1137;
}

string term__bg_cyan(string msg) {
	string _t1138 = term__format(msg, _SLIT("46"), _SLIT("49"));
	return _t1138;
}

string term__bright_bg_cyan(string msg) {
	string _t1139 = term__format(msg, _SLIT("106"), _SLIT("49"));
	return _t1139;
}

string term__bg_green(string msg) {
	string _t1140 = term__format(msg, _SLIT("42"), _SLIT("49"));
	return _t1140;
}

string term__bright_bg_green(string msg) {
	string _t1141 = term__format(msg, _SLIT("102"), _SLIT("49"));
	return _t1141;
}

string term__bg_magenta(string msg) {
	string _t1142 = term__format(msg, _SLIT("45"), _SLIT("49"));
	return _t1142;
}

string term__bright_bg_magenta(string msg) {
	string _t1143 = term__format(msg, _SLIT("105"), _SLIT("49"));
	return _t1143;
}

string term__bg_red(string msg) {
	string _t1144 = term__format(msg, _SLIT("41"), _SLIT("49"));
	return _t1144;
}

string term__bright_bg_red(string msg) {
	string _t1145 = term__format(msg, _SLIT("101"), _SLIT("49"));
	return _t1145;
}

string term__bg_white(string msg) {
	string _t1146 = term__format(msg, _SLIT("47"), _SLIT("49"));
	return _t1146;
}

string term__bright_bg_white(string msg) {
	string _t1147 = term__format(msg, _SLIT("107"), _SLIT("49"));
	return _t1147;
}

string term__bg_yellow(string msg) {
	string _t1148 = term__format(msg, _SLIT("43"), _SLIT("49"));
	return _t1148;
}

string term__bright_bg_yellow(string msg) {
	string _t1149 = term__format(msg, _SLIT("103"), _SLIT("49"));
	return _t1149;
}

string term__black(string msg) {
	string _t1150 = term__format(msg, _SLIT("30"), _SLIT("39"));
	return _t1150;
}

string term__bright_black(string msg) {
	string _t1151 = term__format(msg, _SLIT("90"), _SLIT("39"));
	return _t1151;
}

string term__blue(string msg) {
	string _t1152 = term__format(msg, _SLIT("34"), _SLIT("39"));
	return _t1152;
}

string term__bright_blue(string msg) {
	string _t1153 = term__format(msg, _SLIT("94"), _SLIT("39"));
	return _t1153;
}

string term__bold(string msg) {
	string _t1154 = term__format(msg, _SLIT("1"), _SLIT("22"));
	return _t1154;
}

string term__cyan(string msg) {
	string _t1155 = term__format(msg, _SLIT("36"), _SLIT("39"));
	return _t1155;
}

string term__bright_cyan(string msg) {
	string _t1156 = term__format(msg, _SLIT("96"), _SLIT("39"));
	return _t1156;
}

string term__dim(string msg) {
	string _t1157 = term__format(msg, _SLIT("2"), _SLIT("22"));
	return _t1157;
}

string term__green(string msg) {
	string _t1158 = term__format(msg, _SLIT("32"), _SLIT("39"));
	return _t1158;
}

string term__bright_green(string msg) {
	string _t1159 = term__format(msg, _SLIT("92"), _SLIT("39"));
	return _t1159;
}

string term__gray(string msg) {
	string _t1160 = term__bright_black(msg);
	return _t1160;
}

string term__hidden(string msg) {
	string _t1161 = term__format(msg, _SLIT("8"), _SLIT("28"));
	return _t1161;
}

string term__italic(string msg) {
	string _t1162 = term__format(msg, _SLIT("3"), _SLIT("23"));
	return _t1162;
}

string term__inverse(string msg) {
	string _t1163 = term__format(msg, _SLIT("7"), _SLIT("27"));
	return _t1163;
}

string term__magenta(string msg) {
	string _t1164 = term__format(msg, _SLIT("35"), _SLIT("39"));
	return _t1164;
}

string term__bright_magenta(string msg) {
	string _t1165 = term__format(msg, _SLIT("95"), _SLIT("39"));
	return _t1165;
}

string term__reset(string msg) {
	string _t1166 = term__format(msg, _SLIT("0"), _SLIT("0"));
	return _t1166;
}

string term__red(string msg) {
	string _t1167 = term__format(msg, _SLIT("31"), _SLIT("39"));
	return _t1167;
}

string term__bright_red(string msg) {
	string _t1168 = term__format(msg, _SLIT("91"), _SLIT("39"));
	return _t1168;
}

string term__strikethrough(string msg) {
	string _t1169 = term__format(msg, _SLIT("9"), _SLIT("29"));
	return _t1169;
}

string term__underline(string msg) {
	string _t1170 = term__format(msg, _SLIT("4"), _SLIT("24"));
	return _t1170;
}

string term__white(string msg) {
	string _t1171 = term__format(msg, _SLIT("37"), _SLIT("39"));
	return _t1171;
}

string term__bright_white(string msg) {
	string _t1172 = term__format(msg, _SLIT("97"), _SLIT("39"));
	return _t1172;
}

string term__yellow(string msg) {
	string _t1173 = term__format(msg, _SLIT("33"), _SLIT("39"));
	return _t1173;
}

string term__bright_yellow(string msg) {
	string _t1174 = term__format(msg, _SLIT("93"), _SLIT("39"));
	return _t1174;
}
...
bool term__can_show_color_on_stdout(void) {
	bool _t1175 = term__supports_escape_sequences(1);
	return _t1175;
}

bool term__can_show_color_on_stderr(void) {
	bool _t1176 = term__supports_escape_sequences(2);
	return _t1176;
}

string term__ok_message(string s) {
	if (term__can_show_color_on_stdout()) {
		string _t1177 = term__green( str_intp(2, _MOV((StrIntpData[]){{_SLIT(" "), 0xfe10, {.d_s = s}}, {_SLIT(" "), 0, { .d_c = 0 }}})) );
		return _t1177;
	}
	return s;
}
...
string term__fail_message(string s) {
	if (term__can_show_color_on_stdout()) {
		string _t1179 = term__inverse(term__bg_white(term__bold(term__red( str_intp(2, _MOV((StrIntpData[]){{_SLIT(" "), 0xfe10, {.d_s = s}}, {_SLIT(" "), 0, { .d_c = 0 }}})) ))));
		return _t1179;
	}
	return s;
}
...
string term__warn_message(string s) {
	if (term__can_show_color_on_stdout()) {
		string _t1181 = term__bright_yellow( str_intp(2, _MOV((StrIntpData[]){{_SLIT(" "), 0xfe10, {.d_s = s}}, {_SLIT(" "), 0, { .d_c = 0 }}})) );
		return _t1181;
	}
	return s;
}
...
string term__strip_ansi(string text) {
	...
	string _t1186 = Array_byte_bytestr(output);
	return _t1186;
}
...
string term__h_divider(string divider) {
	...
	string _t1187 = string_substr(result, 0, cols);
	return _t1187;
}
...
string term__header_left(string text, string divider) {
	...
	string _t1188 =  str_intp(4, _MOV((StrIntpData[]){{_SLIT0, 0xfe10, {.d_s = hstart}}, {_SLIT(" "), 0xfe10, {.d_s = text}}, {_SLIT(" "), 0xfe10, {.d_s = hend}}, {_SLIT0, 0, { .d_c = 0 }}})) ;
	return _t1188;
}

string term__header(string text, string divider) {
	if (text.len == 0) {
		string _t1189 = term__h_divider(divider);
		return _t1189;
	}
	...
	if (ln.len == 1) {
		string _t1190 = string__plus(string__plus(string__plus(string__plus(ln, _SLIT(" ")), string_substr(text, 0, tlimit)), _SLIT(" ")), ln);
		return _t1190;
	}
	string _t1191 = string__plus(string__plus(string__plus(string__plus(string_substr(ln, 0, tstart), _SLIT(" ")), string_substr(text, 0, tlimit)), _SLIT(" ")), string_substr(ln, tstart + tlimit + 2, cols));
	return _t1191;
}
...
term__Coord term__get_cursor_position(void) {
	if (os__is_atty(1) <= 0 || string__eq(os__getenv(_SLIT("TERM")), _SLIT("dumb"))) {
		term__Coord _t1199 = (term__Coord){.x = 0,.y = 0,};
		return _t1199;
	}
	...
	term__Coord _t1200 = (term__Coord){.x = x,.y = y,};
	return _t1200;
}
...
VV_LOCAL_SYMBOL bool main__can_use_relative_paths(void) {
	string _t1204 = os__getenv(_SLIT("VERROR_PATHS"));
		bool _t1203 = ((string__eq(_t1204, _SLIT("absolute"))) ? (false) : (true));
	return _t1203;
}
...
VV_LOCAL_SYMBOL void main__cb_propagate_test_error(int line_nr, string file, string mod, string fn_name, string errmsg) {
	...
	if (os__is_file(file)) {
		Option_Array_string _t1205 = os__read_lines(file);
		if (_t1205.state != 0) { /*or block*/ 
			IError err = _t1205.err;
			*(Array_string*) _t1205.data = __new_array_with_default(line_nr + 1, 0, sizeof(string), &(string[]){_SLIT("")});
		}
 		Array_string source_lines =  (*(Array_string*)_t1205.data);
...
VV_LOCAL_SYMBOL void main__test_defer(void) {
	// assert
	string _t1208 = main__foo2();
	if (string__eq(_t1208, _SLIT("foo"))) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t1209 = {0};
		v_assert_meta_info__t1209.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1209.line_nr = 18;
		v_assert_meta_info__t1209.fn_name = _SLIT("main.test_defer");
		v_assert_meta_info__t1209.src = _SLIT("foo2() == 'foo'");
		v_assert_meta_info__t1209.op = _SLIT("==");
		v_assert_meta_info__t1209.llabel = _SLIT("foo2()");
		v_assert_meta_info__t1209.rlabel = _SLIT("'foo'");
		v_assert_meta_info__t1209.lvalue = _t1208 /* typeof: v.ast.CTempVar type: 18 */ ;
		v_assert_meta_info__t1209.rvalue = _SLIT("foo") /* typeof: v.ast.StringLiteral type: 18 */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t1209);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t1210 = {0};
		v_assert_meta_info__t1210.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1210.line_nr = 18;
		v_assert_meta_info__t1210.fn_name = _SLIT("main.test_defer");
		v_assert_meta_info__t1210.src = _SLIT("foo2() == 'foo'");
		v_assert_meta_info__t1210.op = _SLIT("==");
		v_assert_meta_info__t1210.llabel = _SLIT("foo2()");
		v_assert_meta_info__t1210.rlabel = _SLIT("'foo'");
		v_assert_meta_info__t1210.lvalue = _t1208 /* typeof: v.ast.CTempVar type: 18 */ ;
		v_assert_meta_info__t1210.rvalue = _SLIT("foo") /* typeof: v.ast.StringLiteral type: 18 */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t1210);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
...
main__Num** _t1212; // n
	...
	_t1212 = &(n);
	Option_int _t1213;
	opt_ok(&(int[]) { 99 }, (Option*)(&_t1213), sizeof(int));
	// Defer begin
...
VV_LOCAL_SYMBOL void main__test_defer_early_exit(void) {
	...
	println( str_intp(2, _MOV((StrIntpData[]){{_SLIT("sum: "), 0xfe07, {.d_i32 = sum.val}}, {_SLIT0, 0, { .d_c = 0 }}})) );
	// assert
	if (sum.val == 15) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t1215 = {0};
		v_assert_meta_info__t1215.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1215.line_nr = 56;
		v_assert_meta_info__t1215.fn_name = _SLIT("main.test_defer_early_exit");
		v_assert_meta_info__t1215.src = _SLIT("sum.val == 15");
		v_assert_meta_info__t1215.op = _SLIT("==");
		v_assert_meta_info__t1215.llabel = _SLIT("sum.val");
		v_assert_meta_info__t1215.rlabel = _SLIT("15");
		v_assert_meta_info__t1215.lvalue = int_str(sum.val) /* typeof: v.ast.SelectorExpr type: 7 */ ;
		v_assert_meta_info__t1215.rvalue = int_str(15) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t1215);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t1216 = {0};
		v_assert_meta_info__t1216.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1216.line_nr = 56;
		v_assert_meta_info__t1216.fn_name = _SLIT("main.test_defer_early_exit");
		v_assert_meta_info__t1216.src = _SLIT("sum.val == 15");
		v_assert_meta_info__t1216.op = _SLIT("==");
		v_assert_meta_info__t1216.llabel = _SLIT("sum.val");
		v_assert_meta_info__t1216.rlabel = _SLIT("15");
		v_assert_meta_info__t1216.lvalue = int_str(sum.val) /* typeof: v.ast.SelectorExpr type: 7 */ ;
		v_assert_meta_info__t1216.rvalue = int_str(15) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t1216);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
...
VV_LOCAL_SYMBOL void main__test_defer_option(void) {
	...
	Option_int _t1217 = main__set_num_opt((voidptr)&/*qq*/ok);
	if (_t1217.state != 0) { /*or block*/ 
		IError err = _t1217.err;
	};
	// assert
	if (ok.val == 1) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t1218 = {0};
		v_assert_meta_info__t1218.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1218.line_nr = 62;
		v_assert_meta_info__t1218.fn_name = _SLIT("main.test_defer_option");
		v_assert_meta_info__t1218.src = _SLIT("ok.val == 1");
		v_assert_meta_info__t1218.op = _SLIT("==");
		v_assert_meta_info__t1218.llabel = _SLIT("ok.val");
		v_assert_meta_info__t1218.rlabel = _SLIT("1");
		v_assert_meta_info__t1218.lvalue = int_str(ok.val) /* typeof: v.ast.SelectorExpr type: 7 */ ;
		v_assert_meta_info__t1218.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t1218);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t1219 = {0};
		v_assert_meta_info__t1219.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1219.line_nr = 62;
		v_assert_meta_info__t1219.fn_name = _SLIT("main.test_defer_option");
		v_assert_meta_info__t1219.src = _SLIT("ok.val == 1");
		v_assert_meta_info__t1219.op = _SLIT("==");
		v_assert_meta_info__t1219.llabel = _SLIT("ok.val");
		v_assert_meta_info__t1219.rlabel = _SLIT("1");
		v_assert_meta_info__t1219.lvalue = int_str(ok.val) /* typeof: v.ast.SelectorExpr type: 7 */ ;
		v_assert_meta_info__t1219.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t1219);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
...
main__Num** _t1220; // f
	...
	_t1220 = &(f);
	// go
	thread_arg_anon_fn_9cdeab255c06f7ad__863 *arg__t1221 = malloc(sizeof(thread_arg_anon_fn_9cdeab255c06f7ad__863));
	pthread_t thread__t1221;
	pthread_create(&thread__t1221, NULL, (void*)anon_fn_9cdeab255c06f7ad__863_thread_wrapper, arg__t1221);
	pthread_detach(thread__t1221);
	// endgo
	...
	if (main__test_defer_with_anon_fn_defer_0) {
		// assert
		int _t1222 = main__Num_add(/*rec*/*/*defer*/(*_t1220), 1);
		if (_t1222 == 111) {
			g_test_oks++;
			VAssertMetaInfo v_assert_meta_info__t1223 = {0};
			v_assert_meta_info__t1223.fpath = _SLIT("vlib/v/tests/defer_test.v");
			v_assert_meta_info__t1223.line_nr = 70;
			v_assert_meta_info__t1223.fn_name = _SLIT("main.test_defer_with_anon_fn");
			v_assert_meta_info__t1223.src = _SLIT("_t1220.add(1) == 111");
			v_assert_meta_info__t1223.op = _SLIT("==");
			v_assert_meta_info__t1223.llabel = _SLIT("_t1220.add(1)");
			v_assert_meta_info__t1223.rlabel = _SLIT("111");
			v_assert_meta_info__t1223.lvalue = int_str(_t1222) /* typeof: v.ast.CTempVar type: 7 */ ;
			v_assert_meta_info__t1223.rvalue = int_str(111) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
			main__cb_assertion_ok(&v_assert_meta_info__t1223);
		} else {
			g_test_fails++;
			VAssertMetaInfo v_assert_meta_info__t1224 = {0};
			v_assert_meta_info__t1224.fpath = _SLIT("vlib/v/tests/defer_test.v");
			v_assert_meta_info__t1224.line_nr = 70;
			v_assert_meta_info__t1224.fn_name = _SLIT("main.test_defer_with_anon_fn");
			v_assert_meta_info__t1224.src = _SLIT("_t1220.add(1) == 111");
			v_assert_meta_info__t1224.op = _SLIT("==");
			v_assert_meta_info__t1224.llabel = _SLIT("_t1220.add(1)");
			v_assert_meta_info__t1224.rlabel = _SLIT("111");
			v_assert_meta_info__t1224.lvalue = int_str(_t1222) /* typeof: v.ast.CTempVar type: 7 */ ;
			v_assert_meta_info__t1224.rvalue = int_str(111) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
			main__cb_assertion_failed(&v_assert_meta_info__t1224);
			longjmp(g_jump_buffer, 1);
			// TODO
			// Maybe print all vars in a test function if it fails?
		}
...
VV_LOCAL_SYMBOL void main__test_defer_with_if(void) {
	...
	if (n.val == 10) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t1227 = {0};
		v_assert_meta_info__t1227.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1227.line_nr = 99;
		v_assert_meta_info__t1227.fn_name = _SLIT("main.test_defer_with_if");
		v_assert_meta_info__t1227.src = _SLIT("n.val == 10");
		v_assert_meta_info__t1227.op = _SLIT("==");
		v_assert_meta_info__t1227.llabel = _SLIT("n.val");
		v_assert_meta_info__t1227.rlabel = _SLIT("10");
		v_assert_meta_info__t1227.lvalue = int_str(n.val) /* typeof: v.ast.SelectorExpr type: 7 */ ;
		v_assert_meta_info__t1227.rvalue = int_str(10) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t1227);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t1228 = {0};
		v_assert_meta_info__t1228.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1228.line_nr = 99;
		v_assert_meta_info__t1228.fn_name = _SLIT("main.test_defer_with_if");
		v_assert_meta_info__t1228.src = _SLIT("n.val == 10");
		v_assert_meta_info__t1228.op = _SLIT("==");
		v_assert_meta_info__t1228.llabel = _SLIT("n.val");
		v_assert_meta_info__t1228.rlabel = _SLIT("10");
		v_assert_meta_info__t1228.lvalue = int_str(n.val) /* typeof: v.ast.SelectorExpr type: 7 */ ;
		v_assert_meta_info__t1228.rvalue = int_str(10) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t1228);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	...
	if (n.val == 10) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t1229 = {0};
		v_assert_meta_info__t1229.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1229.line_nr = 101;
		v_assert_meta_info__t1229.fn_name = _SLIT("main.test_defer_with_if");
		v_assert_meta_info__t1229.src = _SLIT("n.val == 10");
		v_assert_meta_info__t1229.op = _SLIT("==");
		v_assert_meta_info__t1229.llabel = _SLIT("n.val");
		v_assert_meta_info__t1229.rlabel = _SLIT("10");
		v_assert_meta_info__t1229.lvalue = int_str(n.val) /* typeof: v.ast.SelectorExpr type: 7 */ ;
		v_assert_meta_info__t1229.rvalue = int_str(10) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t1229);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t1230 = {0};
		v_assert_meta_info__t1230.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1230.line_nr = 101;
		v_assert_meta_info__t1230.fn_name = _SLIT("main.test_defer_with_if");
		v_assert_meta_info__t1230.src = _SLIT("n.val == 10");
		v_assert_meta_info__t1230.op = _SLIT("==");
		v_assert_meta_info__t1230.llabel = _SLIT("n.val");
		v_assert_meta_info__t1230.rlabel = _SLIT("10");
		v_assert_meta_info__t1230.lvalue = int_str(n.val) /* typeof: v.ast.SelectorExpr type: 7 */ ;
		v_assert_meta_info__t1230.rvalue = int_str(10) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t1230);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
...
if (main__test_defer_order_defer_2) {
	...
	if (/*defer*/(*_t1231) == 1) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t1232 = {0};
		v_assert_meta_info__t1232.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1232.line_nr = 116;
		v_assert_meta_info__t1232.fn_name = _SLIT("main.test_defer_order");
		v_assert_meta_info__t1232.src = _SLIT("_t1231 == 1");
		v_assert_meta_info__t1232.op = _SLIT("==");
		v_assert_meta_info__t1232.llabel = _SLIT("_t1231");
		v_assert_meta_info__t1232.rlabel = _SLIT("1");
		v_assert_meta_info__t1232.lvalue = str_intp(1, _MOV((StrIntpData[]){_SLIT(""), 0xfe10 ,{.d_s=int_str(/*defer*/(*_t1231))}})) /* typeof: v.ast.Ident type: 65543 */ ;
		v_assert_meta_info__t1232.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t1232);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t1233 = {0};
		v_assert_meta_info__t1233.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1233.line_nr = 116;
		v_assert_meta_info__t1233.fn_name = _SLIT("main.test_defer_order");
		v_assert_meta_info__t1233.src = _SLIT("_t1231 == 1");
		v_assert_meta_info__t1233.op = _SLIT("==");
		v_assert_meta_info__t1233.llabel = _SLIT("_t1231");
		v_assert_meta_info__t1233.rlabel = _SLIT("1");
		v_assert_meta_info__t1233.lvalue = str_intp(1, _MOV((StrIntpData[]){_SLIT(""), 0xfe10 ,{.d_s=int_str(/*defer*/(*_t1231))}})) /* typeof: v.ast.Ident type: 65543 */ ;
		v_assert_meta_info__t1233.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t1233);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
...
if (main__test_defer_order_defer_1) {
	...
	if (/*defer*/(*_t1231) == 2) {
		...
		v_assert_meta_info__t1234.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1234.line_nr = 112;
		v_assert_meta_info__t1234.fn_name = _SLIT("main.test_defer_order");
		v_assert_meta_info__t1234.src = _SLIT("_t1231 == 2");
		v_assert_meta_info__t1234.op = _SLIT("==");
		v_assert_meta_info__t1234.llabel = _SLIT("_t1231");
		v_assert_meta_info__t1234.rlabel = _SLIT("2");
		v_assert_meta_info__t1234.lvalue = str_intp(1, _MOV((StrIntpData[]){_SLIT(""), 0xfe10 ,{.d_s=int_str(/*defer*/(*_t1231))}})) /* typeof: v.ast.Ident type: 65543 */ ;
		v_assert_meta_info__t1234.rvalue = int_str(2) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t1234);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t1235 = {0};
		v_assert_meta_info__t1235.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1235.line_nr = 112;
		v_assert_meta_info__t1235.fn_name = _SLIT("main.test_defer_order");
		v_assert_meta_info__t1235.src = _SLIT("_t1231 == 2");
		v_assert_meta_info__t1235.op = _SLIT("==");
		v_assert_meta_info__t1235.llabel = _SLIT("_t1231");
		v_assert_meta_info__t1235.rlabel = _SLIT("2");
		v_assert_meta_info__t1235.lvalue = str_intp(1, _MOV((StrIntpData[]){_SLIT(""), 0xfe10 ,{.d_s=int_str(/*defer*/(*_t1231))}})) /* typeof: v.ast.Ident type: 65543 */ ;
		v_assert_meta_info__t1235.rvalue = int_str(2) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
...
if (main__test_defer_order_defer_0) {
	...
	if (/*defer*/(*_t1231) == 3) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t1236 = {0};
		v_assert_meta_info__t1236.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1236.line_nr = 108;
		v_assert_meta_info__t1236.fn_name = _SLIT("main.test_defer_order");
		v_assert_meta_info__t1236.src = _SLIT("_t1231 == 3");
		v_assert_meta_info__t1236.op = _SLIT("==");
		v_assert_meta_info__t1236.llabel = _SLIT("_t1231");
		v_assert_meta_info__t1236.rlabel = _SLIT("3");
		v_assert_meta_info__t1236.lvalue = str_intp(1, _MOV((StrIntpData[]){_SLIT(""), 0xfe10 ,{.d_s=int_str(/*defer*/(*_t1231))}})) /* typeof: v.ast.Ident type: 65543 */ ;
		v_assert_meta_info__t1236.rvalue = int_str(3) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t1236);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t1237 = {0};
		v_assert_meta_info__t1237.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1237.line_nr = 108;
		v_assert_meta_info__t1237.fn_name = _SLIT("main.test_defer_order");
		v_assert_meta_info__t1237.src = _SLIT("_t1231 == 3");
		v_assert_meta_info__t1237.op = _SLIT("==");
		v_assert_meta_info__t1237.llabel = _SLIT("_t1231");
		v_assert_meta_info__t1237.rlabel = _SLIT("3");
		v_assert_meta_info__t1237.lvalue = str_intp(1, _MOV((StrIntpData[]){_SLIT(""), 0xfe10 ,{.d_s=int_str(/*defer*/(*_t1231))}})) /* typeof: v.ast.Ident type: 65543 */ ;
		v_assert_meta_info__t1237.rvalue = int_str(3) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t1237);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
...
if (main__test_defer_access_defer_0) {
	...
	if (/*defer*/(*_t1238) == 1) {
		g_test_oks++;
		VAssertMetaInfo v_assert_meta_info__t1239 = {0};
		v_assert_meta_info__t1239.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1239.line_nr = 125;
		v_assert_meta_info__t1239.fn_name = _SLIT("main.test_defer_access");
		v_assert_meta_info__t1239.src = _SLIT("_t1238 == 1");
		v_assert_meta_info__t1239.op = _SLIT("==");
		v_assert_meta_info__t1239.llabel = _SLIT("_t1238");
		v_assert_meta_info__t1239.rlabel = _SLIT("1");
		v_assert_meta_info__t1239.lvalue = str_intp(1, _MOV((StrIntpData[]){_SLIT(""), 0xfe10 ,{.d_s=int_str(/*defer*/(*_t1238))}})) /* typeof: v.ast.Ident type: 65543 */ ;
		v_assert_meta_info__t1239.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t1239);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t1240 = {0};
		v_assert_meta_info__t1240.fpath = _SLIT("vlib/v/tests/defer_test.v");
		v_assert_meta_info__t1240.line_nr = 125;
		v_assert_meta_info__t1240.fn_name = _SLIT("main.test_defer_access");
		v_assert_meta_info__t1240.src = _SLIT("_t1238 == 1");
		v_assert_meta_info__t1240.op = _SLIT("==");
		v_assert_meta_info__t1240.llabel = _SLIT("_t1238");
		v_assert_meta_info__t1240.rlabel = _SLIT("1");
		v_assert_meta_info__t1240.lvalue = str_intp(1, _MOV((StrIntpData[]){_SLIT(""), 0xfe10 ,{.d_s=int_str(/*defer*/(*_t1238))}})) /* typeof: v.ast.Ident type: 65543 */ ;
		v_assert_meta_info__t1240.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t1240);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
...
VV_LOCAL_SYMBOL void main__test_defer_arrays(void) {
bool main__test_defer_arrays_defer_0 = false;
Array_int* _t1241; // ia
	Array_int ia = __new_array_with_default(0, 0, sizeof(int), 0);
	main__test_defer_arrays_defer_0 = true;
	_t1241 = &(ia);
// Defer begin
if (main__test_defer_arrays_defer_0) {
	array_push((array*)&/*defer*/(*_t1241), _MOV((int[]){ 1 }));
}
// Defer end
}

VV_LOCAL_SYMBOL void main__test_defer_str_interpol(void) {
bool main__test_defer_str_interpol_defer_0 = false;
Array_string* _t1243; // t
	Array_string t = __new_array_with_default(0, 0, sizeof(string), 0);
	main__test_defer_str_interpol_defer_0 = true;
	_t1243 = &(t);
	_t1243 = &(t);
// Defer begin
if (main__test_defer_str_interpol_defer_0) {
	array_push((array*)&/*defer*/(*_t1243), _MOV((string[]){ string_clone(_SLIT("test")) }));
	array_push((array*)&/*defer*/(*_t1243), _MOV((string[]){ string_clone( str_intp(2, _MOV((StrIntpData[]){{_SLIT0, 0xfe10, {.d_s = (*(string*)/*ee elem_typ */array_get(/*defer*/(*_t1243), 0))}}, {_SLIT0, 0, { .d_c = 0 }}})) ) }));
}
// Defer end
}
...
int main(int ___argc, char** ___argv){
	...
	if (!setjmp(g_jump_buffer)) main__test_defer_arrays();
	if (!setjmp(g_jump_buffer)) main__test_defer_str_interpol();
