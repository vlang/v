...
VV_LOCAL_SYMBOL void main__test_orm();
...
static string indent_orm__Primitive_str(orm__Primitive x, int indent_count) {
	switch(x._typ) {
		case 16: return str_intp(2, _MOV((StrIntpData[]){
				...
				{_SLIT("')"), 0, {.d_c = 0 }}
			}));		case 131: return str_intp(2, _MOV((StrIntpData[]){
...
static inline orm__Primitive time__Time_to_sumtype_orm__Primitive(time__Time* x) {
	...
	return (orm__Primitive){ ._time__Time = ptr, ._typ = 131};
}
...
VV_LOCAL_SYMBOL Option_void sqlite__sqlite_stmt_binder(sqlite__Stmt stmt, orm__OrmQueryData d, string query) {
	...
	for (int _t1391 = 0; _t1391 < d.data.len; ++_t1391) {
		...
		else if (data._typ == 131 /* time.Time */) {
...
VV_LOCAL_SYMBOL void main__test_orm(void) {
	Option_sqlite__DB _t1430 = sqlite__connect(_SLIT(":memory:"));
	if (_t1430.state != 0) { /*or block*/ 
		IError err = _t1430.err;
		v_panic(IError_str(err));
	}
 	sqlite__DB sdb =  (*(sqlite__DB*)_t1430.data);
	orm__OrmConnection db = I_sqlite__DB_to_Interface_orm__OrmConnection(&sdb);
	Option_void _t1431 = orm__OrmConnection_name_table[db._typ]._method_create(db._object, _SLIT("userlist"), new_array_from_c_array(4, 4, sizeof(orm__OrmTableField), _MOV((orm__OrmTableField[4]){(orm__OrmTableField){.name = _SLIT("id"),.typ = 7,.is_time = 0,.default_val = (string){.str=(byteptr)"", .is_lit=1},.attrs = new_array_from_c_array(2, 2, sizeof(StructAttribute), _MOV((StructAttribute[2]){(StructAttribute){.name = _SLIT("primary"),.has_arg = 0,.arg = (string){.str=(byteptr)"", .is_lit=1},.kind = 0,}, (StructAttribute){.name = _SLIT("sql"),.has_arg = true,.arg = _SLIT("serial"),.kind = AttributeKind_plain,}})),}, (orm__OrmTableField){.name = _SLIT("age"),.typ = 7,.is_time = 0,.default_val = (string){.str=(byteptr)"", .is_lit=1},.attrs = __new_array(0, 0, sizeof(StructAttribute)),}, (orm__OrmTableField){.name = _SLIT("name"),.typ = 18,.is_time = 0,.default_val = (string){.str=(byteptr)"", .is_lit=1},.attrs = new_array_from_c_array(1, 1, sizeof(StructAttribute), _MOV((StructAttribute[1]){(StructAttribute){.name = _SLIT("sql"),.has_arg = true,.arg = _SLIT("username"),.kind = AttributeKind_string,}})),}, (orm__OrmTableField){.name = _SLIT("is_customer"),.typ = 16,.is_time = 0,.default_val = (string){.str=(byteptr)"", .is_lit=1},.attrs = __new_array(0, 0, sizeof(StructAttribute)),}})));
	if (_t1431.state != 0 && _t1431.err._typ != _IError_None___index) {
		IError err = _t1431.err;
		v_panic(IError_str(err));
	};
}
...
int main(int ___argc, char** ___argv){
	...
	if (!setjmp(g_jump_buffer)) main__test_orm();
