	...
	#define V_COMMIT_HASH "9c710b2"
#endif

#ifndef V_CURRENT_COMMIT_HASH
	#define V_CURRENT_COMMIT_HASH "8503717"
#endif
...
typedef enum {
	orm__OrmOrderType__asc, // 
	orm__OrmOrderType__desc, // +1
} orm__OrmOrderType;
...
typedef struct orm__OrmConnection orm__OrmConnection;
struct orm__OrmConnection {
	union {
		void* _object;
...
static string MYSQL_STMT_str(MYSQL_STMT it); // auto
static string indent_MYSQL_STMT_str(MYSQL_STMT it, int indent_count); // auto
static string MYSQL_str(MYSQL it); // auto
static string indent_MYSQL_str(MYSQL it, int indent_count); // auto
...
VV_LOCAL_SYMBOL Option_Array_orm__Primitive mysql__buffer_to_primitive(Array_mysql__Prims data_list, Array_int types);
...
void mysql__Stmt_bind_res(mysql__Stmt* stmt, MYSQL_FIELD* fields, Array_mysql__Prims dataptr, Array_u32 _v_toheap_lens, int num_fields);
...
static string MYSQL_STMT_str(MYSQL_STMT it) { return indent_MYSQL_STMT_str(it, 0);}
static string MYSQL_str(MYSQL it) { return indent_MYSQL_str(it, 0);}
...
static inline orm__Primitive time__Time_to_sumtype_orm__Primitive(time__Time* x) {
	...
	return (orm__Primitive){ ._time__Time = ptr, ._typ = 138};
}
...
static string indent_MYSQL_str(MYSQL it, int indent_count) {
	return _SLIT("MYSQL{}");
}

static string indent_MYSQL_STMT_str(MYSQL_STMT it, int indent_count) {
	string indents = string_repeat(_SLIT("    "), indent_count);
	string res = str_intp( 11, _MOV((StrIntpData[]){
		{_SLIT("MYSQL_STMT{\n"), 0, {.d_c=0}},
		{_SLIT0, 0xfe10, {.d_s=indents}}, {_SLIT("    mysql: &"), 0, {.d_c=0}}, {_SLIT(""), 16, {.d_s=isnil(it.mysql) ? _SLIT("nil") : indent_MYSQL_str(*it.mysql, indent_count + 1)}}, {_SLIT(""), 0, {.d_c=0}},
		{_SLIT("\n"), 0xfe10, {.d_s=indents}}, {_SLIT("    stmt_id: "), 0, {.d_c=0}}, {_SLIT(""), 6, {.d_u32=it.stmt_id}}, {_SLIT(""), 0, {.d_c=0}},
...
static string indent_orm__Primitive_str(orm__Primitive x, int indent_count) {
	switch(x._typ) {
		case 16: return str_intp(2, _MOV((StrIntpData[]){
				...
				{_SLIT("')"), 0, {.d_c = 0 }}
			}));		case 138: return str_intp(2, _MOV((StrIntpData[]){
...
FILE* fp;
	...
	int cseek = fseek(fp, 0, SEEK_END);
	if (cseek != 0) {
		Option_string _t3 = (Option_string){ .state=2, .err=v_error(_SLIT("fseek failed")), .data={EMPTY_STRUCT_INITIALIZATION} };
		// Defer begin
	...
	int fsize = ftell(fp);
	if (fsize < 0) {
		Option_string _t4 = (Option_string){ .state=2, .err=v_error(_SLIT("ftell failed")), .data={EMPTY_STRUCT_INITIALIZATION} };
		// Defer begin
		if (os__read_file_defer_0) {
			fclose(fp);
		}
	...
	{ // Unsafe block
		...
		if (is_eof == 0 && is_error != 0) {
			v_free(str);
			Option_string _t5 = (Option_string){ .state=2, .err=v_error(_SLIT("fread failed")), .data={EMPTY_STRUCT_INITIALIZATION} };
			// Defer begin
			if (os__read_file_defer_0) {
				fclose(fp);
			}
...
VV_LOCAL_SYMBOL int os__vpclose(voidptr f) {
	...
	#else
	{
		multi_return_int_bool mr_8815 = os__posix_wait4_to_exit_status(pclose(f));
		int ret = mr_8815.arg0;
		return ret;
	}
	#endif
	return 0;
...
int os__system(string cmd) {
	...
	#if !defined(_WIN32)
	{
		multi_return_int_bool mr_9824 = os__posix_wait4_to_exit_status(ret);
		int pret = mr_9824.arg0;
		bool is_signaled = mr_9824.arg1;
		if (is_signaled) {
...
string pcmd;
	...
	c->f = os__vpopen(pcmd);
	if (isnil(c->f)) {
		Option_void _t1 = (Option_void){ .state=2, .err=v_error( str_intp(2, _MOV((StrIntpData[]){{_SLIT("exec(\""), 0xfe10, {.d_s = c->path}}, {_SLIT("\") failed"), 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
		// Defer begin
...
inline bool time__Time__eq(time__Time t1, time__Time t2) {
	bool _t1 = t1.v_unix == t2.v_unix && t1.microsecond == t2.microsecond;
	return _t1;
}

// Attr: [inline]
inline bool time__Time__lt(time__Time t1, time__Time t2) {
	bool _t1 = t1.v_unix < t2.v_unix || (t1.v_unix == t2.v_unix && t1.microsecond < t2.microsecond);
	return _t1;
}
...
Option_time__Time time__parse(string s) {
	...
	if (_t1.state != 0) { /*or block*/ 
		IError err = _t1.err;
		return (Option_time__Time){ .state=2, .err=v_error( str_intp(2, _MOV((StrIntpData[]){{_SLIT("Invalid time format: "), 0xfe10, {.d_s = s}}, {_SLIT0, 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
...
VV_LOCAL_SYMBOL Option_multi_return_int_int_int time__parse_iso8601_date(string s) {
	...
	if (count != 3) {
		return (Option_multi_return_int_int_int){ .state=2, .err=v_error(_const_time__err_invalid_8601), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
...
VV_LOCAL_SYMBOL Option_multi_return_int_int_int_int_i64_bool time__parse_iso8601_time(string s) {
	...
	if (count < 4) {
		return (Option_multi_return_int_int_int_int_i64_bool){ .state=2, .err=v_error(_const_time__err_invalid_8601), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	...
	if (plus_min_z != '+' && plus_min_z != '-' && !is_utc && !is_local_time) {
		return (Option_multi_return_int_int_int_int_i64_bool){ .state=2, .err=v_error(_SLIT("Invalid 8601 format, expected `Z` or `+` or `-` as time separator")), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	int unix_offset = 0;
	if (offset_hour > 0) {
...
Option_time__Time time__parse_iso8601(string s) {
	...
	if (!(parts.len == 1 || parts.len == 2)) {
		return (Option_time__Time){ .state=2, .err=v_error(_const_time__err_invalid_8601), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
...
time__Time time__new_time(time__Time t) {
	...
	time__Time _t2 = (time__Time){t.year,t.month,t.day,t.hour,t.minute,t.second,t.microsecond,.v_unix = utime,};
	return _t2;
}

// Attr: [inline]
inline int time__Time_unix_time(time__Time t) {
	int _t1 = ((int)(t.v_unix));
	return _t1;
}

// Attr: [inline]
inline u64 time__Time_unix_time_milli(time__Time t) {
	u64 _t1 = t.v_unix * 1000U + ((u64)(t.microsecond / 1000));
	return _t1;
}
...
Option_int time__days_in_month(int month, int year) {
	if (month > 12 || month < 1) {
		return (Option_int){ .state=2, .err=v_error( str_intp(2, _MOV((StrIntpData[]){{_SLIT("Invalid month: "), 0xfe07, {.d_i32 = month}}, {_SLIT0, 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
...
int time__offset(void) {
	...
	int _t1 = ((int)(local.v_unix - t.v_unix));
	return _t1;
}
...
time__Time time__darwin_now(void) {
	time__Time _t1 = (time__Time){.year = 0,.month = 0,.day = 0,.hour = 0,.minute = 0,.second = 0,.microsecond = 0,.v_unix = 0,};
	return _t1;
}
...
VV_LOCAL_SYMBOL string orm__OrmOrderType_to_str(orm__OrmOrderType kind) {
	string _t1 = ((kind == (orm__OrmOrderType__desc)) ? (_SLIT("DESC")) : (_SLIT("ASC")));
	return _t1;
}
...
Option_string orm__orm_table_gen(string table, string para, bool defaults, int def_unique_len, Array_orm__OrmTableField fields, Option_string (*sql_from_v)(int ), bool alternative) {
	...
	for (int _t1 = 0; _t1 < fields.len; ++_t1) {
		orm__OrmTableField field = ((orm__OrmTableField*)fields.data)[_t1];
		if (field.is_arr) {
			continue;
		}
		bool no_null = false;
		...
		if ((ctyp).len == 0) {
			return (Option_string){ .state=2, .err=v_error( str_intp(4, _MOV((StrIntpData[]){{_SLIT("Unknown type ("), 0xfe07, {.d_i32 = field.typ}}, {_SLIT(") for field "), 0xfe10, {.d_s = field.name}}, {_SLIT(" in struct "), 0xfe10, {.d_s = table}}, {_SLIT0, 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
		}
	...
	if ((primary).len == 0) {
		return (Option_string){ .state=2, .err=v_error( str_intp(2, _MOV((StrIntpData[]){{_SLIT("A primary key is required for "), 0xfe10, {.d_s = table}}, {_SLIT0, 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	if (unique.len > 0) {
...
Option_Array_Array_orm__Primitive mysql__Connection_select(mysql__Connection db, orm__OrmSelectConfig config, orm__OrmQueryData data, orm__OrmQueryData where) {
	...
	mysql__Stmt_bind_res(&stmt, fields, dataptr, lens, num_fields);
	Option_void _t20 = mysql__Stmt_bind_result_buffer(&stmt);
	if (_t20.state != 0 && _t20.err._typ != _IError_None___index) {
		Option_Array_Array_orm__Primitive _t21;
		memcpy(&_t21, &_t20, sizeof(Option));
		return _t21;
	}
	;
	Option_void _t22 = mysql__Stmt_store_result(&stmt);
	...
	;
	for (;;) {
		Option_int _t24 = mysql__Stmt_fetch_stmt(stmt);
		if (_t24.state != 0) { /*or block*/ 
 		...
 		status =  (*(int*)_t24.data);
		if (status == 100) {
			eprintln(_SLIT("no data anymore"));
			break;
		}
		Option_Array_orm__Primitive _t26 = mysql__buffer_to_primitive(dataptr, config.types);
		if (_t26.state != 0) { /*or block*/ 
 		...
 		Array_orm__Primitive data_list =  (*(Array_orm__Primitive*)_t26.data);
		array_push((array*)&ret, _MOV((Array_orm__Primitive[]){ data_list }));
	}
	Option_void _t29 = mysql__Stmt_close(stmt);
	if (_t29.state != 0 && _t29.err._typ != _IError_None___index) {
		Option_Array_Array_orm__Primitive _t30;
		memcpy(&_t30, &_t29, sizeof(Option));
		return _t30;
	}
	;
	Option_Array_Array_orm__Primitive _t31;
	opt_ok(&(Array_Array_orm__Primitive[]) { ret }, (Option*)(&_t31), sizeof(Array_Array_orm__Primitive));
	return _t31;
}

Option_void mysql__Connection_insert(mysql__Connection db, string table, orm__OrmQueryData data) {
	string query = orm__orm_stmt_gen(table, _SLIT("`"), orm__StmtKind__insert, false, _SLIT("?"), 1, data, (orm__OrmQueryData){.fields = __new_array(0, 0, sizeof(string)),.data = __new_array(0, 0, sizeof(orm__Primitive)),.types = __new_array(0, 0, sizeof(int)),.kinds = __new_array(0, 0, sizeof(orm__OperationKind)),.is_and = __new_array(0, 0, sizeof(bool)),});
	Option_void _t1 = mysql__mysql_stmt_worker(db, query, data, (orm__OrmQueryData){.fields = __new_array(0, 0, sizeof(string)),.data = __new_array(0, 0, sizeof(orm__Primitive)),.types = __new_array(0, 0, sizeof(int)),.kinds = __new_array(0, 0, sizeof(orm__OperationKind)),.is_and = __new_array(0, 0, sizeof(bool)),});
	if (_t1.state != 0 && _t1.err._typ != _IError_None___index) {
		Option_void _t2;
		memcpy(&_t2, &_t1, sizeof(Option));
		return _t2;
...
Option_void mysql__Connection_delete(mysql__Connection db, string table, orm__OrmQueryData where) {
	string query = orm__orm_stmt_gen(table, _SLIT("`"), orm__StmtKind__delete, false, _SLIT("?"), 1, (orm__OrmQueryData){.fields = __new_array(0, 0, sizeof(string)),.data = __new_array(0, 0, sizeof(orm__Primitive)),.types = __new_array(0, 0, sizeof(int)),.kinds = __new_array(0, 0, sizeof(orm__OperationKind)),.is_and = __new_array(0, 0, sizeof(bool)),}, where);
	Option_void _t1 = mysql__mysql_stmt_worker(db, query, (orm__OrmQueryData){.fields = __new_array(0, 0, sizeof(string)),.data = __new_array(0, 0, sizeof(orm__Primitive)),.types = __new_array(0, 0, sizeof(int)),.kinds = __new_array(0, 0, sizeof(orm__OperationKind)),.is_and = __new_array(0, 0, sizeof(bool)),}, where);
	if (_t1.state != 0 && _t1.err._typ != _IError_None___index) {
		Option_void _t2;
		memcpy(&_t2, &_t1, sizeof(Option));
		return _t2;
...
orm__Primitive mysql__Connection_last_id(mysql__Connection db) {
	orm__Primitive _t1 = int_to_sumtype_orm__Primitive(ADDR(int, (0)));
	return _t1;
}
...
Option_void mysql__Connection_drop(mysql__Connection db, string table) {
	...
	Option_void _t1 = mysql__mysql_stmt_worker(db, query, (orm__OrmQueryData){.fields = __new_array(0, 0, sizeof(string)),.data = __new_array(0, 0, sizeof(orm__Primitive)),.types = __new_array(0, 0, sizeof(int)),.kinds = __new_array(0, 0, sizeof(orm__OperationKind)),.is_and = __new_array(0, 0, sizeof(bool)),}, (orm__OrmQueryData){.fields = __new_array(0, 0, sizeof(string)),.data = __new_array(0, 0, sizeof(orm__Primitive)),.types = __new_array(0, 0, sizeof(int)),.kinds = __new_array(0, 0, sizeof(orm__OperationKind)),.is_and = __new_array(0, 0, sizeof(bool)),});
	if (_t1.state != 0 && _t1.err._typ != _IError_None___index) {
		Option_void _t2;
		memcpy(&_t2, &_t1, sizeof(Option));
		return _t2;
...
VV_LOCAL_SYMBOL Option_void mysql__mysql_stmt_binder(mysql__Stmt* stmt, orm__OrmQueryData d) {
	for (int _t1 = 0; _t1 < d.data.len; ++_t1) {
		...
		else if (data._typ == 138 /* time.Time */) {
...
VV_LOCAL_SYMBOL Option_Array_orm__Primitive mysql__buffer_to_primitive(Array_mysql__Prims data_list, Array_int types) {
	...
	for (int _t1 = 0; _t1 < data_list.len; ++_t1) {
		mysql__Prims data = ((mysql__Prims*)data_list.data)[_t1];
		if (data._typ == 18 /* string */) {
			eprintln((*data._string));
		}
	...
	Option_Array_orm__Primitive _t2;
	opt_ok(&(Array_orm__Primitive[]) { res }, (Option*)(&_t2), sizeof(Array_orm__Primitive));
	return _t2;
}
...
VV_LOCAL_SYMBOL Option_string mysql__mysql_type_from_v(int typ) {
	...
	if ((str).len == 0) {
		return (Option_string){ .state=2, .err=v_error( str_intp(2, _MOV((StrIntpData[]){{_SLIT("Unknown type "), 0xfe07, {.d_i32 = typ}}, {_SLIT0, 0, { .d_c = 0 }}}))), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	Option_string _t2;
...
Option_int mysql__Stmt_fetch_stmt(mysql__Stmt stmt) {
	eprintln(str_intp(1, _MOV((StrIntpData[]){{_SLIT("&"), 0xfe10 ,{.d_s=MYSQL_STMT_str(*stmt.stmt)}}})));
...
IError mysql__Stmt_error(mysql__Stmt stmt, int code) {
	...
	IError _t1 = I_mysql__SQLError_to_Interface_IError((mysql__SQLError*)memdup(&(mysql__SQLError){.msg =  str_intp(4, _MOV((StrIntpData[]){{_SLIT0, 0xfe10, {.d_s = msg}}, {_SLIT(" ("), 0xfe07, {.d_i32 = code}}, {_SLIT(") ("), 0xfe10, {.d_s = stmt.query}}, {_SLIT(")"), 0, { .d_c = 0 }}})),.code = code,}, sizeof(mysql__SQLError)));
	return _t1;
}
...
Array_u32* lens = HEAP(Array_u32, _v_toheap_lens);
	for (int i = 0; i < num_fields; ++i) {
		...
		array_push((array*)&stmt->res, _MOV((MYSQL_BIND[]){ (MYSQL_BIND){.buffer_type = fields[i].type,.buffer = mysql__Prims_get_data_ptr((*(mysql__Prims*)/*ee elem_typ */array_get(dataptr, i))),.buffer_length = *&(*(len)),.length = &(*(u32*)/*ee elem_typ */array_get((*(lens)), i)),} }));
	}
}
...
VV_LOCAL_SYMBOL void main__test_mysql_orm(void) {
	mysql__Connection *mdb = HEAP(mysql__Connection, ((mysql__Connection){.host = _SLIT("localhost"),.port = 3306U,.username = _SLIT("root"),.password = _SLIT("L[ypGVQ95x[;U/Tk"),.dbname = _SLIT("mysql"),.flag = 0,}));
	Option_bool _t1 = mysql__Connection_connect(&(*(mdb)));
	if (_t1.state != 0) { /*or block*/ 
		IError err = _t1.err;
		v_panic(IError_str(err));
		VUNREACHABLE();
	...
	;
 	...
 	Array_Array_orm__Primitive res =  (*(Array_Array_orm__Primitive*)_t4.data);
	...
	if ((id)._typ == 7 /* int */) {
		...
		v_assert_meta_info__t5.fpath = _SLIT("vlib/mysql/mysql_orm_test.v");
		v_assert_meta_info__t5.line_nr = 62;
		v_assert_meta_info__t5.fn_name = _SLIT("main.test_mysql_orm");
		v_assert_meta_info__t5.src = _SLIT("id is TypeNode(7)");
		v_assert_meta_info__t5.op = _SLIT("is");
		v_assert_meta_info__t5.llabel = _SLIT("id");
		v_assert_meta_info__t5.rlabel = _SLIT("TypeNode(7)");
		v_assert_meta_info__t5.lvalue = orm__Primitive_str(id) /* typeof: v.ast.Ident type: 89 */ ;
		v_assert_meta_info__t5.rvalue = _SLIT("int") /* typeof: v.ast.TypeNode type: 7 */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t5);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t6 = {0};
		v_assert_meta_info__t6.fpath = _SLIT("vlib/mysql/mysql_orm_test.v");
		v_assert_meta_info__t6.line_nr = 62;
		v_assert_meta_info__t6.fn_name = _SLIT("main.test_mysql_orm");
		v_assert_meta_info__t6.src = _SLIT("id is TypeNode(7)");
		v_assert_meta_info__t6.op = _SLIT("is");
		v_assert_meta_info__t6.llabel = _SLIT("id");
		v_assert_meta_info__t6.rlabel = _SLIT("TypeNode(7)");
		v_assert_meta_info__t6.lvalue = orm__Primitive_str(id) /* typeof: v.ast.Ident type: 89 */ ;
		v_assert_meta_info__t6.rvalue = _SLIT("int") /* typeof: v.ast.TypeNode type: 7 */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t6);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	if ((id)._typ == 7 /* int */) {
		...
		if ((*id._int) == 1) {
			...
			v_assert_meta_info__t7.fpath = _SLIT("vlib/mysql/mysql_orm_test.v");
			v_assert_meta_info__t7.line_nr = 64;
			v_assert_meta_info__t7.fn_name = _SLIT("main.test_mysql_orm");
			v_assert_meta_info__t7.src = _SLIT("id == 1");
			v_assert_meta_info__t7.op = _SLIT("==");
			v_assert_meta_info__t7.llabel = _SLIT("id");
			v_assert_meta_info__t7.rlabel = _SLIT("1");
			v_assert_meta_info__t7.lvalue = int_str((*id._int)) /* typeof: v.ast.Ident type: 7 */ ;
			v_assert_meta_info__t7.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
			main__cb_assertion_ok(&v_assert_meta_info__t7);
		} else {
			g_test_fails++;
			VAssertMetaInfo v_assert_meta_info__t8 = {0};
			v_assert_meta_info__t8.fpath = _SLIT("vlib/mysql/mysql_orm_test.v");
			v_assert_meta_info__t8.line_nr = 64;
			v_assert_meta_info__t8.fn_name = _SLIT("main.test_mysql_orm");
			v_assert_meta_info__t8.src = _SLIT("id == 1");
			v_assert_meta_info__t8.op = _SLIT("==");
			v_assert_meta_info__t8.llabel = _SLIT("id");
			v_assert_meta_info__t8.rlabel = _SLIT("1");
			v_assert_meta_info__t8.lvalue = int_str((*id._int)) /* typeof: v.ast.Ident type: 7 */ ;
			v_assert_meta_info__t8.rvalue = int_str(1) /* typeof: v.ast.IntegerLiteral type: 7 */ ;
			main__cb_assertion_failed(&v_assert_meta_info__t8);
			longjmp(g_jump_buffer, 1);
			// TODO
			// Maybe print all vars in a test function if it fails?
		}
	...
	if ((name)._typ == 18 /* string */) {
		...
		v_assert_meta_info__t9.fpath = _SLIT("vlib/mysql/mysql_orm_test.v");
		v_assert_meta_info__t9.line_nr = 67;
		v_assert_meta_info__t9.fn_name = _SLIT("main.test_mysql_orm");
		v_assert_meta_info__t9.src = _SLIT("name is TypeNode(18)");
		v_assert_meta_info__t9.op = _SLIT("is");
		v_assert_meta_info__t9.llabel = _SLIT("name");
		v_assert_meta_info__t9.rlabel = _SLIT("TypeNode(18)");
		v_assert_meta_info__t9.lvalue = orm__Primitive_str(name) /* typeof: v.ast.Ident type: 89 */ ;
		v_assert_meta_info__t9.rvalue = _SLIT("string") /* typeof: v.ast.TypeNode type: 18 */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t9);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t10 = {0};
		v_assert_meta_info__t10.fpath = _SLIT("vlib/mysql/mysql_orm_test.v");
		v_assert_meta_info__t10.line_nr = 67;
		v_assert_meta_info__t10.fn_name = _SLIT("main.test_mysql_orm");
		v_assert_meta_info__t10.src = _SLIT("name is TypeNode(18)");
		v_assert_meta_info__t10.op = _SLIT("is");
		v_assert_meta_info__t10.llabel = _SLIT("name");
		v_assert_meta_info__t10.rlabel = _SLIT("TypeNode(18)");
		v_assert_meta_info__t10.lvalue = orm__Primitive_str(name) /* typeof: v.ast.Ident type: 89 */ ;
		v_assert_meta_info__t10.rvalue = _SLIT("string") /* typeof: v.ast.TypeNode type: 18 */ ;
		main__cb_assertion_failed(&v_assert_meta_info__t10);
		longjmp(g_jump_buffer, 1);
		// TODO
		// Maybe print all vars in a test function if it fails?
	}
	if ((name)._typ == 18 /* string */) {
		...
		if (string__eq((*name._string), _SLIT("Louis"))) {
			...
			v_assert_meta_info__t11.fpath = _SLIT("vlib/mysql/mysql_orm_test.v");
			v_assert_meta_info__t11.line_nr = 69;
			v_assert_meta_info__t11.fn_name = _SLIT("main.test_mysql_orm");
			v_assert_meta_info__t11.src = _SLIT("name == 'Louis'");
			v_assert_meta_info__t11.op = _SLIT("==");
			v_assert_meta_info__t11.llabel = _SLIT("name");
			v_assert_meta_info__t11.rlabel = _SLIT("'Louis'");
			v_assert_meta_info__t11.lvalue = (*name._string) /* typeof: v.ast.Ident type: 18 */ ;
			v_assert_meta_info__t11.rvalue = _SLIT("Louis") /* typeof: v.ast.StringLiteral type: 18 */ ;
			main__cb_assertion_ok(&v_assert_meta_info__t11);
		} else {
			g_test_fails++;
			VAssertMetaInfo v_assert_meta_info__t12 = {0};
			v_assert_meta_info__t12.fpath = _SLIT("vlib/mysql/mysql_orm_test.v");
			v_assert_meta_info__t12.line_nr = 69;
			v_assert_meta_info__t12.fn_name = _SLIT("main.test_mysql_orm");
			v_assert_meta_info__t12.src = _SLIT("name == 'Louis'");
			v_assert_meta_info__t12.op = _SLIT("==");
			v_assert_meta_info__t12.llabel = _SLIT("name");
			v_assert_meta_info__t12.rlabel = _SLIT("'Louis'");
			v_assert_meta_info__t12.lvalue = (*name._string) /* typeof: v.ast.Ident type: 18 */ ;
			v_assert_meta_info__t12.rvalue = _SLIT("Louis") /* typeof: v.ast.StringLiteral type: 18 */ ;
			main__cb_assertion_failed(&v_assert_meta_info__t12);
			longjmp(g_jump_buffer, 1);
			// TODO
			// Maybe print all vars in a test function if it fails?
		}
	...
	if ((age)._typ == 8 /* i64 */) {
		...
		v_assert_meta_info__t13.fpath = _SLIT("vlib/mysql/mysql_orm_test.v");
		v_assert_meta_info__t13.line_nr = 72;
		v_assert_meta_info__t13.fn_name = _SLIT("main.test_mysql_orm");
		v_assert_meta_info__t13.src = _SLIT("age is TypeNode(8)");
		v_assert_meta_info__t13.op = _SLIT("is");
		v_assert_meta_info__t13.llabel = _SLIT("age");
		v_assert_meta_info__t13.rlabel = _SLIT("TypeNode(8)");
		v_assert_meta_info__t13.lvalue = orm__Primitive_str(age) /* typeof: v.ast.Ident type: 89 */ ;
		v_assert_meta_info__t13.rvalue = _SLIT("i64") /* typeof: v.ast.TypeNode type: 8 */ ;
		main__cb_assertion_ok(&v_assert_meta_info__t13);
	} else {
		g_test_fails++;
		VAssertMetaInfo v_assert_meta_info__t14 = {0};
		v_assert_meta_info__t14.fpath = _SLIT("vlib/mysql/mysql_orm_test.v");
		v_assert_meta_info__t14.line_nr = 72;
		v_assert_meta_info__t14.fn_name = _SLIT("main.test_mysql_orm");
		v_assert_meta_info__t14.src = _SLIT("age is TypeNode(8)");
		v_assert_meta_info__t14.op = _SLIT("is");
		v_assert_meta_info__t14.llabel = _SLIT("age");
		v_assert_meta_info__t14.rlabel = _SLIT("TypeNode(8)");
		v_assert_meta_info__t14.lvalue = orm__Primitive_str(age) /* typeof: v.ast.Ident type: 89 */ ;
		v_assert_meta_info__t14.rvalue = _SLIT("i64") /* typeof: v.ast.TypeNode type: 8 */ ;
	...
	if ((age)._typ == 8 /* i64 */) {
		...
		if ((*age._i64) == 100) {
			...
			v_assert_meta_info__t15.fpath = _SLIT("vlib/mysql/mysql_orm_test.v");
			v_assert_meta_info__t15.line_nr = 74;
			v_assert_meta_info__t15.fn_name = _SLIT("main.test_mysql_orm");
			v_assert_meta_info__t15.src = _SLIT("age == 100");
			v_assert_meta_info__t15.op = _SLIT("==");
			v_assert_meta_info__t15.llabel = _SLIT("age");
			v_assert_meta_info__t15.rlabel = _SLIT("100");
			v_assert_meta_info__t15.lvalue = i64_str((*age._i64)) /* typeof: v.ast.Ident type: 8 */ ;
			v_assert_meta_info__t15.rvalue = i64_str(100) /* typeof: v.ast.IntegerLiteral type: 8 */ ;
			main__cb_assertion_ok(&v_assert_meta_info__t15);
		} else {
			g_test_fails++;
			VAssertMetaInfo v_assert_meta_info__t16 = {0};
			v_assert_meta_info__t16.fpath = _SLIT("vlib/mysql/mysql_orm_test.v");
			v_assert_meta_info__t16.line_nr = 74;
			v_assert_meta_info__t16.fn_name = _SLIT("main.test_mysql_orm");
			v_assert_meta_info__t16.src = _SLIT("age == 100");
			v_assert_meta_info__t16.op = _SLIT("==");
			v_assert_meta_info__t16.llabel = _SLIT("age");
			v_assert_meta_info__t16.rlabel = _SLIT("100");
			v_assert_meta_info__t16.lvalue = i64_str((*age._i64)) /* typeof: v.ast.Ident type: 8 */ ;
			v_assert_meta_info__t16.rvalue = i64_str(100) /* typeof: v.ast.IntegerLiteral type: 8 */ ;
			main__cb_assertion_failed(&v_assert_meta_info__t16);
			longjmp(g_jump_buffer, 1);
			// TODO
			// Maybe print all vars in a test function if it fails?
		}
