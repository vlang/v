...
// type/init/cap/len expression FlatNodeIds and a slice of already-flat
// element expression FlatNodeIds. Mirrors the add_expr(ArrayInitExpr)
// encoding exactly: edges = [typ, init, cap, len, exprs...]. The flat-write
// port for `transform_array_init_expr`'s identity branches (invalid data,
// fixed array, eval-backend dynamic array) uses this to skip the
// `ast.ArrayInitExpr` wrapper struct allocation.
pub fn (mut b FlatBuilder) emit_array_init_expr_by_ids(typ_id FlatNodeId, init_id FlatNodeId, cap_id FlatNodeId, len_id FlatNodeId, expr_ids []FlatNodeId, pos token.Pos) FlatNodeId {
	mut edges := []FlatEdge{cap: 4 + expr_ids.len}
	edges << FlatEdge{
		child_id: typ_id
	}
	edges << FlatEdge{
