...
// type in env — exactly which nodes the propagate walk visited + typed. This is
// the parity signal a future cursor-native propagate port must preserve.
fn prop_typed_ids_sig(t &Transformer) string {
	mut ids := []string{}
	for id in 1 .. t.env.expr_type_values.len {
		if _ := t.env.get_expr_type(id) {
			ids << id.str()
		}
	}
	return ids.join(',')
