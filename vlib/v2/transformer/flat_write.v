...
//
// Sessions 2..N port these in roughly increasing complexity. The categories
// are deliberately coarse — within a category the rewrites share most of
// their helper state (e.g. all or-block expansions share `expand_*_or_expr`).
//
...
// session port. It rehydrates a single file from `input_flat`, runs it
// through the legacy per-file transform, and appends the transformed result
// to `out`. Returns the FlatNodeId of the appended file root, or
// `ast.invalid_flat_node_id` for an empty / missing source file.
//
// Today the body is rehydrate + `transform_file` + `append_file`: a behaviour-
// preserving decomposition of the loop inside `transform_files_from_flat`.
// Sessions 2..N progressively replace pieces of `transform_file`'s output with
// direct `out.emit(...)` calls so the rehydration shrinks toward zero and the
// post-transform legacy `ast.File` eventually drops out.
//
// Callers must invoke `pre_pass_from_flat(input_flat)` before the per-file
// loop and `post_pass(mut collected_files)` after, mirroring the wedge.
// The per-file API does not run those passes itself so sessions 2..N can
...
pub fn (mut t Transformer) transform_file_index_to_flat(input_flat &ast.FlatAst, fi int, mut out ast.FlatBuilder) ast.FlatNodeId {
	...
	transformed := t.transform_file(src_arr[0])
	return out.append_file(transformed)
}
