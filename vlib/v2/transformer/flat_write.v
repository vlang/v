...
//     cap_id, len_id, expr_ids, pos)` in `vlib/v2/ast/flat.v` mirrors
//     `add_expr(ArrayInitExpr)` encoding (`emit_simple(.expr_array_init,
//     pos, [typ, init, cap, len, exprs...])`).
//
	...
	match expr {
		...
		ast.ArrayInitExpr {
			...
			if result is ast.ArrayInitExpr {
				...
				return out.emit_array_init_expr_by_ids(typ_id, init_id, cap_id, len_id, expr_ids,
					result.pos)
			}
			if result is ast.CallExpr {
				lhs_id := out.emit_expr(result.lhs)
