...
fn (mut g Gen) gen_enum_decl(node ast.EnumDecl) {
	...
	g.register_alias_type(option_name)
	mut enum_from_string_expr := '({ ${option_name} _opt = {0}; _opt.state = 2; _opt; })'
	for i := node.fields.len - 1; i >= 0; i-- {
		field := node.fields[i]
		enum_from_string_expr = '(string__eq(s, ${c_static_v_string_expr(field.name)}) ? ({ ${option_name} _opt = {0}; ${name} _val = ${g.enum_member_c_name(name,
			field.name)}; _option_ok(&_val, (_option*)&_opt, sizeof(_val)); _opt; }) : ${enum_from_string_expr})'
	}
	g.sb.writeln('#define ${name}__from_string(s) ${enum_from_string_expr}')
	// Runtime enum values array for lowered `EnumType.values` usages.
	values_name := '__enum_values_${name}'
...
fn (g &Gen) can_emit_map_str_for_type(type_name string) bool {
	...
	// Pointer-like types that may not be valid C type names. Do not treat
	// composite aliases like Array_Fooptr as pointers just because their
	// element component ends in "ptr".
	is_composite_alias := type_name.starts_with('Array_') || type_name.starts_with('Map_')
		|| type_name.starts_with('Tuple_')
	if type_name.ends_with('*') || (!is_composite_alias
		&& (type_name in ['intptr', 'charptr', 'byteptr']
		|| type_name.ends_with('ptr'))) {
		return true
	}
...
fn (mut g Gen) emit_map_str_write_val(var_name string, type_name string) {
	...
	} else if type_name in ['voidptr', 'intptr', 'u8ptr', 'charptr', 'byteptr'] {
		// Print pointer as hex
		g.sb.writeln('\t\t{ char _buf[20]; snprintf(_buf, sizeof(_buf), "%p", (void*)(${var_name})); strings__Builder__write_string(&sb, (string){.str = _buf, .len = strlen(_buf), .is_lit = 0}); }')
...
fn (mut g Gen) emit_map_eq_functions() {
	...
	for name in map_names {
		...
		if value_type == 'string' {
			...
			|| value_type.ends_with('ptr') || value_type.ends_with('*') {
...
fn (mut g Gen) emit_struct_field_eq(s types.Struct, va string, vb string) {
	for field in s.fields {
		...
		eq_expr := g.eq_expr_for_type(field.typ, '${va}.${field_name}', '${vb}.${field_name}')
		g.sb.writeln('\t\tif (!(${eq_expr})) return false;')
	}
}
...
fn (g &Gen) parse_map_kv_types(kv_str string) (string, string) {
	if info := g.collected_map_types['Map_${kv_str}'] {
	...
	for i := 1; i < kv_str.len; i++ {
		if kv_str[i] == `_` {
			...
			if 'Map_${key}_' in g.map_aliases.keys().map(it + '_') || key in g.array_aliases
				|| key in g.emitted_types || 'body_${key}' in g.emitted_types
				|| 'alias_${key}' in g.emitted_types || 'enum_${key}' in g.emitted_types
				|| key in simple_types {
				return key, value
			}
		}
