...
fn (mut g Gen) infer_tuple_field_types_from_return_stmt(ret ast.ReturnStmt) ?[]string {
	...
	mut tuple_exprs := ret.exprs.clone()
	...
	match node {
		...
		ast.InfixExpr {
			...
			if node.op in [.eq, .ne] && is_lhs_array && is_rhs_array {
				lhs_is_ptr := g.expr_is_pointer(node.lhs)
				rhs_is_ptr := g.expr_is_pointer(node.rhs)
				sep_l := if lhs_is_ptr { '->' } else { '.' }
				sep_r := if rhs_is_ptr { '->' } else { '.' }
				if node.op == .ne {
					g.sb.write_string('!')
				}
				g.sb.write_string('(((')
				g.gen_expr(node.lhs)
				g.sb.write_string('${sep_l}len == ')
				g.gen_expr(node.rhs)
				g.sb.write_string('${sep_r}len) && (')
				g.gen_expr(node.lhs)
				g.sb.write_string('${sep_l}element_size == ')
				g.gen_expr(node.rhs)
				g.sb.write_string('${sep_r}element_size) && (memcmp(')
				g.gen_expr(node.lhs)
				g.sb.write_string('${sep_l}data, ')
				g.gen_expr(node.rhs)
				g.sb.write_string('${sep_r}data, (')
				g.gen_expr(node.lhs)
				g.sb.write_string('${sep_l}len * ')
				g.gen_expr(node.lhs)
				g.sb.write_string('${sep_l}element_size)) == 0)))')
				return
			}
...
fn (mut g Gen) gen_inline_filter_or_map(c_name string, call_args []ast.Expr) {
	...
	if !is_filter {
		result_type := g.get_expr_type(body_expr)
		if result_type != '' && result_type != 'int_literal' {
			result_elem = result_type
		}
	}
	g.tmp_counter++
	tmp := g.tmp_counter
