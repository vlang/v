	...
	for stmt in file.stmts {
		if stmt is ast.FnDecl {
			...
			if stmt.is_method {
				...
				receiver_module := b.type_modules[receiver_type_name]
...
fn (mut b Builder) generate_array_contains_stub(name string) {
	...
	array_t := if st := b.struct_types['array'] { st } else { i64_t }
...
fn (mut b Builder) generate_str_stub(name string) {
	...
	string_t := if st := b.struct_types['string'] { st } else { i64_t }
...
fn (mut b Builder) generate_array_eq_stub() {
	...
	// Load a1.len (field 1)
	a1_len_ptr := b.mod.add_instr(.get_element_ptr, b.cur_block, ptr_t, [a1_param, field1_idx])
	a1_len := b.mod.add_instr(.load, b.cur_block, i64_t, [a1_len_ptr])
	// Load a2.len (field 1)
	a2_len_ptr := b.mod.add_instr(.get_element_ptr, b.cur_block, ptr_t, [a2_param, field1_idx])
	...
	field0_idx := b.mod.add_value_node(.constant, i64_t, '0', 0)
	field4_idx := b.mod.add_value_node(.constant, i64_t, '4', 4)
	...
	// Load a1.element_size (field 4)
	a1_esz_ptr := b.mod.add_instr(.get_element_ptr, b.cur_block, ptr_t, [a1_param, field4_idx])
...
fn (mut b Builder) stmt(node ast.Stmt) {
	...
	match node {
		ast.AssignStmt {
			...
			if node.op == .decl_assign {
				...
				if is_map {
					...
					default_map_sizes := [8, 8]!
					b.var_map_types[name] = default_map_sizes
		...
		ast.StructDecl {
			...
			// Track which module this type belongs to
				...
				if node.base_type is ast.Ident {
					...
					match node.base_type {
						...
						ast.MapType {
							...
							if map_t := b.struct_types['map'] {
								b.struct_types[node.name] = map_t
							}
							b.type_modules[node.name] = b.cur_module
						}
						else {}
...
fn (mut b Builder) expr_call(node ast.CallExpr) ValueID {
	...
	if lhs is ast.Ident {
		...
		if !name.contains('__') && b.cur_module != 'main' && b.cur_module != 'builtin' {
			...
			if prefixed in b.func_ret_types {
				name = prefixed
			}
		}
		...
		if name !in b.func_ret_types && name.contains('_') {
			...
			for i := name.len - 1; i >= 0; i-- {
				if name[i] == `_` && (i == 0 || name[i - 1] != `_`)
					...
					if candidate in b.func_ret_types {
						name = candidate
						break
					}
				}
	...
	if name.starts_with('array__contains_') && name !in b.func_ret_types {
		b.generate_array_contains_stub(name)
	}
	if name == 'array__eq' && name !in b.func_ret_types {
		b.generate_array_eq_stub()
	}
	// Generate no-op stubs for sync RwMutex methods (single-threaded in native backend)
	if name !in b.func_ret_types && name.contains('RwMutex') {
	...
	if name !in b.func_ret_types && name.ends_with('__str') {
	...
	if node.lhs is ast.Ident {
		...
		if node.lhs.lhs is ast.Ident {
			...
			if receiver_name == 'C' {
				...
				receiver_module2 := b.type_modules[struct_type_name]
				mut mangled_type := struct_type_name
				if receiver_module2 != '' && receiver_module2 != 'main'
					&& receiver_module2 != 'builtin' {
	...
	match node {
		...
		ast.IndexExpr {
			...
			if elem_typ.kind == .ptr_t {
				...
				i64_t := b.mod.type_store.get_int(64)
				data_ptr_t := b.mod.type_store.get_ptr(i64_t)
