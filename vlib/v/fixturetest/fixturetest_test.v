...
fn test_forwarded_compiler_options_preserve_configuration_flags() {
	args := ['-silent', '-cc', 'clang', '-d', 'test', '-dfoo', '-o', 'ignored-output', 'test',
		...
		'-no-parallel', '-keepc']
	assert forwarded_compiler_options(args) == ['-cc', 'clang', '-d', 'test', '-dfoo', '-os', 'windows',
		'-thread-stack-size', '4194304', '-keepc']
}
...
fn test_fixture_repo_root_falls_back_to_requested_suite() {
	...
	os.mkdir_all(os.join_path(repo_root, 'vlib', 'v')) or { panic(err) }
