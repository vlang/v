...
fn (mut e Eval) register_files() ! {
	...
	if !e.has_main_function() && e.implicit_main.len == 0 {
		return error('v.eval: missing main.main entry point')
	}
}
...
fn (mut e Eval) run_implicit_main() ![]Value {
	...
	if escaped.kind != .normal {
		return error('v.eval: unexpected `${escaped.kind}` escaped implicit main')
	}
	return values
}
...
fn (e &Eval) unknown_variable_error(name string) IError {
	return error('v.eval: unknown variable `${name}` in `${e.current_function_label()}` stack `${e.call_stack_trace()}`')
}
...
fn (mut e Eval) call_function(module_name string, fn_name string, args []Value) !CallResult {
	...
	if target_module !in e.functions {
		return error('v.eval: unknown module `${target_module}`')
	}
	if fn_name !in e.functions[target_module] {
		return error('v.eval: unknown function `${target_module}.${fn_name}`')
	}
	...
	if args.len < min_args {
		return error('v.eval: `${target_module}.${fn_name}` expected ${min_args} arguments, got ${args.len}')
	}
	e.open_scope()
	e.call_stack << CallFrame{
	...
	if signal.kind != .normal {
		return error('v.eval: unexpected `${signal.kind}` escaped `${target_module}.${fn_name}`')
	}
	return CallResult{
		values:       [void_value()]
		mutated_args: mutated_args
...
fn (mut e Eval) run_deferred_stmts() ! {
	...
	for i := defer_stmts.len; i > 0; i-- {
		...
		for child_id in e.children(defer_node) {
			...
			if signal.kind != .normal {
				return error('v.eval: unexpected `${signal.kind}` escaped defer')
			}
		}
	}
}
...
fn (mut e Eval) assign_target(id flat.NodeId, value Value, declare bool) ! {
	...
	if signal.kind != .normal {
		return error('v.eval: unexpected `${signal.kind}` escaped assignment target')
	}
}
...
fn (mut e Eval) assign_target_typed_flow(id flat.NodeId, value Value, declare bool, type_name string) !FlowSignal {
	...
	match node.kind {
		...
		.index {
			...
			return error('v.eval: unsupported assignment target `${node.kind}`')
		}
	}
	...
	match container {
		ArrayValue {
			...
			if idx < 0 || idx >= arr.values.len {
				return error('v.eval: array index out of bounds')
			}
		...
		MapValue {
			...
			return error('v.eval: indexed assignment expects array or map')
		}
	}
}
...
fn (mut e Eval) set_selector_value(container Value, field_name string, value Value) !Value {
	match container {
		...
		SumValue {
			...
			return error('v.eval: selector assignment expects struct, got `${e.runtime_type_name(container)}`')
		}
	}
}
...
fn (mut e Eval) exec_array_append(node &flat.Node) !FlowSignal {
	...
	if current !is ArrayValue {
		return error('v.eval: `<<` is only supported for arrays')
	}
...
fn (mut e Eval) exec_for_in(node &flat.Node, loop_label string) !FlowSignal {
	...
	match container {
		...
		string {
			...
			return error('v.eval: unsupported for-in iterable `${e.runtime_type_name(container)}`')
		}
	}
...
fn (mut e Eval) eval_expr(id flat.NodeId) !Value {
	...
	match node.kind {
		...
		.fn_literal {
			...
			return error('v.eval: unsupported expression `${node.kind}`')
		}
	}
}
...
fn (mut e Eval) eval_fn_literal(id flat.NodeId, node &flat.Node) !FnValue {
	...
	for child_id in e.children(node) {
		...
		if !found.found {
			return error('v.eval: unknown captured variable `${child.value}`')
		}
...
fn (mut e Eval) eval_call_flow(id flat.NodeId, node &flat.Node) !FlowSignal {
	...
	_ = id
	return error('v.eval: unsupported call target `${callee.kind}`')
}
...
fn (mut e Eval) write_back_mutated_arg_values(node &flat.Node, mutated_args map[int]Value, mut_lvalues map[int]ResolvedLvalue) ! {
	for child_index := 1; child_index < node.children_count; child_index++ {
		...
		if resolved := mut_lvalues[arg_index] {
			...
			if signal.kind != .normal {
				return error('v.eval: unexpected `${signal.kind}` escaped mut argument writeback')
			}
			continue
		}
		arg_id := e.child(node, child_index)
...
fn (mut e Eval) call_fn_value(fv FnValue, args []Value) !CallResult {
	...
	if signal.kind != .normal {
		return error('v.eval: unexpected `${signal.kind}` escaped anonymous function')
	}
	return CallResult{
		values:           [void_value()]
		mutated_args:     mutated_args
...
fn (mut e Eval) eval_selector_value(left Value, field string) !Value {
	...
	return error('v.eval: unsupported selector `${field}` on `${e.runtime_type_name(left)}`')
}
...
fn (e &Eval) eval_sum_selector(value SumValue, field string) !Value {
	...
	return error('v.eval: unknown sum type field `${field}` on `${value.type_name}`')
}
	...
	match container {
		...
		string {
			...
			if idx < 0 || idx >= container.len {
				return error('v.eval: string index out of bounds')
			}
			return Value(i64(container[idx]))
		}
		else {
			return error('v.eval: unsupported index target `${e.runtime_type_name(container)}`')
		}
	}
}
...
fn (e &Eval) slice_value(container Value, start int, end int) !Value {
	match container {
		...
		string {
			...
			return error('v.eval: slicing is only supported for arrays and strings')
		}
	}
}
...
fn (mut e Eval) eval_assoc_flow(node &flat.Node) !FlowSignal {
	...
	if base !is StructValue {
		return error('v.eval: assoc base must be struct')
	}
...
fn (mut e Eval) call_value_method(receiver Value, receiver_type_name string, method_name string, args []Value) !MethodCallResult {
	...
	return error('v.eval: unsupported method `${method_name}` on `${e.runtime_type_name(receiver)}`')
}
...
fn (mut e Eval) call_string_method(receiver string, method_name string, args []Value) !Value {
	match method_name {
		...
		'trim_space' {
			...
			return error('v.eval: unsupported string method `${method_name}`')
		}
	}
}
	...
	match method_name {
		...
		'first' {
			if receiver.values.len == 0 {
				return error('v.eval: array.first on empty array')
			}
			return receiver.values[0]
		}
		'last' {
			if receiver.values.len == 0 {
				return error('v.eval: array.last on empty array')
			}
		...
		'join' {
			...
			return error('v.eval: unsupported array method `${method_name}`')
		}
	}
}
...
fn (mut e Eval) call_map_method(receiver MapValue, method_name string, args []Value) !Value {
	match method_name {
		...
		'delete' {
			...
			return error('v.eval: unsupported map method `${method_name}`')
		}
	}
}
...
fn (mut e Eval) apply_infix(op flat.Op, left Value, right Value) !Value {
	...
	match op {
		...
		.right_shift_unsigned {
			...
			return error('v.eval: unsupported infix operator `${op}`')
		}
	}
}
...
fn (e &Eval) expect_string_arg(args []Value, index int) !string {
	if index >= args.len {
		return error('v.eval: missing argument ${index}')
	}
...
fn (e &Eval) value_as_bool(value Value) !bool {
	match value {
		...
		else { return error('v.eval: `${e.runtime_type_name(value)}` can not be used as bool') }
	}
}
...
fn (e &Eval) value_as_int(value Value) !i64 {
	...
	return error('v.eval: `${e.runtime_type_name(value)}` can not be used as int')
}
...
fn (e &Eval) value_as_f64(value Value) !f64 {
	match value {
		...
		else { return error('v.eval: `${e.runtime_type_name(value)}` can not be used as float') }
	}
}
