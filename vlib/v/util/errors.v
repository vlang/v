...
pub fn formatted_error(kind string, omsg string, filepath string, pos token.Position) string {
	...
	position := '$path:${pos.line_nr + 1}:${mu.imax(1, column + 1)}:'
...
pub fn source_context(kind string, source string, column int, pos token.Position) []string {
	...
	bline := mu.imax(0, pos.line_nr - util.error_context_before)
	aline := mu.imax(0, mu.imin(source_lines.len - 1, pos.line_nr + util.error_context_after))
	tab_spaces := '    '
	for iline := bline; iline <= aline; iline++ {
		sline := source_lines[iline]
		start_column := mu.imax(0, mu.imin(column, sline.len))
		end_column := mu.imax(0, mu.imin(column + mu.imax(0, pos.len), sline.len))
