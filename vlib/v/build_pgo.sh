#!/usr/bin/env bash
# Build a standalone V compiler with Clang's profile-guided optimization.
set -euo pipefail
...
bootstrap="${1:-$script_dir/../../v}"
output="${2:-$script_dir/v-pgo}"
...
pgo_dir="$(mktemp -d /tmp/v-pgo.XXXXXXXX)"
