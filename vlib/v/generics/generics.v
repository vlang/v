...
pub fn (mut g Generics) expr(mut node ast.Expr) ast.Expr {
	match mut node {
		...
		ast.InfixExpr {
			if g.cur_concrete_types.len > 0 {
				resolved_left_type := g.unwrap_generic(node.left_type)
				resolved_right_type := g.unwrap_generic(node.right_type)
				resolved_promoted := if node.promoted_type.has_flag(.generic)
					|| (node.left_type.has_flag(.generic) && node.promoted_type.is_number()) {
					ast.promote_num(resolved_left_type, resolved_right_type)
				} else {
					g.unwrap_generic(node.promoted_type)
				}
				return ast.Expr(ast.InfixExpr{
					...node
					left_type:     resolved_left_type
					right_type:    resolved_right_type
					promoted_type: resolved_promoted
					left:          g.expr(mut node.left)
