...
fn test_silent() {
	...
	path := os.real_path('./${os.dir(parser_dir)}/sql.v')
	source := os.read_file(path) or { '' }
	os.exec('$vexe -cg -o $parser_dir/test_parse_silent $parser_dir/test_parse_silent.v') or {
		assert false
	}
	assert source != ''
	len := source.len - 1
	mut success := true
	for i in 0..len {
		res := os.exec('./$parser_dir/test_parse_silent --path $path --index $i') or { os.Result{output: err} }
		if res.output != '' {
			success = false
			part := source[..i]
			line := part.count('\n')
			...
			err := red('error:')
			path_to_line := bold('$path:$line')
			err_line := italic(last_line.trim_left('\t'))
			eprintln('$err $path_to_line')
			eprintln('\t$line | $err_line')
			eprintln('')
			eprintln(res.output)
		}
	}
