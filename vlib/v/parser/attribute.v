...
fn (mut p Parser) parse_attr_arg(err_context string) (ast.AttrKind, string) {
	if p.tok.kind == .name { // `name: arg`
		...
		arg := p.tok.lit
		p.next()
		return ast.AttrKind.string, arg
...
fn (mut p Parser) parse_attr(is_at bool) []ast.Attr {
	...
	if p.tok.kind == .key_if {
		...
		if p.tok.kind == .colon {
			has_arg = true
			p.next()
			kind, arg = p.parse_attr_arg(' after `:`')
