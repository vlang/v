...
fn (mut p Parser) fn_decl() ast.FnDecl {
	...
	if p.tok.kind == .name {
		...
		type_sym := p.table.type_symbol(rec.typ)
		if is_method {
	...
	if is_method && rec.typ.has_flag(.generic) {
		sym := p.table.type_symbol(rec.typ)
		if sym.info is ast.Struct {
			rec_generic_names := sym.info.generic_types.map(p.table.type_symbol(it).name)
	...
	// Register
	if is_method {
		mut type_sym := p.table.type_symbol(rec.typ)
		...
		if !is_non_local && !(p.builtin_mod && p.pref.is_fmt) && type_sym.kind in [.array, .map] {
			elem_type_sym := p.table.type_symbol(p.table.value_type(rec.typ))
...
fn (mut p Parser) fn_args() ([]ast.Param, bool, bool) {
	...
	if types_only {
		...
		for p.tok.kind != .rpar {
			...
			alanguage := p.table.type_symbol(arg_type).language
			if alanguage != .v {
		...
		for p.tok.kind != .rpar {
			...
			for i, arg_name in arg_names {
				alanguage := p.table.type_symbol(typ).language
				if alanguage != .v {
