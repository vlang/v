...
const requested_vlib_tests = [
	...
	'vlib/v/tests/testdata/multiple_generic_struct_fields.v',
]
...
// their own runners. The compiler unit-test batcher must not rediscover their
// fixture programs as standalone unit tests.
const separately_run_test_prefixes = [
	'vlib/v/build_constraint/',
	'vlib/v/checker/tests/',
	'vlib/v/debug/',
	'vlib/v/embed_file/',
	'vlib/v/live/',
	'vlib/v/parser/tests/',
...
fn main() {
	...
	converged_size := assert_same_file_bytes('v5/v6 generated C output', v5_bin + '.c', v6_bin + '.c')
...
fn enabled_unit_tests(cfg Config, paths []string) []string {
	...
	for path in paths {
		...
		if relative_path == 'vlib/v/compiler_errors_test.v'
			|| separately_run_test_prefixes.any(relative_path.starts_with(it)) {
			continue
		}
...
fn parse_config() Config {
	...
	tests_dir := os.join_path(script_dir, 'compiler_tests')
...
fn host_v_cmd(cfg Config) string {
	// The canonical compiler must be able to build itself. Keep the compatibility
	// compiler confined to the unit-test harness for source constructs that those
	// tests intentionally exercise outside the new frontend.
