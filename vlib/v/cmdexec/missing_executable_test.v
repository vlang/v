...
// A file that exists but carries no execute bit is not "missing". The launch has
// to reach execve() so the caller sees the real permission error, rather than a
// message claiming an existing compiler is not there.
fn test_a_non_executable_file_is_not_reported_as_missing() {
	$if windows {
		return
	}
	tmp := os.join_path(os.vtmp_dir(), 'v_cmdexec_noexec_${os.getpid()}')
	os.mkdir_all(tmp) or { panic(err) }
	defer {
		os.rmdir_all(tmp) or {}
	}
	present := os.join_path(tmp, 'wrapper')
	os.write_file(present, '#!/bin/sh\necho nope\n') or { panic(err) }
	os.chmod(present, 0o644) or { panic(err) }
	res := run(present, [])
	assert res.exit_code != 0
	assert !res.output.contains('failed to find executable'), res.output
}

// ExpandEnvironmentStringsW substitutes `%NAME%` and leaves anything it cannot
// resolve verbatim. The preflight has to match that, because os.Process hands
// CreateProcessW the *unexpanded* filename as lpApplicationName, so an
// unexpanded `%COMSPEC%` would abort the compiler rather than launch a shell.
fn test_windows_env_var_expansion_matches_the_api() {
	os.setenv('V_CMDEXEC_TEST_VAR', 'expanded', true)
	defer {
		os.unsetenv('V_CMDEXEC_TEST_VAR')
	}
	assert expand_windows_env_vars('plain') == 'plain'
	assert expand_windows_env_vars('%V_CMDEXEC_TEST_VAR%') == 'expanded'
	assert expand_windows_env_vars('a%V_CMDEXEC_TEST_VAR%b') == 'aexpandedb'
	assert expand_windows_env_vars(r'C:\tools\%V_CMDEXEC_TEST_VAR%\cc.exe') == r'C:\tools\expanded\cc.exe'
	// An unset name, an empty name and a dangling `%` are all kept as written.
	assert expand_windows_env_vars('%V_CMDEXEC_NOT_SET_VAR%') == '%V_CMDEXEC_NOT_SET_VAR%'
	assert expand_windows_env_vars('%%') == '%%'
	assert expand_windows_env_vars('50%') == '50%'
}
