...
const buf_len = os.getenv_opt('BUF_LEN') or { '10_000_000' }.int()
const nthreads = os.getenv_opt('VJOBS') or { '2' }.int()
const max_iterations = os.getenv_opt('MAX_ITERATIONS') or { '1' }.int()
...
fn worker(mut chunk WorkChunk) {
	...
	elapsed_time := sw.elapsed().nanoseconds()
	us_fill := u64(f64(elapsed_time) / f64(max_iterations * 1000))
	ns_rand_character := f64(elapsed_time) / f64(max_iterations * chunk.part.len)
	println('thread ${chunk.thread_id:2}, took ${elapsed_time / 1_000_000:6}ms, per fill: ${us_fill:9}us, per character: ${ns_rand_character:5.0}ns, part: ${chunk.part#[..3]}...${chunk.part#[-3..]}, part.len: ${chunk.part.len:6}')
}
