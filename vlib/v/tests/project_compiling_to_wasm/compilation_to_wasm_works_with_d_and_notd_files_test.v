...
fn test_normal() {
	...
	dump(vexe)
	res := os.system('${vexe} -old-compiler -o normal.exe ${os.quoted_path(project_folder)}')
	assert res == 0
	dump(res)
...
fn test_emcc() {
	...
	dump(emcc)
	res :=
		os.system('${vexe} -old-compiler -os wasm32_emscripten -o wasm_check.html ${os.quoted_path(project_folder)}')
	assert res == 0
	dump(res)
