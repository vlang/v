...
struct OptionRefCounter {
mut:
	calls int
}

fn (mut counter OptionRefCounter) next() int {
	counter.calls++
	return 0
}
...
fn test_string_interpolation_reference_to_option_value() {
	...
	assert option_payload_alias_option_alias_ref_string(?OptionRefInt(none)) == '&Option(none)'
...
fn test_string_interpolation_reference_to_option_value_is_evaluated_lazily() {
	...
	assert !(evaluate && '${&options[counter.next()]}' == '&Option(42)')
