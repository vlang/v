...
fn (mut c Checker) register_trace_call(node ast.CallExpr) {
	...
	if is_traceable {
		...
		$if trace_fns ? {
			eprintln('>> trace_fns | checker | ${node.name} - ${generic_name}')
		}
		calling_fn := if generic_name != '' {
