				...
				if call_arg.expr is ast.Ident && !func.is_method {
					...
					if ident.obj is ast.Var {
						...
						if var_obj.is_arg && c.table.cur_fn != unsafe { nil }
							...
							cur_fn_param_type := c.table.cur_fn.params.last().typ
							cur_fn_param_sym := c.table.sym(cur_fn_param_type)
							if cur_fn_param_sym.info is ast.Array {
								cur_elem_type := cur_fn_param_sym.info.elem_type
								allow_variadic_pass = cur_elem_type == expected_type
							}
						}
					}
				}
