...
pub fn (mut c Checker) alias_type_decl(node ast.AliasTypeDecl) {
	...
	typ_sym := c.table.type_symbol(node.parent_type)
	if typ_sym.kind in [.placeholder, .int_literal, .float_literal] {
		c.error('unknown type `$typ_sym.name`', node.type_pos)
	} else if typ_sym.kind == .alias {
		orig_sym := c.table.type_symbol((typ_sym.info as ast.Alias).parent_type)
...
pub fn (mut c Checker) fn_type_decl(node ast.FnTypeDecl) {
	...
	typ_sym := c.table.type_symbol(node.typ)
	fn_typ_info := typ_sym.info as ast.FnType
	fn_info := fn_typ_info.func
	c.ensure_type_exists(fn_info.return_type, fn_info.return_type_pos) or {}
	ret_sym := c.table.type_symbol(fn_info.return_type)
	if ret_sym.kind == .placeholder {
		c.error('unknown type `$ret_sym.name`', fn_info.return_type_pos)
	}
	for arg in fn_info.params {
		c.ensure_type_exists(arg.typ, arg.type_pos) or { return }
		arg_sym := c.table.type_symbol(arg.typ)
...
pub fn (mut c Checker) sum_type_decl(node ast.SumTypeDecl) {
	...
	for variant in node.variants {
		...
		mut sym := c.table.type_symbol(variant.typ)
...
pub fn (mut c Checker) struct_decl(mut node ast.StructDecl) {
	...
	if mut struct_sym.info is ast.Struct {
		for embed in node.embeds {
			...
			embed_sym := c.table.type_symbol(embed.typ)
		...
		for i, field in node.fields {
			...
			sym := c.table.type_symbol(field.typ)
			for j in 0 .. i {
				...
				c.check_expected(field_expr_type, field.typ) or {
					if sym.kind == .interface_
						...
						if !field_expr_type.is_ptr() && !field_expr_type.is_pointer()
							&& !c.inside_unsafe {
							field_expr_type_sym := c.table.type_symbol(field_expr_type)
...
pub fn (mut c Checker) struct_init(mut node ast.StructInit) ast.Type {
	if node.typ == ast.void_type {
		...
		sym := c.table.type_symbol(c.expected_type)
		if sym.kind == .array {
	...
	struct_sym := c.table.type_symbol(node.typ)
	if struct_sym.info is ast.Struct {
		...
		parent_sym := c.table.type_symbol(struct_sym.info.parent_type)
	...
	type_sym := c.table.type_symbol(node.typ)
	...
	match type_sym.kind {
		...
		.struct_, .string, .array, .alias {
			...
			if type_sym.kind == .alias {
				...
				sym := c.table.type_symbol(info_t.parent_type)
			...
			for i, mut field in node.fields {
				...
				field_type_sym := c.table.type_symbol(field_info.typ)
					...
					if mut field.expr is ast.Ident {
						if mut field.expr.obj is ast.Var {
							...
							if obj.is_stack_obj && !c.inside_unsafe {
								sym := c.table.type_symbol(obj.typ.set_nr_muls(0))
	...
	if node.has_update_expr {
		...
		if c.table.type_kind(update_type) != .struct_ {
			...
			from_sym := c.table.type_symbol(update_type)
			to_sym := c.table.type_symbol(node.typ)
...
pub fn (mut c Checker) infix_expr(mut node ast.InfixExpr) ast.Type {
	...
	mut right_sym := c.table.type_symbol(right_type)
	right_final := c.table.final_type_symbol(right_type)
	mut left_sym := c.table.type_symbol(left_type)
	left_final := c.table.final_type_symbol(left_type)
	...
	match node.op {
		...
		.plus, .minus, .mul, .div, .mod, .xor, .amp, .pipe { // binary operators that expect matching types
			if right_sym.info is ast.Alias && (right_sym.info as ast.Alias).language != .c
				...
				&& c.table.type_symbol((right_sym.info as ast.Alias).parent_type).is_primitive() {
				right_sym = c.table.type_symbol((right_sym.info as ast.Alias).parent_type)
			}
			if left_sym.info is ast.Alias && (left_sym.info as ast.Alias).language != .c
				&& c.mod == c.table.type_to_str(left_type).split('.')[0]
				&& c.table.type_symbol((left_sym.info as ast.Alias).parent_type).is_primitive() {
				left_sym = c.table.type_symbol((left_sym.info as ast.Alias).parent_type)
			}
		...
		.left_shift {
			if left_final.kind == .array {
				...
				left_value_sym := c.table.type_symbol(c.unwrap_generic(left_value_type))
		...
		.unsigned_right_shift {
			modified_left_type := if !left_type.is_int() {
				c.error('invalid operation: shift on type `${c.table.type_symbol(left_type).name}`',
					left_pos)
				ast.void_type_idx
		...
		.key_is, .not_is {
			...
			if typ != ast.Type(0) {
				typ_sym := c.table.type_symbol(typ)
				op := node.op.str()
...
fn (mut c Checker) fail_if_immutable(expr ast.Expr) (string, token.Position) {
	...
	match mut expr {
		...
		ast.IndexExpr {
			left_sym := c.table.type_symbol(expr.left_type)
		...
		ast.SelectorExpr {
			...
			mut typ_sym := c.table.final_type_symbol(c.unwrap_generic(expr.expr_type))
			match typ_sym.kind {
				.struct_ {
					mut has_field := true
...
fn (mut c Checker) type_implements(typ ast.Type, interface_type ast.Type, pos token.Position) bool {
	...
	if mut inter_sym.info is ast.Interface {
		...
		if inter_sym.info.parent_type.has_flag(.generic) {
			parent_sym := c.table.type_symbol(inter_sym.info.parent_type)
...
pub fn (mut c Checker) selector_expr(mut node ast.SelectorExpr) ast.Type {
	...
	if field_name.len > 0 && field_name[0].is_capital() && sym.info is ast.Struct
		...
		for embed in sym.info.embeds {
			embed_sym := c.table.type_symbol(embed)
		...
		if typ.has_flag(.generic) && !has_field {
			gs := c.table.type_symbol(c.unwrap_generic(typ))
	...
	if has_field {
		if sym.mod != c.mod && !field.is_pub && sym.language != .c {
			unwrapped_sym := c.table.type_symbol(c.unwrap_generic(typ))
...
fn (mut c Checker) check_array_init_para_type(para string, expr ast.Expr, pos token.Position) {
	sym := c.table.type_symbol(c.expr(expr))
...
pub fn (mut c Checker) ensure_sumtype_array_has_default_value(node ast.ArrayInit) {
	sym := c.table.type_symbol(node.elem_type)
...
pub fn (mut c Checker) array_init(mut node ast.ArrayInit) ast.Type {
	...
	if node.typ != ast.void_type {
		...
		if node.has_len {
			if node.has_len && !node.has_default {
				elem_type_sym := c.table.type_symbol(node.elem_type)
	...
	if node.exprs.len == 0 {
		...
		mut type_sym := c.table.type_symbol(c.expected_type)
	...
	if node.exprs.len > 0 && node.elem_type == ast.void_type {
		...
		if c.expected_type != 0 {
			...
			if c.table.type_symbol(expected_value_type).kind == .interface_ {
				// Array of interfaces? (`[dog, cat]`) Save the interface type (`Animal`)
				expecting_interface_array = true
			}
		}
		// expecting_interface_array := c.expected_type != 0 &&
		// c.table.type_symbol(c.table.value_type(c.expected_type)).kind ==			.interface_
		//
			...
			if !c.pref.is_repl && (c.stmt_level == 1 || (c.stmt_level > 1 && !c.is_last_stmt)) {
				if node.expr is ast.InfixExpr {
					if node.expr.op == .left_shift {
						left_sym := c.table.final_type_symbol(node.expr.left_type)
...
fn (mut c Checker) assert_stmt(node ast.AssertStmt) {
	...
	if assert_type != ast.bool_type_idx {
		atype_name := c.table.type_symbol(assert_type).name
...
fn (mut c Checker) for_in_stmt(mut node ast.ForInStmt) {
	...
	if node.is_range {
		...
		sym := c.table.final_type_symbol(typ)
		if sym.kind == .struct_ {
			// iterators
	...
	if node.cond is ast.InfixExpr {
		...
		if infix.op == .key_is {
			if infix.left in [ast.Ident, ast.SelectorExpr] && infix.right is ast.TypeNode {
				...
				left_sym := c.table.type_symbol(left_type)
				if is_variable {
					if left_sym.kind in [.sum_type, .interface_] {
...
pub fn (mut c Checker) expr(node ast.Expr) ast.Type {
	...
	match mut node {
		...
		ast.ArrayDecompose {
			...
			type_sym := c.table.type_symbol(typ)
		...
		ast.AsCast {
			...
			expr_type_sym := c.table.type_symbol(node.expr_type)
			type_sym := c.table.type_symbol(node.typ)
		...
		ast.ComptimeSelector {
			...
			expr_sym := c.table.type_symbol(expr_type)
		...
		ast.DumpExpr {
			...
			tsym := c.table.type_symbol(node.expr_type)
		...
		ast.Likely {
			...
			if !c.check_types(ltype, ast.bool_type) {
				ltype_sym := c.table.type_symbol(ltype)
...
pub fn (mut c Checker) cast_expr(mut node ast.CastExpr) ast.Type {
	...
	to_type := node.typ
	//
	from_type_sym := c.table.type_symbol(from_type)
	from_type_sym_final := c.table.final_type_symbol(from_type)
	to_type_sym := c.table.type_symbol(to_type) // type to be used as cast
	to_type_sym_final := c.table.final_type_symbol(to_type)
	//
	...
	node.typname = c.table.type_symbol(to_type).name
		...
		if obj := node.scope.find(node.name) {
			match mut obj {
				...
				ast.Var {
					...
					// if typ == ast.t_type {
					// sym := c.table.type_symbol(c.cur_generic_type)
...
fn (c Checker) smartcast(expr ast.Expr, cur_type ast.Type, to_type_ ast.Type, mut scope ast.Scope) {
	sym := c.table.type_symbol(cur_type)
	to_type := if sym.kind == .interface_ { to_type_.ref() } else { to_type_ }
	match expr {
		ast.SelectorExpr {
			mut is_mut := false
			mut smartcasts := []ast.Type{}
			expr_sym := c.table.type_symbol(expr.expr_type)
			mut orig_type := 0
			...
			ast.ExprStmt {
				if branch.is_timeout {
					if !branch.stmt.typ.is_int() {
						tsym := c.table.type_symbol(branch.stmt.typ)
...
pub fn (mut c Checker) prefix_expr(mut node ast.PrefixExpr) ast.Type {
	...
	if node.op == .amp && !right_type.is_ptr() {
		...
		if mut node.right is ast.IndexExpr {
			typ_sym := c.table.type_symbol(node.right.left_type)
			mut is_mut := false
	...
	right_sym := c.table.final_type_symbol(c.unwrap_generic(right_type))
...
fn (mut c Checker) check_index(typ_sym &ast.TypeSymbol, index ast.Expr, index_type ast.Type, pos token.Position, range_index bool) {
	index_type_sym := c.table.type_symbol(index_type)
...
pub fn (mut c Checker) index_expr(mut node ast.IndexExpr) ast.Type {
	...
	mut typ_sym := c.table.final_type_symbol(typ)
	node.left_type = typ
	for {
		match typ_sym.kind {
			.map {
			...
			.any {
				...
				node.left_type = typ
				typ_sym = c.table.final_type_symbol(typ)
				if typ.is_ptr() {
					continue
				} else {
	...
	if mut node.index is ast.RangeExpr { // [1..2]
		...
		if typ_sym.kind == .map {
			...
			value_sym := c.table.type_symbol(info.value_type)
...
pub fn (mut c Checker) enum_val(mut node ast.EnumVal) ast.Type {
	...
	mut typ_sym := c.table.type_symbol(typ)
...
pub fn (mut c Checker) chan_init(mut node ast.ChanInit) ast.Type {
	if node.typ != 0 {
		info := c.table.type_symbol(node.typ).chan_info()
...
pub fn (mut c Checker) offset_of(node ast.OffsetOf) ast.Type {
	sym := c.table.final_type_symbol(node.struct_type)
	if sym.kind != .struct_ {
...
pub fn (mut c Checker) map_init(mut node ast.MapInit) ast.Type {
	...
	if node.typ != 0 {
		info := c.table.type_symbol(node.typ).map_info()
	...
	if node.keys.len > 0 && node.vals.len > 0 {
		...
		use_expected_type := c.expected_type != ast.void_type && !c.inside_const
			&& c.table.type_symbol(c.expected_type).kind == .map
