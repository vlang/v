...
pub fn (mut c Checker) array_init(mut array_init ast.ArrayInit) ast.Type {
	...
	if array_init.exprs.len > 0 && array_init.elem_type == ast.void_type {
		...
		if array_init.is_fixed {
			idx := c.table.find_or_register_array_fixed(elem_type, array_init.exprs.len, ast.EmptyExpr{})
		...
		idx := c.table.find_or_register_array_fixed(array_init.elem_type, fixed_size, init_expr)
