...
pub fn (mut c Checker) assign_stmt(mut node ast.AssignStmt) {
	...
	for i, right in node.right {
		if right in [ast.CallExpr, ast.IfExpr, ast.LockExpr, ast.MatchExpr] {
			...
			right_type_sym := c.table.type_symbol(right_type)
	...
	for i, left in node.left {
		...
		if !is_decl && !is_blank_ident {
			...
			// `map = {}`
			if left_type != 0 {
				sym := c.table.type_symbol(left_type)
				if sym.kind == .map {
					if node.right.len <= i {
					...
					if left_type != 0 {
						match mut left.obj {
							ast.Var {
								...
								if !left_type.is_ptr() {
									if c.table.type_symbol(left_type).is_heap() {
		...
		left_sym := c.table.type_symbol(left_type_unwrapped)
		right_sym := c.table.type_symbol(right_type_unwrapped)
		...
		match node.op {
			...
			.mult_assign, .div_assign {
				if !left_sym.is_number() && !c.table.final_type_symbol(left_type_unwrapped).is_int()
					&& left_sym.kind !in [.struct_, .alias] {
					c.error('operator $node.op.str() not defined on left operand type `$left_sym.name`',
						left.position())
				} else if !right_sym.is_number()
					&& !c.table.final_type_symbol(left_type_unwrapped).is_int()
					&& left_sym.kind !in [.struct_, .alias] {
			...
			.right_shift_assign {
				if !left_sym.is_int() && !c.table.final_type_symbol(left_type_unwrapped).is_int() {
					c.error('operator $node.op.str() not defined on left operand type `$left_sym.name`',
						left.position())
				} else if !right_sym.is_int()
					&& !c.table.final_type_symbol(right_type_unwrapped).is_int() {
					c.error('operator $node.op.str() not defined on right operand type `$right_sym.name`',
						right.position())
				}
			}
		...
		if node.op in [.plus_assign, .minus_assign, .mod_assign, .mult_assign, .div_assign]
			...
			parent_sym := c.table.final_type_symbol(left_type_unwrapped)
		...
		if left_first is ast.Ident {
			...
			if right_node.op == .arrow {
				if assigned_var.is_mut {
					right_sym := c.table.type_symbol(right_type0)
