	...
	if node.kind == .fields {
		if sym.kind == .struct_ {
			...
			for field in sym_info.fields {
				// TODO: is this a typo? should `c.comptime_fields_type[node.val_var]` be set to field.typ?
				// c.comptime_fields_type[node.val_var] = field.typ.derive(node.typ).clear_flag(.generic)
