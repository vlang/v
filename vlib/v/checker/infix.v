...
pub fn (mut c Checker) infix_expr(mut node ast.InfixExpr) ast.Type {
	...
	if left_type.is_any_kind_of_pointer()
		...
		if !c.pref.translated && ((right_type.is_any_kind_of_pointer() && node.op != .minus)
			...
			if !c.inside_unsafe_block() && !node.left.is_auto_deref_var()
					...
					if right_final_sym.kind != .array {
						...
						if c.type_implements(right_type, left_value_type, right_pos) {
							if !right_type.is_ptr() && !right_type.is_pointer()
								&& !c.inside_unsafe_block() && right_sym.kind != .interface_ {
	...
	if node.left !is ast.ComptimeCall && node.right !is ast.ComptimeCall {
		if !(c.symmetric_check(left_type, right_type) && c.symmetric_check(right_type, left_type))
			...
			if left_sym.language == .v && !c.pref.translated && !c.inside_unsafe_block()
				&& !for_ptr_op && right_type.is_int() {
