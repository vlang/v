...
pub fn (mut c Checker) check_types(got ast.Type, expected ast.Type) bool {
	...
	if c.pref.translated {
		...
		if expected.is_any_kind_of_pointer() { //&& !got.is_any_kind_of_pointer() {
			...
			got_sym := c.table.type_symbol(got)
	...
	if got.is_number() && expected.is_number() {
		...
		if c.promote_num(expected, got) != expected {
			// println('could not promote ${c.table.type_symbol(got).name} to ${c.table.type_symbol(expected).name}')
			return false
		}
	}
...
pub fn (mut c Checker) check_expected_call_arg(got ast.Type, expected_ ast.Type, language ast.Language, arg ast.CallArg) ? {
	...
	if expected.has_flag(.variadic) {
		exp_type_sym := c.table.type_symbol(expected_)
	...
	if language == .c {
		...
		exp_sym := c.table.type_symbol(expected)
	...
	if c.check_types(got, expected) {
		if language != .v || expected.is_ptr() == got.is_ptr() || arg.is_mut
			...
			|| c.table.type_symbol(expected_).kind !in [.array, .map] {
			return
		}
	}
...
pub fn (mut c Checker) check_basic(got ast.Type, expected ast.Type) bool {
	...
	got_sym, exp_sym := c.table.type_symbol(got), c.table.type_symbol(expected)
...
fn (mut c Checker) check_shift(mut node ast.InfixExpr, left_type ast.Type, right_type ast.Type) ast.Type {
	...
	match node.op {
		.left_shift, .right_shift, .unsigned_right_shift {
			...
			left_sym_final := c.table.final_type_symbol(left_type)
...
pub fn (mut c Checker) get_default_fmt(ftyp ast.Type, typ ast.Type) byte {
	if ftyp.has_flag(.optional) {
		...
		mut sym := c.table.type_symbol(c.unwrap_generic(ftyp))
		if sym.kind == .alias {
			// string aliases should be printable
			info := sym.info as ast.Alias
			sym = c.table.type_symbol(info.parent_type)
	...
	for gi, gt_name in func.generic_names {
		...
		for i, param in func.params {
			...
			if i == 0 && node.is_method && param.typ.has_flag(.generic) {
				sym := c.table.type_symbol(node.receiver_type)
				match sym.info {
					ast.Struct, ast.Interface, ast.SumType {
						if c.table.cur_fn.generic_names.len > 0 { // in generic fn
							...
							receiver_generic_names := sym.info.generic_types.map(c.table.type_symbol(it).name)
			...
			if param.typ.has_flag(.generic) && param_type_sym.name == gt_name {
				...
				if param.typ.has_flag(.variadic) {
					...
					mut arg_elem_sym := c.table.type_symbol(arg_elem_info.elem_type)
					mut param_elem_sym := c.table.type_symbol(param_elem_info.elem_type)
					for {
						if arg_elem_sym.kind == .array && param_elem_sym.kind == .array
							...
							arg_elem_sym = c.table.type_symbol(arg_elem_info.elem_type)
							param_elem_info = param_elem_sym.info as ast.Array
							param_elem_sym = c.table.type_symbol(param_elem_info.elem_type)
						} else {
					...
					if param_map_info.key_type.has_flag(.generic)
						&& c.table.type_symbol(param_map_info.key_type).name == gt_name {
						typ = arg_map_info.key_type
					}
					if param_map_info.value_type.has_flag(.generic)
						&& c.table.type_symbol(param_map_info.value_type).name == gt_name {
					...
					generic_names := generic_types.map(c.table.type_symbol(it).name)
			...
			if to_set != ast.void_type {
				if typ != ast.void_type {
					...
					if !c.check_types(typ, to_set) {
						c.error('inferred generic type `$gt_name` is ambiguous: got `${c.table.type_symbol(to_set).name}`, expected `${c.table.type_symbol(typ).name}`',
							arg.pos)
					}
				}
				typ = to_set
