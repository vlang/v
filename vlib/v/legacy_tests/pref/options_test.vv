...
const vexe = @VEXE
const tfolder = os.join_path(os.vtmp_dir(), 'custom_compile')
...
fn custom_compile(fname string, cflags_options string) Results {
	...
	res.compilation =
		os.execute('${os.quoted_path(vexe)} -cflags "${cflags_options}" -o ${os.quoted_path(res.exe)} examples/hello_world.v')
