...
fn (mut list []CommentAndExprAlignInfo) add_info(attrs_len int, type_len int, line int) {
	...
	d_len := mu.iabs(list[i].max_attrs_len - attrs_len) + mu.iabs(list[i].max_type_len - type_len)
	...
	for i, field in node.fields {
		if i == node.mut_pos {
			...
			if node.fields[i - 1].comments.len > 0 {
				before_last_line = mu.imax(before_last_line, node.fields[i - 1].comments.last().pos.last_line)
			}
			if node.fields[i - 1].has_default_expr {
				before_last_line = mu.imax(before_last_line, node.fields[i - 1].default_expr.position().last_line)
			}

			mut next_first_line := field.pos.line_nr
			if field.comments.len > 0 {
				next_first_line = mu.imin(next_first_line, field.comments[0].pos.line_nr)
			}
