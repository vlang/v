	...
	for sym in g.table.type_symbols {
		...
		match sym.kind {
			.array {
				...
				elem_sym := g.table.type_symbol(info.elem_type)
...
pub fn (mut g Gen) write_interface_typesymbol_declaration(sym ast.TypeSymbol) {
	...
	for variant in info.types {
		vcname := g.table.type_symbol(variant).cname
...
fn (mut g Gen) stmt(node ast.Stmt) {
	...
	match node {
		...
		ast.InterfaceDecl {
			...
			ts := g.table.type_symbol(node.typ)
		...
		ast.StructDecl {
			...
			// field_type_sym := g.table.type_symbol(field.typ)
...
fn (mut g Gen) for_in_stmt(node ast.ForInStmt) {
	...
	if node.is_range {
		...
		val_sym := g.table.type_symbol(node.val_type)
		mut cond_var := ''
		...
		cond_sym := g.table.type_symbol(node.cond_type)
		...
		cond_type_sym := g.table.type_symbol(node.cond_type)
		...
		receiver_sym := g.table.type_symbol(receiver_typ)
...
fn (mut g Gen) get_sumtype_casting_fn(got_ ast.Type, exp_ ast.Type) string {
	...
	got_cname, exp_cname := g.table.type_symbol(got).cname, g.table.type_symbol(exp).cname
...
fn (mut g Gen) write_sumtype_casting_fn(fun SumtypeCastingFn) {
	...
	got_sym, exp_sym := g.table.type_symbol(got), g.table.type_symbol(exp)
	...
	for field in (exp_sym.info as ast.SumType).fields {
		...
		if embed_types.len > 0 {
			embed_sym := g.table.type_symbol(embed_types.last())
...
fn (mut g Gen) expr_with_cast(expr ast.Expr, got_type_raw ast.Type, expected_type ast.Type) {
	...
	got_sym := g.table.type_symbol(got_type)
	...
	if expected_type != ast.void_type {
		...
		unwrapped_exp_sym := g.table.type_symbol(unwrapped_expected_type)
		unwrapped_got_sym := g.table.type_symbol(unwrapped_got_type)
	...
	if got_is_ptr && !expected_is_ptr && neither_void && exp_sym.kind != .placeholder
		...
		deref_sym := g.table.type_symbol(got_deref_type)
...
fn (mut g Gen) gen_assign_stmt(assign_stmt ast.AssignStmt) {
	...
	if af {
		...
		first_left_sym := g.table.type_symbol(assign_stmt.left_types[0])
	...
	*/
	...
	if return_type != ast.void_type && return_type != 0 {
		sym := g.table.type_symbol(return_type)
		if sym.kind == .multi_return {
			// multi return
		...
		for i, left in assign_stmt.left {
			match left {
				ast.Ident {
					...
					left_sym := g.table.type_symbol(left_typ)
				...
				ast.IndexExpr {
					sym := g.table.type_symbol(left.left_type)
					if sym.kind == .array {
						info := sym.info as ast.Array
						elem_typ := g.table.type_symbol(info.elem_type)
						...
						val_typ := g.table.type_symbol(info.value_type)
						if val_typ.kind == .function {
							left_type := assign_stmt.left_types[i]
							left_sym := g.table.type_symbol(left_type)
							g.write_fn_ptr_decl(left_sym.info as ast.FnType, '_var_$left.pos.pos')
	...
	for i, left in assign_stmt.left {
		...
		left_sym := g.table.type_symbol(g.unwrap_generic(var_type))
		...
		right_sym := g.table.type_symbol(unwrapped_val_type)
		unaliased_right_sym := g.table.final_type_symbol(unwrapped_val_type)
			...
			if right_sym.kind == .function && is_decl {
				...
				if is_decl {
					...
					if ident.name !in g.defer_vars {
						val_sym := g.table.type_symbol(val_type)
...
fn (mut g Gen) gen_cross_tmp_variable(left []ast.Expr, val ast.Expr) {
	...
	match val {
		...
		ast.InfixExpr {
			sym := g.table.type_symbol(val.left_type)
...
fn (mut g Gen) gen_clone_assignment(val ast.Expr, typ ast.Type, add_eq bool) bool {
	...
	right_sym := g.table.type_symbol(typ)
	if g.is_autofree {
		if add_eq {
			g.write('=')
		}
...
fn (mut g Gen) map_fn_ptrs(key_typ ast.TypeSymbol) (string, string, string, string) {
	...
	match key_typ.kind {
		...
		.voidptr {
			ts := if g.pref.m64 {
				unsafe { g.table.type_symbol_by_idx(ast.u64_type_idx) }
			} else {
				unsafe { g.table.type_symbol_by_idx(ast.u32_type_idx) }
			}
	...
	match mut node {
		...
		ast.CallExpr {
			...
			if g.is_shared && !ret_type.has_flag(.shared_f) {
				ret_sym := g.table.type_symbol(ret_type)
		...
		ast.IsRefType {
			...
			sym := g.table.type_symbol(node_typ)
...
fn (mut g Gen) typeof_expr(node ast.TypeOf) {
	...
	if sym.kind == .sum_type {
		...
		varg_elem_type_sym := g.table.type_symbol(g.table.value_type(typ))
...
fn (mut g Gen) selector_expr(node ast.SelectorExpr) {
	...
	sym := g.table.type_symbol(g.unwrap_generic(node.expr_type))
	...
	if f := g.table.find_field(sym, node.field_name) {
		field_sym := g.table.type_symbol(f.typ)
			...
			if !prevent_sum_type_unwrapping_once {
				...
				if field := scope.find_struct_field(node.expr.str(), node.expr_type, node.field_name) {
					...
					for i, typ in field.smartcasts {
						...
						cast_sym := g.table.type_symbol(typ)
						if i != 0 {
							dot := if field.typ.is_ptr() { '->' } else { '.' }
							sum_type_deref_field += ')$dot'
						}
						if mut cast_sym.info is ast.Aggregate {
							agg_sym := g.table.type_symbol(cast_sym.info.types[g.aggregate_type_idx])
...
fn (mut g Gen) match_expr_sumtype(node ast.MatchExpr, is_expr bool, cond_var string, tmp_var string) {
	for j, branch in node.branches {
		...
		for {
			...
			if branch.is_else || (node.is_expr && is_last && tmp_var.len == 0) {
				...
				if sym.kind == .sum_type {
					...
					if branch.exprs[sumtype_index] is ast.TypeNode {
						...
						branch_sym := g.table.type_symbol(g.unwrap_generic(typ.typ))
			...
			if is_expr && tmp_var.len > 0 && g.table.type_symbol(node.return_type).kind == .sum_type {
...
fn (mut g Gen) map_init(node ast.MapInit) {
	...
	value_typ := g.table.type_symbol(unwrap_val_typ)
	key_typ := g.table.final_type_symbol(unwrap_key_typ)
...
fn (mut g Gen) ident(node ast.Ident) {
	...
	if node_info is ast.IdentVar {
		...
		if v := scope.find_var(node.name) {
			...
			if v.smartcasts.len > 0 {
				v_sym := g.table.type_symbol(v.typ)
				if !prevent_sum_type_unwrapping_once {
...
fn (mut g Gen) cast_expr(node ast.CastExpr) {
	...
	if sym.kind in [.sum_type, .interface_] {
		...
		g.expr(node.expr)
		g.write('))')
	} else if sym.kind == .alias && g.table.final_type_symbol(node.typ).kind == .array_fixed {
		g.expr(node.expr)
	} else {
		styp := g.typ(node.typ)
...
fn (g &Gen) expr_is_multi_return_call(expr ast.Expr) bool {
	match expr {
		ast.CallExpr { return g.table.type_symbol(expr.return_type).kind == .multi_return }
		else { return false }
	}
}
...
fn (mut g Gen) return_stmt(node ast.Return) {
	...
	if fn_return_is_multi && node.exprs.len > 0 && !g.expr_is_multi_return_call(node.exprs[0]) {
		...
		// typ_sym := g.table.type_symbol(g.fn_decl.return_type)
		...
		for i, expr in node.exprs {
			...
			if g.expr_is_multi_return_call(expr) {
				...
				expr_sym := g.table.type_symbol(c.return_type)
		...
		// normal return
		return_sym := g.table.type_symbol(node.types[0])
		expr0 := node.exprs[0]
...
fn (mut g Gen) struct_init(node ast.StructInit) {
	...
	mut sym := g.table.final_type_symbol(g.unwrap_generic(node.typ))
	is_amp := g.is_amp
	...
	if sym.kind == .struct_ {
		...
		for mut field in info.fields {
			...
			if field.name in inited_fields {
				...
				g.write('.$field_name = ')
				field_type_sym := g.table.type_symbol(sfield.typ)
				mut cloned := false
...
fn (mut g Gen) write_builtin_types() {
	...
	for builtin_name in c.builtins {
		sym := g.table.type_symbol_by_idx(g.table.type_idxs[builtin_name])
...
fn (mut g Gen) write_types(symbols []&ast.TypeSymbol) {
	for sym in symbols {
		...
		match mut sym.info {
			...
			ast.ArrayFixed {
				elem_sym := g.table.type_symbol(sym.info.elem_type)
...
fn (g &Gen) sort_structs(typesa []&ast.TypeSymbol) []&ast.TypeSymbol {
	...
	for sym in typesa {
		...
		match mut sym.info {
			ast.ArrayFixed {
				dep := g.table.type_symbol(sym.info.elem_type).name
			...
			ast.Struct {
				for embed in sym.info.embeds {
					dep := g.table.type_symbol(embed).name
				...
				for field in sym.info.fields {
					dep := g.table.type_symbol(field.typ).name
					// skip if not in types list or already in deps
	...
	for node in dep_graph_sorted.nodes {
		sorted_symbols << g.table.type_symbol_by_idx(g.table.type_idxs[node.name])
	}
	...
	match sym.kind {
		...
		.array {
			...
			if typ.has_flag(.shared_f) {
				atyp := '__shared__Array_${g.table.type_symbol(elem_typ).cname}'
		...
		.map {
			info := sym.map_info()
			key_typ := g.table.type_symbol(info.key_type)
			hash_fn, key_eq_fn, clone_fn, free_fn := g.map_fn_ptrs(key_typ)
			...
			if typ.has_flag(.shared_f) {
				mtyp := '__shared__Map_${key_typ.cname}_${g.table.type_symbol(info.value_type).cname}'
		...
		.struct_ {
			...
			if sym.language == .v && !typ_is_shared_f {
				for field in info.fields {
					...
					if field.has_default_expr
						...
						if field.has_default_expr {
							mut expr_str := ''
							if g.table.type_symbol(field.typ).kind in [.sum_type, .interface_] {
			...
			if typ.has_flag(.shared_f) {
				styp := '__shared__${g.table.type_symbol(typ).cname}'
...
fn (mut g Gen) go_expr(node ast.GoExpr) {
	...
	if expr.is_method {
		receiver_sym := g.table.type_symbol(expr.receiver_type)
	...
	if g.pref.obfuscate && g.cur_mod.name == 'main' && name.starts_with('main__') {
		mut key := expr.name
		if expr.is_method {
			sym := g.table.type_symbol(expr.receiver_type)
	...
	if node.call_expr.return_type == ast.void_type {
		...
		gohandle_name = '__v_thread_$opt${g.table.type_symbol(g.unwrap_generic(node.call_expr.return_type)).cname}'
	}
	if g.pref.os == .windows {
	...
	if should_register {
		...
		if expr.args.len > 0 {
			...
			for i in 0 .. expr.args.len {
				if g.table.type_symbol(expr.expected_arg_types[i]).kind == .interface_
					&& g.table.type_symbol(expr.args[i].typ).kind != .interface_ {
					has_cast = true
					break
				}
			}
...
fn (mut g Gen) as_cast(node ast.AsCast) {
	...
	styp := g.typ(node.typ)
	sym := g.table.type_symbol(node.typ)
	mut expr_type_sym := g.table.type_symbol(node.expr_type)
	if mut expr_type_sym.info is ast.SumType {
		...
		expected_sym := g.table.type_symbol(node.typ)
					...
					if _ := st_sym.find_field(field.name) {
						...
						if st == ast.voidptr_type {
							...
							for embed_type in st_sym.struct_info().embeds {
								embed_sym := g.table.type_symbol(embed_type)
	...
	return $cast_struct_str;
			...
			match st_sym.info {
				ast.Struct, ast.Interface, ast.SumType {
					if st_sym.info.parent_type.has_flag(.generic) {
						parent_sym := g.table.type_symbol(st_sym.info.parent_type)
			...
			for method in methods {
				...
				if inter_info.parent_type.has_flag(.generic) {
					parent_sym := g.table.type_symbol(inter_info.parent_type)
				...
				if !method.params[0].typ.is_ptr() {
					...
					if embed_types.len > 0 && method.name !in method_names {
						...
						for embed in embed_types {
							esym := g.table.type_symbol(embed)
...
pub fn (mut g Gen) get_array_depth(el_typ ast.Type) int {
	...
	sym := g.table.final_type_symbol(typ)
	if sym.kind == .array {
		info := sym.info as ast.Array
