	...
	for field in node.fields {
		...
		match field.expr {
			ast.ArrayInit {
				...
				if field.expr.is_fixed && !field.expr.has_init && g.pref.build_mode != .build_module
				...
				} else if field.expr.is_fixed && !field.expr.has_init
...
fn (mut g Gen) const_decl_init_later(mod string, name string, expr ast.Expr, typ ast.Type, unwrap_option bool, surround_cbr bool) {
	...
	if cname == '_const_os__args' {
		...
		} else if expr is ast.ArrayInit && (expr as ast.ArrayInit).has_init {
