...
fn (g &FlatGen) enum_autostr_c_name_for(type_name string, resolve_file_imports bool) string {
	...
	// name, or the helper is emitted as `Kind__autostr` and never defined. The
	// file's import wins even when the spelling also matches another module's
	// enum, because that is how the checker resolves the source spelling.
	if file_lookup && name.contains('.') {
