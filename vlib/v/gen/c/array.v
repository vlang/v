...
fn (mut g Gen) gen_array_map(node ast.CallExpr) {
	...
	ret_sym := g.table.type_symbol(node.return_type)
	inp_sym := g.table.type_symbol(node.receiver_type)
	...
	match mut expr {
		...
		ast.Ident {
			...
			if expr.kind == .function {
				...
				sym := g.table.type_symbol(var_info.typ)
...
fn (mut g Gen) gen_array_sort(node ast.CallExpr) {
	// println('filter s="$s"')
	rec_sym := g.table.type_symbol(node.receiver_type)
	...
	match mut expr {
		...
		ast.Ident {
			...
			if expr.kind == .function {
				...
				var_info := expr.var_info()
				sym_t := g.table.type_symbol(var_info.typ)
...
fn (mut g Gen) gen_array_insert(node ast.CallExpr) {
	...
	arg2_sym := g.table.type_symbol(node.args[1].typ)
...
fn (mut g Gen) gen_array_prepend(node ast.CallExpr) {
	...
	left_info := left_sym.info as ast.Array
	elem_type_str := g.typ(left_info.elem_type)
	arg_sym := g.table.type_symbol(node.args[0].typ)
...
fn (mut g Gen) get_array_contains_method(typ ast.Type) string {
	t := g.table.final_type_symbol(g.unwrap_generic(typ).set_nr_muls(0)).idx
...
fn (mut g Gen) gen_array_contains_methods() {
	...
	for t in g.array_contains_types {
		left_final_sym := g.table.final_type_symbol(t)
		if left_final_sym.idx in done || g.table.type_symbol(t).has_method('contains') {
			continue
		}
		done << t
		...
		elem_sym := g.table.type_symbol(left_info.elem_type)
...
fn (mut g Gen) gen_array_index_methods() {
	...
	for t in g.array_index_types {
		if t in done || g.table.type_symbol(t).has_method('index') {
			continue
		}
		done << t
		final_left_sym := g.table.final_type_symbol(t)
		mut left_type_str := g.typ(t)
...
fn (mut g Gen) gen_array_wait(node ast.CallExpr) {
	arr := g.table.type_symbol(node.receiver_type)
	thread_type := arr.array_info().elem_type
	thread_sym := g.table.type_symbol(thread_type)
	thread_ret_type := thread_sym.thread_info().return_type
	eltyp := g.table.type_symbol(thread_ret_type).cname
