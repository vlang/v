...
fn (mut g Gen) gen_fn_decl(node &ast.FnDecl, skip bool) {
	...
	*/
	...
	if node.is_method {
		unwrapped_rec_sym := g.table.type_symbol(g.unwrap_generic(node.receiver.typ))
		if unwrapped_rec_sym.kind == .placeholder {
			return
		}
		name = g.cc_type(node.receiver.typ, false) + '_' + name
		// name = g.table.type_symbol(node.receiver.typ).name + '_' + name
	}
	if node.language == .c {
	...
	if g.pref.obfuscate && g.cur_mod.name == 'main' && name.starts_with('main__') && !node.is_main
		...
		mut key := node.name
		if node.is_method {
			sym := g.table.type_symbol(node.receiver.typ)
		...
		for var in defer_stmt.defer_vars {
			...
			if var.kind == .variable {
				if var.name !in g.defer_vars {
					...
					if g.table.type_symbol(info.typ).kind != .function {
...
fn (mut g Gen) fn_args(args []ast.Param, scope &ast.Scope) ([]string, []string, []bool) {
	...
	for i, arg in args {
		...
		arg_type_sym := g.table.type_symbol(typ)
...
fn (mut g Gen) call_expr(node ast.CallExpr) {
	...
	if node.is_method && !node.is_field {
		if node.name == 'writeln' && g.pref.experimental && node.args.len > 0
			...
			&& g.table.type_symbol(node.receiver_type).name == 'strings.Builder' {
	...
	if gen_or { // && !g.autofree {
		...
		if unwrapped_typ == ast.void_type {
			g.write('\n $cur_line')
		} else if g.table.type_symbol(node.return_type).kind == .multi_return {
			g.write('\n $cur_line $tmp_opt /*U*/')
		} else {
			if !g.inside_const {
...
fn (mut g Gen) method_call(node ast.CallExpr) {
		...
		if node.left is ast.ComptimeSelector {
			...
			if node.left.obj is ast.Var {
				if g.comptime_var_type_map.len > 0 {
					...
					cast_sym := g.table.type_symbol(node.left.obj.smartcasts.last())
	...
	if g.is_autofree && node.free_receiver && !g.inside_lambda && !g.is_builtin_mod {
		...
		for embed in node.from_embed_types {
			embed_sym := g.table.type_symbol(embed)
...
fn (mut g Gen) fn_call(node ast.CallExpr) {
	...
	if is_print && (node.args[0].typ != ast.string_type || g.comptime_for_method.len > 0) { // && !free_tmp_arg_vars {
		...
		if typ != ast.string_type || g.comptime_for_method.len > 0 {
			...
			typ_sym := g.table.type_symbol(typ)
				...
				if expr is ast.ComptimeSelector {
					...
					if expr.obj is ast.Var {
						...
						if expr.obj.smartcasts.len > 0 {
							cast_sym := g.table.type_symbol(expr.obj.smartcasts.last())
							if cast_sym.info is ast.Aggregate {
					...
					ast.Var {
						if obj.smartcasts.len > 0 {
							...
							for i, typ in obj.smartcasts {
								cast_sym := g.table.type_symbol(g.unwrap_generic(typ))
								mut is_ptr := false
								if i == 0 {
									g.write(node.name)
								...
								if mut cast_sym.info is ast.Aggregate {
									sym := g.table.type_symbol(cast_sym.info.types[g.aggregate_type_idx])
...
fn (mut g Gen) call_args(node ast.CallExpr) {
	...
	if is_variadic {
		...
		arr_sym := g.table.type_symbol(varg_type)
...
fn (mut g Gen) keep_alive_call_pregen(node ast.CallExpr) int {
	...
	for i, arg in node.args {
		...
		typ := g.table.type_symbol(expected_type).cname
...
fn (mut g Gen) ref_or_deref_arg(arg ast.CallArg, expected_type ast.Type, lang ast.Language) {
	...
	exp_sym := g.table.type_symbol(expected_type)
	...
	if arg.is_mut && !arg_is_ptr {
		...
		if !g.is_json_fn {
			...
			arg_typ_sym := g.table.type_symbol(arg_typ)
			expected_deref_type := if expected_type.is_ptr() {
				expected_type.deref()
			} else {
				expected_type
			}
			deref_sym := g.table.type_symbol(expected_deref_type)
