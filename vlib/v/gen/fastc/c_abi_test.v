...
fn fastc_c_abi_check_source(host_os string, prelude string) string {
	...
	mut macros := ['O_RDONLY', 'O_WRONLY', 'O_RDWR', 'O_APPEND', 'O_CREAT', 'O_TRUNC', 'O_EXCL',
		...
		'STDERR_FILENO', 'FIONREAD', 'FD_SETSIZE', 'PTHREAD_CREATE_DETACHED', 'PTHREAD_PROCESS_PRIVATE']
	if macos {
		macros << ['MAP_ANON', 'F_SETLK', 'F_SETLKW', 'F_RDLCK', 'F_UNLCK', 'F_WRLCK', 'LOCK_SH',
			...
			'MACH_TASK_BASIC_INFO_COUNT', 'TASK_BASIC_INFO', 'KERN_SUCCESS', 'QOS_CLASS_USER_INITIATED',
			'RUSAGE_SELF']
	}
	for name in macros {
