...
fn (mut tc TypeChecker) check_comptime_static_metadata_if(node flat.Node, var_name string, loop_kind string, field_cases ComptimeStaticFieldCases, value_cases ComptimeStaticValueCases) {
	if loop_kind == 'values' {
		tc.check_comptime_static_value_metadata_if(node, var_name, loop_kind, field_cases, value_cases)
		return
	}
...
fn (mut tc TypeChecker) check_comptime_static_value_metadata_if(node flat.Node, var_name string, loop_kind string, field_cases ComptimeStaticFieldCases, value_cases ComptimeStaticValueCases) {
	...
	if check_then && node.children_count > 0 {
		tc.check_comptime_static_body(tc.a.child(&node, 0), var_name, loop_kind, field_cases, value_cases)
	}
	if check_else && node.children_count > 1 {
		tc.check_comptime_static_body(tc.a.child(&node, 1), var_name, loop_kind, field_cases, value_cases)
	}
}
...
fn (mut tc TypeChecker) check_comptime_static_call(id flat.NodeId, node flat.Node, var_name string, loop_kind string, field_cases ComptimeStaticFieldCases, value_cases ComptimeStaticValueCases) {
	...
	if loop_kind == 'methods' && callee_node.kind == .ident
		...
		tc.check_comptime_static_call_args(node, var_name, loop_kind, field_cases, value_cases)
		tc.record_error_at(.call_arg_mismatch, '`${callee_node.value}` can not print void expressions', id, node.pos)
		return
	}
...
fn (tc &TypeChecker) comptime_static_expr_has_void_method_call(id flat.NodeId, var_name string, value_cases ComptimeStaticValueCases) bool {
	...
	if node.kind == .paren && node.children_count > 0 {
		return tc.comptime_static_expr_has_void_method_call(tc.a.child(node, 0), var_name, value_cases)
	}
	return false
}
...
fn (mut tc TypeChecker) check_comptime_static_call_args(node flat.Node, var_name string, loop_kind string, field_cases ComptimeStaticFieldCases, value_cases ComptimeStaticValueCases) {
	for i in 1 .. node.children_count {
		tc.check_comptime_static_body(tc.call_arg_value(tc.a.child(&node, i)), var_name, loop_kind, field_cases, value_cases)
	}
}
...
fn (mut tc TypeChecker) comptime_static_enum_value_cases(base_type string) ComptimeStaticValueCases {
	...
	names := tc.enum_fields[enum_name] or {
...
fn (mut tc TypeChecker) comptime_static_field_cases(base_type string) ComptimeStaticFieldCases {
	...
	fields := tc.structs[struct_name] or {
		return ComptimeStaticFieldCases{
			known: true
		}
	}
	...
	for field in fields {
		...
		cases << ComptimeStaticFieldCase{
			name:          comptime_static_reflected_field_name(field.name, raw_typ, decl_meta.is_embed)
			typ:           typ
...
fn (tc &TypeChecker) comptime_static_enum_decl_value_cases(enum_name string) []ComptimeStaticValueCase {
	...
	if tc.top_level_idx.len > 0 {
		for idx in tc.top_level_idx {
			...
			if cases := tc.comptime_static_enum_decl_value_cases_for_node(enum_name, cur_mod, tc.a.nodes[idx]) {
				return cases
			}
		}
...
fn (tc &TypeChecker) comptime_static_enum_decl_value_cases_for_node(enum_name string, cur_mod string, node flat.Node) ?[]ComptimeStaticValueCase {
	...
	for field_name in field_order {
		...
		if expr_id := field_exprs[field_name] {
			...
			if ev := tc.comptime_static_enum_field_value(expr_id, cur_mod, enum_name, mut field_values, field_exprs, mut resolving) {
				val = ev
			}
		}
...
fn (tc &TypeChecker) comptime_static_enum_field_value(id flat.NodeId, enum_module string, enum_name string, mut field_values map[string]int, field_exprs map[string]flat.NodeId, mut resolving map[string]bool) ?int {
	...
	match node.kind {
		...
		.ident {
			...
			if ev := tc.comptime_static_enum_field_ref_value(node.value, enum_module, enum_name, mut field_values, field_exprs, mut resolving) {
				return ev
			}
			return none
		}
		...
		.selector {
			if field := tc.comptime_static_enum_selector_ref_field(id, enum_module, enum_name) {
				return tc.comptime_static_enum_field_ref_value(field, enum_module, enum_name, mut field_values, field_exprs, mut resolving)
			}
			return none
		}
		else {}
...
fn (tc &TypeChecker) comptime_static_enum_field_ref_value(field_name string, enum_module string, enum_name string, mut field_values map[string]int, field_exprs map[string]flat.NodeId, mut resolving map[string]bool) ?int {
	...
	val := tc.comptime_static_enum_field_value(expr_id, enum_module, enum_name, mut field_values, field_exprs, mut resolving) or {
...
fn (tc &TypeChecker) comptime_static_field_decl_metas(base_type string) map[string]ComptimeStaticFieldDeclMeta {
	...
	if tc.top_level_idx.len > 0 {
		for idx in tc.top_level_idx {
			...
			if metas := tc.comptime_static_field_decl_metas_for_node(decl_name, cur_mod, tc.a.nodes[idx]) {
				return metas
			}
		}
		return out
...
fn (mut tc TypeChecker) check_node(id flat.NodeId) {
	...
	if node.kind == .string_literal {
		...
		if !tc.string_literal_source_is_valid_utf8(literal_pos) {
			tc.record_notice_at(.unknown_ident, 'invalid utf8 byte sequence in string literal', id, literal_pos)
		}
		return
	}
	...
	if node.kind == .const_decl {
		...
		tc.record_error_at(.duplicate_decl, 'const can only be defined at the top level (outside of functions)', id, token.new_span(node.pos.id, node.pos.offset, node.pos.offset + 5))
		for i in 0 .. node.children_count {
	...
	if node.kind == .sizeof_expr {
		...
		if should_check_named_type(node.value) && !tc.type_name_known(node.value)
			...
			if node.value.len == 1 {
				...
				tc.record_error_severity_at(.compile_error, 'unknown type `${node.value}`', id, pos, 'cgen error:')
			} else {
	...
	if node.kind == .defer_stmt {
		if node.value.starts_with('invalid:') {
			...
			tc.record_error_at(.unknown_ident, 'unknown `defer` mode: `${mode}`', id, tc.defer_mode_diagnostic_pos(node, mode))
			return
		}
	...
	if node.kind == .string_interp {
		...
		for i in 0 .. node.children_count {
			...
			if (expr_type is Void && !tc.expr_subtree_has_undefined_variable_error(expr_id))
				...
				tc.record_error_at(.call_arg_mismatch, 'expression returning type `char` cannot be used in string interpolation directly, print its address or cast it to an integer instead', expr_id, tc.string_interpolation_expr_pos(expr_id))
			}
	...
	if node.kind == .paren && node.value == '__v3_comptime_d' && node.children_count > 0 {
		...
		if default.kind !in [.bool_literal, .char_literal, .float_literal, .string_literal,
			.int_literal] {
			tc.record_error_at(.assignment_mismatch, '$d() values can only be pure literals', id, node.pos)
		}
	...
	if node.kind == .array_literal {
		...
		if generic_name := tc.bare_generic_decl_type_name(node.value) {
			...
			if generic_name in tc.struct_generic_params || qualified in tc.struct_generic_params {
				tc.record_error_at(.unknown_type, 'generic struct `${generic_name}` must specify type parameter, e.g. ${generic_name}[int]', id, node.pos)
			}
		}
		if type_text_contains_any(node.value) {
			tc.record_notice_at(.unknown_type, 'the `any` type is deprecated and will be removed soon - either use an empty interface, or a sum type', id, node.pos)
	...
	if node.kind == .infix && node.op in [.logical_and, .logical_or] && node.children_count >= 2 {
		...
		if smartcasts.len > 0 {
			...
			tc.merge_unsafe_reference_alias_short_circuit_state(node.op, lhs_id, unsafe_alias_skipped_rhs)
	...
	for i in 0 .. node.children_count {
		...
		if node.kind == .infix && node.op in [.logical_and, .logical_or] && i == 1 {
			tc.merge_unsafe_reference_alias_short_circuit_state(node.op, tc.a.child(&node, 0), unsafe_alias_skipped_rhs)
			tc.merge_pointer_binding_short_circuit_state(node.op, tc.a.child(&node, 0), pointer_alias_skipped_rhs)
		}
	...
	if node.kind == .map_init {
		...
		if map_type is Map && unalias_type(map_type.value_type) is ResultType
			...
			tc.record_error_at(.unknown_type, 'cannot use Result type as map value type', id, node.pos)
		}
		if map_type is Map && unalias_type(map_type.key_type) is Struct {
			if map_type.key_type.name() != 'any' {
				tc.record_error_at(.unknown_type, 'map key type `${map_type.key_type.name()}` not supported', id, node.pos)
			}
		}
...
fn (mut tc TypeChecker) check_loop_control_statement(id flat.NodeId, node flat.Node) {
	...
	for _ in 0 .. 64 {
		...
		if parent.kind == .comptime_for {
			...
			tc.record_error_at(.assignment_mismatch, '${keyword} is not allowed within a compile-time loop', id, tc.loop_control_keyword_pos(id, keyword))
			return
		}
		if parent.kind == .defer_stmt {
			keyword := if node.kind == .break_stmt { 'break' } else { 'continue' }
			tc.record_error_at(.assignment_mismatch, '`${keyword}` is not allowed in defer statements', id, tc.loop_control_keyword_pos(id, keyword))
			break
		}
		if parent.kind in [.fn_decl, .fn_literal, .lambda_expr] {
			break
	...
	keyword := if node.kind == .break_stmt { 'break' } else { 'continue' }
	tc.record_error_at(.assignment_mismatch, '${keyword} statement not within a loop', id, tc.loop_control_keyword_pos(id, keyword))
}
...
fn (mut tc TypeChecker) check_missing_array_init_interface_type_args(id flat.NodeId, node flat.Node) {
	...
	tc.record_error_at(.unknown_type, 'generic interface `${name}` must specify type parameter, e.g. ${name}[int]', id, tc.type_diagnostic_pos(id, name))
}
...
fn (mut tc TypeChecker) check_map_none_values(node flat.Node) {
	...
	if inferred is Void {
		tc.record_error_at(.assignment_mismatch, 'map value cannot be only `none`', first_none_id, tc.a.node(first_none_id).pos)
		return
	}
	...
	for none_id in none_ids {
		tc.record_error_at(.assignment_mismatch, 'invalid map value: expected `${inferred.name()}`, not `none`', none_id, tc.a.node(none_id).pos)
	}
}
...
fn (mut tc TypeChecker) check_map_literal_element_types(id flat.NodeId, node flat.Node) {
	...
	for i := 0; i + 1 < node.children_count; i += 2 {
		...
		if map_type_from_receiver(tc.resolve_type(update_id)) == none {
			...
			tc.record_error_at(.assignment_mismatch, 'invalid map update: non-map type', update_id, update.pos)
		}
	}
...
fn (mut tc TypeChecker) check_map_literal_slot_type(value_id flat.NodeId, expected Type, slot string) {
	...
	tc.record_error_at(.assignment_mismatch, 'invalid map ${slot}: expected `${expected.name()}`, not `${tc.diagnostic_expr_type_name(value_id, actual)}`', value_id, value.pos)
}
...
fn (mut tc TypeChecker) check_spawn_expr(id flat.NodeId, node flat.Node) {
	...
	if child.kind == .or_expr && child.children_count >= 2 {
		...
		if call.kind == .call {
			...
			tc.record_error_at(.call_arg_mismatch, 'option handling cannot be done in `spawn` call. Do it when calling `.wait()`', child_id, tc.or_block_operator_pos(call_id, fallback_id))
			return
		}
	}
	error_count := tc.errors.len
	if child.kind != .call {
		tc.record_error_at(.call_arg_mismatch, 'expression in `spawn` must be a function call', child_id, child.pos)
	}
	if child.kind == .call {
		if info := tc.resolve_call_info(child_id, *child) {
			...
			if callee.kind == .selector && callee.children_count > 0
				...
				if receiver_type !is Pointer
					...
					tc.record_error_at(.call_arg_mismatch, 'method in `spawn` statement cannot have non-reference mutable receiver', receiver_id, tc.a.node(receiver_id).pos)
				}
			}
			for i in 1 .. child.children_count {
				...
				if arg.is_mut && tc.call_param_is_mut(info, param_idx)
					...
					tc.record_error_at(.call_arg_mismatch, 'function in `spawn` statement cannot contain mutable non-reference arguments', arg_id, arg.pos)
				}
			}
		}
	}
...
fn (mut tc TypeChecker) check_map_duplicate_keys(node flat.Node) {
	...
	for i := 0; i + 1 < node.children_count; i += 2 {
		...
		if duplicate {
			tc.record_error_at(.duplicate_decl, 'duplicate key "${display}" in map literal', key_id, key.pos)
		}
	}
}
...
fn (mut tc TypeChecker) check_array_literal_element_types(id flat.NodeId, node flat.Node) {
	if node.children_count == 0 {
		if node.typ.len == 0 {
			...
			tc.record_error_at(.assignment_mismatch, 'array_init: no type specified (maybe: `[]Type{}` instead of `[]`)', id, node.pos)
		}
		return
	}
	if node.typ.len > 0 {
		...
		if declared is ArrayFixed && declared_len != int(node.children_count) {
			tc.record_error_at(.assignment_mismatch, 'fixed array expects ${declared_len} value(s), but got ${node.children_count}', id, tc.fixed_array_value_list_pos(node))
		}
	}
	...
	if node.typ.len == 0 {
		...
		if tc.type_contains_open_generic_placeholder(array_type) {
			...
			if !tc.type_contains_open_generic_placeholder(refined) {
				array_type = refined
				tc.register_synth_type(id, refined)
			}
		}
	}
	...
	for i in 1 .. node.children_count {
		...
		if spread_id := tc.spread_arg_value(child_id) {
			...
			if spread_type is Array || spread_type is ArrayFixed {
				if !tc.type_compatible(spread_elem_type, elem_type) {
					tc.record_error_at(.assignment_mismatch, 'invalid array spread element: expected `${elem_type.name()}`, not `${spread_elem_type.name()}`', child_id, tc.array_element_diagnostic_pos(child_id))
				}
				continue
			}
		}
		...
		if actual is OptionType && elem_type !is OptionType {
			tc.record_error_at(.assignment_mismatch, 'cannot use `?${actual.base_type.name()}` as `${elem_name}`, it must be unwrapped first', child_id, tc.array_element_value_pos(child_id))
			continue
		}
		if elem_type is OptionType && actual !is OptionType {
			tc.record_error_at(.assignment_mismatch, 'cannot use `${actual_name}` as `?${elem_type.base_type.name()}`', child_id, tc.array_element_value_pos(child_id))
			continue
		}
		...
		if elem_type is Pointer && elem_type.base_type !is Void && actual !is Pointer {
			tc.record_error(.assignment_mismatch, 'cannot have non-pointer of type `${actual_name}` in a pointer array of type `&${elem_type.base_type.name()}`', child_id)
			continue
		}
		tc.record_error_at(.assignment_mismatch, 'invalid array element: expected `${elem_name}`, not `${actual_name}`', child_id, tc.array_element_diagnostic_pos(child_id))
	}
}
...
fn (mut tc TypeChecker) record_implicit_slice_clone_notice(id flat.NodeId) {
	...
	detail := compiler_errors.formatted_source_error('details:', 'To silence this notice, use either an explicit `a[..].clone()`,\nor use an explicit `unsafe{ a[..] }`, if you do not want a copy of the slice.', file, pos)
	tc.record_notice_with_details_at(.assignment_mismatch, 'an implicit clone of the slice was done here', id, pos, [
		detail,
	])
}
...
fn (mut tc TypeChecker) check_dump_expr(node flat.Node) {
	...
	if unalias_type(expr_type) is Char {
		tc.record_error_at(.assignment_mismatch, '`char` values cannot be dumped directly, use dump(u8(x)) or dump(int(x)) instead', expr_id, tc.a.node(expr_id).pos)
		return
	}
	...
	if private_type is Struct {
		if visibility := tc.private_declaration(private_type.name) {
			if tc.concrete_method_signature_key(private_type.name, 'str') == none {
				...
				tc.record_error_at(.assignment_mismatch, 'cannot dump private type `${type_name}` outside module `${module_name}` without an explicit `str()` method', expr_id, tc.a.node(expr_id).pos)
				return
			}
		}
	}
...
fn (mut tc TypeChecker) check_assert_stmt(node flat.Node) {
	...
	if node.children_count > 1 {
		...
		if !unalias_type(message_type).is_string() {
			tc.record_error_at(.condition_mismatch, 'assert allows only a single string as its second argument, but found `${tc.diagnostic_expr_type_name(message_id, message_type)}` instead', message_id, tc.a.node(message_id).pos)
		}
	}
	...
	if tc.expr_contains_invalid_variable_error(condition_id)
		...
		tc.record_error_at(.condition_mismatch, 'assert can be used only with `bool` expressions, but found `void` instead', condition_id, tc.assert_condition_diagnostic_pos(condition_id))
		return
	}
	...
	tc.record_error_at(.condition_mismatch, 'assert can be used only with `bool` expressions, but found `${condition_name}` instead', condition_id, tc.assert_condition_diagnostic_pos(condition_id))
}
...
fn (mut tc TypeChecker) check_string_interpolation_format(id flat.NodeId, node flat.Node, expr_id flat.NodeId) {
	...
	if letters.len > 1 {
		tc.record_error_at(.call_arg_mismatch, 'format specifier may only be one letter', id, node.pos)
		return
	}
	if letters.len == 0 {
		return
	...
	known := spec in [`d`, `u`, `x`, `X`, `o`, `b`, `c`, `s`, `S`, `e`, `E`, `f`, `F`, `g`, `G`,
		`r`, `R`, `p`]
	if !known {
		tc.record_error_at(.call_arg_mismatch, 'unknown format specifier `${spec.ascii_str()}`', id, spec_pos)
	}
	...
	if fmt.contains('.') && !clean.is_float() {
		tc.record_error_at(.call_arg_mismatch, 'precision specification only valid for float types', id, spec_pos)
		return
	}
	mut allowed := false
	if spec == `p`
		...
		allowed = true
	} else if actual.name() == 'voidptr' && spec in [`x`, `X`] {
		allowed = true
	...
	if !allowed {
		...
		tc.record_error_at(.call_arg_mismatch, 'illegal format specifier `${spec.ascii_str()}` for type `${actual_name}`', id, spec_pos)
	}
}
...
fn (mut tc TypeChecker) check_prefix_expr(id flat.NodeId, node flat.Node) {
	...
	if node.value == 'unexpected_amp' {
		tc.record_error_at(.assignment_mismatch, 'unexpected `&`, expecting expression', id, node.pos)
		return
	}
	...
	if node.op == .amp && child.kind == .paren && child.children_count == 1 {
		...
		if inner.kind == .struct_init {
			tc.record_notice_at(.assignment_mismatch, 'unnecessary `()`, use `&${inner.value}{....}` instead of `&(${inner.value}{....})`', child_id, child.pos)
		}
	}
	...
	if node.op == .amp && tc.node_source_starts_with(id, '&') && tc.unsafe_depth == 0
		...
		if fixed_array_id := tc.fixed_array_reference_ident(child_id) {
			if tc.fixed_array_reference_is_const(fixed_array_id) {
				...
				tc.record_error_at(.assignment_mismatch, 'cannot reference fixed array `${name}` outside `unsafe` blocks as it is supposed to be stored on stack', fixed_array_id, tc.node_value_diagnostic_pos(fixed_array_id))
				return
			}
		}
	}
	if node.op == .arrow {
		...
		if channel_type !is Channel {
			tc.record_error_at(.assignment_mismatch, 'operator `<-` can only be used with `chan` types, but the value after `<-` is of type `${child_type.name()}` instead', id, tc.prefix_operator_pos(id, '<-'))
		}
		return
	}
	...
	if node.op == .mul && child_type !is Pointer && child_type !is OptionType
		...
		tc.record_error_at(.assignment_mismatch, 'invalid indirect of `${child_type.name()}`, the type `${child_type.name()}` is not a pointer', id, tc.prefix_operator_pos(id, '*'))
		return
	}
	if node.op == .mul && child_type is Pointer {
		if child.kind == .nil_literal {
			tc.record_error_at(.assignment_mismatch, 'cannot deference a `nil` pointer', child_id, child.pos)
			return
		}
		diagnostic_name := tc.pointer_diagnostic_binding_type_name(child_id, child_type)
		if diagnostic_name == 'nil' {
			tc.record_error_at(.assignment_mismatch, 'cannot deference a `nil` pointer', id, tc.node_value_diagnostic_pos(child_id))
			return
		}
	...
	if node.op == .mul && child_type is OptionType {
		...
		if payload is Pointer {
			...
			tc.record_error_at(.assignment_mismatch, 'type `?${payload.base_type.name()}` is an Option, it must be unwrapped first; use `*${child_text}?` to do it', child_id, tc.node_value_diagnostic_pos(child_id))
	...
	if node.op == .amp && address_child.kind == .selector && address_child.children_count > 0 {
		...
		if selector_base.kind in [.int_literal, .float_literal, .bool_literal, .char_literal,
			...
			tc.record_error_at(.assignment_mismatch, 'cannot take the address of a literal value', id, token.new_span(node.pos.id, operator_pos.offset, address_child.pos.end))
			return
		}
		if declared := tc.selector_declared_value_type(address_child) {
			if unalias_type(declared) is OptionType {
				tc.record_error_at(.assignment_mismatch, 'cannot take the address of an Option field', id, node.pos)
				return
			}
		}
		if selector_base.kind == .struct_init {
			tc.record_error_at(.assignment_mismatch, 'should not create object instance on the heap to simply access a member', id, node.pos)
			return
		}
	}
	...
	if node.op == .amp && tc.unsafe_depth == 0 && !tc.expr_is_inside_unsafe_block(id)
		...
		tc.record_error_at(.assignment_mismatch, 'cannot take the address of map values outside `unsafe`', id, tc.address_operator_pos(id))
		return
	}
	if node.op == .amp && (tc.unsafe_depth > 0 || tc.expr_is_inside_unsafe_block(id))
		&& tc.address_path_contains_map_index(child_id) {
...
fn (mut tc TypeChecker) check_ownership_array_spread_clone(id flat.NodeId, node flat.Node) {
	...
	if bad_type := tc.ownership_default_clone_missing_method(array_type.elem_type) {
		tc.record_error(.call_arg_mismatch, 'cannot clone array spread elements: `${bad_type}` requires ownership destruction but has no `clone()` method', id)
	}
}
...
fn (mut tc TypeChecker) check_ownership_map_spread_clone(id flat.NodeId, node flat.Node) {
	...
	if bad_type := tc.ownership_default_clone_missing_method(map_type.key_type) {
		tc.record_error(.call_arg_mismatch, 'cannot clone map spread keys: `${bad_type}` requires ownership destruction but has no `clone()` method', id)
	}
...
fn (mut tc TypeChecker) check_cast_expr(id flat.NodeId, node flat.Node) {
	...
	if generic_name := tc.bare_generic_decl_type_name(node.value) {
		...
		if sum_params.len > 0 {
			tc.record_error_at(.unknown_type, 'generic sumtype `${generic_name}` must specify type parameter, e.g. ${generic_name}[int]', id, node.pos)
			return
		}
		...
		if params.len > 0 {
			...
			for method in tc.interface_abstract_method_names(interface_name) {
				if tc.concrete_method_signature_key(actual_name, method) == none {
					tc.record_error_at(.assignment_mismatch, 'can not find method `${method}` on `${actual_name.all_after_last('.')}`, needed for interface: `${generic_name.all_after_last('.')}`', id, node.pos)
					return
				}
			}
			tc.record_error_at(.unknown_type, 'could not infer generic type `${params[0]}` in interface `${generic_name}`', id, node.pos)
			return
		}
	}
	...
	if target is Unknown {
		if target_is_generic_param {
			...
			if fn_param_is_voidptr_type(actual_generic_source) {
				tc.record_error_at(.assignment_mismatch, 'cannot cast `voidptr` to struct', id, node.pos)
			}
			return
		}
	...
	if target is Alias && target.base_type is OptionType && !node.value.starts_with('?') {
		tc.record_error(.assignment_mismatch, 'alias to Option type requires to be used as Option type (?${node.value}(...))', id)
		return
	}
	actual := tc.resolve_type(child_id)
	if actual is Void {
		tc.record_error_at(.assignment_mismatch, 'expression does not return a value so it cannot be cast', child_id, tc.a.nodes[int(child_id)].pos)
		return
	}
	...
	if target_struct := struct_type_from_type(target) {
		...
		if actual_is_voidptr {
			...
			if target is Alias {
				tc.record_error_at(.assignment_mismatch, 'cannot cast `voidptr` to `${target.name}` (alias to `${target.base_type.name()}`)', id, node.pos)
			} else {
				tc.record_error_at(.assignment_mismatch, 'cannot cast `voidptr` to struct', id, node.pos)
			}
			_ = target_struct
	...
	if clean_pointer_target is Pointer {
		...
		if target_name.starts_with('&') && alias_target_text.starts_with('map[')
			...
			tc.record_error_at(.assignment_mismatch, 'cannot cast to alias pointer `${target_name}` because `${alias_target_text}` is a value', id, tc.cast_expression_diagnostic_pos(node, target_name))
			return
		}
		if target_base is Alias && unalias_type(target_base.base_type) is Map
			&& unalias_type(actual) is Map {
			tc.record_error_at(.assignment_mismatch, 'cannot cast to alias pointer `${target_name}` because `${target_base.base_type.name()}` is a value', id, node.pos)
			return
		}
	...
	if (clean_target is Array || clean_target is ArrayFixed)
		...
		if !points_to_target {
			...
			tc.record_error_at(.assignment_mismatch, 'cannot cast pointer type `${actual.name()}` to array type `${target.name()}`', id, token.new_pos(full_pos.id, full_pos.offset))
			return
		}
	}
	if clean_target is SumType && !tc.sum_type_contains_variant(clean_target, actual)
	...
	if tc.a.node(child_id).kind in [.int_literal, .float_literal, .bool_literal, .char_literal,
		.string_literal]
		&& (clean_target is Array || clean_target is ArrayFixed || clean_target is Map) {
		tc.record_error_at(.assignment_mismatch, 'cannot cast literal value to ${target.name()} type', id, node.pos)
		return
	}
	...
	if clean_target is Primitive
		...
		if clean_actual is Array {
			tc.record_error_at(.assignment_mismatch, 'cannot cast array `${actual.name()}` to `${target.name()}`', id, node.pos)
			return
		}
		...
		if clean_actual is SumType {
			tc.record_error_at(.assignment_mismatch, 'cannot cast `${actual.name()}` sum type value to `${target.name()}`, use `${tc.source_text_for_node(child_id)} as ${target.name()}` instead', id, node.pos)
			return
		}
	}
	if clean_target is Rune && clean_actual is Struct {
		tc.record_error_at(.assignment_mismatch, 'cannot cast struct `${actual.name()}` to `rune`', id, node.pos)
		return
	}
	if target is Alias && unalias_type(target.base_type) is String && clean_actual !is String {
		tc.record_error_at(.assignment_mismatch, 'cannot cast `${tc.diagnostic_expr_type_name(child_id, actual)}` to `${target.name}` (alias to `${target.base_type.name()}`)', id, node.pos)
		return
	}
	...
	if target_struct := struct_type_from_type(target) {
		...
		if target is Alias && clean_actual_pointer is Pointer {
			...
			if source_struct := actual_base {
				if source_struct.name == target_struct.name {
					...
					tc.record_error_at(.assignment_mismatch, 'cannot cast `${actual.name()}` to `${target.name}`, you must dereference it first (e.g. ${target.name}(*var))', id, node.pos)
					return
				}
			}
		}
		...
		if source_struct := actual_struct {
			...
			if source_struct.name != target_struct.name {
				tc.record_error(.assignment_mismatch, 'cannot convert struct `${actual.name()}` to struct `${target.name()}`', id)
			}
			return
		}
		...
		if unalias_type(actual) is SumType {
			tc.record_error(.assignment_mismatch, 'cannot cast `${actual.name()}` sum type value to `${target.name()}`, use `${tc.source_text_for_node(child_id)} as ${target.name()}` instead', id)
		}
		return
	}
	...
	if tc.interface_embedding_exceeds(target_iface.name, 0, mut embedding_path) {
		...
		tc.record_error_at(.unknown_type, 'too many interface embedding levels: 101, for interface `${target_iface.name.all_after_last('.')}`', decl_id, decl_pos)
		return
	}
	if clean_actual is FnType {
		tc.record_error_at(.assignment_mismatch, 'function does not implement interface `${target_iface.name}`; cannot implement interface `${target_iface.name}` using function', child_id, tc.a.node(child_id).pos)
		return
	}
...
pub fn (mut tc TypeChecker) check_interface_embedding_limits() bool {
	...
	for idx in tc.top_level_idx {
		...
		tc.errors << tc.make_type_error_at(.unknown_type, 'too many interface embedding levels: 101, for interface `${iface_name.all_after_last('.')}`', decl_id, decl_pos)
		return true
	}
	return false
}
...
fn (mut tc TypeChecker) record_interface_implementation_error_with_mut_receiver(kind TypeErrorKind, actual Type, expected Interface, id flat.NodeId, pos token.Pos, allow_mut_receiver bool) bool {
	...
	for method in tc.interface_abstract_method_names(expected_name) {
		...
		if message != '' {
			...
			tc.record_error_with_details_at(kind, message, id, pos, [
				'${expected_owner} has `${tc.interface_diagnostic_method_signature(expected_key, expected_owner, method, true)}`',
				'         ${actual_owner} has `${tc.interface_diagnostic_method_signature(actual_key, actual_owner, method, false)}`',
			])
			return true
		}
	}
	...
	for field in tc.interface_field_list(expected_name) {
		actual_field := tc.interface_actual_field(actual_name, field.name) or {
			tc.record_error_at(kind, "`${actual_display}` doesn't implement field `${field.name}` of interface `${expected_display}`", id, pos)
			return true
		}
		if !tc.type_compatible(actual_field.typ, field.typ)
			|| !tc.type_compatible(field.typ, actual_field.typ) {
			tc.record_error_at(kind, '`${actual_display}` incorrectly implements field `${field.name}` of interface `${expected_display}`, expected `${field.typ.name()}`, got `${actual_field.typ.name()}`', id, pos)
			return true
		}
		if field.is_mut && !actual_field.is_mut {
			tc.record_error_at(kind, '`${actual_display}` incorrectly implements interface `${expected_display}`, field `${field.name}` must be mutable', id, pos)
			return true
		}
	}
	return false
...
fn (mut tc TypeChecker) check_function_cast(id flat.NodeId, node flat.Node, child_id flat.NodeId, actual Type, target Type) bool {
	...
	if tc.expr_tail_is_nil(child_id) {
		...
		if !is_option {
			tc.record_warning_at(.assignment_mismatch, 'casting `nil` to function value should be done inside `unsafe{}` blocks', id, node.pos)
		}
		tc.record_error_at(.assignment_mismatch, '`nil` is only allowed in `unsafe` code', child_id, tc.a.node(child_id).pos)
		if is_option {
			tc.record_error_at(.assignment_mismatch, 'casting number to Option function is not allowed, only compatible function or `none`', id, tc.cast_expression_diagnostic_pos(node, node.value.trim_left('?')))
		}
		return true
	}
	if infix_power_type_is_numeric(actual) {
		...
		if is_option {
			...
			tc.record_warning_at(.assignment_mismatch, 'casting number to function value should be done inside `unsafe{}` blocks', id, node.pos)
		}
		return true
	}
	...
	if actual_fn := fn_type_from_type(actual) {
		if tc.unsafe_depth == 0
			...
			tc.record_error_at(.assignment_mismatch, 'casting a function value from one function signature, to another function signature, should be done inside `unsafe{}` blocks', id, if is_option {
...
fn (mut tc TypeChecker) warn_invalid_enum_cast(id flat.NodeId, node flat.Node, child_id flat.NodeId, target Enum) {
	...
	if !valid {
		tc.record_warning_at(.assignment_mismatch, '${value} does not represent a value of enum ${target.name}', id, node.pos)
	}
}
...
fn (mut tc TypeChecker) check_cast_to_string(id flat.NodeId, node flat.Node, child_id flat.NodeId, actual Type, target Type) bool {
	...
	if clean_actual is Enum {
		tc.record_error_at(.assignment_mismatch, 'cannot cast enum to string, use ${expr}.str() instead.', id, node.pos)
		return true
	}
	if clean_actual is SumType {
		tc.record_error_at(.assignment_mismatch, 'cannot cast sumtype `${actual.name()}` to string, use `${expr}.str()` instead.', id, node.pos)
		tc.record_error_at(.assignment_mismatch, 'cannot cast `${actual.name()}` sum type value to `string`, use `${expr} as string` instead', id, node.pos)
		return true
	}
	if clean_actual is Struct {
		tc.record_error_at(.assignment_mismatch, 'cannot cast struct `${actual.name()}` to `string`', id, node.pos)
		return true
	}
	...
	if clean_actual is Array && is_byte_type(clean_actual.elem_type) {
		tc.record_error_at(.assignment_mismatch, 'cannot cast []u8 to string, use `${expr}.bytestr()` or `${expr}.str()` instead.', id, node.pos)
		return true
	}
	if clean_actual is Pointer {
		if is_byte_type(unalias_type(clean_actual.base_type)) {
			tc.record_error_at(.assignment_mismatch, 'to convert a C string buffer pointer to a V string, use x.vstring() instead of string(x)', id, node.pos)
		}
		tc.record_error_at(.assignment_mismatch, 'cannot cast pointer type `${actual.name()}` to string, use `&u8(${expr}).vstring()` or `cstring_to_vstring(${expr})` instead.', id, node.pos)
		return true
	}
	if clean_actual is FnType {
		tc.record_error_at(.assignment_mismatch, 'cannot cast function `${expr}` to string', id, node.pos)
		return true
	}
	return false
}
...
fn (mut tc TypeChecker) check_cast_from_string(id flat.NodeId, node flat.Node, child_id flat.NodeId, actual Type, target Type) bool {
	...
	if clean_target is Primitive
		...
		tc.record_error_at(.assignment_mismatch, 'cannot cast string to `${target_name}`, use `${expr}.${target_name}()` instead.', id, node.pos)
		return true
	}
	if clean_target is Char {
		tc.record_error_at(.assignment_mismatch, 'cannot cast string to `char`, use `${expr}[index]` instead.', id, node.pos)
		return true
	}
	if clean_target is Rune {
		tc.record_error_at(.assignment_mismatch, 'cannot cast `string` to rune, use `${expr}.runes()` instead.', id, node.pos)
		return true
	}
	if clean_target is Enum {
		...
		tc.record_error_with_details_at(.assignment_mismatch, 'cannot cast `string` to `enum`', id, node.pos, [
	...
	if clean_target is Pointer {
		...
		if target_name in ['voidptr', 'byteptr', 'charptr'] {
			...
			tc.record_error_at(.assignment_mismatch, 'cannot cast string to `${target_name}`${outside_unsafe}, use ${target_name}(s.str) instead', id, node.pos)
		} else if struct_type_from_type(clean_target.base_type) != none {
			tc.record_error_at(.assignment_mismatch, 'cannot cast `${actual.name()}` to `${target_name}`', id, node.pos)
		} else {
			tc.record_error_at(.assignment_mismatch, 'cannot cast string to `${target_name}`, use `${expr}.str` instead.', id, node.pos)
		}
		return true
	}
	return false
...
fn (mut tc TypeChecker) warn_if_integer_literal_outside_known_type_range(id flat.NodeId, expected Type, warning_pos token.Pos) {
	...
	tc.record_warning_at(.call_arg_mismatch, 'value `${literal}` is outside the range of `${type_range.name}` in argument, this will be considered hard error soon', id, warning_pos)
	if integer_literal_overflows_signed_64(literal) {
		tc.record_error_at(.assignment_mismatch, 'integer literal ${literal} overflows int', id, tc.a.nodes[int(id)].pos)
	}
}
...
fn (mut tc TypeChecker) check_as_expr(id flat.NodeId, node flat.Node) {
	...
	if target_type_name.contains('.') && !interface_pattern_is_collapsed_container(node.value)
		...
		if target.len > 0 && field.len > 0 {
			...
			tc.record_error_with_details_at(.condition_mismatch, 'indeterminate `as` cast, use parenthesis to clarity', id, tc.as_operator_pos(id, node, child_id), [
	...
	if clean_child !is SumType && clean_child !is Interface {
		...
		tc.record_error_at(.assignment_mismatch, 'cannot cast non-sum type `${clean_child.name()}` using `as`${suffix}', id, tc.as_operator_pos(id, node, child_id))
		return
	}
	if clean_child is SumType {
		...
		if !tc.sum_type_contains_variant(clean_child, target) {
			tc.record_error_at(.assignment_mismatch, 'cannot cast `${clean_child.name}` to `${target.name()}`', id, tc.as_operator_pos(id, node, child_id))
			return
		}
	}
	if node.value.len == 0 || !interface_pattern_is_collapsed_container(node.value) {
...
fn (tc &TypeChecker) generic_type_arg_diagnostic_pos(id flat.NodeId) token.Pos {
	...
	file := tc.a.source_files[node.pos.id] or { return node.pos }
	source := tc.source_texts_by_file[file.name] or { return node.pos }
	return token.new_span(node.pos.id, int_max(0, node.pos.offset - 1), int_min(node.pos.end + 1, source.len))
}
...
fn (mut tc TypeChecker) check_in_expr(id flat.NodeId, node flat.Node) {
		...
		if container.kind == .array_literal {
			...
			for i in 0 .. container.children_count {
				...
				if should_check_named_type(pattern) && !tc.type_name_known(pattern) {
					tc.record_error_at(.unknown_type, tc.unknown_type_message(pattern, variant_id), variant_id, tc.node_value_diagnostic_pos(variant_id))
				} else {
					tc.record_error_at(.condition_mismatch, '`${value_sum_type.name}` has no variant `${tc.sum_variant_diagnostic_name(pattern)}`', variant_id, tc.node_value_diagnostic_pos(variant_id))
				}
			}
	...
	if tc.a.node(value_id).kind == .none_expr && container.kind == .none_expr {
		tc.record_error_at(.condition_mismatch, 'invalid operator `${op}` to `none` and `none`', id, node.pos)
		tc.record_error_at(.condition_mismatch, '`${op}` can only be used with arrays and maps', id, tc.in_operator_pos(value_id, container_id))
		return
	}
	if value_type is OptionType || value_type is ResultType {
		kind := if value_type is OptionType { 'Option' } else { 'Result' }
		tc.record_error_at(.condition_mismatch, 'unwrapped ${kind} cannot be used with `${op}`', value_id, tc.wrapped_operand_diagnostic_pos(value_id))
	...
	if container_type is Array || container_type is ArrayFixed {
		...
		if !pointer_element_compatible && !tc.expr_compatible(value_id, value_type, element_type) {
			tc.record_error_at(.condition_mismatch, 'left operand to `${op}` does not match the array element type: expected `${element_type.name()}`, not `${tc.diagnostic_expr_type_name(value_id, value_type_raw)}`', id, node.pos)
		}
		return
	}
	...
	tc.record_error_at(.condition_mismatch, '`${tc.diagnostic_expr_type_name(value_id, value_type)}` is an invalid type for range expression', id, tc.in_operator_pos(value_id, container_id))
}
...
fn (mut tc TypeChecker) check_in_array_duplicate_items(array flat.Node) {
	...
	for i in 0 .. array.children_count {
		...
		if seen[key] && !reported[key] {
			...
			tc.record_notice_at(.duplicate_decl, 'item `${display}` is duplicated in the list', item_id, item.pos)
			reported[key] = true
		}
		seen[key] = true
	}
...
fn (mut tc TypeChecker) check_comptime_match_diagnostics(id flat.NodeId, node flat.Node) bool {
	...
	mut has_error := false
	if explicit_mut {
		tc.record_error_at(.condition_mismatch, '`\$match` condition `${subject}` can not be mutable', id, subject_pos)
		has_error = true
	} else if tc.ident_is_mutable_lvalue(subject) {
		tc.record_error_at(.condition_mismatch, '`${subject}` is mut and may have changed since its definition', id, subject_pos)
		has_error = true
	} else if subject_is_unresolved_local && match_kind == 'value' {
		tc.record_error_at(.condition_mismatch, 'definition of `${subject}` is unknown at compile time', id, subject_pos)
		has_error = true
	}
	...
	for offset := 7; offset + 3 < metadata.len; offset += 4 {
		...
		if match_kind == 'value' && pattern.len >= 2 && pattern[0] in [`'`, `"`]
			&& subject_type != 'string' {
			tc.record_error_at(.condition_mismatch, 'can not matching a string value(`${pattern}`) in a non string type `\$match`, `${subject}` type is `${subject_type}`', id, pattern_pos)
			has_error = true
		}
	}
	return has_error
	...
	for op in [' !is ', ' is '] {
		...
		if left.starts_with('$') && !right.starts_with('$') {
			if !tc.lvalue_ident_is_known(right) {
				tc.record_error_at(.unknown_ident, 'undefined ident: `${right}`', id, tc.comptime_condition_part_pos(node, right))
				return true
			}
			return false
		}
...
fn comptime_condition_is_builtin_threads_guarded_with_negation(cond string, negated bool) bool {
	...
	or_idx := comptime_condition_top_level_index(clean, '||')
	if or_idx >= 0 {
		left := comptime_condition_is_builtin_threads_guarded_with_negation(clean[..or_idx], negated)
		right := comptime_condition_is_builtin_threads_guarded_with_negation(clean[or_idx + 2..], negated)
		return if negated { left || right } else { left && right }
	}
	and_idx := comptime_condition_top_level_index(clean, '&&')
	if and_idx >= 0 {
		left := comptime_condition_is_builtin_threads_guarded_with_negation(clean[..and_idx], negated)
		right := comptime_condition_is_builtin_threads_guarded_with_negation(clean[and_idx + 2..], negated)
		return if negated { left && right } else { left || right }
	}
	if clean.starts_with('!') {
...
fn (mut tc TypeChecker) check_infix(id flat.NodeId, node flat.Node) {
	...
	if lhs_node.kind == .none_expr && rhs_node.kind == .none_expr {
		...
		if node.op in [.left_shift, .right_shift, .right_shift_unsigned] {
			tc.record_error_at(.condition_mismatch, 'invalid operation: shift on type `none`', lhs_id, lhs_node.pos)
			tc.register_synth_type(id, Type(void_))
			return
		}
		if node.op in [.logical_and, .logical_or] {
			tc.record_error_at(.condition_mismatch, 'left operand for `${op}` is not a boolean', lhs_id, lhs_node.pos)
			tc.record_error_at(.condition_mismatch, 'right operand for `${op}` is not a boolean', rhs_id, rhs_node.pos)
			tc.register_synth_type(id, Type(bool_))
			return
		}
		if node.op == .arrow {
			tc.record_error_at(.assignment_mismatch, 'cannot push on non-channel `none`', lhs_id, lhs_node.pos)
			tc.register_synth_type(id, Type(void_))
			return
		}
		return
	...
	if lhs_is_nil || rhs_is_nil {
		...
		if node.op in [.eq, .ne] {
			...
			if clean_other !is Pointer && clean_other !is FnType && !mut_receiver
				&& !optional_pointer {
				tc.record_error_at(.condition_mismatch, 'cannot compare with `nil` because `${other_type.name()}` is not a pointer', id, tc.infix_operator_pos(node, op))
				return
			}
		} else {
			tc.record_error_at(.condition_mismatch, 'cannot use `${op}` with `nil`', id, tc.infix_operator_pos(node, op))
			return
		}
	}
	...
	if lhs_clean is MultiReturn || rhs_clean is MultiReturn {
		op := infix_operator_name(node.op) or { '' }
		tc.record_error_at(.assignment_mismatch, 'invalid number of operand for `${op}`. Only one allowed on each side.', id, node.pos)
	...
	if node.op == .plus && lhs_clean is Map && rhs_clean is Map {
		tc.record_error_at(.assignment_mismatch, 'undefined operation `${lhs_type.name()}` + `${rhs_type.name()}`', id, node.pos)
		tc.register_synth_type(id, Type(void_))
		return
	}
	if node.op == .left_shift && lhs_clean is OptionType
		&& unalias_type(lhs_clean.base_type) is Array {
		tc.record_error_at(.assignment_mismatch, 'cannot push to Option array that was not unwrapped first', lhs_id, tc.wrapped_operand_diagnostic_pos(lhs_id))
	...
	if node.op == .left_shift {
		if lhs_array := array_type_from_receiver(lhs_type) {
			if unalias_type(rhs_type) is OptionType
				...
				tc.record_error_at(.assignment_mismatch, 'unwrapped Option cannot be used in an infix expression', rhs_id, tc.infix_operator_pos(node, '<<'))
				return
			}
		}
	}
	if node.op == .plus && lhs_clean is OptionType && rhs_clean !is OptionType
		...
		tc.record_wrapped_infix_operand_error(lhs_id, lhs_type, tc.diagnostic_expr_type_name(rhs_id, rhs_type))
		if !compatible {
			tc.record_error(.assignment_mismatch, 'infix expr: cannot use `${tc.diagnostic_expr_type_name(rhs_id, rhs_type)}` (right expression) as `${payload.name()}`', id)
			tc.register_synth_type(id, Type(void_))
		}
		return
	}
	if node.op == .plus && rhs_clean is OptionType && lhs_clean !is OptionType
		...
		tc.record_wrapped_infix_operand_error(rhs_id, rhs_type, tc.diagnostic_expr_type_name(lhs_id, lhs_type))
		if !compatible {
			tc.register_synth_type(id, Type(void_))
		}
		return
	...
	if node.op in [.logical_and, .logical_or] {
		...
		if !rhs_is_bool {
			tc.record_error_at(.condition_mismatch, 'right operand for `${op}` is not a boolean', rhs_id, tc.a.node(rhs_id).pos)
			if node.op == .logical_or && rhs_clean !is MultiReturn {
				tc.record_error(.condition_mismatch, 'infix expr: cannot use `${tc.diagnostic_expr_type_name(rhs_id, rhs_type)}` (right expression) as `bool`', id)
			}
		}
	...
	if node.op == .plus && (lhs_clean is SumType || rhs_clean is SumType) {
		...
		if lhs_clean !is SumType {
			tc.record_error(.assignment_mismatch, 'infix expr: cannot use `${tc.diagnostic_expr_type_name(rhs_id, rhs_type)}` (right expression) as `${tc.diagnostic_expr_type_name(lhs_id, lhs_type)}`', id)
		}
		return
	}
	if node.op in [.eq, .ne] {
		...
		compatible := if lhs_is_sum != rhs_is_sum {
			...
			tc.type_compatible(lhs_type, rhs_type) || tc.type_compatible(rhs_type, lhs_type)
				...
				|| pointer_value_comparison_allowed || c_literal_scalar_comparison
		}
		...
		if !compatible && !unsafe_zero_struct_comparison
			...
			tc.record_error_at(.assignment_mismatch, 'infix expr: cannot use `${rhs_name}` (right expression) as `${lhs_name}`${suffix}', id, diagnostic_pos)
		}
		return
	}
	...
	if node.op in [.lt, .gt, .le, .ge] && lhs_is_fn_pointer && rhs_is_fn_pointer {
		...
		tc.record_error_at(.condition_mismatch, 'undefined operation `${lhs_name}` ${op} `${rhs_name}`', id, node.pos)
		return
	}
	...
	if node.op in [.lt, .gt, .le, .ge] && lhs_order_type is Struct && rhs_order_type is Struct {
		...
		if !tc.type_has_infix_operator_method(lhs_order_type, required_op) {
			...
			diagnostic_pos := if lhs_node.kind == .index && lhs_node.children_count > 1 {
				...
				token.new_span(node.pos.id, int_max(node.pos.offset, index_node.pos.offset - 1), node.pos.end)
			} else {
				node.pos
			}
			...
			if lhs_name == rhs_name {
				tc.record_error_at(.condition_mismatch, 'undefined operation `${lhs_name}` ${op} `${rhs_name}`', id, diagnostic_pos)
			} else {
	...
	if node.op == .arrow {
		...
		if channel_type !is Channel {
			tc.record_error_at(.assignment_mismatch, 'cannot push on non-channel `${lhs_type.name()}`', lhs_id, lhs_node.pos)
			tc.register_synth_type(id, Type(void_))
			return
		}
	...
	if node.op == .left_shift {
		if lhs_array := array_type_from_receiver(lhs_type) {
			...
			if const_id := tc.addressed_const_ident(rhs_id) {
				...
				tc.record_error_at(.assignment_mismatch, 'cannot have mutable reference to const `${const_node.value}`', const_id, tc.node_value_diagnostic_pos(const_id))
			}
			...
			if !tc.array_append_rhs_compatible(rhs_id, append_rhs_type, lhs_array.elem_type)
				...
				if elem_type is Interface {
					if !tc.record_interface_implementation_error(.assignment_mismatch, append_rhs_type, elem_type, diagnostic_id, tc.a.node(diagnostic_id).pos) {
						tc.record_error(.assignment_mismatch, 'cannot append `${rhs_name}` to `${lhs_type.name()}`', diagnostic_id)
					}
				} else {
					tc.record_error(.assignment_mismatch, 'cannot append `${rhs_name}` to `${lhs_type.name()}`', diagnostic_id)
				}
			...
			if !tc.array_append_is_standalone_statement(id) {
				tc.record_error_at(.assignment_mismatch, 'array append cannot be used in an expression', id, tc.infix_operator_pos(node, '<<'))
			}
	...
	if node.op in [.left_shift, .right_shift, .right_shift_unsigned] {
		...
		if !skip_inactive_compact_else && node.op != .right_shift_unsigned && bit_size > 0
			...
			if shift_count := v_int_literal_value(rhs_node.value) {
				if shift_count >= bit_size {
					tc.record_error_at(.assignment_mismatch, 'shift count for type `${lhs_type.name()}` too large (maximum: ${bit_size - 1} bits)', rhs_id, rhs_node.pos)
				}
			}
		}
		if !skip_inactive_compact_else && node.op == .left_shift && lhs_node.kind == .ident
			...
			tc.record_notice_at(.assignment_mismatch, 'shifting a value from a signed type `${lhs_type.name()}` can change the sign', lhs_id, tc.node_value_diagnostic_pos(lhs_id))
		}
	...
	if node.op in [.plus, .minus] && lhs_type.name() == 'voidptr' {
		op := infix_operator_name(node.op) or { '' }
		tc.record_error_at(.assignment_mismatch, '`${op}` cannot be used with `voidptr`', lhs_id, lhs_node.pos)
		return
	}
	if node.op == .mod && (unalias_type(lhs_type).is_float() || unalias_type(rhs_type).is_float()) {
		...
		tc.record_error_at(.assignment_mismatch, 'float modulo not allowed, use math.fmod() instead', float_id, tc.a.node(float_id).pos)
		return
	}
	if node.op in [.mul, .div, .mod] {
		...
		if lhs_pointer || rhs_pointer {
			op := infix_operator_name(node.op) or { '' }
			if lhs_pointer {
				tc.record_error(.assignment_mismatch, 'invalid operator `${op}` to `${lhs_type.name()}` and `${tc.diagnostic_expr_type_name(rhs_id, rhs_type)}`', id)
			}
			tc.record_error(.assignment_mismatch, 'infix `${op}` is not defined for pointer values', id)
			return
		}
	}
	if node.op in [.minus, .mul, .div, .mod] {
		...
		if node.op == .minus && lhs_type is Alias && rhs_type is Alias
			...
			tc.record_error(.assignment_mismatch, 'undefined operation `${lhs_type.name}` - `${rhs_type.name}`', id)
			return
		}
		...
		if lhs_clean is Struct && rhs_clean is Struct && lhs_name == rhs_name
			...
			op := infix_operator_name(node.op) or { '' }
			tc.record_error(.assignment_mismatch, 'undefined operation `${lhs_name}` ${op} `${rhs_name}`', id)
			return
		}
	...
	if node.op == .power {
		...
		if !infix_power_type_is_numeric(lhs_type) || !infix_power_type_is_numeric(rhs_type) {
			tc.record_error(.assignment_mismatch, 'operator `**` requires numeric operands; got `${lhs_type.name()}` and `${rhs_type.name()}`', id)
		}
		return
	}
	if node.op != .plus {
	...
	if lhs_clean is ResultType {
		...
		if tc.valid_node_id(parent_id) {
			...
			if parent.kind == .infix && parent.op == .plus
				...
				tc.record_error(.assignment_mismatch, 'operator `+` cannot concatenate `${lhs_type.name()}` and `${rhs_type.name()}`', parent_id)
			}
		}
	...
	if rhs_clean is OptionType {
		...
		if unalias_and_unwrap_pointer_type(lhs_type) is Struct && interface_base is Interface {
			...
			if tc.valid_node_id(parent_id) && tc.a.node(parent_id).kind == .infix {
				tc.record_error(.assignment_mismatch, 'infix expr: cannot use `${interface_base.name}` (right expression) as `${tc.diagnostic_expr_type_name(lhs_id, lhs_type)}`', parent_id)
			}
		}
		return true
	}
...
fn (mut tc TypeChecker) record_unwrapped_result_infix_error(id flat.NodeId, result_type ResultType) {
	tc.record_error_at(.assignment_mismatch, 'unwrapped Result cannot be used in an infix expression', id, tc.wrapped_operand_diagnostic_pos(id))
...
fn (mut tc TypeChecker) record_wrapped_infix_operand_error(id flat.NodeId, wrapped Type, expected_name string) {
	...
	tc.record_error_at(.assignment_mismatch, '`${display_name}` cannot be used as `${expected_name}`, unwrap the option first', id, tc.wrapped_operand_diagnostic_pos(id))
}
...
fn (mut tc TypeChecker) check_select_stmt(node flat.Node) {
	...
	if tc.valid_node_id(conflict_id) {
		tc.record_error(.condition_mismatch, '`else` and timeout value are mutually exclusive `select` keys', conflict_id)
	}
	if tc.valid_node_id(duplicate_timeout_id) {
		tc.record_error(.condition_mismatch, 'at most one timeout branch allowed in `select` block', duplicate_timeout_id)
	}
	$if ownership ? {
	...
	for i in 0 .. node.children_count {
		...
		if is_assignment_case && !has_receive_rhs {
			tc.record_error(.assignment_mismatch, 'select assignment case requires a channel receive on the right side', branch_id)
		...
		if branch.value.starts_with('recv_compound:') && has_receive_rhs {
			...
			tc.record_error(.assignment_mismatch, 'compound receive assignment `${op}` is not supported in `select`; use `=` or `:=`', branch_id)
			body_start = 2
		}
			...
			if branch.value == 'recv' {
				...
				if tc.valid_node_id(var_id) {
					...
					if var_node.kind == .ident && var_node.value.len > 0 {
						...
						tc.record_error(.assignment_mismatch, 'select receive declaration requires a plain identifier on the left side', var_id)
					}
				}
			} else {
				var_node := tc.a.node(var_id)
				if var_node.kind == .ident && var_node.value == '_' {
					tc.record_error_at(.assignment_mismatch, 'cannot send on `_`, use `_ := <- quit` instead', var_id, tc.node_value_diagnostic_pos(var_id))
					tc.check_node(recv_id)
				} else {
					...
					if !tc.assignment_types_compatible(recv_id, rhs_type, expected_type, .assign) {
						tc.type_mismatch(.assignment_mismatch, 'cannot assign `${rhs_type.name()}` to `${expected_type.name()}`', branch_id)
					}
					lhs_key := tc.expr_key(var_id)
					if lhs_key.len > 0 {
						tc.smartcasts.delete(lhs_key)
					}
					tc.update_pointer_binding_alias_assignment(var_id, recv_id, lhs_type, .assign, tc.fn_context.pointer_binding_value_keys)
				}
			}
			body_start = 2
		}
...
fn (mut tc TypeChecker) check_array_init(id flat.NodeId, node flat.Node) {
	if tc.fn_context.generic_params.len == 0
		...
		tc.record_error_at(.unsupported_generic, 'generic struct cannot be used in non-generic function', id, tc.array_init_head_pos(node))
	...
	if type_text_contains_any(node.typ) {
		...
		tc.record_error_at(.unknown_type, 'cannot use type `any` here', id, token.new_span(node.pos.id, node.pos.offset, end))
	}
	...
	for i in 0 .. node.children_count {
		...
		if child.kind == .field_init && child.value !in ['init', 'len', 'cap'] {
			tc.record_error_at(.unknown_field, 'wrong field `${child.value}`, expecting `len`, `cap`, or `init`', child_id, tc.node_value_diagnostic_pos(child_id))
		}
	}
		...
		for i in 0 .. node.children_count {
			...
			if field_name == 'init' {
				if expected := init_elem_type {
					...
					if actual !is Unknown && clean_actual !is OptionType
						...
						tc.record_error_at(.assignment_mismatch, 'expected `${expected.name()}`, not `${tc.diagnostic_expr_type_name(expr_id, actual)}`', expr_id, tc.a.node(expr_id).pos)
					}
				}
			}
			if field_name in ['len', 'cap'] {
				if value := tc.index_literal_value(expr_id) {
					if value < 0 {
						tc.record_error(.assignment_mismatch, 'array ${field_name} can not be negative', expr_id)
					}
				}
			}
...
fn (mut tc TypeChecker) check_array_element_reference_fields(id flat.NodeId, node flat.Node) {
	...
	tc.record_uninitialized_reference_field_notices(struct_elem, struct_elem.name, id, pos, mut checked)
}
...
fn (mut tc TypeChecker) record_uninitialized_reference_field_notices(struct_type Struct, linked_name string,
	...
	for field in tc.struct_fields_for_init(struct_type.name) {
		...
		if field_type is Pointer {
			tc.record_notice_at(.assignment_mismatch, 'reference field `${linked_name}.${field.name}` must be initialized (part of struct `${struct_type.name}`)', node_id, pos)
			continue
		}
		if field_type is Struct && !field.is_embed {
			tc.record_uninitialized_reference_field_notices(field_type, '${linked_name}.${field.name}', node_id, pos, mut checked)
		}
	}
}
	...
	if node.children_count >= 2 {
		...
		if inner.kind == .call && inner.children_count > 0 {
			...
			if callee.kind == .selector && callee.children_count > 0
				...
				tc.record_error_at(.assignment_mismatch, 'unexpected `or` block, the function `${callee.value}` does not return an Option or a Result', id, tc.or_block_operator_pos(inner_id, fallback_id))
				return
			}
		}
	}
	if node.children_count >= 2 && node.value !in ['!', '?']
		...
		if (source_type !is Unknown || unresolved_method_call) && tc.should_diagnose(inner_id) {
			if inner.kind == .call {
				...
				tc.record_error_at(.assignment_mismatch, 'unexpected `or` block, the function `${name}` does not return an Option or a Result', id, tc.or_block_operator_pos(inner_id, fallback_id))
				return
			} else if inner.kind == .ident {
				if smart_type := tc.smartcast_type(inner_id) {
					tc.record_error(.assignment_mismatch, 'cannot use `or {}` block on non-option variable `${inner.value}` (it was already unwrapped to `${smart_type.name()}` by an earlier none check; use `${inner.value}` directly)', inner_id)
				} else {
					tc.record_error(.assignment_mismatch, 'cannot use `or {}` block on non-option variable', inner_id)
				}
			} else {
				tc.record_error(.assignment_mismatch, 'unexpected `or` block, expression of type `${source_type.name()}` is not an Option or a Result', inner_id)
			}
		}
	}
	$if ownership ? {
...
fn (mut tc TypeChecker) check_option_propagation(id flat.NodeId, source_id flat.NodeId) {
	...
	if source.kind == .call && clean_source_type is OptionType {
		if tc.valid_node_id(propagation_parent_id)
			...
			tc.record_error_at(.return_mismatch, '`?` is not needed, use `return ${tc.call_display_name(*source)}()`', source_id, source.pos)
			return
		}
	...
	if source.kind == .call && clean_source_type !is OptionType && clean_source_type !is Unknown {
		...
		tc.record_error_at(.assignment_mismatch, 'unexpected `?`, the function `${name}` does not return an Option', id, tc.propagation_operator_pos(source_id, id, '?'))
		return
	}
	if source.kind == .ident {
		if smart_type := tc.smartcast_type(source_id) {
			...
			if tc.valid_node_id(parent_id) && tc.a.node(parent_id).kind == .return_stmt
				...
				tc.record_warning_at(.return_mismatch, 'unwrapping option is redundant as the function returns option', parent_id, token.new_span(return_line.id, return_line.offset, operator_pos.offset))
			}
			tc.record_error_at(.assignment_mismatch, 'cannot use `?` on non-option variable `${source.value}` (it was already unwrapped to `${smart_type.name()}` by an earlier none check; use `${source.value}` directly)', source_id, tc.node_value_diagnostic_pos(source_id))
			return
		}
		if clean_source_type is OptionType {
			...
			if tc.valid_node_id(parent_id) && tc.a.node(parent_id).kind == .return_stmt
				...
				if tc.valid_node_id(fn_id) {
					...
					detail := compiler_errors.formatted_source_error('details:', 'prepend ? before the declaration of the return type of `${fn_name}`', tc.a.source_files[return_pos.id] or {
						return
					}, return_pos)
					tc.record_error_with_details_at(.return_mismatch, 'to propagate the Option, `${fn_name}` must return an Option type', id, tc.node_value_diagnostic_pos(source_id), [
						detail,
					])
				}
			}
		...
		if clean_source_type !is ResultType {
			tc.record_error_at(.assignment_mismatch, 'cannot use `?` on non-option variable', source_id, tc.wrapped_operand_diagnostic_pos(source_id))
			return
		}
	}
	if source.kind == .selector && source_type !is OptionType {
		...
		tc.record_error_at(.assignment_mismatch, 'unexpected `?`, the field `${source.value}` is not an Option', id, tc.propagation_operator_pos(source_id, id, '?'))
		return
	}
	...
	tc.record_error_with_details_at(.return_mismatch, 'to propagate the call, `${fn_name}` must return an Option type', id, tc.propagation_operator_pos(source_id, id, '?'), [
		detail,
	])
}
...
fn (mut tc TypeChecker) check_result_propagation(id flat.NodeId, source_id flat.NodeId) {
	...
	if source.kind == .call && clean_source_type !is ResultType && clean_source_type !is Unknown {
		...
		tc.record_error_at(.return_mismatch, 'unexpected `!`, the function `${tc.call_display_name(*source)}` does not return a Result', id, tc.propagation_operator_pos(source_id, id, '!'))
		return
	}
...
fn (mut tc TypeChecker) record_nested_propagation_return_errors(id flat.NodeId, source_id flat.NodeId, wrapper string, marker string) {
	...
	fn_name := fn_node.value.all_after_last('.')
	return_pos := tc.fn_return_type_diagnostic_pos(*fn_node)
	detail := compiler_errors.formatted_source_error('details:', 'prepend ${marker} before the declaration of the return type of `${fn_name}`', tc.a.source_files[return_pos.id] or {
		return
	}, return_pos)
	operator_pos := tc.propagation_operator_pos(source_id, id, marker)
	article := if wrapper == 'Option' { 'an' } else { 'a' }
	tc.record_error_with_details_at(.return_mismatch, 'to propagate the call, `${fn_name}` must return ${article} ${wrapper} type', id, operator_pos, [
		detail,
	])
}
...
fn (mut tc TypeChecker) record_specific_propagation_return_error(id flat.NodeId, source_id flat.NodeId, wrapper string, marker string) {
	...
	operator_pos := tc.propagation_operator_pos(source_id, id, marker)
	article := if wrapper == 'Option' { 'an' } else { 'a' }
	tc.record_error_with_details_at(.return_mismatch, 'to propagate the ${wrapper} call, `${fn_name}` must return ${article} ${wrapper}', id, operator_pos, [
		detail,
	])
}
...
fn (mut tc TypeChecker) check_or_fallback_type(or_id flat.NodeId, source_id flat.NodeId, fallback_id flat.NodeId, outer_expected Type) {
	...
	if tail.kind == .assert_stmt && tc.direct_parent_kind(or_id) != .expr_stmt {
		...
		tc.record_error_at(.assignment_mismatch, 'last statement in the `or {}` block should be an expression of type `${expected.name()}` or exit parent scope', cond_id, token.new_span(tc.a.node(cond_id).pos.id, tc.a.node(cond_id).pos.offset, fallback.pos.end))
		return
	}
	...
	if tail.kind == .block && expected !is MultiReturn {
		tc.record_error_at(.assignment_mismatch, 'last statement in the `or {}` block should be an expression of type `${expected.name()}` or exit parent scope', tail_id, token.new_span(tail.pos.id, tail.pos.offset, tail.pos.offset + 1))
		return
	}
		...
		if groups := tc.tuple_tail_value_groups(fallback_id, expected.types.len, false) {
			if groups.len > 0 {
				...
				if !compatible {
					...
					tc.record_error_at(.assignment_mismatch, 'wrong return type `${actual_multi.name()}` in the `or {}` block, expected `${Type(expected).name()}`', values[0], tc.a.node(values[0]).pos)
					return
				}
				return
			}
	...
	if tc.or_expr_payload_is_shared(source_id) && !tc.expr_is_shared_arg(tail_id)
		...
		tc.record_error_at(.assignment_mismatch, 'wrong return type `${actual_name}` in the `or {}` block, expected `shared ${expected.name()}`', tail_id, tc.or_fallback_value_pos(tail_id, tail))
		return
	}
...
fn (mut tc TypeChecker) check_sql_expr(id flat.NodeId, node flat.Node) {
	...
	if node.children_count > 0 {
		...
		if db_type is OptionType {
			tc.record_error_at(.call_arg_mismatch, 'expected `${db_type.base_type.name()}`, not `?${db_type.base_type.name()}`', db_id, tc.a.node(db_id).pos)
			...
			if db_name.len > 0 && db_type !is Unknown
				...
				for method in tc.interface_abstract_method_names('orm.Connection') {
					tc.record_error_at(.call_arg_mismatch, "`${db_name}` doesn't implement method `${method}` of interface `orm.Connection`", db_id, tc.a.node(db_id).pos)
				}
			}
		}
	}
	...
	for i in 0 .. tokens.len {
		...
		rhs := tokens[i + 1]
		if rhs == 'none' {
			tc.record_warning_at(.condition_mismatch, 'comparison with none probably isn\'t intended; use "is none" and "!is none" to select by NULL', id, tc.sql_expr_text_pos(node, ' ${op} ', 1, op.len))
			continue
		}
		if rhs_type := tc.cur_scope.lookup(rhs) {
			if unalias_type(rhs_type) is OptionType {
				tc.record_warning_at(.condition_mismatch, 'comparison with Option value probably isn\'t intended; use "is none" and "!is none" to select by NULL', id, tc.sql_expr_text_pos(node, ' ${op} ', 1, op.len))
			}
		}
	}
...
fn (mut tc TypeChecker) check_sql_orm_table_kind(id flat.NodeId, node flat.Node, table_name string) {
	...
	tc.record_sql_error_at(.assignment_mismatch, 'ORM: the table symbol `${table_name}` has to be a struct', id, tc.sql_expr_text_pos(node, ' ${table_name}', 1, table_name.len))
}
...
fn (mut tc TypeChecker) check_sql_orm_struct_constraints(id flat.NodeId, table_name string) {
	...
	if pos := tc.declaration_attribute_without_value_pos(decl_id, 'table') {
		tc.record_sql_error_at(.assignment_mismatch, 'ORM: table attribute must have an argument', id, pos)
		return
	}
	...
	for i in 0 .. decl.children_count {
		...
		if struct_field_has_attr(field, 'primary') {
			if has_primary {
				tc.record_sql_error_at(.assignment_mismatch, 'ORM: a struct can only have one primary key', id, tc.sql_orm_field_declaration_pos(field))
				return
			}
			has_primary = true
		}
	...
	for field in fields {
		...
		if info.is_multidim {
			tc.record_sql_error_at(.assignment_mismatch, 'ORM: multi-dimension array fields are not supported', id, tc.sql_orm_field_declaration_pos(field))
			return
		}
		if info.base_name.len > 0 && tc.sql_orm_field_is_sub_struct(info.base_name)
			...
			tc.record_sql_error_at(.assignment_mismatch, 'ORM: invalid recursive struct `${info.base_name.all_after_last('.')}`', id, tc.sql_orm_field_declaration_pos(field))
			return
		}
	}
	for field in fields {
		...
		if info.is_array && !struct_field_has_attr(field, 'fkey') {
			tc.record_sql_error_at(.assignment_mismatch, 'ORM: array fields must have an fkey attribute', id, tc.sql_orm_field_declaration_pos(field))
			return
		}
		if info.is_array && !has_primary {
			tc.record_sql_error_at(.assignment_mismatch, 'ORM: a struct that has a field that holds an array must have a primary key', id, tc.sql_orm_field_declaration_pos(field))
			return
		}
		...
		if meta.len > 1 {
			for raw_attr in meta[1..] {
				if raw_attr.trim_space() == 'fkey' {
					tc.record_sql_error_at(.assignment_mismatch, 'ORM: the `fkey` attribute must have an argument', id, tc.sql_orm_field_attribute_pos(field, 'fkey'))
					return
				}
			}
		}
		if !info.is_array && !sql_orm_field_is_embedded(field)
			...
			if !foreign_has_primary {
				tc.record_sql_error_at(.assignment_mismatch, "ORM: struct `${info.base_name.all_after_last('.')}` used as ORM sub-struct field `${field.value}` must have a `@[primary]` field, or use `@[sql: '-']` to skip this field", id, tc.sql_orm_field_declaration_pos(field))
				return
			}
		}
	}
	...
	for i := 0; i + 3 < tokens.len; i++ {
		...
		mut actual := tc.smartcasts[value_name] or {
			tc.cur_scope.lookup(value_name) or {
				tc.record_sql_error_at(.unknown_ident, 'undefined ident: `${value_name}`', id, tc.sql_expr_text_pos(node, 'insert ${value_name}', 'insert '.len, value_name.len))
				continue
			}
		}
		clean_actual := unalias_type(actual)
		...
		tc.record_sql_error_at(.assignment_mismatch, 'cannot use `${actual.name()}` as `${expected.name()}`', id, tc.sql_expr_text_pos(node, 'insert ${value_name}', 'insert '.len, value_name.len))
	}
}
...
fn (mut tc TypeChecker) check_sql_update_fields(id flat.NodeId, node flat.Node, tokens []string) {
	for update_idx := 0; update_idx + 3 < tokens.len; update_idx++ {
		...
		for i := set_idx + 1; i + 1 < tokens.len && tokens[i] != 'where'; i++ {
			...
			tc.record_sql_error_at(.assignment_mismatch, "ORM: `${field.value}` is a foreign column of `${table_name}`, it can't update here", id, tc.sql_expr_text_pos(node, 'update ${table_name}', 'update '.len, table_name.len))
			return
		}
	}
}
...
fn (mut tc TypeChecker) check_sql_where_constraints(id flat.NodeId, node flat.Node, tokens []string) {
	...
	for i := where_idx + 1; i + 1 < tokens.len; i++ {
		...
		tc.record_sql_error_at(.unknown_field, message, id, tc.sql_expr_text_pos(node, ' ${tokens[i]} ', 1, tokens[i].len))
		break
	}
	for i := where_idx + 1; i + 2 < tokens.len; i++ {
		...
		if rhs in field_names && rhs != tokens[i] {
			tc.record_sql_error_at(.unknown_field, 'ORM: right side of the `${op}` expression cannot reference another `${table_name}` field; field-to-field comparisons are not supported', id, tc.sql_expr_text_pos(node, ' ${rhs}', 1, rhs.len))
		} else {
			tc.record_sql_error_at(.unknown_ident, 'undefined variable: `${rhs}`', id, tc.sql_expr_text_last_pos(node, rhs, rhs.len))
		}
		break
	}
	if !has_comparison && (tokens.len == 0 || tokens[0] != 'dynamic') {
		value := tokens[where_idx + 1]
		tc.record_sql_error_at(.condition_mismatch, 'ORM: `where` expression must have at least one comparison for filtering rows', id, tc.sql_expr_text_pos(node, 'where ${value}', 'where '.len, value.len))
	}
	for i := where_idx + 1; i + 1 < tokens.len; i++ {
		fn_name := tokens[i]
...
fn (mut tc TypeChecker) check_sql_order_by_constraint(id flat.NodeId, node flat.Node, tokens []string) {
	...
	for i := table_end; i + 2 < tokens.len; i++ {
		...
		tc.record_sql_error_at(.unknown_field, message, id, tc.sql_expr_text_pos(node, 'by ${field_name}', 'by '.len, field_name.len))
...
fn (mut tc TypeChecker) check_sql_limit_constraint(id flat.NodeId, node flat.Node, tokens []string) {
	for i := 0; i + 1 < tokens.len; i++ {
		...
		if limit < 0 {
			tc.record_sql_error_at(.assignment_mismatch, 'ORM: `limit` must be greater than or equal to zero', id, tc.sql_expr_text_pos(node, 'limit ${value}', 'limit '.len, value.len))
		}
	}
}
...
fn (mut tc TypeChecker) check_sql_aggregate_constraints(id flat.NodeId, node flat.Node, tokens []string) {
	for select_idx, token in tokens {
		...
		if close_idx != aggregate_idx + 3 || !should_check_named_type(tokens[aggregate_idx + 2]) {
			...
			tc.record_error_at(.assignment_mismatch, 'ORM aggregate functions only support a single field name argument', id, tc.sql_expr_text_pos(node, ' ${operator} ', 1, operator.len))
			continue
		}
		...
		if !clean_field.is_integer() && !clean_field.is_float() {
			tc.record_error_at(.assignment_mismatch, 'ORM: `${aggregate}` aggregate requires a numeric field', id, tc.sql_expr_head_pos(node))
		}
	}
}
...
fn (mut tc TypeChecker) check_sql_bulk_pointer_arrays(id flat.NodeId, node flat.Node, tokens []string) {
	for i := 0; i + 3 < tokens.len; i++ {
		if tokens[i] == 'insert' {
			...
			table_name, _ := sql_checker_table_type_token_from(tokens, i + 3) or { continue }
			tc.record_error_at(.assignment_mismatch, 'ORM: bulk insert currently supports only arrays of `${table_name}` values', id, tc.sql_expr_text_pos(node, ' ${value_name} ', 1, value_name.len))
		}
		if tokens[i] == 'update' {
			...
			if value_name.len > 0 {
				tc.record_error_at(.assignment_mismatch, 'ORM: bulk update currently supports only arrays of `${table_name}` values', id, tc.sql_expr_text_last_pos(node, value_name, value_name.len))
			}
		}
	}
}
...
fn (mut tc TypeChecker) check_sql_like_operands(id flat.NodeId, node flat.Node, tokens []string) {
	...
	for i := 1; i + 1 < tokens.len; i++ {
		...
		if !lhs_is_string_field {
			tc.record_error_at(.condition_mismatch, 'the left operand of the `${tokens[i]}` operator must be an identifier with a string type', id, tc.sql_expr_text_pos(node, ' ${lhs} ', 1, lhs.len))
		}
		rhs_type := tc.sql_like_token_type(rhs, table_type)
		if !((rhs.len >= 2 && rhs[0] in [`'`, `"`] && rhs[rhs.len - 1] == rhs[0])
			|| unalias_type(rhs_type) is String) {
			tc.record_error_at(.condition_mismatch, 'the right operand of the `${tokens[i]}` operator must be a string type', id, tc.sql_expr_text_pos(node, ' ${rhs}', 1, rhs.len))
		}
		if table_name.len > 0 && !tc.sql_table_has_field(table_type, lhs) {
			tc.record_error_at(.unknown_field, "ORM: left side of the `${tokens[i]}` expression must be one of the `${table_name}`'s fields", id, tc.sql_expr_text_pos(node, ' ${lhs} ', 1, lhs.len))
		}
	}
}
...
fn (mut tc TypeChecker) check_sql_anonymous_struct_fields(tokens []string) {
	...
	for i in 0 .. decl.children_count {
		...
		tc.record_error_at(.assignment_mismatch, "ORM: field `${field.value}` uses an anonymous struct type, which ORM does not support; use a named struct, or skip it with `@[skip]` or `@[sql: '-']`", field_id, tc.struct_field_declaration_pos(field))
	}
}
...
fn (mut tc TypeChecker) check_dynamic_sql_query_data_fields(id flat.NodeId, tokens []string) bool {
	...
	for i := 0; i + 1 < query_tokens.len; i++ {
		...
		if query_tokens[i + 1] !in ['==', '!=', '<', '>', '<=', '>=', 'in', '!in', 'like', 'ilike',
			'is', '!is', '=']
		...
		message :=
			util.new_suggestion(field_name, field_names).say('ORM: `${table_name}` structure has no field with name `${field_name}`')
		tc.record_error_at(.unknown_field, message, id, tc.dynamic_sql_field_pos(query_id, field_name))
		return true
	}
	return false
}
...
fn (tc &TypeChecker) sql_aggregate_or_expr_type(node flat.Node) ?Type {
	...
	for start, token in tokens {
		...
		field_type := tc.sql_aggregate_field_type(table_name, field_name) or { continue }
		return tc.parse_canonical_type(sql_aggregate_optional_type_name(tokens[select_start], field_type))
	}
	return none
}
...
fn (mut tc TypeChecker) check_asm_stmt(id flat.NodeId, node flat.Node) {
	...
	for line_start < end {
		...
		if trimmed == 'mov' {
			...
			tc.record_error_at(.assignment_mismatch, 'asm instruction `mov` expects 2 operands, but got 0', id, token.new_span(node.pos.id, line_start + relative, line_start + relative + 'mov'.len))
		}
		if line_end >= end {
			break
		}
...
fn (mut tc TypeChecker) check_fn_literal(id flat.NodeId, node flat.Node) {
	...
	for i in 0 .. node.children_count {
		...
		if param.value.len == 0 {
			tc.record_error_at(.duplicate_decl, 'use `_` to name an unused parameter', param_id, param.pos)
			continue
		}
	...
	for i in 0 .. node.children_count {
		...
		if capture.kind == .ident && capture.is_mut && capture.value.len > 0
			...
			capture_id := tc.a.child(&node, i)
			tc.record_error_at(.assignment_mismatch, 'original `${capture.value}` is immutable, declare it with `mut` to make it mutable', capture_id, tc.node_value_diagnostic_pos(capture_id))
		...
		if capture.kind == .ident && capture.value.len > 0 {
			if capture_type := tc.cur_scope.lookup(capture.value) {
				if tc.type_has_declaration_attribute(capture_type, 'nocopy') {
					capture_id := tc.a.child(&node, i)
					tc.record_error_at(.assignment_mismatch, 'cannot capture @[nocopy] struct by value: use a reference instead', capture_id, tc.node_value_diagnostic_pos(capture_id))
				}
					...
					if generic_name !in literal_generic_params
						...
						if !missing_capture_generic {
							if tc.checker_fixture_mode && literal_generic_params.len == 0 {
								tc.record_error_at(.unsupported_generic, 'generic closure fn must specify type parameter, e.g. fn [foo] [T]()', id, node.pos)
							} else {
								capture_id := tc.a.child(&node, i)
	...
	for i in 0 .. node.children_count {
		...
		key := owner.storage_key()
		if key.len > 0 {
			captured_pointer_values[key] = tc.pointer_binding_ident_values(owner, saved_fn_context.pointer_binding_value_keys)
		}
	}
	...
	if !type_allows_implicit_return(return_type) && !missing_capture_generic {
		...
		if !tc.stmt_sequence_definitely_returns(&node, body_start) {
			tc.record_error_at(.return_mismatch, 'missing return at the end of an anonymous function', id, node.pos)
		}
	}
	for i in 0 .. node.children_count {
		capture_id := tc.a.child(&node, i)
...
fn (mut tc TypeChecker) record_invalid_comptime_for_type(decl VisibleMutationFnDecl, generic_name string) bool {
	...
	for stack.len > 0 {
		...
		if node.kind == .comptime_for && node.typ.len > 0 {
			...
			tc.record_error_at(.unknown_type, '\$for expects a type name or variable name to be used here, but ${generic_name} is not a type or variable name', id, pos)
			return true
		}
		for i in 0 .. node.children_count {
			stack << tc.a.child(node, i)
...
fn (mut tc TypeChecker) record_malformed_fn_literal_call_mismatch(literal flat.Node, param_id flat.NodeId, param flat.Node) {
	...
	tc.record_error_at(.call_arg_mismatch, 'cannot use `${actual.name()}` as `${param.typ}` in argument 1 to `${name}`', param_id, token.new_span(literal.pos.id, arg_start, arg_end))
}
...
fn (mut tc TypeChecker) check_lambda_expr(id flat.NodeId, node flat.Node) {
	...
	if tc.lambda_is_uncontextualized_if_branch_value(id) {
		tc.record_error_at(.call_arg_mismatch, 'lambda expressions are allowed only in places expecting function callbacks', id, node.pos)
	}
	...
	if (body_type is OptionType || body_type is ResultType)
		...
		tc.record_error_at(.return_mismatch, 'cannot use `${body_type.name()}` as type `${expected_fn.return_type.name()}` in return argument', body_id, tc.a.node(body_id).pos)
	}
	tc.pop_scope()
	$if ownership ? {
		tc.ownership_end_fn()
...
fn (mut tc TypeChecker) check_for_stmt(node flat.Node) {
	...
	if loop_may_skip_body || body_may_break {
		for state in pointer_alias_continue_states {
			pointer_alias_paths << tc.pointer_binding_state_after_loop_post(node, state, pointer_alias_base, pointer_alias_post, has_cond_smartcasts, saved_smartcasts)
		}
	}
	if body_reaches_post && (loop_may_skip_body || body_may_break) {
		unsafe_alias_paths << apply_unsafe_reference_alias_state_delta(unsafe_alias_base, unsafe_alias_post, unsafe_alias_body)
		pointer_alias_paths << tc.pointer_binding_state_after_loop_post(node, pointer_alias_body, pointer_alias_base, pointer_alias_post, has_cond_smartcasts, saved_smartcasts)
	}
...
fn (mut tc TypeChecker) check_lock_expr(id flat.NodeId, node flat.Node) {
	...
	if tc.lock_depth > 0 {
		tc.record_error_at(.assignment_mismatch, 'nested `lock`/`rlock` not allowed', id, tc.lock_keyword_diagnostic_pos(node))
	}
	...
	for i in 0 .. node.children_count - 1 {
		...
		if !is_shared {
			...
			tc.record_error_at(.assignment_mismatch, '`${tc.source_text_for_node(object_id)}` must be declared as `shared` ${object_type} to be locked', object_id, object.pos)
		}
		if is_shared {
			if unstable_id := tc.lock_object_unstable_dependency(object_id) {
				tc.record_error_at(.assignment_mismatch, 'cannot lock `${tc.source_text_for_node(object_id)}`: selector bases and indices must be stable expressions', unstable_id, tc.a.node(unstable_id).pos)
			}
		}
	...
	match node.kind {
		...
		.index {
			for i in 0 .. int_min(node.children_count, 2) {
				tc.record_locked_shared_dependency_keys(tc.a.child(node, i), lock_name, mut added_keys)
			}
		}
		.paren, .prefix {
			if node.children_count > 0 {
...
fn (tc &TypeChecker) locked_shared_struct_field_path(struct_name string, field_name string, mut seen map[string]bool) ?[]StructField {
	...
	for field in fields {
		...
		if clean_embedded_type is Struct {
			if path := tc.locked_shared_struct_field_path(clean_embedded_type.name, field_name, mut seen) {
...
fn (tc &TypeChecker) locked_shared_base_owner_alias_keys(id flat.NodeId) []string {
	...
	match node.kind {
		.ident {
			...
			mut keys := []string{}
			for value_key in tc.pointer_binding_ident_values(owner, tc.fn_context.pointer_binding_value_keys) {
				if value_key.len > 0 {
...
fn (mut tc TypeChecker) check_for_in_stmt(node flat.Node) {
	...
	if header == 4 {
		...
		if raw_container_type is ResultType {
			pos := token.new_span(container_node.pos.id, container_node.pos.offset + 3, container_node.pos.offset + 10)
			tc.record_error_at(.cannot_index, 'for in: cannot index `${raw_container_name}`', container_id, pos)
			...
			if option_base !is Array && option_base !is ArrayFixed {
				tc.record_error_at(.cannot_index, 'for in: cannot index `${raw_container_name}`', container_id, container_node.pos)
			}
			clean = option_base
		}
		...
		if same_low && container_node.kind == .range {
			...
			if has_val {
				tc.record_error_at(.cannot_index, 'for in: cannot index `int`', container_id, tc.node_value_diagnostic_pos(container_id))
				tc.insert_loop_var(key_id, Type(int_))
			...
			tc.check_for_in_mutable_container(node, key_id, val_id, container_id, clean)
			if !has_val {
				tc.record_error_at(.cannot_index, 'declare a key and a value variable when ranging a map: `for key, val in map {`\nuse `_` if you do not need the variable', key_id, tc.node_value_diagnostic_pos(key_id))
			}
			...
			tc.record_error_with_details_at(.cannot_index, 'cannot infer from generic type `${generic_name}`', key_id, tc.node_value_diagnostic_pos(key_id), [
			...
			if unalias_type(elem_type) is MultiReturn {
				tc.record_error_at(.cannot_index, 'iterator method `next()` must not return multiple values', container_id, container_node.pos)
			}
			if has_val {
				tc.insert_loop_var(key_id, Type(int_))
				if node.op == .amp {
...
fn (mut tc TypeChecker) check_for_in_const_conflicts_preflight() {
	for i in tc.preflight_nodes(.for_in_stmt) {
		...
		for child_index in 0 .. count {
			...
			tc.record_error_at(.duplicate_decl, 'duplicate of a const name `${binding.value}`', flat.NodeId(i), tc.node_value_diagnostic_pos(binding_id))
...
fn (mut tc TypeChecker) check_for_in_binding_name(id flat.NodeId, role string) {
	...
	if reserved_const_type_name(node.value) {
		tc.record_error_at(.duplicate_decl, 'invalid use of reserved type `${node.value}` as ${role} name', id, tc.node_value_diagnostic_pos(id))
		return
	}
	...
	if parent != unsafe { nil } {
		for name in parent.names {
			if name == node.value {
				tc.record_error_at(.duplicate_decl, 'redefinition of ${role} iteration variable `${node.value}`', id, tc.node_value_diagnostic_pos(id))
				return
			}
		}
	}
...
fn (mut tc TypeChecker) check_for_in_same_variable(header int, key_id flat.NodeId, val_id flat.NodeId, low_id flat.NodeId, high_id flat.NodeId) (bool, bool) {
	...
	if same_low {
		...
		tc.record_error_at(.cannot_index, 'in a `for x in ${kind}` loop, the key or value iteration variable `${iteration_name}` can not be the same as the low variable', actual_low_id, tc.node_value_diagnostic_pos(actual_low_id))
	}
	...
	if same_high && !same_low {
		tc.record_error_at(.cannot_index, 'in a `for x in <range>` loop, the key or value iteration variable `${iteration_name}` can not be the same as the high variable', actual_high_id, tc.node_value_diagnostic_pos(actual_high_id))
	}
...
fn (mut tc TypeChecker) check_for_in_mutable_container(loop flat.Node, key_id flat.NodeId, val_id flat.NodeId, container_id flat.NodeId, container_type Type) {
	...
	if unalias_type(container_type) is String {
		tc.record_error_at(.assignment_mismatch, 'string type is immutable, it cannot be changed', binding_id, tc.for_in_mut_keyword_pos(binding_id))
		return
	}
	...
	if container.kind == .ident {
		tc.record_error_at(.assignment_mismatch, '`${container.value}` is immutable, it cannot be changed', container_id, tc.node_value_diagnostic_pos(diagnostic_id))
		return
	}
	if container.kind == .selector && container.children_count > 0 {
		...
		base_type := unalias_and_unwrap_pointer_type(tc.resolve_type(base_id))
		if base_type is Struct {
			tc.record_error_at(.assignment_mismatch, 'field `${base_type.name}.${container.value}` is immutable, it cannot be changed', container_id, tc.node_value_diagnostic_pos(diagnostic_id))
			return
		}
	}
	if container.kind == .map_init {
		pos := token.new_span(container.pos.id, container.pos.offset, container.pos.offset + 1)
		tc.record_error_at(.assignment_mismatch, 'map literal is immutable, it cannot be changed', container_id, pos)
		return
	}
	if container.kind in [.array_literal, .array_init] {
		tc.record_error_at(.assignment_mismatch, 'array literal is immutable, it cannot be changed', container_id, tc.a.node(container_id).pos)
	}
}
...
fn (mut tc TypeChecker) record_for_in_invalid_selector(id flat.NodeId, node flat.Node) {
	...
	tc.record_error_at(.cannot_index, '`${base.value}` does not return a value', id, tc.node_value_diagnostic_pos(id))
}
...
fn (mut tc TypeChecker) check_for_in_range_types(low_id flat.NodeId, high_id flat.NodeId) {
	...
	if low_type is MultiReturn || high_type is MultiReturn {
		tc.record_error_at(.condition_mismatch, 'multi-returns cannot be used in ranges. A range is from a single value to a single higher value.', low_id, tc.range_endpoints_pos(low_id, high_id))
		return
	}
	...
	if low_type is OptionType || low_type is ResultType {
		tc.record_error_at(.condition_mismatch, 'the `low` value in a `for x in low..high {` loop, cannot be Result or Option', low_id, tc.a.node(low_id).pos)
		return
	}
	if high_type is OptionType || high_type is ResultType {
		tc.record_error_at(.condition_mismatch, 'the `high` value in a `for x in low..high {` loop, cannot be Result or Option', high_id, tc.range_bound_diagnostic_pos(high_id))
		return
	}
	...
	if low_is_numeric && high_is_numeric && low_type.is_integer() != high_type.is_integer() {
		tc.record_error_at(.condition_mismatch, 'range types do not match', low_id, tc.a.node(low_id).pos)
		return
	}
	...
	if low := tc.match_condition_int_value(low_id) {
		if high := tc.match_condition_int_value(high_id) {
			if low > high {
				tc.record_error_at(.condition_mismatch, 'empty range: `${low} .. ${high}` will never execute', low_id, tc.range_endpoints_pos(low_id, high_id))
			}
		}
	}
}
...
fn (mut tc TypeChecker) check_map_delete_snapshot_clones(map_type Map, pos flat.NodeId) {
	if bad_type := tc.ownership_default_clone_missing_method(map_type.key_type) {
		tc.record_error(.call_arg_mismatch, 'cannot snapshot map keys for iteration with delete: `${bad_type}` requires ownership destruction but has no `clone()` method', pos)
	}
	if bad_type := tc.ownership_default_clone_missing_method(map_type.value_type) {
		tc.record_error(.call_arg_mismatch, 'cannot snapshot map values for iteration with delete: `${bad_type}` requires ownership destruction but has no `clone()` method', pos)
	}
}
...
fn (mut tc TypeChecker) check_for_in_binding_clone(target_id flat.NodeId, typ Type, role string) {
	...
	if bad_type := tc.ownership_default_clone_missing_method(typ) {
		tc.record_error(.call_arg_mismatch, 'cannot iterate over ownership-bearing ${role}: `${bad_type}` requires ownership destruction but has no `clone()` method', target_id)
	}
}
...
fn (mut tc TypeChecker) check_decl_assign(id flat.NodeId, node flat.Node) {
	...
	for i + 1 < node.children_count {
		...
		if lhs_node.kind != .ident {
			...
			if lhs_node.kind == .selector && lhs_node.children_count > 0 {
				...
				if !tc.is_namespace_selector(lhs_node, base) {
					tc.record_error_at(.assignment_mismatch, 'use assignment `=` instead of declaration `:=` when modifying struct fields', lhs_id, tc.assignment_operator_pos(node, lhs_id, rhs_id))
					i += 2
					continue
				}
			}
			tc.record_error_at(.assignment_mismatch, 'non-name `${tc.source_text_for_node(lhs_id)}` on left side of `:=`', lhs_id, tc.decl_non_name_lhs_pos(lhs_id))
			i += 2
			continue
		}
		if node.value == 'static' && tc.unsafe_depth == 0 && !tc.translated_files[tc.cur_file]
			&& !tc.current_fn_declared_unsafe() {
			tc.record_error_at(.assignment_mismatch, 'static variables are supported only in -translated mode, `unsafe{}` blocks, or in `@[unsafe] fn`', lhs_id, tc.node_value_diagnostic_pos(lhs_id))
		}
		...
		if reserved_const_type_name(lhs_node.value) && tc.should_diagnose(lhs_id) {
			tc.record_error_at(.duplicate_decl, 'invalid use of reserved type `${lhs_node.value}` as a variable name', lhs_id, lhs_node.pos)
		}
		if tc.should_check_source_name(lhs_id) && !snake_case_name_is_valid(lhs_node.value) {
			tc.check_snake_case_name(lhs_id, lhs_node.value, 'variable name', tc.node_value_diagnostic_pos(lhs_id))
		}
		if lhs_node.value != '_' {
		...
		if rhs_node.kind == .ident && tc.lock_depth == 0
			...
			tc.record_error_at(.assignment_mismatch, '`${rhs_node.value}` is `shared` and must be `rlock`ed or `lock`ed to be used as non-mut right-hand side of assignment', rhs_id, rhs_node.pos)
			if lhs_node.value != '_' {
		...
		if rhs_node.kind == .map_init && rhs_node.children_count == 0 && rhs_node.value.len == 0 {
			tc.record_error_at(.assignment_mismatch, 'invalid empty map initialisation syntax, use e.g. map[string]int{} instead', rhs_id, rhs_node.pos)
			if lhs_node.value != '_' {
				tc.cur_scope.insert(lhs_node.value, Type(void_))
			}
			i += 2
		...
		if rhs_node.kind == .fn_literal && rhs_node.generic_params().len > 0 {
			tc.record_error_at(.assignment_mismatch, 'cannot assign generic function to a variable', rhs_id, rhs_node.pos)
			if lhs_node.value != '_' {
				tc.cur_scope.insert(lhs_node.value, tc.resolve_type(rhs_id))
			}
			i += 2
		...
		if rhs_node.kind == .infix && rhs_node.op == .arrow {
			tc.record_error_at(.assignment_mismatch, 'cannot use `<-` on the right-hand side of an assignment, as it does not return any values', rhs_id, tc.infix_operator_pos(rhs_node, '<-'))
			i += 2
			continue
		}
		if unsafe_none_id := tc.unsafe_block_none_expr_id(rhs_id) {
			none_node := tc.a.node(unsafe_none_id)
			tc.record_error_at(.assignment_mismatch, 'cannot use `none` in `unsafe` blocks', unsafe_none_id, none_node.pos)
		} else if rhs_node.kind == .none_expr {
			tc.record_error_at(.assignment_mismatch, 'cannot assign a `none` value to a variable', rhs_id, rhs_node.pos)
			if lhs_node.value != '_' {
		...
		if rhs_node.kind == .nil_literal && tc.unsafe_depth == 0 {
			tc.record_error_at(.assignment_mismatch, '`nil` is only allowed in `unsafe` code', rhs_id, rhs_node.pos)
			tc.record_error_at(.assignment_mismatch, 'use of untyped nil in assignment (use `unsafe` | false)', rhs_id, rhs_node.pos)
			if lhs_node.value != '_' {
		...
		if tc.new_error_kind_since(error_count, .unknown_ident) {
			if tc.new_errors_are_forward_decl_unknowns(error_count) {
				if lhs_node.value != '_' {
					tc.cur_scope.insert(lhs_node.value, unknown_type('initializer contains an unresolved forward declaration'))
				}
				i += 2
				continue
			}
		...
		if tc.assignment_rhs_is_void_selector(rhs_id) && !tc.unknown_imported_enum_selector(rhs_id)
			...
			tc.record_error_at(.assignment_mismatch, 'assignment mismatch: 1 variable 0 values', id, tc.assignment_operator_pos(node, lhs_id, rhs_id))
		}
		...
		if tc.unsafe_depth == 0 && lhs_node.value != '_' && rhs_node.kind == .ident
			...
			tc.record_error_at(.assignment_mismatch, 'assignment from one fixed array to another with a pointer element type is prohibited outside of `unsafe`', id, tc.assignment_operator_pos(node, lhs_id, rhs_id))
		}
		if rhs_node.kind == .ident && tc.type_has_declaration_attribute(rhs_type, 'nocopy') {
			tc.record_error_at(.assignment_mismatch, 'cannot copy @[nocopy] struct: use a reference instead', rhs_id, rhs_node.pos)
		}
		tc.record_non_heap_pointer_param_escape(rhs_id)
		if node.value == 'static' && !tc.static_initializer_is_constant(rhs_id) {
			tc.record_error_at(.assignment_mismatch, 'cannot initialized static variable with non-constant value', lhs_id, tc.node_value_diagnostic_pos(lhs_id))
		}
		if node.value == 'static' && unalias_type(rhs_type) is Map {
			tc.record_error_at(.assignment_mismatch, 'maps cannot be static', lhs_id, tc.node_value_diagnostic_pos(lhs_id))
		}
		if assigned_fn := fn_type_from_type(rhs_type) {
			...
			if unknown_return && rhs_node.kind in [.index, .selector] {
				tc.record_error_at(.unknown_type, 'unknown return type: cannot assign `${tc.source_text_for_node(rhs_id)}` as a function variable', rhs_id, tc.index_suffix_diagnostic_pos(rhs_id))
				if lhs_node.value != '_' {
		...
		if rhs_type is MultiReturn {
			...
			tc.record_error_at(.assignment_mismatch, message, id, tc.assignment_operator_pos(node, lhs_id, rhs_id))
			if lhs_node.value != '_' {
				tc.cur_scope.insert(lhs_node.value, rhs_type)
			}
			i += 2
		...
		if explicit_expected is Void && rhs_type.name() == 'int'
			...
			tc.record_error_at(.assignment_mismatch, 'overflow in implicit type `int`, use explicit type casting instead', rhs_id, rhs_node.pos)
		}
		...
		if tc.unsafe_depth == 0 && lhs_is_mut && unalias_type(rhs_type) is Array
			...
			tc.record_error_at(.assignment_mismatch, 'use `mut array2 := array1.clone()` instead of `mut array2 := array1` (or use `unsafe`)', id, tc.assignment_operator_pos(node, lhs_id, rhs_id))
		}
		...
		if explicit_expected !is Void {
			...
			if !tc.expr_compatible(rhs_id, rhs_type, expected)
				...
				tc.type_mismatch(.assignment_mismatch, 'cannot assign `${rhs_type.name()}` to `${expected.name()}`', id)
			}
		}
		...
		if lhs_node.value != '_' && lhs_is_mut {
			...
			if immutable_source_id := tc.immutable_reference_source(rhs_id, rhs_type) {
				...
				tc.record_notice_at(.assignment_mismatch, '`${source.value}` is immutable, cannot have a mutable reference to an immutable object', immutable_source_id, tc.node_value_diagnostic_pos(immutable_source_id))
...
fn (mut tc TypeChecker) check_multi_value_list_decl_assign(id flat.NodeId, node flat.Node) bool {
	...
	if multi_ids.len > 0 {
		...
		tc.record_error_at(.assignment_mismatch, 'cannot use multi-value ${Type(multi_type).name()} in single-value context', multi_id, tc.multi_value_single_context_pos(multi_id))
	}
	if lhs_ids.len != right_len {
		...
		tc.record_error_at(.assignment_mismatch, message, id, tc.assignment_operator_pos(node, lhs_ids[0], first_rhs_id))
	}
	return true
}
...
fn (mut tc TypeChecker) check_mutable_array_immutable_references(id flat.NodeId) {
	...
	for i in 0 .. node.children_count {
		...
		if source.kind == .ident && !tc.ident_is_mutable_lvalue(source.value)
			...
			tc.record_warning_at(.assignment_mismatch, 'cannot add a reference to an immutable object to a mutable array', elem_id, tc.address_operator_pos(elem_id))
		}
	}
}
...
fn (mut tc TypeChecker) record_mutable_decl_branch_immutable_notices(id flat.NodeId) {
	...
	if node.kind == .if_expr || node.kind == .match_stmt {
		for i in 1 .. node.children_count {
			tc.record_mutable_decl_branch_immutable_notices(tc.branch_tail_expr_id(tc.a.child(node, i)))
		}
		return
	}
	...
	if typ is Array || typ is ArrayFixed || typ is Map {
		tc.record_notice_at(.assignment_mismatch, 'left-side of assignment expects a mutable reference, but variable `${node.value}` is immutable, declare it with `mut` to make it mutable or clone it', id, tc.node_value_diagnostic_pos(id))
	}
}
...
fn (mut tc TypeChecker) record_invalid_decl_assign_lhs(node flat.Node, lhs_id flat.NodeId, rhs_id flat.NodeId) bool {
	lhs := tc.a.node(lhs_id)
	if lhs.kind == .paren {
		tc.record_error_at(.assignment_mismatch, 'parentheses are not supported on the left side of `:=`', lhs_id, lhs.pos)
	...
	if lhs.op == .amp {
		tc.record_error_at(.assignment_mismatch, 'cannot use a reference on the left side of `:=`', lhs_id, tc.prefix_operator_pos(lhs_id, op))
	} else {
		tc.record_error_at(.assignment_mismatch, 'cannot use `${op}` on the left of `:=`', lhs_id, tc.prefix_operator_pos(lhs_id, op))
	}
	tc.record_error_at(.assignment_mismatch, 'non-name on the left side of `:=`', lhs_id, tc.prefix_operator_pos(lhs_id, op))
	return true
}
...
fn (mut tc TypeChecker) check_const_reference_assignment(lhs_id flat.NodeId, rhs_id flat.NodeId, is_decl bool, decl_is_mut bool) {
	...
	if const_id := tc.addressed_const_ident(rhs_id) {
		...
		if is_decl {
			if expr_id := tc.const_exprs[key] {
				...
				if expr.kind == .int_literal {
					...
					tc.record_error_with_details_at(.assignment_mismatch, 'cannot assign a pointer to a constant with an integer literal value', const_id, tc.node_value_diagnostic_pos(const_id), [
		...
		if lhs.kind == .ident && lhs.value != '_' {
			tc.record_error_at(.assignment_mismatch, 'cannot have mutable reference to const `${const_node.value}`', rhs_id, tc.address_operator_pos(rhs_id))
		}
	}
	...
	if requests_mutable_reference && tc.unsafe_depth == 0 && !tc.expr_is_inside_unsafe_block(rhs_id)
		...
		tc.record_error_at(.assignment_mismatch, '`${addressed.value}` is immutable, cannot have a mutable reference to it', rhs_id, tc.address_operator_pos(rhs_id))
	}
}
...
fn (mut tc TypeChecker) check_multi_return_decl_assign(id flat.NodeId, node flat.Node) bool {
	...
	if lhs_ids.len > 1 {
		if call_name := tc.unresolved_multi_assign_method_call_name(rhs_id) {
			tc.check_node(rhs_id)
			if tc.should_diagnose(id) {
				tc.record_error_at(.assignment_mismatch, 'assignment mismatch: ${lhs_ids.len} variables but `${call_name}()` returns 0 values', id, tc.assignment_operator_pos(node, lhs_ids[0], rhs_id))
			}
			for lhs_id in lhs_ids {
				tc.insert_decl_lhs(lhs_id, unknown_type('invalid variable'), tc.decl_lhs_is_mut(node, lhs_id))
			}
			tc.fn_context.continue_after_unknown_ident = true
			return true
		}
	...
	if lhs_ids.len > 1 && rhs.kind == .ident && !tc.lvalue_ident_is_known(rhs.value) {
		variables := if lhs_ids.len == 1 { 'variable' } else { 'variables' }
		tc.record_error_at(.assignment_mismatch, 'assignment mismatch: ${lhs_ids.len} ${variables} 1 value', id, tc.assignment_operator_pos(node, lhs_ids[0], rhs_id))
		for lhs_id in lhs_ids {
			tc.insert_decl_lhs(lhs_id, unknown_type('invalid variable'), tc.decl_lhs_is_mut(node, lhs_id))
		}
		return true
		...
		if wrapped_multi := multi_return_payload_type(rhs_type) {
			for i, lhs_id in lhs_ids {
				if i < wrapped_multi.types.len {
					tc.insert_decl_lhs(lhs_id, wrapped_multi.types[i], tc.decl_lhs_is_mut(node, lhs_id))
				}
			}
		}
		return true
...
fn (tc &TypeChecker) multi_expr_tail_type_groups(expr_id flat.NodeId, count int) ?[][]Type {
	...
	match node.kind {
		...
		.lock_expr {
			if node.children_count > 0 {
				return tc.multi_expr_tail_type_groups(tc.a.child(&node, node.children_count - 1), count)
			}
		}
		.expr_stmt {
			if node.children_count > 0 {
	...
	for group in groups {
		...
		for i, value_id in group {
			...
			if !tc.type_compatible(actual, lhs_types[i]) {
				tc.type_mismatch(.assignment_mismatch, 'cannot assign `${actual.name()}` to `${lhs_types[i].name()}`', id)
			}
		}
	}
	return lhs_types
...
fn (tc &TypeChecker) multi_expr_tail_value_groups(expr_id flat.NodeId, count int, explicit_comma_tail bool) ?[][]flat.NodeId {
	...
	match node.kind {
		.if_expr {
			...
			then_groups := tc.tuple_tail_value_groups(tc.a.child(&node, 1), count, explicit_comma_tail) or { return none }
		...
		.lock_expr {
			if node.children_count > 0 {
				return tc.multi_expr_tail_value_groups(tc.a.child(&node, node.children_count - 1), count, explicit_comma_tail)
			}
		}
		.expr_stmt {
			if node.children_count > 0 {
				return tc.multi_expr_tail_value_groups(tc.a.child(&node, 0), count, explicit_comma_tail)
			}
		}
		else {}
	}
...
fn (tc &TypeChecker) wrapped_multi_return_value_groups(expr_id flat.NodeId, count int, explicit_comma_tail bool, wrapper Type) ?[][]flat.NodeId {
	...
	match node.kind {
		.if_expr {
			...
			mut groups := [][]flat.NodeId{}
			then_groups := tc.wrapped_multi_return_branch_groups(tc.a.child(&node, 1), count, explicit_comma_tail, wrapper) or { return none }
			for group in then_groups {
				groups << group
			}
			else_id := tc.a.child(&node, 2)
			else_groups := tc.wrapped_multi_return_branch_groups(else_id, count, explicit_comma_tail, wrapper) or { return none }
			for group in else_groups {
				groups << group
			}
			return groups
		...
		.match_stmt {
			...
			mut groups := [][]flat.NodeId{}
			for i in 1 .. node.children_count {
				branch_groups := tc.wrapped_multi_return_branch_groups(tc.a.child(&node, i), count, true, wrapper) or { return none }
				for group in branch_groups {
					groups << group
				}
			}
		...
		.block, .match_branch {
			return tc.wrapped_multi_return_branch_groups(expr_id, count, explicit_comma_tail, wrapper)
		}
		.lock_expr {
			if node.children_count > 0 {
				return tc.wrapped_multi_return_value_groups(tc.a.child(&node, node.children_count - 1), count, explicit_comma_tail, wrapper)
			}
		}
		.expr_stmt {
			if node.children_count > 0 {
				return tc.wrapped_multi_return_value_groups(tc.a.child(&node, 0), count, explicit_comma_tail, wrapper)
			}
		}
		else {}
	}
...
fn (tc &TypeChecker) visible_local_scope_owns_name(name string) bool {
	...
	for scope != unsafe { nil } && voidptr(scope) != voidptr(tc.file_scope) {
...
fn (mut tc TypeChecker) check_assign(id flat.NodeId, node flat.Node) {
	...
	if node.kind == .index_assign && tc.reject_unlowered_map_mutation
		...
		if tc.should_diagnose(id) {
			tc.record_error(.assignment_mismatch, 'internal compiler error: unlowered map index assignment reached post-transform checker', id)
		}
		for i := 1; i < node.children_count; i += 2 {
			tc.check_node(tc.a.child(&node, i))
		}
	...
	if node.is_mut {
		lhs_id := tc.a.child(&node, 0)
		rhs_id := tc.a.child(&node, 1)
		tc.record_error_at(.assignment_mismatch, 'expecting `:=` (e.g. `mut x :=`)', id, tc.assignment_operator_pos(node, lhs_id, rhs_id))
		return
	}
	...
	for i + 1 < node.children_count {
		...
		if node.kind == .index_assign && effective_lhs_node.kind == .index
			...
			tc.check_node(rhs_id)
			tc.record_error_at(.assignment_mismatch, 'cannot reassign using range expression on the left side of an assignment', effective_lhs_id, tc.index_brackets_pos(effective_lhs_node))
			i += 2
			continue
		}
		if effective_lhs_node.kind == .call {
			...
			tc.record_error_at(.assignment_mismatch, 'cannot call function `${name}()` on the left side of an assignment', effective_lhs_id, effective_lhs_node.pos)
			i += 2
			continue
		}
		if effective_lhs_node.kind == .prefix && effective_lhs_node.op != .mul {
			...
			tc.record_error_at(.assignment_mismatch, message, effective_lhs_id, tc.prefix_operator_pos(effective_lhs_id, op))
			i += 2
			continue
		}
		if effective_lhs_node.kind == .prefix && effective_lhs_node.op == .mul
		...
		if lhs_node.kind == .selector && lhs_node.children_count > 0 {
			...
			if tc.unsafe_depth == 0 && lhs_node.value == 'len'
				...
				tc.check_node(rhs_id)
				tc.record_error_at(.assignment_mismatch, '`${kind}` can not be modified', lhs_id, tc.node_value_diagnostic_pos(lhs_id))
				i += 2
				continue
			}
		}
		if lhs_node.kind == .ident && lhs_node.value == '_' && node.op != .assign {
			tc.record_error_at(.assignment_mismatch, 'cannot modify blank `_` identifier', lhs_id, tc.node_value_diagnostic_pos(lhs_id))
			tc.check_node(rhs_id)
			i += 2
			continue
		}
		...
		if lhs_node.kind == .selector && lhs_type is Void && lhs_node.children_count > 0 {
			...
			if tc.expr_subtree_has_error(base_id) {
				tc.record_error_at(.assignment_mismatch, 'unexpected symbol `void`', lhs_id, tc.node_value_diagnostic_pos(lhs_id))
			}
		}
		...
		if lhs_node.kind == .index && lhs_node.children_count > 0
			...
			tc.record_error_at(.assignment_mismatch, 'cannot assign to s[i] since V strings are immutable\n(note, that variables may be mutable but string values are always immutable, like in Go and Java)', lhs_id, tc.decl_non_name_lhs_pos(lhs_id))
			i += 2
			continue
		}
		...
		if rhs_type is Void {
			...
			if call_name := tc.assignment_rhs_call_name(rhs_id) {
				tc.record_error_at(.assignment_mismatch, 'assignment mismatch: 1 variable but `${call_name}()` returns 0 values', id, tc.assignment_operator_pos(node, lhs_id, rhs_id))
				if node.kind == .decl_assign && lhs_node.kind == .ident && lhs_node.value != '_' {
					tc.record_error_at(.unknown_ident, 'unknown identifier `${lhs_node.value}`', lhs_id, lhs_node.pos)
				}
				i += 2
				continue
			}
		...
		if node.op == .assign && rhs_node.kind == .ident && lhs_node.kind == .ident
			...
			if tc.unsafe_depth == 0 && (is_array_copy || is_map_copy) {
				...
				if is_array_copy {
					tc.record_error_at(.assignment_mismatch, 'use `array2 = array1.clone()` instead of `array2 = array1` (or use `unsafe`)', id, tc.assignment_operator_pos(node, lhs_id, rhs_id))
				} else {
					tc.record_error_at(.assignment_mismatch, 'cannot copy map: call `move` or `clone` method (or use a reference)', rhs_id, rhs_node.pos)
				}
		...
		if clean_expected_type is OptionType && tc.expr_tail_is_nil(rhs_id) {
			...
			if lhs_node.kind == .ident {
				...
				tc.record_warning_at(.assignment_mismatch, 'cannot assign a reference to a value (this will be an error soon) left=${base_type.name()} false right=nil true ptr=false', id, tc.assignment_operator_pos(node, lhs_id, rhs_id))
			}
			tc.record_error_at(.assignment_mismatch, 'cannot assign `nil` to option value', rhs_id, tc.array_element_diagnostic_pos(rhs_id))
			i += 2
			continue
		}
		if tc.anonymous_struct_assignment_mismatch(rhs_id, rhs_type, expected_type) {
			tc.record_error_at(.assignment_mismatch, 'cannot assign anonymous `struct` to a typed `struct`', rhs_id, tc.anonymous_struct_literal_brace_pos(rhs_id))
			i += 2
			continue
		}
		...
		if node.op != .assign && rhs_node.kind == .call && rhs_call_has_no_value
			...
			tc.record_error_at(.assignment_mismatch, 'assignment mismatch: 1 variable but `${call_name}()` returns 0 values', id, tc.compound_assignment_operator_pos(lhs_id, rhs_id, op_text))
			i += 2
			continue
		}
		if node.op != .assign {
		...
		if deref_pointer_mismatch {
			tc.record_error_at(.assignment_mismatch, 'cannot use `${rhs_type.name()}` (right side) as `${expected_type.name()}` (left side) in assignment', id, tc.assignment_operator_pos(node, lhs_id, rhs_id))
		} else {
			...
			if !defer_open_generic_mismatch && !string_char_append && (sum_variant_mismatch
				...
				if clean_expected_type is Pointer && unalias_type(rhs_type) is Struct {
					...
					tc.record_error_at(.assignment_mismatch, 'cannot assign an Option value to a non-option variable', rhs_id, tc.array_element_diagnostic_pos(rhs_id))
				} else {
		...
		if node.op == .power_assign && expected_type !is Unknown && rhs_type !is Unknown
			...
			_ := tc.infix_operator_return_type(.power, expected_type, rhs_type) or {
				tc.record_error(.assignment_mismatch, 'operator `**=` requires numeric operands; got `${expected_type.name()}` and `${rhs_type.name()}`', id)
				Type(void_)
			}
		}
		$if ownership ? {
			tc.check_ownership_map_assignment_key(lhs_id, node.op)
			tc.check_ownership_uncloneable_overlapping_map_assignment(lhs_id, rhs_id, lhs_type, node.op, id)
...
fn (mut tc TypeChecker) record_pointer_binding_alias(owner ScopeBindingOwner, rhs_id flat.NodeId, typ Type) {
	...
	values := tc.pointer_binding_alias_values(left_key, rhs_id, typ, tc.fn_context.pointer_binding_value_keys) or { return }
	tc.fn_context.pointer_binding_value_keys[left_key] = values
}
...
fn (mut tc TypeChecker) record_invalid_literal_assign_lhs(lhs_id flat.NodeId, op flat.Op) bool {
	...
	if lhs.kind == .infix {
		...
		tc.record_error_at(.assignment_mismatch, 'cannot use infix expression on the left side of `${assignment_operator_text(op)}`', diagnostic_id, tc.infix_operator_pos(lhs, op_text))
		if !tc.expr_subtree_contains_ident(diagnostic_id) {
			display := tc.source_text_for_node(diagnostic_id)
			tc.record_error_at(.assignment_mismatch, 'non-name literal value `${display}` on left side of `${assignment_operator_text(op)}`', diagnostic_id, lhs.pos)
		}
		return true
...
fn (mut tc TypeChecker) record_compound_assignment_operand_errors(op flat.Op, lhs_id flat.NodeId, rhs_id flat.NodeId, lhs_type Type, rhs_type Type) {
	...
	if op == .plus_assign && type_is_string_like(lhs_type) && rhs_type.name() in [
		'char',
		'rune',
	] {
		return
	...
	if op in [.plus_assign, .minus_assign] && unalias_type(lhs_type) is Pointer
		...
		if tc.unsafe_depth == 0 && !tc.translated_files[tc.cur_file]
			...
			tc.record_warning_at(.assignment_mismatch, 'pointer arithmetic is only allowed in `unsafe` blocks', lhs_id, tc.compound_assignment_operator_pos(lhs_id, rhs_id, op_text))
		}
		return
	}
	if infix_op := compound_assignment_infix_op(op) {
		if signature := tc.infix_operator_signature(infix_op, lhs_type) {
			...
			if lhs_type is Alias && rhs_type is Alias && lhs_type.name != rhs_type.name
				...
				tc.record_error_at(.assignment_mismatch, 'cannot assign to `${lhs_text}`: expected `${lhs_type.name}`, not `${rhs_type.name}`', rhs_id, tc.array_element_diagnostic_pos(rhs_id))
			} else if op in [.plus_assign, .minus_assign] {
				if unalias_type(lhs_type) !is Struct {
					rhs_name := tc.diagnostic_expr_type_name(rhs_id, rhs_type)
					tc.record_error_at(.assignment_mismatch, 'invalid right operand: ${lhs_name} ${op_text} ${rhs_name}', rhs_id, tc.array_element_diagnostic_pos(rhs_id))
				}
			}
			return
		}
	...
	if op == .minus_assign && (array_type_from_receiver(lhs_type) != none
		...
		tc.record_error_at(.assignment_mismatch, 'undefined operation `${lhs_type.name()}` - `${rhs_type.name()}`', lhs_id, tc.compound_assignment_operator_pos(lhs_id, rhs_id, op_text))
		return
	}
	...
	if op in [.plus_assign, .minus_assign] {
		tc.record_error(.assignment_mismatch, 'invalid right operand: ${lhs_name} ${op_text} ${rhs_name}', rhs_id)
	} else {
