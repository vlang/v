...
import v.pref
import v.tempname
import v.token as compiler_token
import v.transform
import v.types
import v.util
import v.workers
...
fn input_implies_building_v(input_file string) bool {
	...
	if os.is_dir(input_file) {
		...
		return normalized_dir.ends_with('/vlib/v')
	}
	return false
}
...
fn input_is_compiler_tree(input_file string) bool {
	real_input := os.real_path(input_file).replace('\\', '/').trim_right('/')
	if real_input.ends_with('/vlib/v') {
		return true
	}
	marker := '/vlib/v/'
	marker_index := real_input.last_index(marker) or { return false }
	relative := real_input[marker_index + marker.len..]
	if !relative.contains('/') {
		return relative == 'v.v'
	}
	// These are preserved compatibility modules and test suites, not the promoted
	// compiler source tree. Keep their ordinary user-program memory guard.
	for prefix in ['build_constraint/', 'checker/', 'debug/', 'embed_file/', 'live/', 'parser/tests/',
		'profile/', 'reflection/', 'scanner/tests/', 'slow_tests/', 'tests/', 'trace_calls/', 'vmod/'] {
		if relative == prefix.trim_right('/') || relative.starts_with(prefix) {
			return false
		}
	}
	return true
...
fn restore_v3_type_diagnostics(mut a flat.FlatAst, diagnostics []V3CachedTypeDiagnostic) []types.TypeError {
	...
	mut file_set := compiler_token.FileSet.new()
...
fn v3_impure_v_diagnostics(a &flat.FlatAst) []parser.Diagnostic {
	...
	for node in a.nodes {
		...
		for namespace in ['C', 'JS'] {
			...
			pos := if node.pos.is_valid() { node.pos } else { compiler_token.new_pos(file_id, 0) }
...
fn v3_fallback_backend_specific_builtin_source(path string, builtin_root string) bool {
	return (path == builtin_root || path.starts_with(builtin_root + os.path_separator))
		&& os.file_name(path) in ['map.v', 'map_d_v3_backend.v', 'ownership_interface_d_v3_backend.v',
...
pub fn run(args []string) {
	...
	compiler_tree_input := input_is_compiler_tree(input_file)
	...
	if fastc_selfhost_build {
		...
		// `-b fastc vlib/v/v.v` build is a self-host build too; do not require the
	...
	if backend == 'fastc' {
		$if skip_fastc ? {
			...
			if fastc_bench {
				...
				total_lines := source_file_line_count(fastc_generation.source_paths, map[int]&compiler_token.File{})
	...
	builtin_files = builtin_files.filter(os.file_name(it) != 'map.v')
	if no_builtin {
		builtin_files = []
	}
	...
	// A test binary keeps the harness counters (`__test_failures` and the
...
fn unsupported_backend_node_error(a &flat.FlatAst, tc &types.TypeChecker, id flat.NodeId, backend string, diagnose_aggregates bool, fallback_location string, mut visited []bool) ?string {
	...
	if backend == 'wasm' && diagnose_aggregates {
		...
		if unsupported_type.len > 0 {
			return '${fallback_location}error: the V wasm backend does not support type `${unsupported_type}` yet'
		}
		if node.kind == .infix && node.children_count >= 2 && node.op in [.eq, .ne] {
			lhs_type := tc.resolve_type(a.child(&node, 0))
			rhs_type := tc.resolve_type(a.child(&node, 1))
...
fn source_file_line_count(paths []string, files map[int]&compiler_token.File) int {
	if paths.len == 0 {
		return 0
	}
