...
fn test_pkgconfig_flag_queries_keep_explicit_output_filters() {
	for option in ['--cflags', '--cflags-only-I', '--cflags-only-other', '--libs',
		...
		assert pkgconfig_flags_args('--static ${option} library')! == ['--static', option,
			'library']
	}
	assert pkgconfig_flags_args('--libs-only-l --libs-only-L one two')! == ['--libs-only-l',
		'--libs-only-L', 'one', 'two']
	assert pkgconfig_flags_args('--cflags --libs one two')! == ['--cflags', '--libs', 'one',
		'two']
}
...
fn test_pkgconfig_flag_queries_retain_default_actions_and_modifiers() {
	...
	assert pkgconfig_flags_args('--static library')! == ['--cflags', '--libs', '--static',
		'library']
	assert pkgconfig_flags_args('--define-prefix library')! == ['--cflags', '--libs',
		'--define-prefix', 'library']
	assert pkgconfig_flags_args('--dont-define-prefix --static library')! == ['--cflags',
		'--libs', '--dont-define-prefix', '--static', 'library']
...
fn test_pkgconfig_flag_queries_keep_literal_argument_boundaries() {
	...
	assert pkgconfig_flags_args('--libs --define-variable=prefix="café dir" library')! == [
		'--libs', '--define-variable=prefix=café dir', 'library']
	assert pkgconfig_flags_args('--define-variable=filter=--libs library')! == ['--cflags',
		'--libs', '--define-variable=filter=--libs', 'library']
	...
	assert pkgconfig_flags_args(r'--libs ";" "$(not-a-command)"')! == ['--libs', ';',
		r'$(not-a-command)']
}
