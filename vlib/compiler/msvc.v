...
fn find_msvc() ?MsvcResult {
	$if windows {
		...
		return MsvcResult{
			full_cl_exe_path: os.abs_path(vs.exe_path + os.path_separator + 'cl.exe')
...
pub fn (v mut V) cc_msvc() {
	...
	out_name_obj := os.abs_path(v.out_name_c + '.obj')
	// Default arguments
	...
	v.pref.out_name = os.abs_path(v.pref.out_name)
	...
	if v.pref.build_mode == .build_module {
		...
		/*
		b := os.abs_path( '$v_modules_path/vlib/builtin.obj' )
		alibs << '"$b"'
		if !os.exists(b) {
		...
		for imp in v.table.imports {
			...
			alibs << '"' + os.abs_path( '$v_modules_path/vlib/${imp}.obj' ) + '"'
		}
		*/
	}
	if v.pref.sanitize {
...
fn build_thirdparty_obj_file_with_msvc(path string, moduleflags []CFlag) {
	...
	for file in files {
		if file.ends_with('.c') {
			cfiles += '"' + os.abs_path(parent + os.path_separator + file) + '" '
		}
	}
