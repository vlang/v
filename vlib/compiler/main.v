...
pub fn new_vlang(pref &pref.Preferences) &Vlang {
...
pub fn (v &Vlang) finalize_compilation() {
	// TODO remove
	if v.pref.autofree {
		/*
...
pub fn (v mut Vlang) add_parser(parser Parser) int {
	pidx := v.parsers.len
	v.parsers << parser
...
pub fn (v &Vlang) get_file_parser_index(file string) ?int {
...
pub fn (v mut Vlang) parse(file string, pass Pass) int {
...
pub fn (v mut Vlang) compile_x64() {
	$if !linux {
...
// make v2 from v1
fn (v &Vlang) new_v2() builder.Builder {
...
pub fn (v mut Vlang) generate_main() {
	mut cgen := v.cgen
	$if js {
		return
	}
...
pub fn (v mut Vlang) gen_main_start(add_os_args bool) {
...
pub fn (v mut Vlang) gen_main_end(return_statement string) {
	v.cgen.genln('')
	v.cgen.genln('  $return_statement;')
	v.cgen.genln('}')
}

pub fn (v &Vlang) v_files_from_dir(dir string) []string {
	mut res := []string
	if !os.exists(dir) {
...
pub fn (v mut Vlang) add_v_files_to_compile() {
	v.set_module_lookup_paths()
...
pub fn (v &Vlang) get_builtin_files() []string {
...
// get user files
pub fn (v &Vlang) get_user_files() []string {
	mut dir := v.pref.path
...
fn (v &Vlang) get_imported_module_files(mod string) []string {
	mut files := []string
	for p in v.parsers {
		if p.mod == mod {
			files << p.file_path
...
pub fn (v mut Vlang) parse_lib_imports() {
