...
fn issue74_checked_flags_for_source(ccompiler string, cflags_value string, ldflags_value string, source string) Issue74CheckedFlags {
	fixture_dir := os.join_path(@VEXEROOT, 'vlib', 'old', 'pkgconfig', 'testdata', 'static_pkgconfig')
	...
	mut file := parser.parse_text(source, os.join_path('/virtual', 'main.v'), mut table, .skip_comments, prefs)
...
fn test_static_token_inside_combined_cflags_value_activates_pkgconfig() {
	for ccompiler in ['gcc', 'clang', 'mingw'] {
		flags := issue74_checked_flags(ccompiler, '-static -Wno-error -Wno-incompatible-pointer-types')
		assert flags.all_linker == issue74_static_linker_segment, ccompiler
	}
}
...
fn test_static_directive_without_actions_adds_only_default_cflags_and_libs() {
	for ccompiler in ['gcc', 'clang'] {
		shorthand := issue74_checked_flags_for_source(ccompiler, '', '', issue74_static_shorthand_source)
		explicit := issue74_checked_flags_for_source(ccompiler, '', '', issue74_explicit_static_source)
...
fn test_short_static_clusters_keep_their_explicit_action() {
	cflags := issue74_checked_flags_for_source('gcc', '', '', issue74_short_static_cflags_debug_source)
