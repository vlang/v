...
fn test_pkgconfig_is_skipped_for_cross_compilation() {
	sample_dir := os.join_path(@VEXEROOT, 'vlib', 'old', 'pkgconfig', 'test_samples')
...
fn checked_pkgconfig_flags(target_os pref.OS) CheckedPkgConfigFlags {
	...
	pref_.os = target_os
	mut file := parser.parse_text(pkgconfig_test_source, os.join_path('/virtual', 'main.v'), mut table, .skip_comments, pref_)
