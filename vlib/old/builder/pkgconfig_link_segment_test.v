...
const issue74_fixture_dir = os.join_path(@VEXEROOT, 'vlib', 'old', 'pkgconfig', 'testdata', 'static_pkgconfig')
...
const issue74_source_order_markers = ['-Wl,--issue74-order-before', '-Wl,--issue74-order-pkg-public',
	'-Wl,--issue74-order-pkg-private', '-Wl,--issue74-order-between', '-Wl,--issue74-order-pkg-public',
	'-Wl,--issue74-order-pkg-private', '-Wl,--issue74-order-after']

const issue74_xlinker_operand_markers = ['--issue74-xlinker-operand', '-Wl,--issue74-order-pkg-public',
...
fn test_static_pkgconfig_structured_archives_link_direct_and_response_file() {
	$if linux {
		...
		os.write_file(main_source, 'int issue74_a(void);\nint main(void) { return issue74_a() == 74 ? 0 : 1; }\n') or {
			panic(err)
		}
		os.write_file(a_source, 'int issue74_b(void);\nint issue74_a(void) { return issue74_b() + 1; }\n') or {
			panic(err)
		}
...
fn test_static_pkgconfig_quoted_search_path_links_direct_and_response_file() {
	$if linux {
		...
		os.write_file(main_source, 'int issue74_quoted_search(void);\nint main(void) { return issue74_quoted_search() == 74 ? 0 : 1; }\n') or {
			panic(err)
		}
...
fn test_static_pkgconfig_raw_spaced_colon_archive_links_direct_and_response_file() {
	$if linux {
		...
		os.write_file(main_source, 'int issue74_raw_spaced(void);\nint main(void) { return issue74_raw_spaced() == 74 ? 0 : 1; }\n') or {
			panic(err)
		}
...
fn test_static_pkgconfig_spaced_wl_rpath_links_direct_and_response_file() {
	$if linux {
		...
		os.write_file(main_source, 'int issue74_spaced_rpath(void);\nint main(void) { return issue74_spaced_rpath() == 74 ? 0 : 1; }\n') or {
			panic(err)
		}
		os.write_file(library_source, 'int issue74_spaced_rpath(void) { return 74; }\n') or {
			panic(err)
		}
		shared_link := issue74_run_tool(gcc, ['-fPIC', '-shared', library_source, '-o', library_path], root)
...
fn issue74_builder_with_pkgconfig_pthread_and_compiler(pkgconfig_flags []string, ccompiler string, extra_args []string) Builder {
	...
	return issue74_builder_with_pkgconfig_pthread_and_compiler_from_environment(pkgconfig_flags, ccompiler, extra_args)
}
...
fn test_pkgconfig_pthread_provenance_excludes_linker_only_sources() {
	...
	os.setenv('LDFLAGS', '-pthread', true)
	env_ldflags_builder := issue74_builder_with_pkgconfig_pthread_and_compiler_from_environment([], 'gcc', [])
...
fn test_linker_only_pthread_is_not_projected_for_object_compilation() {
	...
	os.setenv('LDFLAGS', '-pthread', true)
	environment_builder := issue74_builder_with_pkgconfig_pthread_and_compiler_from_environment([], 'gcc', [
		'-is_o',
	])
...
fn issue74_run_fake_compile_with_args(root string, ccompiler string, name string, source string, extra_args []string) Issue74FakeCompileResult {
	...
	issue74_replace_environment_value_case_insensitive(mut env, 'PATH', '${bin_dir}${os.path_delimiter}${current_path}')
	env['PKG_CONFIG_PATH'] = ''
...
fn test_pkgconfig_posix_fake_compiler_final_argv() {
	$if linux || macos {
		...
		for ccompiler in ['gcc', 'clang'] {
			order_markers := issue74_fake_compile(root, ccompiler, 'source_order', issue74_source_order_source).filter(issue74_is_source_order_marker(it))
			assert order_markers == issue74_source_order_markers, ccompiler
			dynamic_argv := issue74_fake_compile(root, ccompiler, 'dynamic_compat', issue74_dynamic_compat_source)
			...
			xlinker_argv := issue74_fake_compile(root, ccompiler, 'xlinker_operand', issue74_xlinker_operand_source)
			...
			ldflags_static_argv := issue74_fake_compile_with_args(root, ccompiler, 'ldflags_static', issue74_ldflags_static_source, [
				'-ldflags',
				'-static',
			])
			...
			group_alias_argv := issue74_fake_compile(root, ccompiler, 'group_alias', issue74_group_alias_source)
			...
			parallel_group_alias_argv := issue74_fake_compile_with_args(root, ccompiler, 'group_alias_parallel', issue74_group_alias_source, [
				'-parallel-cc',
			])
...
fn test_pkgconfig_posix_generated_project_group_alias_commands() {
	$if linux || macos {
		...
		generated := issue74_run_fake_compile_with_args(root, 'gcc', 'group_alias_project', issue74_group_alias_source, [
			'-cflags',
			unrelated_arg,
			'-generate-c-project',
			project_dir,
...
fn test_pkgconfig_windows_fake_compiler_group_alias_final_argv() {
	$if windows {
		...
		for ccompiler in ['gcc', 'clang'] {
			group_alias_argv := issue74_fake_compile(root, ccompiler, 'group_alias_windows', issue74_group_alias_source)
			group_alias_markers := group_alias_argv.filter(issue74_is_group_alias_marker(it))
			assert group_alias_markers == issue74_group_alias_markers, ccompiler

			parallel_group_alias_argv := issue74_fake_compile_with_args(root, ccompiler, 'group_alias_parallel_windows', issue74_group_alias_source, [
				'-parallel-cc',
			])
			parallel_group_alias_markers :=
				parallel_group_alias_argv.filter(issue74_is_group_alias_marker(it))
...
fn test_pkgconfig_posix_builder_rejects_incomplete_xlinker_operand() {
	$if linux || macos {
		...
		for ccompiler in ['gcc', 'clang'] {
			interrupted := issue74_run_fake_compile(root, ccompiler, 'xlinker_interrupted', issue74_xlinker_interrupted_source)
			...
			end_of_stream := issue74_run_fake_compile(root, ccompiler, 'xlinker_end_of_stream', issue74_xlinker_end_of_stream_source)
			...
			rpath_interrupted := issue74_run_fake_compile(root, ccompiler, 'wl_rpath_interrupted', issue74_wl_rpath_interrupted_source)
			...
			version_script_interrupted := issue74_run_fake_compile(root, ccompiler, 'version_script_interrupted', issue74_version_script_interrupted_source)
			...
			for control, operand in {
				...
				alias_interrupted := issue74_run_fake_compile(root, ccompiler, alias, issue74_wl_operand_source(control, operand, true))
