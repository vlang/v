...
fn test_pkgconfig_static_folds_continued_physical_lines() {
	...
	os.write_file(crlf_fixture, 'Name: continued-static-crlf-74\r\nDescription: CRLF continuation probe\r\nVersion: 1.0.0\r\nLibs: -lissue74_crlf_a \\\r\n\t-lissue74_crlf_b\r\n') or {
		panic(err)
	}
	defer {
...
fn test_pkgconfig_static_accepts_spaced_variable_assignments() {
	...
	invalid_fixture := os.join_path(os.vtmp_dir(), 'static-variable-validation-74-${os.getpid()}.pc')
	os.write_file(invalid_fixture, '=empty\nbad key = internal\ntabbed\t=\ttrimmed\nName: static-variable-validation-74\nDescription: field=value\nVersion: 1.0.0\nLibs: -lissue74_validation\n') or {
		panic(err)
	}
	defer {
...
fn test_pkgconfig_static_preserves_quoted_public_and_private_link_operands() {
	...
	$if linux {
		...
		os.write_file(main_source, 'int issue74_public(void);\nint main(void) { return issue74_public() == 42 ? 0 : 1; }\n') or {
			panic(err)
		}
		os.write_file(public_source, 'int issue74_private(void);\nint issue74_public(void) { return issue74_private() + 1; }\n') or {
			panic(err)
		}
