...
fn set_addr_family(mut a Addr, family AddrFamily, sockaddr_size u32) {
	unsafe {
		// Addr starts with `len, f` on BSD/Darwin, but with `f` elsewhere.
		$if macos || freebsd || openbsd || netbsd || dragonfly {
			mut raw := &u8(&a)
			raw[0] = u8(sockaddr_size)
			raw[1] = u8(family)
		} $else {
			*(&u16(&a)) = u16(family)
		}
...
pub fn new_ip6(port u16, addr [16]u8) Addr {
	...
	set_addr_family(mut a, .ip6, sizeof(C.sockaddr_in6))
...
pub fn new_ip(port u16, addr [4]u8) Addr {
	...
	set_addr_family(mut a, .ip, sizeof(C.sockaddr_in))
