...
fn server() ! {
	...
	// sets `cleanup_done` before freeing, so a second call returns before a
	// second pass through the mbedtls frees. The guard's real purpose is
	// preventing the double-free that the
	...
	// without erroring). A successful second call proves the idempotent early-return
	// path was taken.
	cli.shutdown()!
