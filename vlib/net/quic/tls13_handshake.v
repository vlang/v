...
pub fn (mut h Tls13ClientHandshake) process_server_hello(msg HandshakeMessage, framed_message []u8) !HandshakeSecrets {
	if h.state != .wait_server_hello {
		return handshake_error(.unexpected_message, 'quic: received ServerHello/HelloRetryRequest while in state ${h.state}')
	}
	if msg.typ != .server_hello {
		return handshake_error(.unexpected_message, 'quic: expected ServerHello or HelloRetryRequest, got ${msg.typ}')
	}
	...
	match parsed {
		ParsedHelloRetryRequest {
			...
			if h.got_hello_retry_request {
				return handshake_error(.unexpected_message, 'quic: received a second HelloRetryRequest')
			}
			if parsed.cipher_suite != cipher_suite_tls_aes_128_gcm_sha256 {
				return handshake_error(.illegal_parameter, 'quic: HelloRetryRequest cipher_suite 0x${parsed.cipher_suite:04x} was not offered')
			}
			if selected_group := parsed.selected_group {
				if selected_group != named_group_secp256r1 {
					...
					return handshake_error(.handshake_failure, 'quic: HelloRetryRequest selected an unsupported group 0x${selected_group:04x} (v1 only offers secp256r1)')
				}
			}
			...
			// mishandled.
			return handshake_error(.handshake_failure, 'quic: HelloRetryRequest handling is not yet implemented (only second-HRR rejection is)')
		}
		ParsedServerHello {
			if parsed.cipher_suite != cipher_suite_tls_aes_128_gcm_sha256 {
				return handshake_error(.illegal_parameter, 'quic: ServerHello cipher_suite 0x${parsed.cipher_suite:04x} was not offered')
			}
			if parsed.key_share_group != named_group_secp256r1 {
				return handshake_error(.illegal_parameter, 'quic: ServerHello key_share selected an unoffered group 0x${parsed.key_share_group:04x}')
			}

			peer_public := ecdsa.PublicKey.from_uncompressed_bytes(parsed.key_share_key_exchange,
				nid: .prime256v1
			) or {
				return handshake_error(.decode_error, 'quic: malformed ServerHello key_share: ${err.msg()}')
			}
			defer {
				peer_public.free()
			}
			...
			h.handshake_secrets = derive_handshake_secrets(early_secret, shared_secret, h.transcript_hash()) or { return handshake_error(.handshake_failure, err.msg()) }
...
pub fn (mut h Tls13ClientHandshake) process_encrypted_extensions(msg HandshakeMessage, framed_message []u8, peer_initial_scid []u8, original_dcid []u8, retry_scid ?[]u8) ! {
	if h.state != .wait_encrypted_extensions {
		return handshake_error(.unexpected_message, 'quic: received EncryptedExtensions while in state ${h.state}')
	}
	if msg.typ != .encrypted_extensions {
		return handshake_error(.unexpected_message, 'quic: expected EncryptedExtensions, got ${msg.typ}')
	}
	...
	alpn_ext := find_extension(extensions, ext_alpn) or {
		return handshake_error(.no_application_protocol, 'quic: EncryptedExtensions missing mandatory alpn extension')
	}
	...
	if selected_protocol !in h.alpn_protocols {
		return handshake_error(.no_application_protocol, 'quic: server selected ALPN protocol "${selected_protocol}" which this client did not offer')
	}
	...
	tp_ext := find_extension(extensions, ext_quic_transport_parameters) or {
		...
		return handshake_error(.missing_extension, 'quic: EncryptedExtensions missing mandatory quic_transport_parameters extension')
	}
...
pub fn (mut h Tls13ClientHandshake) process_certificate_or_request(msg HandshakeMessage, framed_message []u8) ! {
	if h.state != .wait_certificate {
		return handshake_error(.unexpected_message, 'quic: received Certificate/CertificateRequest while in state ${h.state}')
	}
	if msg.typ == .certificate_request {
		return handshake_error(.handshake_failure, 'quic: server requested client certificate authentication, which v1 does not support')
	}
	...
	if parsed.certificate_request_context.len != 0 {
		return handshake_error(.illegal_parameter, 'quic: Certificate certificate_request_context must be empty for server authentication, got ${parsed.certificate_request_context.len} bytes')
	}
...
pub fn (mut h Tls13ClientHandshake) process_certificate_verify(msg HandshakeMessage, framed_message []u8) ! {
	if h.state != .wait_certificate_verify {
		return handshake_error(.unexpected_message, 'quic: received CertificateVerify while in state ${h.state}')
	}
	if msg.typ != .certificate_verify {
		return handshake_error(.unexpected_message, 'quic: expected CertificateVerify, got ${msg.typ}')
	}
...
pub fn (mut h Tls13ClientHandshake) process_finished(msg HandshakeMessage, framed_message []u8) !([]u8, ApplicationSecrets) {
	...
	h.accumulate(framed_message)
	h.application_secrets = derive_application_secrets(h.handshake_secrets.handshake_secret, h.transcript_hash()) or { return handshake_error(.handshake_failure, err.msg()) }

	client_verify_data := compute_finished_verify_data(h.handshake_secrets.client_secret, h.transcript_hash()) or { return handshake_error(.handshake_failure, err.msg()) }
