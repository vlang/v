...
pub fn (mut c Client) notify[T](method string, params T) ! {
	...
	intercept_request(c.interceptors.request, &req) or { return err }

	mut enc_req := req.encode().bytes()
	intercept_encoded_request(c.interceptors.encoded_request, enc_req) or { return err }
...
pub fn (mut c Client) request[T](method string, params T, id string) !Response {
	...
	c.stream.read(mut enc_resp) or { return err }

	intercept_encoded_response(c.interceptors.encoded_response, enc_resp)

	resp := decode_response(enc_resp.bytestr()) or { return err }
...
pub fn (mut c Client) batch(reqs []Request) ![]Response {
	...
	intercept_encoded_request(c.interceptors.encoded_request, enc_reqs) or { return err }
	...
	mut resps := decode_batch_response(enc_resp.bytestr()) or { return err }
