...
pub mut:
	...
	verify                   string // the path to a rootca.pem file, containing trusted CA certificate(s); when empty, mbedTLS and HTTP/3 use the Linux system bundle when available and otherwise the vendored bundle, OpenSSL uses its configured default trust paths, and SChannel uses the Windows certificate store
