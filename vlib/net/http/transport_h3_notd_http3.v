...
// the h3_*_d_http3.v files it needs). HTTP/3 is opt-in because importing
// net.quic also pulls its TLS 1.3 and QPACK implementation into every net.http
// program. That substantially increases compile time even for applications
// that use only HTTP/1.1 or HTTP/2. Gating the net.quic import behind
// `-d http3` keeps the common path lean; a program that actually wants HTTP/3
// opts in explicitly.
//
...
fn (mut t Transport) h3_round_trip(_req &Request, _key string, _method Method, _host string, _port int, _path string, _data string, _header Header) !Response {
	return error('http: HTTP/3 (enable_http3) support is not compiled in; rebuild with `-d http3`')
}
