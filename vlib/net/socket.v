...
pub fn (s Socket) send(data string) int {
	buf := data.cstr()
	res := C.send(s.sockfd, &buf, data.len, 0)
	if res < 0 {
...
pub fn (s Socket) sendall(data string) int {
	mut index := 0
	mut len := data.len
	for {
		if len <= 0 {
		...
		i := s.send(data.substr(index, data.len))
		index += i
		len -= i
	}
	return data.len
...
pub fn (s Socket) recv(bufsize int) string {
	buf := string {
