...
// 	mut addr := C.sockaddr_storage{}
// 	len := sizeof(C.sockaddr_storage)
// 	socket_error(C.getpeername(c.sock.handle, &addr, &len)) ?
// 	return new_addr(addr, len)
// }
...
// 	mut new_handle := C.accept(l.sock.handle, &addr, &len)
// 	if new_handle <= 0 {
// 		l.wait_for_accept() ?
// 		new_handle = C.accept(l.sock.handle, &addr, &len)
...
// 	mut addr := SockaddrStorage{}
// 	len := sizeof(SockAddrStorage)
// 	// cast to the correct type
// 	C.getsockname(s.handle, &addr, &len)
// 	return new_addr(&addr, len)
// }
