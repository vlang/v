...
pub fn cp_all(osource_path, odest_path string, overwrite bool) ?bool {
	source_path := os.abs_path(osource_path)
	dest_path := os.abs_path(odest_path)
...
pub fn abs_path(fpath string) string {
...
pub fn resource_abs_path(path string) string {
	mut base_path := os.abs_path(os.dir(os.executable()))
	vresource := os.getenv('V_RESOURCE_PATH')
	if vresource.len != 0 {
		base_path = vresource
	}
	return os.abs_path(os.join_path(base_path, path))
}
