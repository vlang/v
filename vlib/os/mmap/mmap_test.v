...
fn test_mmap_file_lowlevel() {
	...
	// Map the file
	addr := mmap.mmap(len: usize(test_content.len), 
		prot: mmap.prot_read, 
	...
	content := unsafe { string{len: test_content.len, str: mapped_data} }
...
fn test_msync() {
	...
	addr := mmap.mmap(len: usize(content.len), 
		prot: mmap.prot_read, 
		flags: mmap.map_shared, 
		fd: f.fd, 
		offset: 0)!
...
fn test_mmap_protection_flags() {
	...
	read_only_addr := mmap.mmap(len: size, 
		prot: mmap.prot_read, 
		flags: mmap.map_anonymous | mmap.map_private, 
		fd: -1, 
		offset: 0)!
	...
	write_only_addr := mmap.mmap(len: size, 
...
fn test_mmap_mapping_flags() {
	...
	shared_addr := mmap.mmap(len: size, 
		prot: mmap.prot_read | mmap.prot_write, 
	...
	private_addr := mmap.mmap(len: size, 
		prot: mmap.prot_read | mmap.prot_write, 
		flags: mmap.map_anonymous | mmap.map_private, 
		fd: -1, 
		offset: 0)!
...
fn test_mmap_error_handling() {
	...
	mut result := mmap.mmap(len: usize(4096), 
		prot: mmap.prot_read, 
		flags: mmap.map_shared, 
		fd: -999, 
		offset: 0) or { return }
