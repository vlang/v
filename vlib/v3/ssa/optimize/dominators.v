...
// CfgData holds successor/predecessor lists indexed by block id. v3 branch and
// phi operands store raw block ids, so this is built straight from the CFG that
// build_cfg() already wrote into m.blocks.
struct CfgData {
mut:
	succs [][]int
	preds [][]int
...
// cfg_data_from_module snapshots the successor/predecessor sets from m.blocks.
// build_cfg(mut m) must have been called first.
fn cfg_data_from_module(m &ssa.Module) CfgData {
	...
	for bi in 0 .. n_blocks {
		mut s := []int{cap: m.blocks[bi].succs.len}
		for x in m.blocks[bi].succs {
			s << int(x)
		}
		cfg.succs[bi] = s
		mut p := []int{cap: m.blocks[bi].preds.len}
		for x in m.blocks[bi].preds {
			p << int(x)
		}
		cfg.preds[bi] = p
	}
...
// DomInfo holds dominator results in flat arrays indexed by block id.
struct DomInfo {
mut:
	idom     []int
	dom_tree [][]int
...
// compute_dominators runs Lengauer-Tarjan for every function and also writes the
// idom / dom_tree results back into m.blocks for the verifier's dominance checks.
fn compute_dominators(mut m ssa.Module, cfg &CfgData) DomInfo {
	...
	for fi in 0 .. m.funcs.len {
		...
		for i := ctx.n; i >= 2; i-- {
			...
			if semi_w2 >= 0 && semi_w2 < max_id {
				mut b := ctx.bucket[semi_w2]
				b << w
				ctx.bucket[semi_w2] = b
			}
	...
	// Write idom / dom_tree back into the blocks for verifier dominance checks.
	for bi in 0 .. max_id {
		mut blk := m.blocks[bi]
		blk.idom = ctx.idom[bi]
		blk.dom_tree = []ssa.BlockID{cap: ctx.dom_tree[bi].len}
		for c in ctx.dom_tree[bi] {
			blk.dom_tree << ssa.BlockID(c)
		}
		m.blocks[bi] = blk
	}
