...
limit for the split C compilation. They divide the compiler across 32 C units and cache those
production objects by content, compiler, flags, target, and included-file contents. Repeated
production self-builds therefore compile only changed units; `-nocache` disables this reuse.
Other parallel C builds remain limited to two jobs.
