...
fn (mut t Transformer) transform_call_args(id flat.NodeId, node flat.Node) flat.NodeId {
	...
	mut has_concrete_generic_params := false
	if concrete_params := t.concrete_generic_call_param_types(id, node) {
		has_concrete_generic_params = true
...
fn (mut t Transformer) call_param_types_from_decl(call_name string) ?[]types.Type {
	...
	for i in 0 .. node.children_count {
		...
		if child.is_mut && child.op == .amp && param_type is types.Pointer {
			param_type = types.Type(types.Pointer{
				base_type: param_type
			})
		} else if (child.is_mut || child.typ.starts_with('mut ')
...
fn (mut t Transformer) alias_str_wrap(expr flat.NodeId, alias_name string, base_type string, is_ref bool) flat.NodeId {
	...
	if is_ref && t.is_optional_type_name(resolved_base) {
		return t.string_plus(t.make_string_literal('&'), inner)
	}
...
// fn_type_texts_signature_compatible reports whether two fn-type texts such as
...
fn (mut t Transformer) resolved_receiver_arg_compatible(arg_id flat.NodeId, actual_type string, expected_type string) bool {
	...
	// fn-typed arguments: a fn literal is stringified with its parameter types
	// inline (`fn (&Request, &ResponseWriter)`), while the same signature
	// written as a fn-type alias expands to its declared spelling, which may
	// carry `mut` parameter mode (`fn(&Request, mut ResponseWriter)`). Both
...
fn (mut t Transformer) collect_fn_literal_source_type_texts(arg_id flat.NodeId, mut result []string, mut seen map[int]bool) {
	...
	match node.kind {
		...
		.selector {
			...
			if isnil(t.tc) || !t.tc.expr_is_method_value(arg_id) {
				return
			}
...
fn (t &Transformer) collect_callback_nested_assignment_rhs_ids(id flat.NodeId, name string, use_id flat.NodeId, mut result []flat.NodeId) {
	...
	if node.kind in [.for_stmt, .for_in_stmt] {
		child_start = callback_loop_body_start(node)
		if rhs_ids := t.callback_sequence_definite_assignment_rhs_ids(node, child_start, name,
			use_id)
		{
			for rhs_id in rhs_ids {
				if rhs_id !in result {
	...
	for i in child_start .. node.children_count {
		child_id := t.a.child(&node, i)
...
fn (t &Transformer) callback_definite_assignment_rhs_ids(id flat.NodeId, name string, use_id flat.NodeId) ?[]flat.NodeId {
	...
	return t.callback_sequence_definite_assignment_rhs_ids(node, start, name, use_id)
}

fn (t &Transformer) callback_sequence_definite_assignment_rhs_ids(node flat.Node, start int, name string, use_id flat.NodeId) ?[]flat.NodeId {
	mut result := []flat.NodeId{}
	mut assigned := false
	mut name_shadowed := false
	for i in start .. node.children_count {
...
fn callback_loop_body_start(node flat.Node) int {
	return if node.kind == .for_in_stmt { node.value.int() } else { 3 }
}
...
fn (mut t Transformer) callback_expr_source_container_element_type(base_id flat.NodeId) ?string {
	...
	if container_type.len == 0 {
		base := t.a.nodes[int(base_id)]
		if base.kind == .call {
			container_type = t.callback_call_source_return_type(base_id, base) or { '' }
		}
	}
	if container_type.len == 0 {
	...
	if !isnil(t.tc) {
		for candidate in candidates {
			if raw_return := t.tc.fn_ret_type_texts[candidate] {
				...
				file_name := t.tc.fn_type_files[candidate] or { '' }
				return t.callback_source_return_expansion(t.decl_param_type_in_file(raw_return,
