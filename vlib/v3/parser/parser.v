	...
	match p.tok {
		...
		.name, .key_module {
			...
			if p.tok == .lcbr && local_type_name.len > 0
				&& (!p.in_for_container || p.current_lcbr_looks_struct_init())
...
fn (mut p Parser) fn_literal() flat.NodeId {
	...
	// Optional generic parameter list after captures: `fn [capture] [T](...) {}`.
	if p.tok == .lsbr {
		mut depth := 1
		p.next()
		for depth > 0 && p.tok != .eof {
...
fn (mut p Parser) isreftype_expr() flat.NodeId {
	...
	if p.tok == .lsbr {
		...
		p.check(.lpar)
		if p.tok == .name
			&& (is_builtin_type(p.lit) || (p.lit.len > 0 && p.lit[0] >= `A` && p.lit[0] <= `Z`)) {
			type_name := p.parse_type_name()
			arg = p.a.add_val(.sizeof_expr, type_name)
		} else {
			arg = p.expr(.lowest)
