...
fn (mut p Parser) prefix_expr() flat.NodeId {
	...
	match p.tok {
		...
		.name, .key_module {
			...
			if name == '@FILE' {
				return p.add_val_id(5, p.cur_file)
			}
			if name == '@VMODROOT' {
				return p.add_val_id(5, vmod_root_for_file(p.cur_file))
			}
			if name == '@VEXEROOT' {
			...
			if name == '@FILE_LINE' {
				return p.add_val_id(5, '${p.cur_file}:${p.line_nr_for_pos(name_pos)}')
			}
			if name == '@MOD' {
			...
			if name == '@FN' || name == '@METHOD' {
				return p.add_val_id(5, p.cur_fn)
			}
			if name == '@STRUCT' {
				return p.add_val_id(5, p.cur_struct)
			}
			if name == '@LOCATION' {
				return p.add_val_id(5, '${p.cur_file}:${p.line_nr_for_pos(name_pos)}: ${p.cur_fn}')
			}
		...
		.amp {
			...
			if p.tok == .amp {
				...
				if p.tok == .name && is_builtin_type(p.lit) {
					...
					id := p.add_val(.ident, type_name[2..])
...
fn (mut p Parser) call_args(fn_expr flat.NodeId) flat.NodeId {
	...
	for p.tok != .rpar && p.tok != .eof {
		...
		if p.tok == .ellipsis {
			...
			lambda_params << p.add_val(.ident, p.expect_name())
			for p.tok == .comma {
				p.next()
				lambda_params << p.add_val(.ident, p.expect_name())
			}
...
fn (mut p Parser) index_expr(lhs flat.NodeId) flat.NodeId {
	...
	if p.tok == .dotdot {
		...
		p.check(.rsbr)
		start_id := p.add(flat.NodeKind.empty)
		mut ids := []flat.NodeId{}
		ids << lhs
		ids << start_id
		if int(end_id) >= 0 {
