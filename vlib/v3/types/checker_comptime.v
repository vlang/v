...
fn (mut tc TypeChecker) check_node(id flat.NodeId) {
	...
	if node.kind == .sizeof_expr {
		if should_check_named_type(node.value) && !tc.type_name_known(node.value)
			&& (isnil(tc.cur_scope) || tc.cur_scope.lookup(node.value) == none)
	...
	if child.kind == .call {
		if info := tc.resolve_call_info(child_id, *child) {
			...
			if callee.kind == .selector && callee.children_count > 0
				...
				if unalias_type(receiver_type) !is Pointer
					&& !tc.type_has_declaration_attribute(receiver_type, 'heap') {
					tc.record_error_at(.call_arg_mismatch,
...
fn (mut tc TypeChecker) check_array_literal_element_types(id flat.NodeId, node flat.Node) {
	...
	for i in 1 .. node.children_count {
		child_id := tc.a.child(&node, i)
		mut actual_id := child_id
		mut actual := tc.resolve_expr(child_id, elem_type)
		if spread_id := tc.spread_arg_value(child_id) {
			actual_id = spread_id
			spread_type := tc.resolve_expr(spread_id, array_type)
			actual = array_like_elem_type(unwrap_pointer(spread_type)) or {
				tc.record_error_at(.assignment_mismatch,
					'array spread expects an array, not `${spread_type.name()}`', child_id,
		...
		if elem_type is OptionType && actual !is OptionType
			&& tc.expr_compatible(actual_id, actual, elem_type.base_type) {
		...
		if actual is Unknown || (tc.expr_compatible(actual_id, actual, elem_type)
			&& !is_option_mismatch && !is_pointer_mismatch) {
			continue
		}
		actual_name := tc.array_element_diagnostic_type_name(actual_id, actual)
...
fn (mut tc TypeChecker) check_string_interpolation_format(id flat.NodeId, node flat.Node, expr_id flat.NodeId) {
	...
	mut allowed := false
	if spec == `p` && unalias_type(actual) is Pointer {
		allowed = true
...
fn (mut tc TypeChecker) check_cast_expr(id flat.NodeId, node flat.Node) {
	...
	if clean_pointer_target is Pointer {
		...
		if infix_power_type_is_numeric(actual) && !fn_param_is_voidptr_type(target)
			&& target_name !in ['byteptr', 'charptr'] {
...
fn (mut tc TypeChecker) check_comptime_condition_diagnostics(id flat.NodeId, node flat.Node) bool {
	...
	for op in [' !is ', ' is '] {
		...
		root_is_type := tc.type_name_known(root)
		if root.len > 0 && !root_is_generic_type && !root_is_type && !tc.lvalue_ident_is_known(root) {
		...
		if !right.starts_with('$') && should_check_named_type(right) && !tc.type_name_known(right) {
			if !tc.comptime_reflection_condition_type_known(right)
				&& !tc.comptime_condition_type_is_loop_metadata(id, right) {
...
fn (mut tc TypeChecker) check_infix(id flat.NodeId, node flat.Node) {
	...
	if node.op == .left_shift {
		if append_array := array_type_from_receiver(lhs_type) {
			tc.annotate_expected_expr(rhs_id, append_array.elem_type)
			rhs_type = tc.resolve_expr(rhs_id, append_array.elem_type)
		}
	}
	...
	if node.op == .left_shift {
		if append_array := array_type_from_receiver(lhs_type) {
			if unalias_type(rhs_type) is OptionType
				&& unalias_type(append_array.elem_type) !is OptionType {
				tc.record_error_at(.assignment_mismatch,
	...
	// Array append accepts wrapped values when the array element itself is wrapped.
	// Its dedicated validation below compares against the element type and reports
	// the correct error when (for example) a `?T` is appended to a `[]T`.
	if node.op != .left_shift && tc.check_wrapped_infix_operand(lhs_id, lhs_type, rhs_id, rhs_type) {
		return
	}
	...
	if node.op in [.left_shift, .right_shift, .right_shift_unsigned] {
		...
		mut bit_size := tc.integer_shift_bit_size(lhs_type)
		if expected := tc.expected_context_for_expr(id) {
			expected_bits := tc.integer_shift_bit_size(expected)
			if expected_bits > bit_size {
				bit_size = expected_bits
			}
		}
		if enclosing := tc.enclosing_numeric_cast_context(id) {
			enclosing_bits := tc.integer_shift_bit_size(enclosing)
			if enclosing_bits > bit_size {
				bit_size = enclosing_bits
			}
		}
...
fn (mut tc TypeChecker) check_or_fallback_type(or_id flat.NodeId, source_id flat.NodeId, fallback_id flat.NodeId) {
	...
	if parent_kind == .return_stmt && is_ierror_type(actual)
		&& unalias_type(tc.fn_context.return_type) is ResultType {
		return
	}
...
fn (mut tc TypeChecker) check_block(id flat.NodeId, node flat.Node) {
	...
	if node.value == 'comma_exprs' {
		...
		// Parse-time folding of a platform `$if` leaves its selected block nested
		// at the original expression position. Such a block is transparent: when
		// its parent checks it as a value tail, its own tail is a value too.
		value_tail := !tc.is_statement_node(id) && (is_unsafe || tc.block_is_nested_value_tail(id)
			|| tc.expression_node_used_as_value(id))
...
fn (tc &TypeChecker) block_is_nested_value_tail(id flat.NodeId) bool {
	parent_id := tc.direct_parent_id(id)
	if !tc.valid_node_id(parent_id) {
		return false
	}
	...
	return tc.a.child(parent, parent.children_count - 1) == id
}
...
fn (mut tc TypeChecker) check_lock_expr(id flat.NodeId, node flat.Node) {
	...
	for i in 0 .. node.children_count - 1 {
		...
		lock_key := tc.expr_key(object_id)
		if lock_key.len > 0 && valid_string_data(lock_key) {
			existing_modes := tc.fn_context.locked_shared_modes[lock_key] or { []u8{} }
			if existing_modes.len > 0 {
				message := if existing_modes.last() == `r` {
					'`${tc.source_text_for_node(object_id)}` is already read-locked'
				} else {
					'`${tc.source_text_for_node(object_id)}` is already locked'
				}
				tc.record_error_at(.assignment_mismatch, message, object_id, object.pos)
			}
			mut modes := existing_modes.clone()
			modes << mode
			tc.fn_context.locked_shared_modes[lock_key] = modes
			tc.fn_context.locked_shared_names[lock_key]++
			locked_keys << lock_key
		}
	}
	tc.lock_depth++
	tc.check_stmt_node(tc.a.child(&node, node.children_count - 1))
	tc.lock_depth--
	for lock_key in locked_keys {
		tc.fn_context.locked_shared_names[lock_key]--
		mut modes := (tc.fn_context.locked_shared_modes[lock_key] or { []u8{} }).clone()
		if modes.len > 0 {
			modes.delete_last()
		}
		if modes.len == 0 {
			tc.fn_context.locked_shared_modes.delete(lock_key)
		} else {
			tc.fn_context.locked_shared_modes[lock_key] = modes
		}
	}
...
fn (mut tc TypeChecker) check_decl_assign(id flat.NodeId, node flat.Node) {
	...
	for i + 1 < node.children_count {
		...
		if lhs_node.value != '_' && tc.unsafe_depth == 0 && !tc.current_fn_declared_unsafe()
...
fn (tc &TypeChecker) static_initializer_is_constant(id flat.NodeId) bool {
	...
	if node.kind !in [.infix, .prefix, .paren, .cast_expr, .array_literal, .array_init, .map_init,
		.struct_init, .field_init, .block, .expr_stmt] {
		return false
	}
	for i in 0 .. node.children_count {
...
fn (tc &TypeChecker) assignment_target_requests_mutable_reference(lhs_id flat.NodeId, is_decl bool) bool {
	...
	if is_decl {
		// `mut p := &value` makes the pointer slot reassignable; it does not turn
		// the pointee into a mutable reference.
		return false
	}
	if lhs_type := tc.non_file_scope_type(lhs.value) {
		if unalias_type(lhs_type) is Pointer {
			return false
		}
	}
	...
	for i + 1 < node.children_count {
		...
		if node.op == .assign && rhs_node.kind == .ident {
			if tc.unsafe_depth == 0 && ((clean_expected_type is Array && clean_rhs_type is Array
				&& lhs_node.kind == .ident && expected_type.name() == rhs_type.name())
