...
fn test_fn_literal_c_abi_const_mode_matches_alias_inside_generic_fn() {
	...
	run_bad(v3_bin, 'bad_generic_receiver_direct_const_fn_param',
		...
		'cannot use')
	direct_generic := run_good(v3_bin, 'good_parenthesized_direct_generic_const_fn_param',
		'struct C.native_event {\n\tvalue int\n}\nfn apply[T](handler fn (const_event &T), event &T) bool {\n\thandler(event)\n\treturn true\n}\nfn startup[U]() bool {\n\tevent := C.native_event{\n\t\tvalue: 1\n\t}\n\treturn apply[C.native_event]((fn (const_event &C.native_event) {}), &event)\n}\nfn main() {\n\tprintln(startup[int]().str())\n}\n')
	assert direct_generic == 'true'
	run_bad(v3_bin, 'bad_direct_generic_literal_only_const_fn_param',
		'struct C.native_event {}\nfn apply[T](handler fn (event &T)) {}\nfn startup[U]() {\n\tapply[C.native_event](fn (const_event &C.native_event) {})\n}\nfn main() {\n\tstartup[int]()\n}\n',
		'cannot use')
...
fn test_callback_identifier_c_abi_modes_inside_generic_fn() {
	...
	run_bad(v3_bin, 'bad_callback_container_param_in_generic',
		...
		'cannot use')
	variadic_spread_matching := run_good(v3_bin, 'good_callback_variadic_spread_in_generic',
		'type ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nfn const_handler(const_event &C.native_event) {}\nfn consume[T](handlers ...ConstHandler) bool {\n\treturn handlers.len == 1\n}\nfn startup[T]() bool {\n\thandlers := [ConstHandler(const_handler)]\n\treturn consume[int](...handlers)\n}\nfn main() {\n\tprintln(startup[int]().str())\n}\n')
	assert variadic_spread_matching == 'true'
	run_bad(v3_bin, 'bad_callback_variadic_spread_in_generic',
		'type PlainHandler = fn (event &C.native_event)\ntype ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nfn const_handler(const_event &C.native_event) {}\nfn consume[T](handlers ...PlainHandler) {}\nfn startup[T]() {\n\thandlers := [ConstHandler(const_handler)]\n\tconsume[int](...handlers)\n}\nfn main() {\n\tstartup[int]()\n}\n',
		'cannot use')
	indexed_writes_matching := run_good(v3_bin, 'good_callback_container_index_writes_in_generic',
		'type ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nfn const_handler(const_event &C.native_event) {}\nfn consume_array[T](handlers []ConstHandler) bool {\n\treturn handlers.len == 1\n}\nfn consume_map[T](handlers map[string]ConstHandler) bool {\n\treturn handlers.len == 1\n}\nfn startup[T]() bool {\n\tmut array_handlers := [ConstHandler(const_handler)]\n\tarray_handlers[0] = ConstHandler(const_handler)\n\tmut map_handlers := {"event": ConstHandler(const_handler)}\n\tmap_handlers["event"] = ConstHandler(const_handler)\n\treturn consume_array[int](array_handlers) && consume_map[int](map_handlers)\n}\nfn main() {\n\tprintln(startup[int]().str())\n}\n')
	assert indexed_writes_matching == 'true'
	run_bad(v3_bin, 'bad_callback_array_index_write_in_generic',
		'type PlainHandler = fn (event &C.native_event)\ntype ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nfn plain_handler(event &C.native_event) {}\nfn const_handler(const_event &C.native_event) {}\nfn consume[T](handlers []PlainHandler) {}\nfn startup[T]() {\n\tmut handlers := [PlainHandler(plain_handler)]\n\thandlers[0] = ConstHandler(const_handler)\n\tconsume[int](handlers)\n}\nfn main() {\n\tstartup[int]()\n}\n',
		'cannot use')
	run_bad(v3_bin, 'bad_callback_map_index_write_in_generic',
		'type PlainHandler = fn (event &C.native_event)\ntype ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nfn plain_handler(event &C.native_event) {}\nfn const_handler(const_event &C.native_event) {}\nfn consume[T](handlers map[string]PlainHandler) {}\nfn startup[T]() {\n\tmut handlers := {"event": PlainHandler(plain_handler)}\n\thandlers["event"] = ConstHandler(const_handler)\n\tconsume[int](handlers)\n}\nfn main() {\n\tstartup[int]()\n}\n',
		'cannot use')
	...
	run_bad(v3_bin, 'bad_callback_array_append_in_generic',
		'type PlainHandler = fn (event &C.native_event)\ntype ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nfn plain_handler(event &C.native_event) {}\nfn const_handler(const_event &C.native_event) {}\nfn consume[T](handlers []PlainHandler) {}\nfn startup[T]() {\n\tmut handlers := [PlainHandler(plain_handler)]\n\thandlers << ConstHandler(const_handler)\n\tconsume[int](handlers)\n}\nfn main() {\n\tstartup[int]()\n}\n',
		'cannot use')
	mutator_matching := run_good(v3_bin, 'good_callback_array_mutator_calls_in_generic',
		'type ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nfn const_handler(const_event &C.native_event) {}\nfn consume[T](handlers []ConstHandler) bool {\n\treturn handlers.len == 3\n}\nfn startup[T]() bool {\n\tmut handlers := [ConstHandler(const_handler)]\n\thandlers.prepend(ConstHandler(const_handler))\n\thandlers.insert(1, ConstHandler(const_handler))\n\treturn consume[int](handlers)\n}\nfn main() {\n\tprintln(startup[int]().str())\n}\n')
	assert mutator_matching == 'true'
	run_bad(v3_bin, 'bad_callback_array_prepend_in_generic',
		'type PlainHandler = fn (event &C.native_event)\ntype ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nfn plain_handler(event &C.native_event) {}\nfn const_handler(const_event &C.native_event) {}\nfn consume[T](handlers []PlainHandler) {}\nfn startup[T]() {\n\tmut handlers := [PlainHandler(plain_handler)]\n\thandlers.prepend(ConstHandler(const_handler))\n\tconsume[int](handlers)\n}\nfn main() {\n\tstartup[int]()\n}\n',
		'cannot use')
	run_bad(v3_bin, 'bad_callback_array_insert_in_generic',
...
fn test_callback_spread_projection_and_terminating_branches_preserve_c_abi_modes() {
	v3_bin := build_v3()
	run_bad(v3_bin, 'bad_indexed_callback_from_spread_in_generic',
		'type PlainHandler = fn (event &C.native_event)\nstruct C.native_event {}\nfn consume[T](handler PlainHandler) {}\nfn startup[T]() {\n\tbase := [fn (const_event &C.native_event) {}]\n\tcallbacks := [...base]\n\tconsume[int](callbacks[0])\n}\nfn main() {\n\tstartup[int]()\n}\n',
		'cannot use')
	panic_branch := run_good(v3_bin, 'good_noreturn_callback_branch_in_generic',
		'type PlainHandler = fn (event &C.native_event)\ntype ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nfn plain_handler(event &C.native_event) {}\nfn const_handler(const_event &C.native_event) {}\nfn consume[T](handler PlainHandler) bool {\n\treturn true\n}\nfn startup[T](flag bool) bool {\n\tmut callback := PlainHandler(plain_handler)\n\tif flag {\n\t\tcallback = ConstHandler(const_handler)\n\t\tpanic("stop")\n\t}\n\treturn consume[int](callback)\n}\nfn main() {\n\tprintln(startup[int](false).str())\n}\n')
	assert panic_branch == 'true'
	break_branch := run_good(v3_bin, 'good_break_callback_branch_in_generic',
		'type PlainHandler = fn (event &C.native_event)\ntype ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nfn plain_handler(event &C.native_event) {}\nfn const_handler(const_event &C.native_event) {}\nfn consume[T](handler PlainHandler) bool {\n\treturn true\n}\nfn startup[T](flag bool) bool {\n\tmut callback := PlainHandler(plain_handler)\n\tfor {\n\t\tcallback = PlainHandler(plain_handler)\n\t\tif flag {\n\t\t\tcallback = ConstHandler(const_handler)\n\t\t\tbreak\n\t\t}\n\t\treturn consume[int](callback)\n\t}\n\treturn true\n}\nfn main() {\n\tprintln(startup[int](false).str())\n}\n')
	assert break_branch == 'true'
	continue_branch := run_good(v3_bin, 'good_continue_callback_branch_in_generic',
		'type PlainHandler = fn (event &C.native_event)\ntype ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nfn plain_handler(event &C.native_event) {}\nfn const_handler(const_event &C.native_event) {}\nfn consume[T](handler PlainHandler) bool {\n\treturn true\n}\nfn startup[T](flag bool) bool {\n\tmut callback := PlainHandler(plain_handler)\n\tfor _ in 0 .. 1 {\n\t\tcallback = PlainHandler(plain_handler)\n\t\tif flag {\n\t\t\tcallback = ConstHandler(const_handler)\n\t\t\tcontinue\n\t\t}\n\t\treturn consume[int](callback)\n\t}\n\treturn true\n}\nfn main() {\n\tprintln(startup[int](false).str())\n}\n')
...
fn test_callback_array_accessors_preserve_c_abi_modes() {
	...
	for method in ['first', 'last', 'pop', 'pop_left'] {
		run_bad(v3_bin, 'bad_callback_array_${method}_accessor_in_generic',
			'type PlainHandler = fn (event &C.native_event)\ntype ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nfn const_handler(const_event &C.native_event) {}\nfn consume[T](handler PlainHandler) {}\nfn startup[T]() {\n\tmut callbacks := [ConstHandler(const_handler)]\n\tconsume[int](callbacks.${method}())\n}\nfn main() {\n\tstartup[int]()\n}\n',
			'cannot use')
	}
...
fn test_callback_array_mutations_reach_index_projections() {
	...
	shifted_indexes := run_good(v3_bin, 'good_callback_insertions_shift_projected_indexes',
		'type PlainHandler = fn (event &C.native_event)\ntype ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nfn plain_handler(event &C.native_event) {}\nfn const_handler(const_event &C.native_event) {}\nfn consume_plain[T](handler PlainHandler) bool {\n\treturn true\n}\nfn consume_const[T](handler ConstHandler) bool {\n\treturn true\n}\nfn startup[T]() bool {\n\tmut prepended := [PlainHandler(plain_handler), ConstHandler(const_handler)]\n\tprepended.prepend(PlainHandler(plain_handler))\n\tmut inserted := [PlainHandler(plain_handler), ConstHandler(const_handler)]\n\tinserted.insert(1, PlainHandler(plain_handler))\n\tmut owned := [ConstHandler(const_handler)]\n\towned.prepend(PlainHandler(plain_handler))\n\treturn consume_plain[int](prepended[1]) && consume_const[int](inserted[2])\n\t\t&& consume_plain[int](owned[0])\n}\nfn main() {\n\tprintln(startup[int]().str())\n}\n')
	assert shifted_indexes == 'true'
	run_bad(v3_bin, 'bad_conditional_callback_prepend_shifts_prior_index',
		'type PlainHandler = fn (event &C.native_event)\ntype ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nfn plain_handler(event &C.native_event) {}\nfn const_handler(const_event &C.native_event) {}\nfn consume[T](handler ConstHandler) {}\nfn startup[T](cond bool) {\n\tmut handlers := [PlainHandler(plain_handler), ConstHandler(const_handler)]\n\tif cond {\n\t\thandlers.prepend(PlainHandler(plain_handler))\n\t}\n\tconsume[int](handlers[1])\n}\nfn main() {\n\tstartup[int](true)\n}\n',
		'cannot use')
	run_bad(v3_bin, 'bad_callback_index_assignment_shifted_by_prepend',
		'type PlainHandler = fn (event &C.native_event)\ntype ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nfn plain_handler(event &C.native_event) {}\nfn const_handler(const_event &C.native_event) {}\nfn consume[T](handler PlainHandler) {}\nfn startup[T]() {\n\tmut handlers := [PlainHandler(plain_handler)]\n\thandlers[0] = ConstHandler(const_handler)\n\thandlers.prepend(PlainHandler(plain_handler))\n\tconsume[int](handlers[1])\n}\nfn main() {\n\tstartup[int]()\n}\n',
		'cannot use')
...
fn test_callback_mut_arguments_invalidate_prior_provenance() {
	v3_bin := build_v3()
	run_bad(v3_bin, 'bad_callback_struct_rewritten_by_mut_helper_in_generic',
		'type PlainHandler = fn (event &C.native_event)\ntype ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nstruct Options {\nmut:\n\thandler PlainHandler\n}\nfn plain_handler(event &C.native_event) {}\nfn const_handler(const_event &C.native_event) {}\nfn rewrite(mut options Options) {\n\toptions.handler = ConstHandler(const_handler)\n}\nfn consume[T](options Options) {}\nfn startup[T]() {\n\tmut options := Options{\n\t\thandler: PlainHandler(plain_handler)\n\t}\n\trewrite(mut options)\n\tconsume[int](options)\n}\nfn main() {\n\tstartup[int]()\n}\n',
		'cannot use')
	matching := run_good(v3_bin, 'good_callback_mut_helper_then_full_overwrite_in_generic',
		'type PlainHandler = fn (event &C.native_event)\ntype ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nstruct Options {\nmut:\n\thandler PlainHandler\n}\nfn plain_handler(event &C.native_event) {}\nfn const_handler(const_event &C.native_event) {}\nfn rewrite[T](mut options Options) {\n\toptions.handler = ConstHandler(const_handler)\n}\nfn consume[T](options Options) bool {\n\treturn true\n}\nfn startup[T]() bool {\n\tmut options := Options{\n\t\thandler: PlainHandler(plain_handler)\n\t}\n\trewrite[int](mut options)\n\toptions = Options{\n\t\thandler: PlainHandler(plain_handler)\n\t}\n\treturn consume[int](options)\n}\nfn main() {\n\tprintln(startup[int]().str())\n}\n')
	assert matching == 'true'
}
...
fn test_callback_container_clear_kills_element_provenance() {
	...
	matching := run_good(v3_bin, 'good_callback_containers_cleared_in_generic',
		'type PlainHandler = fn (event &C.native_event)\ntype ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nfn const_handler(const_event &C.native_event) {}\nfn consume_array[T](handlers []PlainHandler) bool {\n\treturn handlers.len == 0\n}\nfn consume_map[T](handlers map[string]PlainHandler) bool {\n\treturn handlers.len == 0\n}\nfn startup[T]() bool {\n\tmut array_handlers := [ConstHandler(const_handler)]\n\tarray_handlers.clear()\n\tmut map_handlers := {"event": ConstHandler(const_handler)}\n\tmap_handlers.clear()\n\treturn consume_array[int](array_handlers) && consume_map[int](map_handlers)\n}\nfn main() {\n\tprintln(startup[int]().str())\n}\n')
	assert matching == 'true'
	run_bad(v3_bin, 'bad_callback_array_appended_after_clear_in_generic',
		'type PlainHandler = fn (event &C.native_event)\ntype ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nfn const_handler(const_event &C.native_event) {}\nfn consume[T](handlers []PlainHandler) {}\nfn startup[T]() {\n\tmut handlers := [ConstHandler(const_handler)]\n\thandlers.clear()\n\thandlers << ConstHandler(const_handler)\n\tconsume[int](handlers)\n}\nfn main() {\n\tstartup[int]()\n}\n',
		'cannot use')
}
...
fn test_fn_literal_nested_c_abi_const_mode_matches_alias_inside_generic_fn() {
	...
	run_bad(v3_bin, 'bad_nested_const_callback_alias_in_generic',
		...
		'cannot use')
	late_type := run_good(v3_bin, 'good_const_callback_alias_declared_before_param_type',
		'type Handler = fn (item Later, const_event &C.native_event)\nstruct C.native_event {}\nstruct Later {}\nstruct Router {}\nfn (mut r Router) accept(handler Handler) bool {\n\treturn true\n}\nfn startup[T]() bool {\n\tmut r := Router{}\n\treturn r.accept(fn (item Later, const_event &C.native_event) {})\n}\nfn main() {\n\tprintln(startup[int]().str())\n}\n')
	assert late_type == 'true'
	...
	run_bad(v3_bin, 'bad_option_wrapped_const_callback_alias_in_generic',
		'type Outer = fn (callback ?fn (const_event &C.native_event))\nstruct C.native_event {}\nstruct Router {}\nfn (mut r Router) accept(handler Outer) {}\nfn startup[T]() {\n\tmut r := Router{}\n\tr.accept(fn (callback ?fn (event &C.native_event)) {})\n}\nfn main() {\n\tstartup[int]()\n}\n',
		'cannot use')
	tuple_factory := run_good(v3_bin, 'good_tuple_returned_const_callback_alias_in_generic',
		'type Factory = fn () (fn (const_event &C.native_event), int)\nstruct C.native_event {}\nstruct Router {}\nfn (mut r Router) accept(factory Factory) bool {\n\treturn true\n}\nfn startup[T]() bool {\n\tmut r := Router{}\n\treturn r.accept(fn () (fn (const_event &C.native_event), int) {\n\t\treturn fn (const_event &C.native_event) {}, 1\n\t})\n}\nfn main() {\n\tprintln(startup[int]().str())\n}\n')
	...
	caller_alias_collision := run_good_project(v3_bin,
		'good_imported_generic_const_alias_ignores_caller_homonym', {
		'main.v': 'module main\n\nimport m\n\nstruct C.native_event {}\ntype Outer[T] = fn (event &T)\nstruct Router {}\nfn (mut r Router) accept(handler m.Outer[C.native_event]) bool {\n\treturn true\n}\nfn startup[T]() bool {\n\tmut r := Router{}\n\treturn r.accept(fn (const_event &C.native_event) {})\n}\nfn main() {\n\tprintln(startup[int]().str())\n}\n'
		'm/m.v':  'module m\n\npub type OuterExtra[T] = fn (event &T)\npub type Outer[T] = fn (const_event &T)\n'
	}, 'main.v')
	assert caller_alias_collision == 'true'
	run_bad_project(v3_bin, 'bad_imported_generic_plain_alias_ignores_caller_homonym', {
...
fn test_fn_literal_container_callback_modes_inside_generic_fn() {
	...
	run_bad(v3_bin, 'bad_parenthesized_array_literal_const_fn_param_in_generic',
		'type PlainHandler = fn (event &C.native_event)\nstruct C.native_event {}\nstruct Router {}\nfn (r Router) apply(handlers []PlainHandler) {}\nfn startup[T]() {\n\tr := Router{}\n\tr.apply(([fn (const_event &C.native_event) {}]))\n}\nfn main() {\n\tstartup[int]()\n}\n',
		'cannot use')
	optional := run_good(v3_bin, 'good_optional_array_literal_const_fn_param_in_generic',
		'type ConstHandler = fn (const_event &C.native_event)\nstruct C.native_event {}\nstruct Router {}\nfn (r Router) apply(handlers ?[]ConstHandler) bool {\n\treturn handlers != none\n}\nfn startup[T]() bool {\n\tr := Router{}\n\treturn r.apply([fn (const_event &C.native_event) {}])\n}\nfn main() {\n\tprintln(startup[int]().str())\n}\n')
	assert optional == 'true'
	run_bad(v3_bin, 'bad_optional_array_literal_const_fn_param_in_generic',
		'type PlainHandler = fn (event &C.native_event)\nstruct C.native_event {}\nstruct Router {}\nfn (r Router) apply(handlers ?[]PlainHandler) {}\nfn startup[T]() {\n\tr := Router{}\n\tr.apply([fn (const_event &C.native_event) {}])\n}\nfn main() {\n\tstartup[int]()\n}\n',
		'cannot use')
