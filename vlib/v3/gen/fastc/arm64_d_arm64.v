...
fn (mut p FastArm64Program) register_os_path_runtime() {
	for key, signature in p.functions {
		...
		trimmed_pointer := p.instr2(.add, trim_root, p.ptr_i8, normalized_pointer, one64)
		trimmed_result_length := p.instr2(.sub, trim_root, p.i32_type, normalized_length, one32)
		p.instr2(.store, trim_root, p.void_type, trimmed_pointer, p.string_field_ptr(trim_root, result_slot, 0))
...
fn (mut p FastArm64Program) register_array_new_runtime() {
	...
	empty := p.m.add_block(id, 'array_new_empty')
	allocate := p.m.add_block(id, 'array_new_allocate')
	initialize := p.m.add_block(id, 'array_new_initialize')
	...
	capacity_is_zero := p.instr2(.eq, ready, p.i1_type, normalized_capacity, zero64)
	p.instr3(.br, ready, p.void_type, capacity_is_zero, ssa.ValueID(empty), ssa.ValueID(allocate))
	zero_pointer := p.m.get_or_add_const(p.ptr_i8, '0')
	p.instr2(.store, empty, p.void_type, zero_pointer, p.struct_field_ptr(empty, slot, p.array_type, 0))
	p.instr1(.jmp, empty, p.void_type, ssa.ValueID(initialize))
	...
	p.instr1(.jmp, allocate, p.void_type, ssa.ValueID(initialize))
	zero32 := p.m.get_or_add_const(p.i32_type, '0')
	p.instr2(.store, initialize, p.void_type, zero32, p.struct_field_ptr(initialize, slot, p.array_type, 1))
	length32 := p.instr1(.trunc, initialize, p.i32_type, length)
	capacity32 := p.instr1(.trunc, initialize, p.i32_type, normalized_capacity)
	element_size32 := p.instr1(.trunc, initialize, p.i32_type, element_size)
	for field, value in {
		2: length32
		3: capacity32
		...
		p.instr2(.store, initialize, p.void_type, value, p.struct_field_ptr(initialize, slot, p.array_type, field))
	}
	result := p.instr1(.load, initialize, p.array_type, slot)
	p.instr1(.ret, initialize, p.void_type, result)
}
