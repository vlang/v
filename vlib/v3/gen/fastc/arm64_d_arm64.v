...
pub fn generate_arm64_files(paths []string, prefs &pref.Preferences, output string) !FastArm64Generation {
	...
	type_decls, constant_sources, enum_values := fast_arm64_collect_declarations(sources, prefs, declared_types, type_source_paths)!
...
fn fast_arm64_collect_declarations(sources []FastcSourceFile, prefs &pref.Preferences, declared_types map[string]bool, type_source_paths map[string]bool) !(map[string]FastArm64TypeDecl, map[string]string, map[string]string) {
	...
	for source_file in sources {
		...
		fast_arm64_collect_source_declarations(source_file, prefs, declared_types, mut declarations, mut constants, mut enum_values)!
	}
...
fn fast_arm64_collect_source_declarations(source_file FastcSourceFile, prefs &pref.Preferences, declared_types map[string]bool, mut declarations map[string]FastArm64TypeDecl, mut constants map[string]string, mut enum_values map[string]string) ! {
	mut file_set := token.FileSet.new()
	mut file := file_set.add_file(source_file.path, source_file.source.len)
	file.index_lines_without_digest(source_file.source)
	mut scan := scanner.new_scanner(prefs, .normal)
		...
		if depth == 0 && tok == .dollar {
			...
			if lookahead.scan() == .key_if {
				...
				if selected.source != '' {
					...
					}, prefs, declared_types, mut declarations, mut constants, mut enum_values)!
				}
				tok = selected.tok
				continue
			}
...
fn (mut p FastArm64Program) register_map_get_runtime() {
	...
	missing_val_size := p.instr1(.load, missing_state, p.i64_type, p.struct_field_ptr(missing_state, state, p.map_state_type, 5))
	one := p.m.get_or_add_const(p.i64_type, '1')
	calloc_ref := p.m.add_value(.func_ref, p.ptr_i8, 'calloc', p.fn_ids['calloc'])
	missing_value := p.m.add_instr(.call, missing_state, p.ptr_i8, [calloc_ref, one,
		missing_val_size])
	p.instr1(.ret, missing_state, p.void_type, missing_value)
	empty_size := p.m.get_or_add_const(p.i64_type, '256')
	empty_value := p.m.add_instr(.call, missing_empty, p.ptr_i8, [calloc_ref, one, empty_size])
...
fn (mut p FastArm64Parser) parse_name_statement(after_mut bool) ! {
	...
	if next_token == .comma {
		...
		for i, item_name in names {
			...
			p.locals[item_name] = FastArm64Local{
				addr: address
				typ: item_type
		...
		if p.program.m.type_store.types[local.typ].kind == .ptr_t {
			...
			if op != .assign {
				...
				value = FastArm64Value{
					id: p.program.instr2(fast_arm64_compound_opcode(op), p.cur_block, element_type, current, value.id)
					typ: element_type
				}
			}
		...
		if local.typ == p.program.array_type {
			...
			if op != .assign {
				...
				assigned = FastArm64Value{
					id: p.program.instr2(fast_arm64_compound_opcode(op), p.cur_block, element_type, current, assigned.id)
					typ: element_type
					typ_name: element_type_name
				}
			}
	...
	if next_token in [.assign, .plus_assign, .minus_assign, .mul_assign, .div_assign, .mod_assign,
		...
		if op != .assign {
			...
			if op == .plus_assign && local.typ == p.program.str_type {
				...
				binary := fast_arm64_compound_opcode(op)
				value = FastArm64Value{
	...
	if p.tok in [.assign, .plus_assign, .minus_assign, .mul_assign, .div_assign, .mod_assign,
		...
		if op != .assign {
			if op == .plus_assign && left.typ == p.program.str_type {
				...
				result = p.program.instr2(fast_arm64_compound_opcode(op), p.cur_block, left.typ, left.id, right.id)
			}
		}
...
fn (mut p FastArm64Parser) emit_deferred_statements() ! {
	for i := p.defer_sources.len - 1; i >= 0; i-- {
		p.parse_selected_statements(p.defer_sources[i])!
	}
}
...
fn (mut p FastArm64Parser) parse_for() ! {
	...
	p.mark_terminated(p.cur_block)
	p.break_to << done_block
	p.continue_to << condition_block
...
fn (mut p FastArm64Parser) parse_collection_for(index_name string, name string, collection FastArm64Value) ! {
	...
	if index_name != '_' && index_name != '' {
		p.locals[index_name] = FastArm64Local{
			addr: index_slot
			typ: p.program.i32_type
			typ_name: 'int'
		}
...
fn (mut p FastArm64Parser) parse_map_collection_for(key_name string, value_name string, collection FastArm64Value) ! {
	...
	if key_name != '' && key_name != '_' {
		p.locals[key_name] = FastArm64Local{
			addr: key_address
			typ: key_type
...
fn (mut p FastArm64Parser) parse_c_for() ! {
	...
	if p.tok == .lcbr {
		...
		if p.tok in [.inc, .dec] {
			...
			current := p.program.instr1(.load, increment_block, initial.typ, address)
			next := p.program.instr2(fast_arm64_compound_opcode(op), increment_block, initial.typ, current, step.id)
			p.program.instr2(.store, increment_block, p.program.void_type, next, address)
		} else {
...
fn (mut p FastArm64Parser) parse_interpolated_string(first_literal string, quote u8) !FastArm64Value {
	...
	for p.tok == .str_dollar {
		...
		p.expect(.rcbr)!
		interpolated := p.stringify(expression)!
...
fn (mut p FastArm64Parser) parse_selector(value FastArm64Value) !FastArm64Value {
	...
	if p.tok == .lpar {
		...
		if value.typ == p.program.array_type && member in ['prepend', 'insert'] {
			...
			if member == 'insert' {
				...
				item := p.parse_expression(0)!
				p.expect(.rpar)!
				zero := p.program.m.get_or_add_const(index.typ, '0')
				is_zero := p.program.instr2(.eq, p.cur_block, p.program.i1_type, index.id, zero)
				// FastC self-host currently uses insert only for index zero.
				_ = is_zero
	...
	if value.typ == p.program.map_type && member == 'len' {
		...
		return FastArm64Value{
			id: p.program.instr1(.trunc, p.cur_block, p.program.i32_type, length64)
			typ: p.program.i32_type
			typ_name: 'int'
		}
	}
...
fn (mut p FastArm64Parser) parse_array_index_or_slice(value FastArm64Value) !FastArm64Value {
	...
	new_offset := p.program.instr2(.add, p.cur_block, p.program.i32_type, offset, start.id)
	new_capacity := p.program.instr2(.sub, p.cur_block, p.program.i32_type, capacity, start.id)
	new_length := if has_end {
		p.program.instr2(.sub, p.cur_block, p.program.i32_type, end.id, start.id)
	} else {
		p.program.instr2(.sub, p.cur_block, p.program.i32_type, length, start.id)
	}
...
fn (mut p FastArm64Parser) parse_call(key string, display_name string) !FastArm64Value {
	...
	if display_name in ['C.FD_ZERO', 'C.FD_SET', 'C.FD_ISSET'] {
		...
		if display_name == 'C.FD_ISSET' {
			return p.zero_value(p.program.i32_type, 'int')
		}
		return FastArm64Value{
			typ: p.program.void_type
			typ_name: 'void'
...
fn fast_arm64_compound_opcode(op token.Token) ssa.OpCode {
	return match op {
		.plus_assign { ssa.OpCode.add }
		.minus_assign { ssa.OpCode.sub }
		.mul_assign { ssa.OpCode.mul }
		.div_assign { ssa.OpCode.sdiv }
		.mod_assign { ssa.OpCode.srem }
		.left_shift_assign { ssa.OpCode.shl }
		.right_shift_assign { ssa.OpCode.ashr }
...
fn (mut p FastArm64Parser) emit_array_push(array FastArm64Value, item FastArm64Value, prepend bool) !FastArm64Value {
	...
	new_data := p.program.m.add_instr(.call, grow, p.program.ptr_i8, [realloc_ref, old_data,
		allocation_size])
	p.program.instr2(.store, grow, p.program.void_type, new_data, data_ptr)
	p.program.instr2(.store, grow, p.program.void_type, new_capacity, cap_ptr)
	p.program.instr2(.store, grow, p.program.void_type, element_size32, p.program.struct_field_ptr(grow, array_slot, p.program.array_type, 5))
	p.program.instr1(.jmp, grow, p.program.void_type, ssa.ValueID(append))
	...
	if prepend {
		bytes := p.program.instr2(.mul, append, p.program.i64_type, append_length64, element_size)
		destination := p.program.instr2(.add, append, p.program.ptr_i8, append_data, element_size)
		memmove_ref := p.program.m.add_value(.func_ref, p.program.ptr_i8, 'memmove', p.program.fn_ids['memmove'])
		p.program.m.add_instr(.call, append, p.program.ptr_i8, [memmove_ref, destination,
			append_data, bytes])
	}
	offset := if prepend {
		p.program.m.get_or_add_const(p.program.i64_type, '0')
	} else {
...
fn (mut p FastArm64Parser) emit_array_append_many(array FastArm64Value, items FastArm64Value, element_type_name string, prepend bool) !FastArm64Value {
	...
	p.mark_terminated(p.cur_block)
	element_size := p.program.m.get_or_add_const(p.program.i64_type, p.program.m.type_size(element_type).str())
	required64 := p.program.instr1(.zext, grow, p.program.i64_type, required)
	allocation_size := p.program.instr2(.mul, grow, p.program.i64_type, required64, element_size)
	data_ptr := p.program.struct_field_ptr(grow, array_slot, p.program.array_type, 0)
	old_data := p.program.instr1(.load, grow, p.program.ptr_i8, data_ptr)
	realloc_ref := p.program.m.add_value(.func_ref, p.program.ptr_i8, 'realloc', p.program.fn_ids['realloc'])
	new_data := p.program.m.add_instr(.call, grow, p.program.ptr_i8, [realloc_ref, old_data,
		allocation_size])
	p.program.instr2(.store, grow, p.program.void_type, new_data, data_ptr)
	p.program.instr2(.store, grow, p.program.void_type, required, capacity_ptr)
	element_size32 := p.program.m.get_or_add_const(p.program.i32_type, p.program.m.type_size(element_type).str())
	p.program.instr2(.store, grow, p.program.void_type, element_size32, p.program.struct_field_ptr(grow, array_slot, p.program.array_type, 5))
	p.program.instr1(.jmp, grow, p.program.void_type, ssa.ValueID(copy_block))
	p.mark_terminated(grow)
