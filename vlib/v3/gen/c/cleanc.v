...
fn (mut g FlatGen) gen_expr_with_expected_type(id flat.NodeId, expected types.Type) {
	...
	if g.gen_sum_pointer_default_expr(node, expected) {
		g.expected_expr_type = old_expected
		g.expected_enum = old_expected_enum
		return
	}
	...
	if expected is types.Pointer && g.gen_pointer_alias_value_cast_addr(id, expected) {
		g.expected_expr_type = old_expected
		g.expected_enum = old_expected_enum
		return
	}
...
fn (mut g FlatGen) gen_pointer_alias_value_cast_addr(id flat.NodeId, expected types.Pointer) bool {
	...
	g.write('(${expected_ct}*)memdup(&(${target_ct}[]){')
	g.gen_expr(g.a.child(child, 0))
	g.write('}[0], sizeof(${target_ct}))')
	return true
}
...
fn (mut g FlatGen) gen_sum_value_expr(id flat.NodeId, expected types.Type) bool {
	...
	if g.variant_references_sum(variant, sum_name) {
		...
		g.gen_sum_variant_memdup_source(id, inner_type)
...
fn (mut g FlatGen) fixed_array_c_type(arr types.ArrayFixed) string {
	...
	return 'Array_fixed_${g.fixed_array_elem_name_part(arr.elem_type)}_${len_name}'
}
...
fn (mut g FlatGen) fixed_array_elem_name_part(elem types.Type) string {
	...
	return naming.type_name_part(g.fixed_array_elem_c_type(elem))
}
...
fn (mut g FlatGen) builtin_abi_decls() {
	...
	g.writeln('#ifdef _WIN32')
	g.writeln('uintptr_t raw = (uintptr_t)malloc((size_t)sz + alignment - 1); if (raw != 0) p = (void*)((raw + alignment - 1) & ~(uintptr_t)(alignment - 1));')
	g.writeln('#else')
