...
fn (mut g FlatGen) gen_method_value_closure(base_id flat.NodeId, base_type types.Type, method string) bool {
	...
	if method_key.len > 0 {
	...
	} else if ci := g.tc.generic_method_value_info['${recv_name}.${method}'] {
		...
		ret = ci.return_type
		cname = g.cname('${recv_name}.${method}')
	} else {
		return false
	}
	if params.len == 0 {
