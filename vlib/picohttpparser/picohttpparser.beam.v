// BEAM Backend: HTTP response builder for picohttpparser.
// Most response methods use pure V string operations and work correctly on BEAM.
// The end() method (which normally calls C.send) returns the byte count but does not
// perform actual network I/O. On BEAM, use gen_tcp:send/2 for real network writes.
//
// NOTE: request.v, misc.v, picohttpparser.v are plain .v files and NOT duplicated here.
// Only response.c.v needs BEAM stubs since it contains C FFI calls.
...
// end finalizes the response and returns the byte count.
// BEAM: computes response size but does not send over network. Use gen_tcp:send/2.
@[inline]
pub fn (mut r Response) end() int {
	n := int(i64(r.buf) - i64(r.buf_start))
	// BEAM: returns byte count. Actual network send requires gen_tcp:send(Socket, Data).
	return n
}
