...
pub fn (x Integer) gcd_binary( y Integer ) Integer {
  ...
  if x.signum < 0 { return x.neg( ).gcd( y )}
  if y.signum < 0 { return x.gcd( y.neg()  )}
...
/*
The following test-cases show, that the best one might gain by entering
'unsafe-territories' is around 1-3-percent on my machine. Thus its
not important :
- pointer-access on digits.data-buffer in Integer.u32msb() and Integer.u64msb()
- 64-bit-pointer access on the u32-buffer

I leave it in here, maybe for educational purposes : How not to do it.
Feel free to remove them.
*/


fn (bi Integer) u32msb() u32 {
  for
    idx := 0;
    idx  < bi.digits.len;
    idx += 1
  {
    word := unsafe { &u32( bi.digits.data )[idx] }
    if word > 0 {
      return u32( (idx*32) + bits.trailing_zeros_32( word ) )
    }
  }
...
pub fn (x Integer) u32binary( y Integer ) Integer {
...
fn (bi Integer) u64msb() u32 {
  ...
  for
    ...
    idx += 1
  {
    word := unsafe { &u64( bi.digits.data )[idx] }
    if word > 0 {
      return u32( (idx * 64) + bits.trailing_zeros_64( word ) )
    }
  }

  if bi.digits.len % 2 > 0 {
    return u32( (bi.digits.len-1)*32) + u32(
      unsafe{
        bits.trailing_zeros_32(
          &u32( bi.digits.data )[bi.digits.len-1]
    )})
  }
  return u32( 32 )
  }
pub fn (x Integer) u64binary( y Integer ) Integer {
