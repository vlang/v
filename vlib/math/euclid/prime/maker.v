...
pub fn( p PrimeSet ) to_primeset () PrimeSet { return p }
...
fn extract_count( s string ) int {
  ...
  if (s=='') || !s.split('').any( it in digits ){ return 0 }
...
fn read_toml_file() map[string][]string {
  ...
  tm_doc := toml.parse_file( './primes.toml' ) or {
    panic( err )
  }
  ...
  for k in tm_primes.keys() {
    p[k] = []string{}
    arr := tm_primes[k] or {
      panic('expected a map[string][]string in TOML-data ? corrupt ?')
    }
    for _, elem in arr.array() { p[k] << elem as string }
  }
  return p
}
...
pub fn random_list( cfg []string ) []string {
  ...
  match cfg.len {
    1 { // prime-size e.g. 'xs' given
      if cfg[0] !in primes { return p_list }
      else { return primes[ cfg[0] ]}
    }
    2 { // prime-size and limiter given e.g 'xs.15'
      prime_size  := cfg[0]
      if prime_size !in primes { return p_list }
      ...
      mut num := ''
      for prime_count != 0 {
        num = primes[ prime_size ][
          rand.int_in_range( 0, primes[prime_size].len - 1 )
        ]
        if num in p_list { continue }
        p_list << num
        prime_count -= 1
