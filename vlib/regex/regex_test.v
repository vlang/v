...
const (
	cgroups_test_suite = [
		TestItemCGroup{'http://www.ciao.mondo/hello/pippo12_/pera.html', r'(?P<format>https?)|(?:ftps?)://(?P<token>[\w_]+[\.|/])+', 0, 42, [
			7, 0, 0, 4, 1, 7, 11, 1, 11, 16, 1, 16, 22, 1, 22, 28, 1, 28, 37, 1, 37, 42], map{
			'format': int(0)
			'token':  1
		}},
		TestItemCGroup{'http://www.ciao.mondo/hello/pippo12_/pera.html', r'(?P<format>https?)|(?P<format>ftps?)://(?P<token>[\w_]+.)+', 0, 46, [
			8, 0, 0, 4, 1, 7, 11, 1, 11, 16, 1, 16, 22, 1, 22, 28, 1, 28, 37, 1, 37, 42, 1, 42,
			46,
		], map{
			'format': int(0)
			'token':  1
		...
		TestItemCGroup{'http://www.ciao.mondo/hello/pippo12_/pera.html', r'(?P<format>https?)|(?P<format>ftps?)://([\w_]+\.)+', 0, 16, [
			3, 0, 0, 4, 1, 7, 11, 1, 11, 16], map{
			'format': int(0)
		}},
		TestItemCGroup{'acc +13 pippo', r'(\w+)\s(.)([0-9]+) \w+', 0, 13, [0, 3, 4, 5, 5, 7], map[string]int{}},
		TestItemCGroup{'acc +13', r'(\w+)\s(.)([0-9]+)', 0, 7, [0, 3, 4, 5, 5, 7], map[string]int{}},
		TestItemCGroup{'ababababac', r'ab(.*)(ac)', 0, 10, [2, 8, 8, 10], map[string]int{}},
	]
)
