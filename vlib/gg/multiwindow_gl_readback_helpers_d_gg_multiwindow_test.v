...
fn C.v_gg_multiwindow_fake_gl_image_case(stale_errors int, operation_error int, permanent_error int, pack_alignment &int, read_framebuffer &int, draw_framebuffer &int, read_buffer &int, get_integer_calls &int, read_calls &int, delete_calls &int, get_error_calls &int) int
...
fn test_gl_image_readback_drains_only_stale_errors_and_restores_separate_framebuffer_state() {
	...
	result := C.v_gg_multiwindow_fake_gl_image_case(2, 0, 0, &pack_alignment, &read_framebuffer,
		&draw_framebuffer, &read_buffer, &get_integer_calls, &read_calls, &delete_calls,
		&get_error_calls)
	assert result == 0
	assert pack_alignment == 8
	...
	for operation_error in [1, 2, 3] {
		...
		failed := C.v_gg_multiwindow_fake_gl_image_case(0, operation_error, 0, &pack_alignment,
			&read_framebuffer, &draw_framebuffer, &read_buffer, &get_integer_calls, &read_calls,
			&delete_calls, &get_error_calls)
		assert failed == 3
		assert pack_alignment == 8
		assert read_framebuffer == 17
		assert draw_framebuffer == 29
