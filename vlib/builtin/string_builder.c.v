...
pub fn (mut b StringBuilder) indent(s string, param VIndentParam) {
	...
	vindent_state_normal := 0 // Normal state, processing regular characters
	vindent_state_in_string := 1 // Inside a string literal, ignoring formatting characters
	mut state := vindent_state_normal
	...
	for i := 0; i < s.len; i++ {
		...
		match state {
			...
			vindent_state_normal {
				match c {
					`"`, `'` { // Note: quote characters for editor display "
						state = vindent_state_in_string
						string_char = c
