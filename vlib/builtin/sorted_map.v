...
fn new_sorted_map(n, value_bytes int) sorted_map { // TODO: Remove `n`
	return sorted_map {
...
fn new_sorted_map_init(n, value_bytes int, keys &string, values voidptr) sorted_map {
...
fn (m mut sorted_map) set(key string, value voidptr) {
	mut node := m.root
	mut child_index := 0
...
fn (m sorted_map) get(key string, out voidptr) bool {
	mut node := m.root
	for {
		mut i := node.size - 1
...
fn (m sorted_map) exists(key string) bool {
...
pub fn (m mut sorted_map) delete(key string) {
	if m.root.size == 0 {
		return
	}
...
pub fn (m &sorted_map) keys() []string {
