...
__global g_autostr_addr_type_stack = [autostr_type_stack_max_depth]int{}
...
fn autostr_addr_push(addr voidptr) {
	...
	g_autostr_addr_type_stack[g_autostr_addr_stack_len] = 0
...
fn autostr_addr_type_in_stack(addr voidptr, typ int) bool {
	for i := 0; i < g_autostr_addr_stack_len; i++ {
		if g_autostr_addr_stack[i] == addr && g_autostr_addr_type_stack[i] == typ {
			return true
		}
	}
	return false
...
fn autostr_addr_type_push(addr voidptr, typ int) {
	...
	g_autostr_addr_stack[g_autostr_addr_stack_len] = addr
	g_autostr_addr_type_stack[g_autostr_addr_stack_len] = typ
	g_autostr_addr_stack_len++
}
