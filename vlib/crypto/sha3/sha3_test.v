module sha3

fn test_api_call() {
	input := 'abc'.bytes()

	// Digest generation
	mut digest_224 := new_digest(rate_224, size_224)!
	mut digest_256 := new_digest(rate_256, size_256)!
	mut digest_384 := new_digest(rate_384, size_384)!
	mut digest_512 := new_digest(rate_512, size_512)!
	mut digest_keccak_224 := new_digest(rate_224, size_224, padding: .keccak)!
	mut digest_keccak_256 := new_digest(rate_256, size_256, padding: .keccak)!
	mut digest_keccak_384 := new_digest(rate_384, size_384, padding: .keccak)!
	mut digest_keccak_512 := new_digest(rate_512, size_512, padding: .keccak)!

	// Result should be same as shortened version
	assert digest_224 == new224()!
	assert digest_256 == new256()!
	assert digest_384 == new384()!
	assert digest_512 == new512()!
	assert digest_keccak_256 == new256keccak()!
	assert digest_keccak_512 == new512keccak()!

	// Should not panic
	digest_224.write(input)!
	digest_256.write(input)!
	digest_384.write(input)!
	digest_512.write(input)!
	digest_keccak_256.write(input)!
	digest_keccak_512.write(input)!

	// Result should be same as shortened version
	assert digest_224.checksum() == sum224(input)
	assert digest_256.checksum() == sum256(input)
	assert digest_384.checksum() == sum384(input)
	assert digest_512.checksum() == sum512(input)
	assert digest_keccak_256.checksum() == keccak256(input)
	assert digest_keccak_512.checksum() == keccak512(input)

	for size in 1 .. 65 {
		// Digest generation
		mut digest_xof_128 := new_digest(xof_rate_128, size, padding: .xof)!
		mut digest_xof_256 := new_digest(xof_rate_256, size, padding: .xof)!

		// Result should be same as shortened version
		assert digest_xof_128 == new_xof_digest(xof_rate_128, size)!
		assert digest_xof_256 == new_xof_digest(xof_rate_256, size)!
		assert digest_xof_128 == new128xof(size)!
		assert digest_xof_256 == new256xof(size)!

		// Should not panic
		digest_xof_128.write(input)!
		digest_xof_256.write(input)!
	}

	// Should be AbsorptionRateError
	if should_be_error := new_digest(rate_224, size_256) {
		assert false, 'new_digest(rate_224, size_256) should be error'
	}
	if should_be_error := new_digest(rate_224, size_256, padding: .keccak) {
		assert false, 'new_digest(rate_224, size_256, padding: .keccak) should be error'
	}
	if should_be_error := new_digest(0, size_256) {
		assert false, 'new_digest(0, size_256) should be error'
	}
	if should_be_error := new_digest(0, size_256, padding: .keccak) {
		assert false, 'new_digest(0, size_256, padding: .keccak) should be error'
	}

	// Should be HashSizeError
	if should_be_error := new_digest(rate_256, -1) {
		assert false, 'new_digest(rate_256, -1) should be error'
	}
	if should_be_error := new_digest(rate_256, -1, padding: .keccak) {
		assert false, 'new_digest(rate_256, -1, padding: .keccak) should be error'
	}
	if should_be_error := new_digest(rate_256, 100) {
		assert false, 'new_digest(rate_256, -1) should be error'
	}
	if should_be_error := new_digest(rate_256, 100, padding: .keccak) {
		assert false, 'new_digest(rate_256, -1, padding: .keccak) should be error'
	}

	// Should be XOFSizeError
	if should_be_error := new128xof(-1) {
		assert false, 'new128xof(-1) should be error'
	}
	if should_be_error := new256xof(-1) {
		assert false, 'new256xof(-1) should be error'
	}
	// Shoud be XOFRateError
	if should_be_error := new_xof_digest(0, size_256) {
		assert false, 'new_xof_digest(0, size_256) should be error'
	}
}

const empty_message_sha3_224 = [u8(0x6B), 0x4E, 0x03, 0x42, 0x36, 0x67, 0xDB, 0xB7, 0x3B, 0x6E,
	0x15, 0x45, 0x4F, 0x0E, 0xB1, 0xAB, 0xD4, 0x59, 0x7F, 0x9A, 0x1B, 0x07, 0x8E, 0x3F, 0x5B, 0x5A,
	0x6B, 0xC7]
const empty_message_sha3_256 = [u8(0xA7), 0xFF, 0xC6, 0xF8, 0xBF, 0x1E, 0xD7, 0x66, 0x51, 0xC1,
	0x47, 0x56, 0xA0, 0x61, 0xD6, 0x62, 0xF5, 0x80, 0xFF, 0x4D, 0xE4, 0x3B, 0x49, 0xFA, 0x82, 0xD8,
	0x0A, 0x4B, 0x80, 0xF8, 0x43, 0x4A]
const empty_message_sha3_384 = [u8(0x0C), 0x63, 0xA7, 0x5B, 0x84, 0x5E, 0x4F, 0x7D, 0x01, 0x10,
	0x7D, 0x85, 0x2E, 0x4C, 0x24, 0x85, 0xC5, 0x1A, 0x50, 0xAA, 0xAA, 0x94, 0xFC, 0x61, 0x99, 0x5E,
	0x71, 0xBB, 0xEE, 0x98, 0x3A, 0x2A, 0xC3, 0x71, 0x38, 0x31, 0x26, 0x4A, 0xDB, 0x47, 0xFB, 0x6B,
	0xD1, 0xE0, 0x58, 0xD5, 0xF0, 0x04]
const empty_message_sha3_512 = [u8(0xA6), 0x9F, 0x73, 0xCC, 0xA2, 0x3A, 0x9A, 0xC5, 0xC8, 0xB5,
	0x67, 0xDC, 0x18, 0x5A, 0x75, 0x6E, 0x97, 0xC9, 0x82, 0x16, 0x4F, 0xE2, 0x58, 0x59, 0xE0, 0xD1,
	0xDC, 0xC1, 0x47, 0x5C, 0x80, 0xA6, 0x15, 0xB2, 0x12, 0x3A, 0xF1, 0xF5, 0xF9, 0x4C, 0x11, 0xE3,
	0xE9, 0x40, 0x2C, 0x3A, 0xC5, 0x58, 0xF5, 0x00, 0x19, 0x9D, 0x95, 0xB6, 0xD3, 0xE3, 0x01, 0x75,
	0x85, 0x86, 0x28, 0x1D, 0xCD, 0x26]
const empty_message_keccak_256 = [u8(0xC5), 0xD2, 0x46, 0x01, 0x86, 0xF7, 0x23, 0x3C, 0x92, 0x7E,
	0x7D, 0xB2, 0xDC, 0xC7, 0x03, 0xC0, 0xE5, 0x00, 0xB6, 0x53, 0xCA, 0x82, 0x27, 0x3B, 0x7B, 0xFA,
	0xD8, 0x04, 0x5D, 0x85, 0xA4, 0x70]
const empty_message_keccak_512 = [u8(0x0E), 0xAB, 0x42, 0xDE, 0x4C, 0x3C, 0xEB, 0x92, 0x35, 0xFC,
	0x91, 0xAC, 0xFF, 0xE7, 0x46, 0xB2, 0x9C, 0x29, 0xA8, 0xC3, 0x66, 0xB7, 0xC6, 0x0E, 0x4E, 0x67,
	0xC4, 0x66, 0xF3, 0x6A, 0x43, 0x04, 0xC0, 0x0F, 0xA9, 0xCA, 0xF9, 0xD8, 0x79, 0x76, 0xBA, 0x46,
	0x9B, 0xCB, 0xE0, 0x67, 0x13, 0xB4, 0x35, 0xF0, 0x91, 0xEF, 0x27, 0x69, 0xFB, 0x16, 0x0C, 0xDA,
	0xB3, 0x3D, 0x36, 0x70, 0x68, 0x0E]

fn test_0_length_hash() {
	input := []u8{}

	output_224 := sum224(input)
	assert output_224 == empty_message_sha3_224

	output_256 := sum256(input)
	assert output_256 == empty_message_sha3_256

	output_384 := sum384(input)
	assert output_384 == empty_message_sha3_384

	output_512 := sum512(input)
	assert output_512 == empty_message_sha3_512

	output_keccak_256 := keccak256(input)
	assert output_keccak_256 == empty_message_keccak_256

	output_keccak_512 := keccak512(input)
	assert output_keccak_512 == empty_message_keccak_512
}

const input_200 = [u8(0xa3), ]

const test_200_message_sha3_224 = [u8(0x93), 0x76, 0x81, 0x6A, 0xBA, 0x50, 0x3F, 0x72, 0xF9, 0x6C,
	0xE7, 0xEB, 0x65, 0xAC, 0x09, 0x5D, 0xEE, 0xE3, 0xBE, 0x4B, 0xF9, 0xBB, 0xC2, 0xA1, 0xCB, 0x7E,
	0x11, 0xE0]

const test_200_message_sha3_256 = [u8(0x79), 0xF3, 0x8A, 0xDE, 0xC5, 0xC2, 0x03, 0x07, 0xA9, 0x8E,
	0xF7, 0x6E, 0x83, 0x24, 0xAF, 0xBF, 0xD4, 0x6C, 0xFD, 0x81, 0xB2, 0x2E, 0x39, 0x73, 0xC6, 0x5F,
	0xA1, 0xBD, 0x9D, 0xE3, 0x17, 0x87]

const test_200_message_sha3_384 = [u8(0x18), 0x81, 0xDE, 0x2C, 0xA7, 0xE4, 0x1E, 0xF9, 0x5D, 0xC4,
	0x73, 0x2B, 0x8F, 0x5F, 0x00, 0x2B, 0x18, 0x9C, 0xC1, 0xE4, 0x2B, 0x74, 0x16, 0x8E, 0xD1, 0x73,
	0x26, 0x49, 0xCE, 0x1D, 0xBC, 0xDD, 0x76, 0x19, 0x7A, 0x31, 0xFD, 0x55, 0xEE, 0x98, 0x9F, 0x2D,
	0x70, 0x50, 0xDD, 0x47, 0x3E, 0x8F]

const test_200_message_sha3_512 = [u8(0xE7), 0x6D, 0xFA, 0xD2, 0x20, 0x84, 0xA8, 0xB1, 0x46, 0x7F,
	0xCF, 0x2F, 0xFA, 0x58, 0x36, 0x1B, 0xEC, 0x76, 0x28, 0xED, 0xF5, 0xF3, 0xFD, 0xC0, 0xE4, 0x80,
	0x5D, 0xC4, 0x8C, 0xAE, 0xEC, 0xA8, 0x1B, 0x7C, 0x13, 0xC3, 0x0A, 0xDF, 0x52, 0xA3, 0x65, 0x95,
	0x84, 0x73, 0x9A, 0x2D, 0xF4, 0x6B, 0xE5, 0x89, 0xC5, 0x1C, 0xA1, 0xA4, 0xA8, 0x41, 0x6D, 0xF6,
	0x54, 0x5A, 0x1C, 0xE8, 0xBA, 0x00]

// The values are taken from the output of Go's sha3 library
const test_200_message_keccak_256 = [u8(0x3A), 0x57, 0x66, 0x6B, 0x04, 0x87, 0x77, 0xF2, 0xC9,
	0x53, 0xDC, 0x44, 0x56, 0xF4, 0x5A, 0x25, 0x88, 0xE1, 0xCB, 0x6F, 0x2D, 0xA7, 0x60, 0x12, 0x2D,
	0x53, 0x0A, 0xC2, 0xCE, 0x60, 0x7D, 0x4A]

const test_200_message_keccak_512 = [u8(0xF4), 0xF8, 0x46, 0xD1, 0x40, 0x84, 0x75, 0x39, 0xF5,
	0x3C, 0x3F, 0x08, 0x2C, 0xC4, 0xE6, 0x81, 0x0E, 0x14, 0x3A, 0x5B, 0x4F, 0xC6, 0x2A, 0x20, 0x59,
	0x7B, 0x5D, 0x76, 0x04, 0x32, 0x46, 0xB8, 0x6B, 0xD7, 0x14, 0x9B, 0x90, 0x61, 0x40, 0xBB, 0x96,
	0x65, 0xA6, 0xCE, 0x83, 0xD9, 0x91, 0xF0, 0x32, 0xF2, 0x29, 0x1D, 0x2F, 0xAE, 0x80, 0xEE, 0xDF,
	0xC6, 0xF8, 0x45, 0xCC, 0x16, 0xD5, 0xAE]

fn test_200_length_hash() {
	output_224 := sum224(input_200)
	assert output_224 == test_200_message_sha3_224

	output_256 := sum256(input_200)
	assert output_256 == test_200_message_sha3_256

	output_384 := sum384(input_200)
	assert output_384 == test_200_message_sha3_384

	output_512 := sum512(input_200)
	assert output_512 == test_200_message_sha3_512

	output_keccak_256 := keccak256(input_200)
	assert output_keccak_256 == test_200_message_keccak_256

	output_keccak_512 := keccak512(input_200)
	assert output_keccak_512 == test_200_message_keccak_512
}

const empty_message_shake128 = [u8(0x7F), ]

const empty_message_shake256 = [u8(0x46), ]

fn test_0_length_xof() {
	input := []u8{}

	output_128 := shake128(input, 512)
	assert output_128 == empty_message_shake128

	output_256 := shake256(input, 512)
	assert output_256 == empty_message_shake256
}

const test_200_message_shake128 = [u8(0x13), ]

const test_200_message_shake256 = [u8(0xCD), ]

fn test_200_length_xof() {
	input := []u8{}

	output_128 := shake128(input_200, 512)
	assert output_128 == test_200_message_shake128

	output_256 := shake256(input_200, 512)
	assert output_256 == test_200_message_shake256
}
