...
pub fn (mut app App) index() vweb.Result {
	...
	return app.text(.ok, 'Welcome to VWeb')
}

pub fn (mut app App) simple() vweb.Result {
	return app.text(.ok, 'A simple result')
}

pub fn (mut app App) html_page() vweb.Result {
	return app.html(.ok, '<h1>ok</h1>')
}
...
pub fn (mut app App) settings(username string) vweb.Result {
	...
	return app.html(.ok, 'username: $username')
}
...
pub fn (mut app App) user_repo_settings(username string, repository string) vweb.Result {
	...
	return app.html(.ok, 'username: $username | repository: $repository')
}

['/json_echo'; post]
pub fn (mut app App) json_echo() vweb.Result {
	// eprintln('>>>>> received http request at /json_echo is: $app.req')
	app.set_content_type(app.request.header.get(.content_type) or { '' })
	return app.text(.ok, app.request.data)
}

['/form_echo'; post]
pub fn (mut app App) form_echo() vweb.Result {
	app.set_content_type(app.request.header.get(.content_type) or { '' })
	return app.text(.ok, app.form['foo'])
}
...
pub fn (mut app App) shutdown() vweb.Result {
	session_key := app.get_cookie('skey') or { return app.send_status(.not_found) }
	if session_key != 'superman' {
		return app.send_status(.not_found)
	}
	go app.gracefull_exit()
	return app.text(.ok, 'good bye')
}
