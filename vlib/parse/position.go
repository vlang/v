...
fn position(r io.Reader, offset int) (line, col int, context string) {
	l := buffer.NewLexer(r)
	...
	for {
		...
		if c == 0 && l.Err() != nil || offset == l.Pos() {
			col = l.Pos() + 1
			context = position_context(l, line, col)
			return
		}
...
func position_context(l *buffer.Lexer, line, col int) (context string) {
	for {
		c := l.Peek(0)
		if c == 0 && l.Err() != nil || c == '\n' || c == '\r' {
			break
