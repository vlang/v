...
pub fn new_two_phase_set[T]() TwoPhaseSet[T] {
...
pub fn (mut t TwoPhaseSet[T]) add(elem T) {
	t.add_set.add(elem)
}

// remove deletes the element from the set.
pub fn (mut t TwoPhaseSet[T]) remove(elem T) {
	t.rm_set.add(elem)
}
...
pub fn (mut t TwoPhaseSet[T]) lookup(elem T) bool {
