...
pub fn protect(mut ctx veb.Context, config &CsrfConfig) bool {
	...
	if config.verbose { eprintln('[CSRF] Token data: ${data}') }
	if data.len < 3 {
		request_is_invalid(mut ctx)
		return false
	}
	...
	if actual_hash != expected_hash {
		if config.verbose { eprintln('[CSRF] The actual hash differs from the expected hash') }
		request_is_invalid(mut ctx)
		return false
	}
	if config.verbose { eprintln('[CSRF] The actual hash matches the expected hash') }
