...
pub fn (mut b BuilderWithBuffer) write_string(s string) {
	...
	if b.char_count + s.len + 1 > b.initial_buffer_cap {
		eprintln('string overflowing inital capacity. To performance improvement increase array capacity from []u8{cap: ${b.initial_buffer_cap}} to []u8{cap: ${
			b.char_count + s.len + 1}} or more')
	}

	unsafe { b.buf.push_many(s.str, s.len) }

	b.char_count += u32(s.len)
}
