...
fn (mut ctx Context) termios_setup() ! {
	...
	if ctx.cfg.capture_events {
		...
		tios.c_iflag &= termios.invert(termios.flag(int(C.IGNBRK) | int(C.BRKINT) | int(C.PARMRK) | int(C.IXON)))
		tios.c_lflag &= termios.invert(termios.flag(int(C.ICANON) | int(C.ISIG) | int(C.ECHO) | int(C.IEXTEN) | int(C.TOSTOP)))
	} else {
		// Set raw input mode by unsetting ICANON and ECHO
