...
[inline]
// write will put the string `s` into the print buffer.
...
[inline]
// flush will flush the print buffer to the screen.
...
[inline]
// bold will set the character state to bold.
pub fn (mut ctx Context) bold() {
	ctx.write('\x1b[1m')
}

[inline]
// set_cursor_position will position the cusor at `x`,`y`.
pub fn (mut ctx Context) set_cursor_position(x int, y int) {
	ctx.write('\x1b[$y;${x}H')
}

[inline]
// set_color will set the current foreground color used by any succeeding `draw_*` calls.
...
[inline]
// set_color will set the current background color used by any succeeding `draw_*` calls.
...
[inline]
// reset_color will set the current foreground color back to it's default value.
pub fn (mut ctx Context) reset_color() {
	ctx.write('\x1b[39m')
}

[inline]
// reset_bg_color will set the current background color back to it's default value.
pub fn (mut ctx Context) reset_bg_color() {
	ctx.write('\x1b[49m')
}

[inline]
// reset will reset all colors and text formats back to their default values.
...
[inline]
// set_window_title will set the string `s` as the window title.
pub fn (mut ctx Context) set_window_title(s string) {
	print('\x1b]0;$s\x07')
}

[inline]
// draw_point will draw a point at position `x`,`y`.
...
[inline]
// draw_text will draw the string `s` starting from position `x`,`y`.
pub fn (mut ctx Context) draw_text(x int, y int, s string) {
	ctx.set_cursor_position(x, y)
	ctx.write(s)
}

// draw_line will draw a line segment starting at point `x`,`y` and ending at point `x2`,`y2`.
...
// draw_dashed_line will draw a dashed line segment starting at point `x`,`y` and ending at point `x2`,`y2`.
...
// draw_rect will draw a rectangle starting at top left `x`,`y` and ending at bottom right `x2`,`y2`.
...
// draw_empty_dashed_rect will draw a rectangle with dashed lines starting at top left `x`,`y` and ending at bottom right `x2`,`y2`.
...
// draw_empty_rect will draw a rectangle with no fill starting at top left `x`,`y` and ending at bottom right `x2`,`y2`.
...
[inline]
// horizontal_separator will draw a horizontal separator spanning the width of the screen.
