...
fn decode_from_buffer(dest &u8, src &u8, src_len int) int {
	...
	unsafe {
		...
		for src_len - si >= 8 {
			...
			datablock_64.data = assemble64(u8(index[d[si + 0]]), u8(index[d[si + 1]]), u8(index[d[si + 2]]), u8(index[d[si + 3]]), u8(index[d[si + 4]]), u8(index[d[si + 5]]), u8(index[d[si + 6]]), u8(index[d[si + 7]]))
		...
		for src_len - si >= 4 {
			datablock_32.data = assemble32(u8(index[d[si + 0]]), u8(index[d[si + 1]]), u8(index[d[si + 2]]), u8(index[d[si + 3]]))
			$if little_endian {
