...
pub fn (mut tm DynamicTemplateManager) serve_dynamic_template(tmpl_path string, tmpl_var TemplateCacheParams) string {
	if tm.dtm_init_is_ok {
		...
		// This code snippet checks if the dynamic content ( If, however, it contains dynamic content ) of an HTML page has been updated.
		// If it has, it creates a checksum of this content for analysis.
		// The checksum is generated by concatenating all dynamic values and applying a SHA256 hash function to the resulting string.
		mut current_content_checksum := ''
		if is_cache_exist && tmpl_var.placeholders.len > 0 {
			mut combined_str := ''
			for _, value in tmpl_var.placeholders {
				match value {
...
fn (tm DynamicTemplateManager) create_temp_cache(html &string, f_path string, ts i64) (bool, string) {
	...
	// Generates a SHA256 hash of the combined string for uniqueness
	tmp_checksum := sha256.hexhash(combined_str)
			...
			mut tc := <-tm.ch_cache_handler {
				...
				if !tm.chandler_prevent_cache_duplicate_request(tc) {
					if tc.cache_request != .delete {
						...
						// The unique checksum is generated by hashing the file data, its path, and the cache generation timestamp using SHA256.
						...
						tc.checksum = sha256.hexhash(combined_str)
...
fn (mut tm DynamicTemplateManager) parse_html_file(file_path string, tmpl_name string, placeholders &map[string]DtmMultiTypeMap, is_compressed bool) (string, string) {
	...
	if placeholders.len > 0 {
		...
		// Calculates the content checksum for the combined string of dynamic content
		current_content_checksum = sha256.hexhash(combined_str)
	}
	...
	return html, current_content_checksum
}
