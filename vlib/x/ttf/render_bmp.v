...
pub fn (mut bmp BitMap) aline(in_x0 int, in_y0 int, in_x1 int, in_y1 int, c u32) {
	...
	if mu.fabs_32(dx) > mu.fabs_32(dy) {
		if x1 < x0 {
			tmp = x0
			x0 = x1
			x1 = tmp
...
pub fn (mut bmp BitMap) get_chars_bbox(in_string string) []int {
	...
	for i < in_string.len {
		...
		width := int((mu.iabs(x_max + x_min) + ax) * bmp.scale)
		// width := int((cw+ax) * bmp.scale)
		w += width + div_space_cw
		h := int(mu.iabs(int(bmp.tf.y_max - bmp.tf.y_min)) * bmp.scale)
		res << w
		res << h
...
pub fn (mut bmp BitMap) get_bbox(in_string string) (int, int) {
	...
	return w, int(mu.iabs(int(bmp.tf.y_max - bmp.tf.y_min)) * bmp.scale)
}
...
fn (mut bmp BitMap) draw_notdef_glyph(in_x int, in_w int) {
	...
	y_h := mu.fabs_32(bmp.tf.y_max - bmp.tf.y_min) * bmp.scale * 0.5
...
pub fn (mut bmp BitMap) draw_text(in_string string) (int, int) {
	...
	for i < in_string.len {
		...
		mut width := int((mu.iabs(x_max + x_min) + ax) * bmp.scale)
