...
fn (c &Context) prepare_v( cdir string, commit string ) {
  ...
  d('make')
  d('./v       -o v     compiler/ ')
  d('./v -prod -o vprod compiler/ ')
  d('cp v     v_stripped')
  d('cp vprod vprod_stripped')
  d('strip *_stripped')
  d('cp v_stripped      v_stripped_upxed')
  d('cp vprod_stripped  vprod_stripped_upxed')
  d('upx -qqq --lzma v_stripped_upxed')
  d('upx -qqq --lzma vprod_stripped_upxed')
  ...
  println("V version is: " + d("$cdir/v --version") + " , local source commit: " + d("git rev-parse --short  --verify HEAD") )
}
